...
//       RP::get("MultiPeak.Vz2PertAbsAmp", this->Vz2PertAbsAmp);
   ...
   Real MultiPeak::calcPhaseSpaceDensity(creal& x, creal& y, creal& z, creal& dx, creal& dy, creal& dz, creal& vx, creal& vy, creal& vz, creal& dvx, creal& dvy, creal& dvz) {
      ...
      const Real v_limit = 0.5e5;
      if (fabs(vz) > v_limit) return 0.0;
      ...
      do {
         ...
         if (fabs(avgCurrent-avgAccum)/(avgAccum+1e-30) < 0.05) ok = true;

         avgTotal += avg;
         N3_sum += N*N*N;
         ++N;

         #warning define max iterations somewhere
         if (N > 10) ok = true;
      } while (ok == false);

      return mul*avgTotal / N3_sum;
   }
