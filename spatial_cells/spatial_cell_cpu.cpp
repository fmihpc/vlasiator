...
namespace spatial_cell {
   ...
   public:
      ...
      template <int PAD> void fetch_data(const vmesh::GlobalID& blockGID, const vmesh::VelocityMesh* vmesh, const Realf* src, Realf* array);
      template <int PAD> void fetch_acc_data(const vmesh::GlobalID& blockGID, const int& dim, vmesh::VelocityMesh* vmesh, const Realf* src, Realf* array, Real cellSizeFractions[2]);
   ...
   inline void SpatialCell::scale_population(creal factor, cuint popID) { (this->populations[popID]).Scale(factor); }
   inline void SpatialCell::increment_population(const Population& pop, creal factor, cuint popID) { (this->populations[popID]).Increment(pop, factor); }
