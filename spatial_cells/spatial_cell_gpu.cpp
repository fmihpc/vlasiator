...
namespace spatial_cell {
   ...
   __global__ static void resize_vmesh_ondevice_kernel(vmesh::VelocityMesh* vmesh, vmesh::LocalID nBlocks) { vmesh->device_setNewSize(nBlocks); }

   /** GPU kernel for scaling a particle population */
   __global__ static void __launch_bounds__(WID3, 4) population_scale_kernel(vmesh::LocalID nBlocks, vmesh::VelocityMesh* vmesh, vmesh::VelocityBlockContainer* blockContainer, const Real factor) {
   ...
   */
   __global__ static void __launch_bounds__(WID3, 4) population_increment_kernel(vmesh::LocalID nBlocks, vmesh::VelocityMesh* vmesh, vmesh::VelocityBlockContainer* blockContainer,
                                                                                 vmesh::VelocityMesh* otherVmesh, vmesh::VelocityBlockContainer* otherBlockContainer, const Real factor
    ...
    */
   __global__ static void __launch_bounds__(WID3, 4)
       population_replace_kernel(vmesh::VelocityMesh* vmesh, vmesh::VelocityBlockContainer* blockContainer, vmesh::VelocityMesh* otherVmesh, vmesh::VelocityBlockContainer* otherBlockContainer) {
...
// Create block in vmesh
    ...
    */
   __global__ static void resize_and_empty_kernel(vmesh::VelocityMesh* vmesh, vmesh::VelocityBlockContainer* blockContainer, vmesh::LocalID newSize) {
   ...
   struct Population {
      ...
      const Population& operator=(const Population& other) {
         ...
         if (newSize > 0) {
            ...
            population_replace_kernel<<<newSize, block, 0, stream>>>(dev_vmesh, dev_blockContainer, other.dev_vmesh, other.dev_blockContainer);
      ...
      void ResizeClear(const uint newSize) {
         ...
         resize_and_empty_kernel<<<1, Hashinator::defaults::MAX_BLOCKSIZE, 0, stream>>>(dev_vmesh, dev_blockContainer, newSize);
         CHK_ERR(gpuPeekAtLastError());
      ...
      void Scale(creal factor) {
         ...
         if (nBlocks > 0) {
            dim3 block(WID, WID, WID);
            population_scale_kernel<<<nBlocks, block, 0, stream>>>(nBlocks, dev_vmesh, dev_blockContainer, factor);
            CHK_ERR(gpuPeekAtLastError());
            CHK_ERR(gpuStreamSynchronize(stream));
         }
      }
      void Increment(const Population& other, creal factor) {
         ...
         if (nBlocks > 0) {
            dim3 block(WID, WID, WID);
            // Now serial
            population_increment_kernel<<<1, block, 0, stream>>>(nBlocks, dev_vmesh, dev_blockContainer, other.dev_vmesh, other.dev_blockContainer, factor);
            CHK_ERR(gpuPeekAtLastError());
            CHK_ERR(gpuStreamSynchronize(stream));
         }
         vmesh->updateCachedSize();
   ...
   */
   ...
   __global__ void add_blocks_from_buffer_kernel(const vmesh::VelocityMesh* vmesh, vmesh::VelocityBlockContainer* blockContainer, const vmesh::LocalID startLID, const vmesh::GlobalID* gpuInitBlocks,
                                                 const fileReal* gpuInitBuffer, const uint nBlocks) {
      const int blocki = blockIdx.x;
   ...
   inline void SpatialCell::dev_upload_population(const uint popID) { populations[popID].Upload(); }
   ...
   inline void SpatialCell::increment_mass_loss(cuint popID, const Real increment) { (this->populations[popID]).RHOLOSSADJUST += increment; }

   inline std::vector<Population>& SpatialCell::get_populations() { return populations; }
   inline const std::vector<Population>& SpatialCell::get_populations() const { return populations; }

   inline const vmesh::LocalID* SpatialCell::get_velocity_grid_length(const uint popID) { return populations[popID].vmesh->getGridLength(); }

   inline const vmesh::GlobalID* SpatialCell::get_velocity_grid(const uint popID) { return (populations[popID].vmesh->getGrid())->data(); }

   inline const Real* SpatialCell::get_velocity_grid_block_size(const uint popID) { return populations[popID].vmesh->getBlockSize(); }

   inline const Real* SpatialCell::get_velocity_grid_cell_size(const uint popID) { return populations[popID].vmesh->getCellSize(); }

   inline void SpatialCell::get_velocity_block_coordinates(const uint popID, const vmesh::GlobalID& globalID, Real* coords) { populations[popID].vmesh->getBlockCoordinates(globalID, coords); }
   ...
   inline vmesh::GlobalID SpatialCell::get_velocity_block(const uint popID, const Real* coords) const { return populations[popID].vmesh->getGlobalID(coords); }
   ...
   inline void SpatialCell::get_velocity_block_size(const uint popID, const vmesh::GlobalID block, Real blockSize[3]) { populations[popID].vmesh->getBlockSize(block, blockSize); }
   ...
   inline unsigned int SpatialCell::invalid_block_index() { return vmesh::VelocityMesh::invalidBlockIndex(); }

   inline vmesh::GlobalID SpatialCell::invalid_global_id() { return vmesh::VelocityMesh::invalidGlobalID(); }

   inline vmesh::GlobalID SpatialCell::invalid_local_id() { return vmesh::VelocityMesh::invalidLocalID(); }
   ...
   inline void SpatialCell::dev_resize_vmesh(const uint popID, const uint nBlocks) {
      ...
      spatial_cell::resize_vmesh_ondevice_kernel<<<1, 1, 0, stream>>>(populations[popID].dev_vmesh, nBlocks);
      CHK_ERR(gpuPeekAtLastError());
   ...
   */
   inline void SpatialCell::setNewSizeClear(const uint popID, const vmesh::LocalID& newSize) { populations[popID].ResizeClear(newSize); }
   inline void SpatialCell::setNewSizeClear(const uint popID) { populations[popID].ResizeClear(populations[popID].N_blocks); }
   ...
   */
   template <typename fileReal> void SpatialCell::add_velocity_blocks(const uint popID, const std::vector<vmesh::GlobalID>& blocks, fileReal* initBuffer) {
      ...
      if (adds != nBlocks) {
         std::cerr << "Failed to add blocks" << __FILE__ << ' ' << __LINE__ << std::endl;
         exit(1);
         return;
      }
      ...
      CHK_ERR(gpuMemcpyAsync(gpuInitBuffer, initBuffer, WID3 * nBlocks * sizeof(fileReal), gpuMemcpyHostToDevice, stream));
      CHK_ERR(gpuMallocAsync((void**)&gpuInitBlocks, nBlocks * sizeof(vmesh::GlobalID), stream));
      CHK_ERR(gpuMemcpyAsync(gpuInitBlocks, blocks.data(), nBlocks * sizeof(vmesh::GlobalID), gpuMemcpyHostToDevice, stream));
      ...
      if (nBlocks > 0) {
         ...
         spatial_cell::add_blocks_from_buffer_kernel<<<nBlocks, block, 0, stream>>>(
             populations[popID].dev_vmesh, populations[popID].dev_blockContainer, startLID, gpuInitBlocks, gpuInitBuffer, nBlocks);
         CHK_ERR(gpuPeekAtLastError());
      }
      CHK_ERR(gpuStreamSynchronize(stream));
