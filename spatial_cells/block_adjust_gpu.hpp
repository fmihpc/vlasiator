...
__global__ void __launch_bounds__(WID3, WID3S_PER_MP)
    batch_update_velocity_block_content_lists_kernel(const vmesh::VelocityMesh* __restrict__ const* vmeshes, const vmesh::VelocityBlockContainer* __restrict__ const* blockContainers,
                                                     Hashinator::Hashmap<vmesh::GlobalID, vmesh::LocalID>** allMaps, const Real* __restrict__ velocity_block_min_values, const bool gatherMass,
                                                     Real* dev_mass) {
 ...
 */
__global__ void __launch_bounds__(Hashinator::defaults::MAX_BLOCKSIZE, FULLBLOCKS_PER_MP) batch_reset_all_to_empty(Hashinator::Hashmap<vmesh::GlobalID, vmesh::LocalID>** maps) {
 ...
 */
__global__ void __launch_bounds__(Hashinator::defaults::MAX_BLOCKSIZE, FULLBLOCKS_PER_MP)
    check_vector_capacities(const Hashinator::Hashmap<vmesh::GlobalID, vmesh::LocalID>* __restrict__ const* maps, const split::SplitVector<vmesh::GlobalID>* __restrict__ const* vecs,
...
__global__ void __launch_bounds__(Hashinator::defaults::MAX_BLOCKSIZE, FULLBLOCKS_PER_MP)
    extract_GIDs_kernel(const Hashinator::Hashmap<vmesh::GlobalID, vmesh::LocalID>* __restrict__ const* input_maps, // buffer of pointers to source maps
                        split::SplitVector<ELEMENT>** output_vecs, vmesh::LocalID* output_sizes, Rule rule,
                        const vmesh::VelocityMesh* __restrict__ const* rule_meshes, // buffer of pointers to vmeshes, sizes used by rules
                        const Hashinator::Hashmap<vmesh::GlobalID, vmesh::LocalID>* __restrict__ const* rule_maps, const split::SplitVector<vmesh::GlobalID>* __restrict__ const* rule_vectors) {
   ...
   const vmesh::LocalID threshold = rule_meshes[cellIndex]->size() + rule_vectors[cellIndex]->size() - rule_maps[hashmapIndex]->size();
...
template <typename Rule, typename ELEMENT, bool FIRSTONLY = false>
void extract_GIDs_kernel_launcher(Hashinator::Hashmap<vmesh::GlobalID, vmesh::LocalID>** input_maps, split::SplitVector<ELEMENT>** output_vecs, vmesh::LocalID* output_sizes, Rule rule,
   ...
   extract_GIDs_kernel<Rule, ELEMENT, FIRSTONLY><<<nCells, Hashinator::defaults::MAX_BLOCKSIZE, 0, stream>>>(input_maps, output_vecs, output_sizes, rule, rule_meshes, rule_maps, rule_vectors);
...
__global__ void __launch_bounds__(Hashinator::defaults::MAX_BLOCKSIZE, FULLBLOCKS_PER_MP)
    extract_overflown_kernel(vmesh::VelocityMesh** vmeshes, // buffer of pointers to vmeshes, contain hashmaps
                             split::SplitVector<Hashinator::hash_pair<vmesh::GlobalID, vmesh::LocalID>>** output_vecs, vmesh::LocalID* output_sizes, Rule rule) {
   // launch parameters: dim3 grid(nMaps,1,1); // As this is a looping reduction
...
template <typename Rule>
void clean_tombstones_launcher(vmesh::VelocityMesh** vmeshes, split::SplitVector<Hashinator::hash_pair<vmesh::GlobalID, vmesh::LocalID>>** overflown_elements, vmesh::LocalID* output_sizes, Rule rule,
                               const uint nCells, gpuStream_t stream) {
   // Extract overflown elements into temporary vector
   extract_overflown_kernel<Rule><<<nCells, Hashinator::defaults::MAX_BLOCKSIZE, 0, stream>>>(vmeshes, overflown_elements, output_sizes, rule);
   CHK_ERR(gpuPeekAtLastError());
}
 ...
 */
__global__ void __launch_bounds__(GPUTHREADS, WARPS_PER_MP) batch_insert_kernel(vmesh::VelocityMesh** vmeshes, // buffer of pointers to vmeshes, contain hashmaps
...
*/
__global__ void __launch_bounds__(26 * 32, FULLBLOCKS_PER_MP)
    batch_update_velocity_halo_kernel(const vmesh::VelocityMesh* __restrict__ const* vmeshes, const split::SplitVector<vmesh::GlobalID>* __restrict__ const* velocity_block_with_content_lists,
...
// Offsets verified in python
         ...
         const vmesh::GlobalID nGID = vmesh->getGlobalID(nind0, nind1, nind2);
...
__global__ void batch_update_velocity_halo_kernel(const vmesh::VelocityMesh* __restrict__ const* vmeshes,
                                                  ...
                                                  Hashinator::Hashmap<vmesh::GlobalID, vmesh::LocalID>** allMaps, const uint warpsPerBlockBatchHalo) {
   // launch grid dim3 grid(launchBlocks,nCells,1);
   // Each block manages a single GID at a time, all velocity neighbours
   // const uint nCells = gridDim.y;
   const uint cellIndex = blockIdx.y;
 ...
 */
__global__ void __launch_bounds__(GPUTHREADS* WARPSPERBLOCK, FULLBLOCKS_PER_MP)
    batch_update_neighbour_halo_kernel(const vmesh::VelocityMesh* __restrict__ const* vmeshes, Hashinator::Hashmap<vmesh::GlobalID, vmesh::LocalID>** allMaps,
...
__global__ void batch_resize_vbc_kernel_pre(vmesh::VelocityMesh** vmeshes, vmesh::VelocityBlockContainer** blockContainers, split::SplitVector<vmesh::GlobalID>** dev_list_with_replace_new,
                                            ...
                                            vmesh::LocalID* dev_nBefore, vmesh::LocalID* dev_nAfter, vmesh::LocalID* dev_nBlocksToChange, vmesh::LocalID* dev_resizeSuccess,
...
__global__ void __launch_bounds__(WID3, WID3S_PER_MP)
    batch_update_velocity_blocks_kernel(vmesh::VelocityMesh** vmeshes, vmesh::VelocityBlockContainer** blockContainers,
                                        ...
                                        const split::SplitVector<Hashinator::hash_pair<vmesh::GlobalID, vmesh::LocalID>>* __restrict__ const* dev_list_with_replace_old, vmesh::LocalID* dev_nBefore,
                                        vmesh::LocalID* dev_nAfter, vmesh::LocalID* dev_nBlocksToChange,
   ...
   **/
   if (index < n_delete) {
      ...
      const Real rm_DV3 = rm_block_parameters[BlockParams::DVX] * rm_block_parameters[BlockParams::DVY] * rm_block_parameters[BlockParams::DVZ];
      // thread-sum for rho
 ...
 */
__global__ void __launch_bounds__(WID3, WID3S_PER_MP) batch_population_scale_kernel(vmesh::VelocityBlockContainer** blockContainers, Real* dev_mass_scale) {
   // launch griddim3 grid(launchBlocks,nCells,1);
   const int cellIndex = blockIdx.y;
