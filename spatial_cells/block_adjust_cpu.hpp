...
#pragma omp critical
      ...
      batch_update_velocity_block_content_lists_kernel<<<grid2, WID3, 0, baseStream>>>(dev_vmeshes,
                                                                                       dev_VBCs,
                                                                                       dev_allMaps,
                                                                                       dev_minValues,
      ...
      auto rule = [] __device__(const Hashinator::Hashmap<vmesh::GlobalID, vmesh::LocalID>* map,
      ...
      extract_GIDs_kernel_launcher<decltype(rule), vmesh::GlobalID, true>(dev_allMaps,      // points to has_content maps
...
#pragma omp critical
      ...
      CHK_ERR(
          gpuMemcpyAsync(dev_lists_to_replace, host_lists_to_replace, nCells * sizeof(split::SplitVector<Hashinator::hash_pair<vmesh::GlobalID, vmesh::LocalID>>*), gpuMemcpyHostToDevice, baseStream));
      CHK_ERR(gpuMemcpyAsync(
          dev_lists_with_replace_old, host_lists_with_replace_old, nCells * sizeof(split::SplitVector<Hashinator::hash_pair<vmesh::GlobalID, vmesh::LocalID>>*), gpuMemcpyHostToDevice, baseStream));
      CHK_ERR(gpuMemcpyAsync(dev_VBCs, host_VBCs, nCells * sizeof(vmesh::VelocityBlockContainer*), gpuMemcpyHostToDevice, baseStream));
      CHK_ERR(gpuStreamSynchronize(baseStream));
      copyTimer.stop();
      ...
      if (largestContentList > 0) {
         ...
         batch_update_velocity_halo_kernel<<<grid_vel_halo, 26 * 32, 0, priorityStream>>>(dev_vmeshes,
                                                                                          dev_vbwcl_vec,
         ...
         batch_update_velocity_halo_kernel<<<grid_vel_halo, block_vel_halo, 0, priorityStream>>>(dev_vmeshes,
                                                                                                 dev_vbwcl_vec,
                                                                                                 dev_allMaps, // Needs both content and no content maps
...
// largestContentListNeighbors accounts for remote (ghost neighbor) content list sizes as well
         ...
         batch_update_neighbour_halo_kernel<<<grid_neigh_halo, WARPSPERBLOCK * GPUTHREADS, 0, baseStream>>>(dev_vmeshes,
      ...
      // reallocated)
      check_vector_capacities<<<nCells, 1, 0, baseStream>>>(dev_allMaps, dev_lists_with_replace_new, dev_overflownElements);
      CHK_ERR(gpuPeekAtLastError());
      ...
      */
      ...
      extract_to_add_caller(dev_allMaps + 0,            // input maps: this is has_content_maps
                            ...
                            dev_vbwcl_vec,              // rule_vectors, not used in this call
                            nCells,
                            baseStream); // This needs to complete before the next 3 extractions
      // Finds Blocks (GID,LID) to be rescued from end of v-space
      extract_to_delete_or_move_caller(dev_allMaps + 0,            // input maps: this is has_content_maps
      ...
      extract_to_delete_or_move_caller(dev_allMaps + 1,            // input maps: this is has_no_content_maps
      ...
      extract_to_replace_caller(dev_allMaps + 1,            // input maps: this is has_no_content_maps
      ...
      auto rule_overflown = [] __device__(Hashinator::Hashmap<vmesh::GlobalID, vmesh::LocalID> * map, Hashinator::hash_pair<vmesh::GlobalID, vmesh::LocalID> & kval) -> bool {
         const vmesh::GlobalID emptybucket = map->get_emptybucket();
         const vmesh::GlobalID tombstone = map->get_tombstone();
      ...
      clean_tombstones_launcher<decltype(rule_overflown)>(dev_vmeshes,                // velocity meshes which include the hash maps to clean
...
#pragma omp critical
      ...
      if (largestOverflow > 0) {
         ...
         batch_insert_kernel<<<grid_reinsert, GPUTHREADS, 0, baseStream>>>(dev_vmeshes,               // velocity meshes which include the hash maps to clean
                                                                           dev_lists_with_replace_old // use this for storing overflown elements
         );
         CHK_ERR(gpuPeekAtLastError());
         CHK_ERR(gpuStreamSynchronize(baseStream));
...
#pragma omp for schedule(dynamic)
      ...
      if ((getObjectWrapper().particleSpecies[popID].sparse_conserve_mass) && (largestBlocksToChange > 0)) {
         ...
         batch_population_scale_kernel<<<grid_mass_conservation, WID3, 0, baseStream>>>(dev_VBCs,
   ...
   void clear_maps_caller(const uint nCells, const size_t largestSizePower, gpuStream_t stream, const size_t offset) {
      ...
      batch_reset_all_to_empty<<<grid1, Hashinator::defaults::MAX_BLOCKSIZE, 0, stream>>>(dev_allMaps + 2 * offset);
      CHK_ERR(gpuPeekAtLastError());
      CHK_ERR(gpuStreamSynchronize(stream));
   }

   void batch_adjust_blocks_caller(dccrg::Dccrg<spatial_cell::SpatialCell, dccrg::Cartesian_Geometry>& mpiGrid, const vector<CellID>& cellsToAdjust, const uint cellOffset,
                                   uint& out_largestBlocksToChange, uint& out_largestBlocksBeforeOrAfter, const uint popID) {
      ...
      batch_resize_vbc_kernel_pre<<<nCells, 1, 0, baseStream>>>(dev_vmeshes + cellOffset,
                                                                dev_VBCs + cellOffset,
      ...
      if (largestBlocksToChange > 0) {
         ...
         batch_update_velocity_blocks_kernel<<<grid_addremove, WID3, 0, baseStream>>>(dev_vmeshes + cellOffset,
                                                                                      dev_VBCs + cellOffset,
                                                                                      dev_lists_with_replace_new + cellOffset,
                                                                                      dev_lists_delete + cellOffset,
                                                                                      dev_lists_to_replace + cellOffset,
         ...
         batch_resize_vbc_kernel_post<<<nCells, 1, 0, baseStream>>>(dev_vmeshes + cellOffset, dev_VBCs + cellOffset, dev_nAfter + cellOffset);
         CHK_ERR(gpuPeekAtLastError());
         CHK_ERR(gpuStreamSynchronize(baseStream));
   ...
   void extract_to_replace_caller(Hashinator::Hashmap<vmesh::GlobalID, vmesh::LocalID>** input_maps, split::SplitVector<Hashinator::hash_pair<vmesh::GlobalID, vmesh::LocalID>>** output_vecs,
                                  vmesh::LocalID* output_sizes, vmesh::VelocityMesh** rule_meshes, Hashinator::Hashmap<vmesh::GlobalID, vmesh::LocalID>** rule_maps,
                                  split::SplitVector<vmesh::GlobalID>** rule_vectors, const uint nCells, gpuStream_t stream) {
      auto rule_to_replace = [] __device__(const Hashinator::Hashmap<vmesh::GlobalID, vmesh::LocalID>* map,
         ...
         const vmesh::GlobalID emptybucket = map->get_emptybucket();
         const vmesh::GlobalID tombstone = map->get_tombstone();
         return kval.first != emptybucket && kval.first != tombstone && kval.first != invalidGID && kval.second < threshold && kval.second != invalidLID;
      };

      // Find Blocks (GID,LID) to be replaced with new ones
      extract_GIDs_kernel_launcher<decltype(rule_to_replace), Hashinator::hash_pair<vmesh::GlobalID, vmesh::LocalID>, false>(
          input_maps, output_vecs, output_sizes, rule_to_replace, rule_meshes, rule_maps, rule_vectors, nCells, stream);
   }

   void extract_to_delete_or_move_caller(Hashinator::Hashmap<vmesh::GlobalID, vmesh::LocalID>** input_maps, split::SplitVector<Hashinator::hash_pair<vmesh::GlobalID, vmesh::LocalID>>** output_vecs,
                                         vmesh::LocalID* output_sizes, vmesh::VelocityMesh** rule_meshes, Hashinator::Hashmap<vmesh::GlobalID, vmesh::LocalID>** rule_maps,
                                         split::SplitVector<vmesh::GlobalID>** rule_vectors, const uint nCells, gpuStream_t stream) {
      ...
      extract_GIDs_kernel_launcher<decltype(rule_delete_move), Hashinator::hash_pair<vmesh::GlobalID, vmesh::LocalID>, false>(
          input_maps, output_vecs, output_sizes, rule_delete_move, rule_meshes, rule_maps, rule_vectors, nCells, stream);
   }

   void extract_to_add_caller(Hashinator::Hashmap<vmesh::GlobalID, vmesh::LocalID>** input_maps, split::SplitVector<vmesh::GlobalID>** output_vecs, vmesh::LocalID* output_sizes,
                              vmesh::VelocityMesh** rule_meshes, Hashinator::Hashmap<vmesh::GlobalID, vmesh::LocalID>** rule_maps, split::SplitVector<vmesh::GlobalID>** rule_vectors,
      ...
      auto rule_add = [] __device__(const Hashinator::Hashmap<vmesh::GlobalID, vmesh::LocalID>* map,
         ...
         const vmesh::GlobalID emptybucket = map->get_emptybucket();
         const vmesh::GlobalID tombstone = map->get_tombstone();
         return kval.first != emptybucket && kval.first != tombstone && kval.first != invalidGID &&
      ...
      extract_GIDs_kernel_launcher<decltype(rule_add), vmesh::GlobalID, true>(input_maps, output_vecs, output_sizes, rule_add, rule_meshes, rule_maps, rule_vectors, nCells, stream);
   }
