...
namespace vmesh {
   ...
   inline size_t VelocityMesh::capacityInBytes() const {
      ...
      const size_t capacityInBytes = currentCapacity * sizeof(vmesh::GlobalID) + currentBucketCount * sizeof(Hashinator::hash_pair<vmesh::GlobalID, vmesh::LocalID>);
   ...
   ARCH_HOSTDEV inline const Real* VelocityMesh::getBlockSize() const { return (*(vmesh::getMeshWrapper()->velocityMeshes))[meshID].blockSize; }
   ...
   ARCH_HOSTDEV inline const Real* VelocityMesh::getCellSize() const { return (*(vmesh::getMeshWrapper()->velocityMeshes))[meshID].cellSize; }
   ...
   ARCH_HOSTDEV inline vmesh::GlobalID VelocityMesh::getGlobalID(const vmesh::LocalID indices[3]) const {
      ...
      return indices[0] + indices[1] * (*(vmesh::getMeshWrapper()->velocityMeshes))[meshID].gridLength[0] +
             indices[2] * (*(vmesh::getMeshWrapper()->velocityMeshes))[meshID].gridLength[1] * (*(vmesh::getMeshWrapper()->velocityMeshes))[meshID].gridLength[0];
   }
   ...
   ARCH_HOSTDEV inline vmesh::GlobalID VelocityMesh::getGlobalID(const vmesh::LocalID i, const vmesh::LocalID j, const vmesh::LocalID k) const {
      ...
      return i + j * (*(vmesh::getMeshWrapper()->velocityMeshes))[meshID].gridLength[0] +
             k * (*(vmesh::getMeshWrapper()->velocityMeshes))[meshID].gridLength[1] * (*(vmesh::getMeshWrapper()->velocityMeshes))[meshID].gridLength[0];
   }

   ARCH_HOSTDEV inline split::SplitVector<vmesh::GlobalID>* VelocityMesh::getGrid() { return &localToGlobalMap; }

   ARCH_HOSTDEV inline const vmesh::LocalID* VelocityMesh::getGridLength() const { return (*(vmesh::getMeshWrapper()->velocityMeshes))[meshID].gridLength; }
   ...
   ARCH_HOSTDEV inline size_t VelocityMesh::getMesh() const { return meshID; }

   ARCH_HOSTDEV inline const Real* VelocityMesh::getMeshMaxLimits() const { return (*(vmesh::getMeshWrapper()->velocityMeshes))[meshID].meshMaxLimits; }

   ARCH_HOSTDEV inline const Real* VelocityMesh::getMeshMinLimits() const { return (*(vmesh::getMeshWrapper()->velocityMeshes))[meshID].meshMinLimits; }
   ...
   ARCH_HOSTDEV inline vmesh::LocalID VelocityMesh::invalidBlockIndex() { return INVALID_VEL_BLOCK_INDEX; }

   ARCH_HOSTDEV inline vmesh::GlobalID VelocityMesh::invalidGlobalID() { return INVALID_GLOBALID; }

   ARCH_HOSTDEV inline vmesh::LocalID VelocityMesh::invalidLocalID() { return INVALID_LOCALID; }

   ARCH_HOSTDEV inline bool VelocityMesh::isInitialized() const { return (*(vmesh::getMeshWrapper()->velocityMeshes))[meshID].initialized; }
   ...
   inline vmesh::LocalID VelocityMesh::push_back(const std::vector<vmesh::GlobalID>& blocks) {
      ...
      if (ltg_size == 0) {
         ...
         for (size_t b = 0; b < blocksSize; ++b) {
            auto position = globalToLocalMap.insert(Hashinator::make_pair(blocks[b], (vmesh::LocalID)(ltg_size + b)));
   ...
   **/
   ...
   ARCH_DEV inline void VelocityMesh::warpReplaceBlock(const vmesh::GlobalID GIDold, const vmesh::LocalID LID, const vmesh::GlobalID GIDnew, const size_t b_tid) {
      ...
      if (b_tid < GPUTHREADS) { // GPUTODO these in parallel?
         ...
         //    printf("Warp error in VelocityMesh::warpReplaceBlock: map size %u does not match expected %u for thread
         //    %u!\n",(vmesh::LocalID)globalToLocalMap.size(),(vmesh::LocalID)(preMapSize-1),(vmesh::LocalID)b_tid); assert(0);
         // }
   ...
   ARCH_DEV inline void VelocityMesh::warpPlaceBlock(const vmesh::GlobalID GID, const vmesh::LocalID LID, const size_t b_tid) {
      ...
      if (b_tid == 0) {
         if (LID > size() - 1) {
            printf("vmesh placeBlock error: LID is too large!\n");
         }
      }
      __syncthreads();
      {
   ...
   inline size_t VelocityMesh::getMeshID() { return this->meshID; }
   ...
   ARCH_HOSTDEV inline size_t VelocityMesh::sizeInBytes() const {
      ...
      #endif
      const size_t sizeInBytes = ltg_size * sizeof(vmesh::GlobalID) + ltg_size * sizeof(Hashinator::hash_pair<vmesh::GlobalID, vmesh::LocalID>);
      return sizeInBytes;
   }
   ...
   ARCH_HOSTDEV inline Hashinator::Hashmap<vmesh::GlobalID, vmesh::LocalID>* VelocityMesh::gpu_expose_map() { return &globalToLocalMap; }
   ...
   inline void VelocityMesh::print_addresses() {
      ...
      printf(
         "GPU localToGlobalMap capacity %zu size %zu \n GPU globalToLocalMap size %zu bucket count %zu\n",
         localToGlobalMap.capacity(),
         localToGlobalMap.size(),
         globalToLocalMap.size(),
         globalToLocalMap.bucket_count()
      );
   ...
   inline void VelocityMesh::print_sizes() {
      printf(
         ...
         ltg_capacity,
         globalToLocalMap.size(),
         (size_t)globalToLocalMap.getSizePower(),
         gtl_sizepower
      );
   }
