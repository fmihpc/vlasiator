...
template <class Reader> static void detect_field_names(Reader& r) {
   ...
   } else if (find(variableNames.begin(), variableNames.end(), std::string("vg_b_background_vol")) !=
                  variableNames.end() &&
              find(variableNames.begin(), variableNames.end(), std::string("vg_b_perturbed_vol")) !=
                  variableNames.end()) {
