...
public:
   ...
   Maxwell_Boltzmann(std::default_random_engine& _rand, Real _mass, Real _charge, Real kT)
       : Distribution(_rand, _mass, _charge), velocity_distribution(0, sqrt(kT / mass)){};
...
public:
   Monoenergetic(std::default_random_engine& _rand, Real _mass, Real _charge, Real _vel)
       : Distribution(_rand, _mass, _charge), vel(_vel) {}
...
private:
   virtual void generate_lookup() {
      ...
      for (int i = 0; i < lookup_size; i++) {
         ...
         lookup.push_back(prefix * (-23328. * vkappa / (pow(under, 6.)) + 1944. * vkappa / (5. * pow(under, 5.)) +
                                    729. * vkappa / (10. * pow(under, 4.)) + 567. * vkappa / (40. * pow(under, 3.)) +
                                    189. * vkappa / (64. * under * under) + 189. * vkappa / (256. * under) +
...
private:
   virtual void generate_lookup() {
      ...
      for (int i = 0; i < lookup_size; i++) {
         ...
         lookup.push_back(prefix * (-2. * vkappa / (under * under) + vkappa / (2. * under) +
