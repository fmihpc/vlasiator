...
public:
   ...
   Maxwell_Boltzmann(std::default_random_engine& _rand, Real _mass, Real _charge, Real kT) : Distribution(_rand, _mass, _charge), velocity_distribution(0, sqrt(kT / mass)) {};
...
public:
   Monoenergetic(std::default_random_engine& _rand, Real _mass, Real _charge, Real _vel) : Distribution(_rand, _mass, _charge), vel(_vel) {}
   ...
   virtual Particle next_particle() {
      ...
      Vec3d dir(sqrt(1 - u * u) * cos(v), sqrt(1 - u * u) * sin(v), u);
...
public:
   ...
   Kappa(std::default_random_engine& _rand) : Distribution(_rand) { maxw0 = 50; }
...
public:
   Kappa6(std::default_random_engine& _rand, double _mass, double _charge, double _w0, double _maxw0) : Kappa(_rand, _mass, _charge, _w0, _maxw0) { generate_lookup(); }
...
private:
   virtual void generate_lookup() {
      ...
      for (int i = 0; i < lookup_size; i++) {
         ...
         lookup.push_back(prefix * (-23328. * vkappa / (pow(under, 6.)) + 1944. * vkappa / (5. * pow(under, 5.)) + 729. * vkappa / (10. * pow(under, 4.)) + 567. * vkappa / (40. * pow(under, 3.)) +
                                    189. * vkappa / (64. * under * under) + 189. * vkappa / (256. * under) + (63. * sqrt(3. / 2.)) * atan2(vkappa, sqrt(6.)) / 256.));
      }
   }
};
...
public:
   Kappa2(std::default_random_engine& _rand, double _mass, double _charge, double _w0, double _maxw0) : Kappa(_rand, _mass, _charge, _w0, _maxw0) { generate_lookup(); }
...
private:
   virtual void generate_lookup() {
      ...
      for (int i = 0; i < lookup_size; i++) {
         ...
         lookup.push_back(prefix * (-2. * vkappa / (under * under) + vkappa / (2. * under) + atan2(vkappa, sqrt(2.)) / (2. * sqrt(2.))));
      }
   }
};

template <typename T> Distribution* createDistribution(std::default_random_engine& rand) { return new T(rand); }
