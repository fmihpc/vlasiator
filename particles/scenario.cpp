...
void singleParticleScenario::afterPush(int step, double time, ParticleContainer& particles, Field& E, Field& B,
   ...
   std::cout << 0 << " " << time << "\t" << x[0] << " " << x[1] << " " << x[2] << "\t" << v[0] << " " << v[1] << " "
             << v[2] << std::endl;
}
...
ParticleContainer analysatorScenario::initialParticles(Field& E, Field& B, Field& V) {
   ...
   while (std::cin) {
      ...
      if (std::cin) {
         particles.push_back(
             Particle(PhysicalConstantsSI::mp, PhysicalConstantsSI::e, Vec3d(x0, x1, x2), Vec3d(v0, v1, v2)));
      }
   }
...
void analysatorScenario::newTimestep(int input_file_counter, int step, double time, ParticleContainer& particles,
   ...
   for (unsigned int i = 0; i < particles.size(); i++) {
      ...
      std::cout << i << " " << time << "\t" << x[0] << " " << x[1] << " " << x[2] << "\t" << v[0] << " " << v[1] << " "
                << v[2] << std::endl;
   }
}
...
void shockReflectivityScenario::newTimestep(int input_file_counter, int step, double time, ParticleContainer& particles,
   ...
   for (unsigned int i = 0; i < num_points; i++) {
      ...
      // Choose y coordinate
      double start_y =
          ParticleParameters::reflect_start_y +
          ((double)i) / num_points * (ParticleParameters::reflect_stop_y - ParticleParameters::reflect_start_y);
