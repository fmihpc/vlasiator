...
// Trace node coordinates outwards until a non-sysboundary cell is encountered or the local fsgrid domain has been left.
   ...
   std::array<std::pair<int, Real>, 3> calculateIonosphereVlasovGridCoupling(std::array<Real, 3> x, std::vector<SBC::SphericalTriGrid::Node>& nodes, creal couplingRadius) {
      ...
      stepFieldLine(
         x, v, stepSize, fieldTracingParameters.min_tracer_dx_ionospere_coupling, fieldTracingParameters.max_tracer_dx_ionospere_coupling, fieldTracingParameters.tracingMethod, dipoleFieldOnly, true
      );
   ...
   void stepCellAcrossTaskDomain(
      ...
      while (true) {
         ...
         if (x.at(0) * x.at(0) + x.at(1) * x.at(1) + x.at(2) * x.at(2) < fieldTracingParameters.innerBoundaryRadius * fieldTracingParameters.innerBoundaryRadius) {
            ...
            Real r_in = sqrt(
               cellTracingCoordinates[n][0] * cellTracingCoordinates[n][0] + cellTracingCoordinates[n][1] * cellTracingCoordinates[n][1] + cellTracingCoordinates[n][2] * cellTracingCoordinates[n][2]
            );
            Real r_out = sqrt(x[0] * x[0] + x[1] * x[1] + x[2] * x[2]);
         ...
         if (cellConnection[n] < TracingLineEndType::N_TYPES) {
            const TReal extension = sqrt(
               (x[0] - (cellInitialCoordinates[n])[0]) * (x[0] - (cellInitialCoordinates[n])[0]) + (x[1] - (cellInitialCoordinates[n])[1]) * (x[1] - (cellInitialCoordinates[n])[1]) +
      ...
      for (int n = 0; n < globalDccrgSize; n++) {
         ...
         if (mpiGrid.is_local(id)) {
            if ((mpiGrid[id]->sysBoundaryFlag != sysboundarytype::NOT_SYSBOUNDARY) || cellFWTracingCoordinates[n][0] > fieldTracingParameters.x_max ||
               ...
               cellCurvatureRadius[n] = 1 / sqrt(
                                               mpiGrid[id]->parameters[CellParams::CURVATUREX] * mpiGrid[id]->parameters[CellParams::CURVATUREX] +
...
// Trace node coordinates forward and backwards until a non-sysboundary cell is encountered or the local fsgrid domain has been left.
            ...
            !(cellsToDoFullBox <= fieldTracingParameters.fullbox_max_incomplete_cells * globalDccrgSize && cellsToDoFluxRopes <= fieldTracingParameters.fluxrope_max_incomplete_cells * globalDccrgSize)
         );
      } // pragma omp parallel
      loopTimer.stop();
