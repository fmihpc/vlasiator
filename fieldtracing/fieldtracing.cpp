...
namespace FieldTracing {
         ...
         #pragma omp parallel
         {
            ...
            for (uint n = 0; n < nodes.size(); n++) {
               ...
               while (true) {
                  ...
                  stepFieldLine(x, v, nodeTracingStepSize[n], fieldTracingParameters.min_tracer_dx_full_box, fsgrid.getGridSpacing()[0] / 2, fieldTracingParameters.tracingMethod, tracingFullField, (no.x[2] < 0));
   ...
   void traceOpenClosedConnection(fsgrids::technicalspan technical, FieldSolverGrid &fsgrid,
      ...
      uint64_t maxTracingSteps = 8 * (gridSize[0]*fsgrid.getGridSpacing()[0] + gridSize[1]*fsgrid.getGridSpacing()[1] + gridSize[2]*fsgrid.getGridSpacing()[2]) / stepSize;
      ...
      TracingFieldFunction<TReal> tracingFullField = [&perb, &dperb, &technical, &fsgrid](std::array<TReal, 3>& r, const bool alongB, std::array<TReal, 3>& b) -> bool {
         return traceFullFieldFunction(perb, dperb, technical, fsgrid, r, alongB, b);
      };
         ...
         #pragma omp parallel
         {
            ...
            for (uint n = 0; n < nodes.size(); n++) {
               ...
               while (true) {
                  ...
                  // Check if the current coordinates (pre-step) are in our own domain.
                  std::array<fsgrid::FsIndex_t, 3> fsgridCell = getLocalFsGridCellIndexForCoord(fsgrid, {(TReal)x[0], (TReal)x[1], (TReal)x[2]});
                  // If it is not in our domain, somebody else takes care of it.
                  if (fsgridCell[0] == -1) {
                     nodeNeedsContinuedTracing[n] = 0;
                     nodeTracingCoordinates[n] = {0,0,0};
                  ...
                  stepFieldLine(x, v, nodeTracingStepSize[n], (TReal)fieldTracingParameters.min_tracer_dx_full_box, (TReal)fsgrid.getGridSpacing()[0] / 2, fieldTracingParameters.tracingMethod, tracingFullField, (no.x[2] < 0));
                  ...
                  if (x[0] > fieldTracingParameters.x_max || x[0] < fieldTracingParameters.x_min ||
                      x[1] > fieldTracingParameters.y_max || x[1] < fieldTracingParameters.y_min ||
   ...
   void stepCellAcrossTaskDomain(cint n, fsgrids::technicalspan technical, FieldSolverGrid &fsgrid,
      ...
      while (true) {
         // Check if the current coordinates (pre-step) are in our own domain.
         std::array<fsgrid::FsIndex_t, 3> fsgridCell = getLocalFsGridCellIndexForCoord(fsgrid, {(Real)x[0], (Real)x[1], (Real)x[2]});
         // If it is not in our domain, somebody else takes care of it.
         if (fsgridCell[0] == -1) {
   ...
   void traceFullBoxConnectionAndFluxRopes(fsgrids::technicalspan technical, FieldSolverGrid &fsgrid,
      ...
      const TReal maxTracingDistance = fieldTracingParameters.fullbox_and_fluxrope_max_distance > 0 ?
                                          ...
                                          gridSize[0]*fsgrid.getGridSpacing()[0] + gridSize[1]*fsgrid.getGridSpacing()[1] + gridSize[2]*fsgrid.getGridSpacing()[2];
      ...
      #pragma omp parallel shared(cellsToDoFullBox, cellsToDoFluxRopes)
      {
         do { // while(either leftover fraction is not achieved
            ...
            // Trace node coordinates forward and backwards until a
            // non-sysboundary cell is encountered or the local fsgrid domain
            // has been left.
            #pragma omp for schedule(dynamic)
            for (int n = 0; n < globalDccrgSize; n++) {
