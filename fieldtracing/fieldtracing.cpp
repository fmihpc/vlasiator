...
namespace FieldTracing {
    ...
    */
   void reduceData(FsGrid<fsgrids::technical, FS_STENCIL_WIDTH>& technicalGrid, FsGrid<std::array<Real, fsgrids::bfield::N_BFIELD>, FS_STENCIL_WIDTH>& perBGrid,
                   FsGrid<std::array<Real, fsgrids::dperb::N_DPERB>, FS_STENCIL_WIDTH>& dPerBGrid, dccrg::Dccrg<SpatialCell, dccrg::Cartesian_Geometry>& mpiGrid,
    ...
    */
   void calculateIonosphereFsgridCoupling(FsGrid<fsgrids::technical, FS_STENCIL_WIDTH>& technicalGrid, FsGrid<std::array<Real, fsgrids::bfield::N_BFIELD>, FS_STENCIL_WIDTH>& perBGrid,
                                          FsGrid<std::array<Real, fsgrids::dperb::N_DPERB>, FS_STENCIL_WIDTH>& dPerBGrid, std::vector<SBC::SphericalTriGrid::Node>& nodes, creal couplingRadius) {
...
// Trace node coordinates outwards until a non-sysboundary cell is encountered or the local fsgrid domain has been left.
            ...
            for (uint n = 0; n < nodes.size(); n++) {
               ...
               while (true) {
                  ...
                  stepFieldLine(
                      x, v, nodeTracingStepSize[n], fieldTracingParameters.min_tracer_dx_full_box, technicalGrid.DX / 2, fieldTracingParameters.tracingMethod, tracingFullField, (no.x[2] < 0));
                  ...
                  if (x[0] * x[0] + x[1] * x[1] + x[2] * x[2] > SBC::Ionosphere::downmapRadius * SBC::Ionosphere::downmapRadius) {
                     ...
                     const std::array<Real, 3> perB =
                         interpolatePerturbedB(perBGrid, dPerBGrid, technicalGrid, fieldTracingParameters.reconstructionCoefficientsCache, fsgridCell[0], fsgridCell[1], fsgridCell[2], no.xMapped);
                     ...
                     nodeDistance[n] =
                         sqrt((no.xMapped[0] - no.x[0]) * (no.xMapped[0] - no.x[0]) + (no.xMapped[1] - no.x[1]) * (no.xMapped[1] - no.x[1]) + (no.xMapped[2] - no.x[2]) * (no.xMapped[2] - no.x[2]));
                     nodeNeedsContinuedTracing[n] = 0;
                     nodeTracingCoordinates[n] = {0, 0, 0};
                     break;
                  }
   ...
   std::array<std::pair<int, Real>, 3> calculateIonosphereVlasovGridCoupling(std::array<Real, 3> x, std::vector<SBC::SphericalTriGrid::Node>& nodes, creal couplingRadius) {
      ...
      while (x[0] * x[0] + x[1] * x[1] + x[2] * x[2] > SBC::Ionosphere::innerRadius * SBC::Ionosphere::innerRadius) {
         ...
         stepFieldLine(x,
                       v,
                       stepSize,
                       fieldTracingParameters.min_tracer_dx_ionospere_coupling,
                       fieldTracingParameters.max_tracer_dx_ionospere_coupling,
      ...
      for (uint toto = 0; toto < 15; toto++) {
         ...
         if (override || (kappa1 > 0 && kappa2 > 0 && kappa3 > 0)) {
            ...
            cerr << "This fell through, strange."
                 << " kappas " << kappa1 << " " << kappa2 << " " << kappa3 << " handedness " << handedness << " r1 " << r1[0] << " " << r1[1] << " " << r1[2] << " r2 " << r2[0] << " " << r2[1] << " "
                 << r2[2] << " r3 " << r3[0] << " " << r3[1] << " " << r3[2] << " rx " << rx[0] << " " << rx[1] << " " << rx[2] << endl;
         }
         if (elementIndex == -1) {
            cerr << __FILE__ << ":" << __LINE__ << ": invalid elementIndex returned for coordinate " << x[0] << " " << x[1] << " " << x[2] << " projected to rx " << rx[0] << " " << rx[1] << " "
                 << rx[2] << ". Last valid elementIndex: " << oldElementIndex << "." << endl;
            return coupling;
         }
      }
      ...
      cerr << "(fieldtracing) Failed to find an ionosphere element to couple to for coordinate " << x[0] << " " << x[1] << " " << x[2] << endl;
      return coupling;
   }

   /*! Trace magnetic field lines out from ionospheric nodes to record whether they are on an open or closed field line.
    */
   void traceOpenClosedConnection(FsGrid<fsgrids::technical, FS_STENCIL_WIDTH>& technicalGrid, FsGrid<std::array<Real, fsgrids::bfield::N_BFIELD>, FS_STENCIL_WIDTH>& perBGrid,
                                  FsGrid<std::array<Real, fsgrids::dperb::N_DPERB>, FS_STENCIL_WIDTH>& dPerBGrid, std::vector<SBC::SphericalTriGrid::Node>& nodes) {
...
// Trace node coordinates outwards until a non-sysboundary cell is encountered or the local fsgrid domain has been left.
            ...
            for (uint n = 0; n < nodes.size(); n++) {
               ...
               while (true) {
                  ...
                  stepFieldLine(x,
                                ...
                                (TReal)fieldTracingParameters.min_tracer_dx_full_box,
                  ...
                  if (x[0] > fieldTracingParameters.x_max || x[0] < fieldTracingParameters.x_min || x[1] > fieldTracingParameters.y_max || x[1] < fieldTracingParameters.y_min ||
                      x[2] > fieldTracingParameters.z_max || x[2] < fieldTracingParameters.z_min) {
                     nodeNeedsContinuedTracing[n] = 0;
                     nodeTracingCoordinates[n] = {0, 0, 0};
    ...
    */
   void stepCellAcrossTaskDomain(cint n, FsGrid<fsgrids::technical, FS_STENCIL_WIDTH>& technicalGrid, TracingFieldFunction<TReal>& tracingFullField,
                                 const std::vector<std::array<TReal, 3>>& cellInitialCoordinates, const std::vector<TReal>& cellCurvatureRadius,
                                 std::vector<std::array<TReal, 3>>& cellTracingCoordinates, std::vector<TReal>& cellTracingStepSize, std::vector<TReal>& cellRunningDistance,
                                 std::vector<TReal>& cellMaxExtension, std::vector<signed char>& cellConnection, bool& warnMaxDistanceExceeded, const TReal maxTracingDistance, cuint DIRECTION) {
      ...
      while (true) {
         ...
         if (x.at(0) * x.at(0) + x.at(1) * x.at(1) + x.at(2) * x.at(2) < fieldTracingParameters.innerBoundaryRadius * fieldTracingParameters.innerBoundaryRadius) {
            ...
            Real r_in = sqrt(cellTracingCoordinates[n][0] * cellTracingCoordinates[n][0] + cellTracingCoordinates[n][1] * cellTracingCoordinates[n][1] +
         ...
         if (cellConnection[n] < TracingLineEndType::N_TYPES) {
            const TReal extension =
                sqrt((x[0] - (cellInitialCoordinates[n])[0]) * (x[0] - (cellInitialCoordinates[n])[0]) + (x[1] - (cellInitialCoordinates[n])[1]) * (x[1] - (cellInitialCoordinates[n])[1]) +
    ...
    */
   void traceFullBoxConnectionAndFluxRopes(FsGrid<fsgrids::technical, FS_STENCIL_WIDTH>& technicalGrid, FsGrid<std::array<Real, fsgrids::bfield::N_BFIELD>, FS_STENCIL_WIDTH>& perBGrid,
                                           FsGrid<std::array<Real, fsgrids::dperb::N_DPERB>, FS_STENCIL_WIDTH>& dPerBGrid, dccrg::Dccrg<SpatialCell, dccrg::Cartesian_Geometry>& mpiGrid) {
      ...
      std::array<FsGridTools::FsSize_t, 3> gridSize = technicalGrid.getGlobalSize();
      // If fullbox_and_fluxrope_max_distance is unset, use this heuristic considering how far an IMF+dipole combo can sensibly stretch in the box before we're safe to assume it's rolled up more or
      // less pathologically.
      const TReal maxTracingDistance = fieldTracingParameters.fullbox_and_fluxrope_max_distance > 0 ? fieldTracingParameters.fullbox_and_fluxrope_max_distance
                                                                                                    : gridSize[0] * technicalGrid.DX + gridSize[1] * technicalGrid.DY + gridSize[2] * technicalGrid.DZ;
      ...
      for (int n = 0; n < globalDccrgSize; n++) {
         ...
         if (mpiGrid.is_local(id)) {
            if ((mpiGrid[id]->sysBoundaryFlag != sysboundarytype::NOT_SYSBOUNDARY) || cellFWTracingCoordinates[n][0] > fieldTracingParameters.x_max ||
                cellFWTracingCoordinates[n][0] < fieldTracingParameters.x_min || cellFWTracingCoordinates[n][1] > fieldTracingParameters.y_max ||
                cellFWTracingCoordinates[n][1] < fieldTracingParameters.y_min || cellFWTracingCoordinates[n][2] > fieldTracingParameters.z_max ||
               ...
               cellCurvatureRadius[n] = 1 / sqrt(mpiGrid[id]->parameters[CellParams::CURVATUREX] * mpiGrid[id]->parameters[CellParams::CURVATUREX] +
...
// Trace node coordinates forward and backwards until a non-sysboundary cell is encountered or the local fsgrid domain has been left.
            ...
            #pragma omp master
            {
               ...
               for (int n = 0; n < globalDccrgSize; n++) {
                  if (storedCellFWConnection[n] % TracingLineEndType::N_TYPES ==
                      TracingLineEndType::UNPROCESSED) { // This still has the allreduce MPI_SUM results from the previous round, so that all globally continuing cells are accounted for.
      ...
      logFile << "(fieldtracing) combined flux rope + full box tracing traced in " << itCount << " iterations of the tracing loop with flux rope " << cellsToDoFluxRopes << ", full box "
              << cellsToDoFullBox << " remaining incomplete field lines (total spatial cells " << globalDccrgSize << ")." << endl;
      ...
      for (int n = 0; n < globalDccrgSize; n++) {
         ...
         if (mpiGrid.is_local(id)) {
            ...
            if (cellFWConnection[n] >= 2 * TracingLineEndType::N_TYPES && cellBWConnection[n] >= 2 * TracingLineEndType::N_TYPES) {
