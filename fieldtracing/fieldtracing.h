...
template <class T>
std::array<int32_t, 3> getLocalFsGridCellIndexWithGhostsForCoord(T& grid, const std::array<Real, 3>& x) {
   std::array<int32_t, 3> retval = getGlobalFsGridCellIndexForCoord(grid, x);
...
bool traceFullFieldFunction(FsGrid<std::array<Real, fsgrids::bfield::N_BFIELD>, FS_STENCIL_WIDTH>& perBGrid,
   ...
   std::array<int32_t, 3> fsgridCell =
   ...
   if (std::isnan(b[0]) || std::isnan(b[1]) || std::isnan(b[2])) {
      cerr << "(fieldtracing) Error: magnetic field is nan in getRadialBfieldDirection at location " << r[0] << ", "
