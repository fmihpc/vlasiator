...
template <class T> std::array<FsGridTools::FsIndex_t, 3> getLocalFsGridCellIndexWithGhostsForCoord(T& grid, const std::array<Real, 3>& x) {
   ...
   std::array<FsGridTools::FsIndex_t, 3> retval = {
       (FsGridTools::FsIndex_t)globalInd[0] - localStart[0], (FsGridTools::FsIndex_t)globalInd[1] - localStart[1], (FsGridTools::FsIndex_t)globalInd[2] - localStart[2]};
   return retval;
}
...
namespace FieldTracing {
   ...
   bool traceFullFieldFunction(FsGrid<std::array<Real, fsgrids::bfield::N_BFIELD>, FS_STENCIL_WIDTH>& perBGrid, FsGrid<std::array<Real, fsgrids::dperb::N_DPERB>, FS_STENCIL_WIDTH>& dPerBGrid,
                               FsGrid<fsgrids::technical, FS_STENCIL_WIDTH>& technicalGrid, std::array<REAL, 3>& r, const bool alongB, std::array<REAL, 3>& b) {
      if (r[0] > P::xmax - 2 * P::dx_ini || r[0] < P::xmin + 2 * P::dx_ini || r[1] > P::ymax - 2 * P::dy_ini || r[1] < P::ymin + 2 * P::dy_ini || r[2] > P::zmax - 2 * P::dz_ini ||
      ...
      if (fsgridCell[0] > localSize[0] || fsgridCell[1] > localSize[1] || fsgridCell[2] > localSize[2] || fsgridCell[0] < -1 || fsgridCell[1] < -1 || fsgridCell[2] < -1) {
         cerr << (string)("(fieldtracing) Error: fsgrid coupling trying to access local ID " + to_string(fsgridCell[0]) + " " + to_string(fsgridCell[1]) + " " + to_string(fsgridCell[2]) +
                          " for local domain size " + to_string(localSize[0]) + " " + to_string(localSize[1]) + " " + to_string(localSize[2]) + " at position " + to_string(r[0]) + " " +
                          to_string(r[1]) + " " + to_string(r[2]) + " radius " + to_string(sqrt(r[0] * r[0] + r[1] * r[1] + r[2] * r[2])) + "\n");
         abort();
         return false;
      } else {
         if (technicalGrid.get(fsgridCell[0], fsgridCell[1], fsgridCell[2])->sysBoundaryFlag == sysboundarytype::NOT_SYSBOUNDARY) {
            const std::array<Real, 3> perB = interpolatePerturbedB(
                perBGrid, dPerBGrid, technicalGrid, fieldTracingParameters.reconstructionCoefficientsCache, fsgridCell[0], fsgridCell[1], fsgridCell[2], {(Real)r[0], (Real)r[1], (Real)r[2]});
            b[0] += perB[0];
            b[1] += perB[1];
            b[2] += perB[2];
         }
      ...
      if (!(std::isfinite(b[0]) && std::isfinite(b[1]) && std::isfinite(b[2]))) {
         cerr << "(fieldtracing) Error: magnetic field is nan or inf in getRadialBfieldDirection at location " << r[0] << ", " << r[1] << ", " << r[2] << ", with B = " << b[0] << ", " << b[1] << ", "
              << b[2] << endl;
         b[0] = 0;
         b[1] = 0;
         b[2] = 0;
    ...
    * */
   template <typename REAL> void modifiedMidpointMethod(std::array<REAL, 3> r, std::array<REAL, 3>& r1, int n, REAL stepSize, TracingFieldFunction<REAL>& BFieldFunction, const bool outwards = true) {
   ...
   template <typename REAL> void richardsonExtrapolation(int i, std::vector<REAL>& table, REAL& maxError, std::array<int, 3> dims) {
      int k;
      maxError = 0;
      for (int dim = 0; dim < 3; dim++) {
         for (k = 1; k < i + 1; k++) {
            ...
            table.at(ijk2Index(i, k, dim, dims)) =
                table.at(ijk2Index(i, k - 1, dim, dims)) + (table.at(ijk2Index(i, k - 1, dim, dims)) - table.at(ijk2Index(i - 1, k - 1, dim, dims))) / (std::pow(4, i) - 1);
         }
   ...
   template <typename REAL>
   bool bulirschStoerStep(std::array<REAL, 3>& r, std::array<REAL, 3>& b, REAL& stepSize, const REAL minStepSize, const REAL maxStepSize, TracingFieldFunction<REAL>& BFieldFunction,
   ...
   template <typename REAL>
   bool dormandPrinceStep(std::array<REAL, 3>& r, std::array<REAL, 3>& b, REAL& stepSize, const REAL minStepSize, const REAL maxStepSize, TracingFieldFunction<REAL>& BFieldFunction,
                          const bool outwards = true) {
   ...
   template <typename REAL>
   bool adaptiveEulerStep(std::array<REAL, 3>& r, std::array<REAL, 3>& b, REAL& stepSize, const REAL minStepSize, const REAL maxStepSize, TracingFieldFunction<REAL>& BFieldFunction,
                          const bool outwards = true) {
      // First evaluation
      std::array<REAL, 3> r1;
      BFieldFunction(r, outwards, b);
   ...
   template <typename REAL> void eulerStep(std::array<REAL, 3>& x, std::array<REAL, 3>& v, REAL& stepSize, TracingFieldFunction<REAL>& BFieldFunction, const bool outwards = true) {
      // Get field direction
   ...
   template <typename REAL>
   void stepFieldLine(std::array<REAL, 3>& x, std::array<REAL, 3>& v, REAL& stepsize, const REAL minStepSize, const REAL maxStepSize, TracingMethod method, TracingFieldFunction<REAL>& BFieldFunction,
                      const bool outwards) {
      bool reTrace;
      uint32_t attempts = 0;
      switch (method) {
   ...
   void calculateIonosphereFsgridCoupling(FsGrid<fsgrids::technical, FS_STENCIL_WIDTH>& technicalGrid, FsGrid<std::array<Real, fsgrids::bfield::N_BFIELD>, FS_STENCIL_WIDTH>& perBGrid,
                                          FsGrid<std::array<Real, fsgrids::dperb::N_DPERB>, FS_STENCIL_WIDTH>& dPerBGrid, std::vector<SBC::SphericalTriGrid::Node>& nodes, creal radius);
