...
namespace vlsvinterface {
   ...
   public:
      ...
      // Reads in a variable:
      template <typename T, size_t N> bool getVariable(const std::string& variableName, const uint64_t& cellId, std::array<T, N>& variable);
   ...
   template <typename T, size_t N> inline bool Reader::getVariable(const std::string& variableName, const uint64_t& cellId, std::array<T, N>& variable) {
      if (cellIdsSet == false) {
