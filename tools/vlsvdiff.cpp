   ...
   for (int i = 1; i <= std::min(nProcs, GlobalSize[0]); i++) {
      ...
      for (int j = 1; j <= std::min(nProcs, GlobalSize[1]); j++) {
         ...
         for (int k = 1; k <= std::min(nProcs, GlobalSize[2]); k++) {
            ...
            double value = 10 * processBox[0] * processBox[1] * processBox[2] +
                           (i > 1 ? processBox[1] * processBox[2] : 0) + (j > 1 ? processBox[0] * processBox[2] : 0) +
...
bool process2Files(const string fileName1,
   ...
   if (strcmp(varToExtract, "proton") == 0 && attributes.find("--no-distrib") == attributes.end()) {
      ...
      bool success = true;
      success =
          convertSILO<vlsvinterface::Reader>(fileName1, varToExtract, compToExtract, &orderedData1, cellOrder, true);
      ...
      success =
          convertSILO<vlsvinterface::Reader>(fileName2, varToExtract, compToExtract, &orderedData2, cellOrder, false);
