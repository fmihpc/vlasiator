...
template <class T> bool convertMeshVariable(T& vlsvReader, const string& meshName, const string& varName) {
   ...
   if (DBPutUcdvar(fileptr, varName.c_str(), meshName.c_str(), vectorSize, &(varNamePtrs[0]), components, arraySize, NULL, 0, SiloType(dataType, dataSize), DB_ZONECENT, NULL) < 0)
      success = false;
...
bool convertMesh(vlsvinterface::Reader& vlsvReader, const string& meshName) {
   ...
   if (DBPutZonelist2(fileptr, zoneListName.c_str(), N_zones, N_dims, nodelist, 8 * cellIds.size(), 0, 0, 0, shapeTypes, shapeSizes, shapeCnt, N_shapes, NULL) < 0)
      success = false;

   // Write grid into silo file:
   if (DBPutUcdmesh(fileptr, meshName.c_str(), N_dims, NULL, coords, N_nodes, N_zones, zoneListName.c_str(), NULL, SiloType(datatype::type::FLOAT, sizeof(Real)), NULL) < 0)
      success = false;
   ...
   if (DBPutUcdvar1(fileptr, "CellID", meshName.c_str(), reinterpret_cast<char*>(cellIds.data()), cellIds.size(), NULL, 0, SiloType(datatype::type::UINT, sizeof(uint64_t)), DB_ZONECENT, NULL) < 0)
      success = false;
   //   delete buffer;
