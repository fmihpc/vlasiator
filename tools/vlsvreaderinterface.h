...
// Returns the vlsv file's version number. Returns 0 if the version does not have a version mark (The old vlsv format
// does not have it)
// Input: File name
...
public:
   ...
   inline uint64_t getBlockOffset(const uint64_t& cellId) {
      ...
      std::unordered_map<uint64_t, std::pair<uint64_t, uint32_t>>::const_iterator it =
...
template <typename T, size_t N>
inline bool Reader::getVariable(const std::string& variableName, const uint64_t& cellId, std::array<T, N>& variable) {
   if (cellIdsSet == false) {
   ...
   if (getArrayInfo("VARIABLE", xmlAttributes, arraySize, vectorSize, dataType, byteSize) == false)
      return false;
   if (vectorSize != N) {
   ...
   if (readArray("VARIABLE", xmlAttributes, begin, amountToReadIn, buffer) == false)
      return false;
