...
void setCellVariables(vlsvinterface::Reader& vlsvReader, CellStructure& cellStruct) {
   ...
   // hence the cast) O: Note: Not actually sure if these are Real valued or not
   ...
   for (int i = 0; i < 3; ++i) {
      if (cellStruct.cell_length[i] == 0 || cellStruct.cell_bounds[i] == 0 || cellStruct.vblock_length[i] == 0 ||
          cellStruct.vcell_bounds[i] == 0) {
...
template <typename T> uint64_t convUInt(const char* ptr, const T& dataType, const uint64_t& dataSize) {
...
template <class T> bool convertMeshVariable(T& vlsvReader, const string& meshName, const string& varName) {
   bool success = true;
   ...
   if (vlsvReader.getArrayInfo("VARIABLE", xmlAttributes, arraySize, vectorSize, dataType, dataSize) == false)
      return false;
   ...
   if (vlsvReader.readArray("VARIABLE", xmlAttributes, startingIndex, arraySize, buffer) == false)
      success = false;
   if (success == false) {
...
bool convertMesh(vlsvinterface::Reader& vlsvReader, const string& meshName) {
   ...
   for (vector<uint64_t>::const_iterator it = cellIds.begin(); it != cellIds.end(); ++it, ++i) {
      ...
      it2 = nodes.find(NodeCrd<Real>(X0, Y0, Z0));
      if (it2 == nodes.end())
         success = false;
      nodelist[i * 8 + 0] = it2->second;
      it2 = nodes.find(NodeCrd<Real>(X1, Y0, Z0));
      if (it2 == nodes.end())
         success = false;
      nodelist[i * 8 + 1] = it2->second;
      it2 = nodes.find(NodeCrd<Real>(X1, Y1, Z0));
      if (it2 == nodes.end())
         success = false;
      nodelist[i * 8 + 2] = it2->second;
      it2 = nodes.find(NodeCrd<Real>(X0, Y1, Z0));
      if (it2 == nodes.end())
         success = false;
      nodelist[i * 8 + 3] = it2->second;
      it2 = nodes.find(NodeCrd<Real>(X0, Y0, Z1));
      if (it2 == nodes.end())
         success = false;
      nodelist[i * 8 + 4] = it2->second;
      it2 = nodes.find(NodeCrd<Real>(X1, Y0, Z1));
      if (it2 == nodes.end())
         success = false;
      nodelist[i * 8 + 5] = it2->second;
      it2 = nodes.find(NodeCrd<Real>(X1, Y1, Z1));
      if (it2 == nodes.end())
         success = false;
      nodelist[i * 8 + 6] = it2->second;
      it2 = nodes.find(NodeCrd<Real>(X0, Y1, Z1));
      if (it2 == nodes.end())
         success = false;
      nodelist[i * 8 + 7] = it2->second;
   }
   // O: REMOVE THIS
   // delete coordsBuffer;
   if (success == false) {
   ...
   for (list<string>::const_iterator it = variables.begin(); it != variables.end(); ++it) {
      if (convertMeshVariable(vlsvReader, meshName, *it) == false)
         success = false;
   }
   return success;
}

template <class T> bool convertSILO(const string& fname) {
   bool success = true;
...
int main(int argn, char* args[]) {
   ...
   for (size_t mask = 0; mask < masks.size(); ++mask) {
      ...
      if (rank == 0) {
         cout << "Comparing mask '" << maskName << "' in folder '" << directory << "'" << endl;
      }
   ...
   if (rank == 0 && totalFilesConverted == 0)
      cout << "\t no files converted" << endl;
