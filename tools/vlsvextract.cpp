...
template <typename REAL> inline NodeCrd<REAL>::NodeCrd(const REAL& x, const REAL& y, const REAL& z) : x(x), y(y), z(z) {}

template <typename REAL> inline bool NodeCrd<REAL>::comp(const NodeCrd<REAL>& n) const {
   REAL EPS1, EPS2, EPS;
   EPS1 = 1.0e-6 * fabs(x);
