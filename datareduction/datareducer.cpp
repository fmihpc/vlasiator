...
void initializeDataReducers(DataReducer * outputReducer, DataReducer * diagnosticReducer)
{
   ...
   for (it = P::outputVariableList.begin();
      ...
      */
      ...
      if(P::systemWriteAllDROs || lowercase == "fg_e_hyper") { // Hyperresistivity electric field at Yee-lattice locations
         outputReducer->addOperator(new DRO::DataReductionOperatorFsGrid("fg_e_hyper",[](
                      FsGrid< std::array<Real, fsgrids::bfield::N_BFIELD>, FS_STENCIL_WIDTH> & perBGrid,
                      FsGrid< std::array<Real, fsgrids::efield::N_EFIELD>, FS_STENCIL_WIDTH> & EGrid,
                      FsGrid< std::array<Real, fsgrids::ehall::N_EHALL>, FS_STENCIL_WIDTH> & EHallGrid,
                      FsGrid< std::array<Real, fsgrids::egradpe::N_EGRADPE>, FS_STENCIL_WIDTH> & EGradPeGrid,
               ...
               // Iterate through fsgrid cells and extract Ehyper values
               for(int z=0; z<gridSize[2]; z++) {
                  for(int y=0; y<gridSize[1]; y++) {
                     for(int x=0; x<gridSize[0]; x++) {
                        retval[3*(gridSize[1]*gridSize[0]*z + gridSize[0]*y + x)] =     (*EHyperGrid.get(x,y,z))[fsgrids::EXHYPER];
                        retval[3*(gridSize[1]*gridSize[0]*z + gridSize[0]*y + x) + 1] = (*EHyperGrid.get(x,y,z))[fsgrids::EYHYPER];
                        retval[3*(gridSize[1]*gridSize[0]*z + gridSize[0]*y + x) + 2] = (*EHyperGrid.get(x,y,z))[fsgrids::EZHYPER];
                     }
                  }
               }
               return retval;
         ...
         outputReducer->addMetadata(outputReducer->size()-1,"V/m","$\\mathrm{V}\\,\\mathrm{m}^{-1}$","$E_\\mathrm{hyper}$","1.0");
         if(!P::systemWriteAllDROs) {
            continue;
         }
      }
...
bool DataReducer::writeFsGridData(
   ...
   if(!DROf) {
      ...
      return DROf->writeFsGridData(perBGrid, EGrid, EHallGrid, EGradPeGrid, EHyperGrid, momentsGrid, dPerBGrid, dMomentsGrid, BgBGrid, volGrid, technicalGrid, meshName, vlsvWriter, writeAsFloat);
   }
}
