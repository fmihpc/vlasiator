...
bool DataReductionOperator::reduceData(const SpatialCell* cell, char* buffer) {
   cerr << "ERROR: DataReductionOperator::reduceData called instead of derived class function! (variable" << getName()
        << ")" << endl;
...
DataReductionOperatorCellParams::DataReductionOperatorCellParams(const std::string& name,
                                                                 const unsigned int parameterIndex,
                                                                 const unsigned int _vectorSize)
...
bool DataReductionOperatorCellParams::setSpatialCell(const SpatialCell* cell) {
   for (uint i = 0; i < vectorSize; i++) {
      if (std::isinf(cell->parameters[_parameterIndex + i]) || std::isnan(cell->parameters[_parameterIndex + i])) {
         string message = "The DataReductionOperator " + this->getName() + " returned a nan or an inf in its " +
...
bool DataReductionOperatorIonosphereElement::writeIonosphereData(SBC::SphericalTriGrid& grid,
                                                                 vlsv::Writer& vlsvWriter) {
...
bool DataReductionOperatorIonosphereNodeInt::writeIonosphereData(SBC::SphericalTriGrid& grid,
                                                                 vlsv::Writer& vlsvWriter) {
...
DataReductionOperatorBVOLDerivatives::DataReductionOperatorBVOLDerivatives(const std::string& name,
                                                                           const unsigned int parameterIndex,
...
bool VariableBVol::setSpatialCell(const SpatialCell* cell) {
   ...
   if (std::isinf(B[0]) || std::isnan(B[0]) || std::isinf(B[1]) || std::isnan(B[1]) || std::isinf(B[2]) ||
       std::isnan(B[2])) {
...
bool VariablePressureSolver::setSpatialCell(const SpatialCell* cell) {
   Pressure =
       1.0 / 3.0 *
       (cell->parameters[CellParams::P_11] + cell->parameters[CellParams::P_22] + cell->parameters[CellParams::P_33]);
   return true;
}
         ...
         for (uint k = 0; k < WID; ++k)
            for (uint j = 0; j < WID; ++j)
               for (uint i = 0; i < WID; ++i) {
                  ...
                  thread_nvxvx_sum +=
                      block_data[n * SIZE_VELBLOCK + cellIndex(i, j, k)] * (VX - averageVX) * (VX - averageVX) * DV3;
                  thread_nvyvy_sum +=
                      block_data[n * SIZE_VELBLOCK + cellIndex(i, j, k)] * (VY - averageVY) * (VY - averageVY) * DV3;
                  thread_nvzvz_sum +=
                      block_data[n * SIZE_VELBLOCK + cellIndex(i, j, k)] * (VZ - averageVZ) * (VZ - averageVZ) * DV3;
               }
      }
         ...
         for (uint k = 0; k < WID; ++k)
            for (uint j = 0; j < WID; ++j)
               for (uint i = 0; i < WID; ++i) {
                  ...
                  thread_nvxvy_sum +=
                      block_data[n * SIZE_VELBLOCK + cellIndex(i, j, k)] * (VX - averageVX) * (VY - averageVY) * DV3;
                  thread_nvzvx_sum +=
                      block_data[n * SIZE_VELBLOCK + cellIndex(i, j, k)] * (VZ - averageVZ) * (VX - averageVX) * DV3;
                  thread_nvyvz_sum +=
                      block_data[n * SIZE_VELBLOCK + cellIndex(i, j, k)] * (VY - averageVY) * (VZ - averageVZ) * DV3;
               }
      }
...
bool MaxDistributionFunction::setSpatialCell(const SpatialCell* cell) { return true; }
...
bool MinDistributionFunction::setSpatialCell(const SpatialCell* cell) { return true; }
...
static void getNonthermalVelocityCells(const Real* block_parameters, vector<uint64_t>& vCellIds, cuint popID) {
   creal HALF = 0.5;
   ...
   for (uint k = 0; k < WID; ++k)
      for (uint j = 0; j < WID; ++j)
         for (uint i = 0; i < WID; ++i) {
            ...
            // Compare the distance of the velocity cell from the center of the maxwellian distribution to the radius of
            // the maxwellian distribution
            if (((thermalV[0] - VX) * (thermalV[0] - VX) + (thermalV[1] - VY) * (thermalV[1] - VY) +
                 (thermalV[2] - VZ) * (thermalV[2] - VZ)) > thermalRadius * thermalRadius) {
...
// Helper function for getting the velocity cell ids that are a part of the nonthermal population:
static void getThermalVelocityCells(const Real* block_parameters, vector<uint64_t>& vCellIds, cuint popID) {
   creal HALF = 0.5;
   const std::array<Real, 3> thermalV = getObjectWrapper().particleSpecies[popID].thermalV;
   creal thermalRadius = getObjectWrapper().particleSpecies[popID].thermalRadius;
   for (uint k = 0; k < WID; ++k)
...
getNonthermalVelocityCellIndices(const Real* block_parameters, vector<array<uint, 3>>& vCellIndices, cuint popID) {
   ...
   for (uint k = 0; k < WID; ++k)
      for (uint j = 0; j < WID; ++j)
         for (uint i = 0; i < WID; ++i) {
            // Get the coordinates of the velocity cell (e.g. VX = block_vx_min_coordinates +
            // (velocity_cell_indice_x+0.5)*length_of_velocity_cell_in_x_direction
            const Real VX = block_parameters[BlockParams::VXCRD] + (i + HALF) * block_parameters[BlockParams::DVX];
            const Real VY = block_parameters[BlockParams::VYCRD] + (j + HALF) * block_parameters[BlockParams::DVY];
            const Real VZ = block_parameters[BlockParams::VZCRD] + (k + HALF) * block_parameters[BlockParams::DVZ];
            // Calculate the distance of the velocity cell from the center of the maxwellian distribution and compare it
            // to the approximate radius of the maxwellian distribution
            if (((thermalV[0] - VX) * (thermalV[0] - VX) + (thermalV[1] - VY) * (thermalV[1] - VY) +
                 (thermalV[2] - VZ) * (thermalV[2] - VZ)) > thermalRadius * thermalRadius) {
...
#pragma omp for
      for (vmesh::LocalID n = 0; n < cell->get_number_of_velocity_blocks(popID); ++n) {
         ...
         for (vector<array<uint, 3>>::const_iterator it = vCellIndices.begin(); it != vCellIndices.end(); ++it) {
            ...
            const Real VZ = parameters[n * BlockParams::N_VELOCITY_BLOCK_PARAMS + BlockParams::VZCRD] +
                            (k + HALF) * parameters[n * BlockParams::N_VELOCITY_BLOCK_PARAMS + BlockParams::DVZ];
            // Add the value of the coordinates and multiply by the AVGS value of the velocity cell and the volume of
            // the velocity cell
...
VariablePTensorNonthermalDiagonal::VariablePTensorNonthermalDiagonal(cuint _popID)
    : DataReductionOperator(), popID(_popID) {
   popName = getObjectWrapper().particleSpecies[popID].name;
   doSkip = (getObjectWrapper().particleSpecies[popID].thermalRadius == 0.0) ? true : false;
}
...
VariablePTensorNonthermalOffDiagonal::VariablePTensorNonthermalOffDiagonal(cuint _popID)
    : DataReductionOperator(), popID(_popID) {
   popName = getObjectWrapper().particleSpecies[popID].name;
   doSkip = (getObjectWrapper().particleSpecies[popID].thermalRadius == 0.0) ? true : false;
}
...
VariablePTensorThermalOffDiagonal::VariablePTensorThermalOffDiagonal(cuint _popID)
    : DataReductionOperator(), popID(_popID) {
   popName = getObjectWrapper().particleSpecies[popID].name;
   doSkip = (getObjectWrapper().particleSpecies[popID].thermalRadius == 0.0) ? true : false;
}
...
VariableEffectiveSparsityThreshold::VariableEffectiveSparsityThreshold(cuint _popID)
    : DataReductionOperator(), popID(_popID) {
   popName = getObjectWrapper().particleSpecies[popID].name;
}
 ...
 * lossConeAngle [deg] The energy channels are saved in bulk files as PrecipitationCentreEnergy{channel_number}.
 */
VariablePrecipitationDiffFlux::VariablePrecipitationDiffFlux(cuint _popID)
    : DataReductionOperatorHasParameters(), popID(_popID) {
   popName = getObjectWrapper().particleSpecies[popID].name;
   ...
   nChannels = getObjectWrapper()
                   .particleSpecies[popID]
                   .precipitationNChannels; // number of energy channels, logarithmically spaced between emin and emax
         ...
         for (uint k = 0; k < WID; ++k)
            for (uint j = 0; j < WID; ++j)
               for (uint i = 0; i < WID; ++i) {
                  ...
                  thread_lossCone_sum[binNumber] +=
                      block_data[n * SIZE_VELBLOCK + cellIndex(i, j, k)] * countAndGate * normV * normV * DV3;
...
#pragma omp critical
   ...
   for (int i = 0; i < nChannels; i++) {
      if (sumWeights[i] != 0) {
         dataDiffFlux[i] *= 1.0 / (getObjectWrapper().particleSpecies[popID].mass * sumWeights[i]) *
                            physicalconstants::CHARGE * 1.0e-4;
      }
   }
 ...
 */
VariablePrecipitationLineDiffFlux::VariablePrecipitationLineDiffFlux(cuint _popID)
    : DataReductionOperatorHasParameters(), popID(_popID) {
   popName = getObjectWrapper().particleSpecies[popID].name;
   emin = getObjectWrapper().particleSpecies[popID].precipitationEmin; // already converted to SI
   emax = getObjectWrapper().particleSpecies[popID].precipitationEmax; // already converted to SI
         ...
         for (uint k = 0; k < WID; ++k)
            for (uint j = 0; j < WID; ++j)
               for (uint i = 0; i < WID; ++i) {
                  ...
                  // We will use a gate function based on criteria that Vi-0.5*DVi <= BnormV[i] <= Vi+0.5*DVi (for
                  // i=x,y,z or 0,1,2)
                  ...
                  bool xyzGateCrit = xGateCrit && yGateCrit &&
                                     zGateCrit; // gate function: 1 if the line goes through the v-cell, else 0.
...
#pragma omp critical
   ...
   for (int i = 0; i < nChannels; i++) {
      if (sumWeights[i] != 0) {
         dataLineDiffFlux[i] *= 1.0 / (getObjectWrapper().particleSpecies[popID].mass * sumWeights[i]) *
                                physicalconstants::CHARGE * 1.0e-4;
      }
   }
...
bool VariablePrecipitationLineDiffFlux::writeParameters(vlsv::Writer& vlsvWriter) {
   for (int i = 0; i < nChannels; i++) {
      const Real channelev = channels[i] / physicalconstants::CHARGE; // in eV
      if (vlsvWriter.writeParameter(popName + "_PrecipitationCentreEnergyLine" + std::to_string(i), &channelev) ==
          false) {
         return false;
      }
   }
 ...
 * Calculates the energy density of particles in three bins: total energy density, above E1limit*solar wind energy, and
 * above E2limit*solar wind energy Energy densities are given in eV/cm^3. Parameters that can be set in cfg file under
 * [{species}_energydensity]:
 *    - solarwindspeed [m/s],
 *    - solarwindenergy [eV],
         ...
         for (uint k = 0; k < WID; ++k)
            for (uint j = 0; j < WID; ++j)
               for (uint i = 0; i < WID; ++i) {
                  ...
                  const Real ENERGY =
                      (VX * VX + VY * VY + VZ * VZ) * HALF * getObjectWrapper().particleSpecies[popID].mass;
                  thread_E0_sum += block_data[n * SIZE_VELBLOCK + cellIndex(i, j, k)] * ENERGY * DV3;
                  if (ENERGY > E1limit)
                     thread_E1_sum += block_data[n * SIZE_VELBLOCK + cellIndex(i, j, k)] * ENERGY * DV3;
                  if (ENERGY > E2limit)
                     thread_E2_sum += block_data[n * SIZE_VELBLOCK + cellIndex(i, j, k)] * ENERGY * DV3;
               }
      }
...
bool VariableEnergyDensity::setSpatialCell(const SpatialCell* cell) { return true; }
            ...
            for (uint j = 0; j < WID; ++j)
               for (uint i = 0; i < WID; ++i) {
                  ...
                  const Real DV3 = parameters[n * BlockParams::N_VELOCITY_BLOCK_PARAMS + BlockParams::DVX] *
                                   parameters[n * BlockParams::N_VELOCITY_BLOCK_PARAMS + BlockParams::DVY] *
                                   parameters[n * BlockParams::N_VELOCITY_BLOCK_PARAMS + BlockParams::DVZ];
                  const Real VSQ = (VX - averageVX) * (VX - averageVX) + (VY - averageVY) * (VY - averageVY) +
