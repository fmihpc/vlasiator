...
namespace DRO {
   ...
   static void
   PTensorDiagonalNonthermalCalculations(const SpatialCell* cell, const bool calculateNonthermal, const Real averageVX, const Real averageVY, const Real averageVZ, cuint popID, Real* PTensor) {
      creal HALF = 0.5;
      const std::array<Real, 3> thermalV = getObjectWrapper().particleSpecies[popID].thermalV;
      creal thermalRadius = getObjectWrapper().particleSpecies[popID].thermalRadius;
   ...
   static void
   PTensorOffDiagonalNonthermalCalculations(const SpatialCell* cell, const bool calculateNonthermal, const Real averageVX, const Real averageVY, const Real averageVZ, cuint popID, Real* PTensor) {
      creal HALF = 0.5;
      const std::array<Real, 3> thermalV = getObjectWrapper().particleSpecies[popID].thermalV;
      creal thermalRadius = getObjectWrapper().particleSpecies[popID].thermalRadius;
