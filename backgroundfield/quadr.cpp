...
double Romberg(const T1DFunction& func, double a, double b, double absacc) {
   ...
   for (j = 1; j <= jmax; j++) {
      ...
      if (j >= min_k) {
         ...
         if (dresult_polrat > dresult)
            dresult = dresult_polrat;
         if (dresult < absacc) {
...
double Romberg(const T2DFunction& func, double a, double b, double c, double d, double absacc) {
   ...
   T1DFunction Tinty = [=](double x) -> double { return Romberg(std::bind(func, x, std::placeholders::_1), c, d, absacc / (b - a)); };
...
double Romberg(const T3DFunction& func, double a, double b, double c, double d, double e, double f, double absacc) {
   T1DFunction Tintxy = [=](double z) -> double { return Romberg(std::bind(func, std::placeholders::_1, std::placeholders::_2, z), a, b, c, d, absacc / (f - e)); };
