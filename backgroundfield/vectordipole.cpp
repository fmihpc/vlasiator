...
void VectorDipole::initialize(const double moment, const double center_x, const double center_y, const double center_z, const double tilt_angle_phi, const double tilt_angle_theta,
                              const double xlimit_f, const double xlimit_z, const double IMF_Bx, const double IMF_By, const double IMF_Bz) {
...
double VectorDipole::operator()(double x, double y, double z, coordinate component, unsigned int derivative, coordinate dcomponent) const {
   ...
   if ((derivative == 1) && (r[0] <= xlimit[0])) {
      ...
      return -5 * B * r[dcomponent] / r2 + (3 * q[dcomponent] * r[component] - 2 * q[component] * r[dcomponent] + 3 * rdotq * sameComponent) / r5;
   }
   ...
   if (derivative == 0) {
      ...
      */
      ...
      const double delB = -5 * B * r[dcomponent] / r2 + (3 * q[dcomponent] * r[component] - 2 * q[component] * r[dcomponent] + 3 * rdotq * sameComponent) / r5;
      ...
      */
      ...
      return S2 * delB + dS2cart[dcomponent] * B + ddS2crossA[component][dcomponent] + IMFS2 * IMFdelB + IMFdS2cart[dcomponent] * IMFB + IMFddS2crossA[component][dcomponent];
   }
