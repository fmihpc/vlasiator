...
double VectorDipole::operator()(double x, double y, double z, coordinate component, unsigned int derivative,
   ...
   if ((derivative == 1) && (r[0] <= xlimit[0])) {
    ...
    3D dipole derivative.  */
      return -5 * B * r[dcomponent] / r2 +
             (3 * q[dcomponent] * r[component] - 2 * q[component] * r[dcomponent] + 3 * rdotq * sameComponent) / r5;
   }
   ...
   if (derivative == 0) {
      ...
      */
      ...
      return S2 * delB + dS2cart[dcomponent] * B + ddS2crossA[component][dcomponent] + IMFS2 * IMFdelB +
             IMFdS2cart[dcomponent] * IMFB + IMFddS2crossA[component][dcomponent];
   }
