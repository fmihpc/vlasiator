...
int main(int argc, char* argv[]) {
   boost::program_options::options_description options("Usage: test1 [options (options given on the command line "
   ...
   */
   ...
   double start[3] = {0, 0, 0}, end[3] = {0, 0, 0}, center[3] = {0, 0, 0};
   ...
   if (verbose) {
      cout << "Cube center at " << center[0] / R_E << ", " << center[1] / R_E << ", " << center[2] / R_E << " (R_E):" << endl;
   }

   double previous_difference = std::numeric_limits<double>::max(), difference = std::numeric_limits<double>::max();
   ...
   for (double size = 1 * R_E; size >= 0.01 * R_E; size /= 2) {
      ...
      double result_x = 0, result_y = 0, result_z = 0, reference_x = 0, reference_y = 0, reference_z = 0;
      ...
      const double dx = result_x - reference_x, dy = result_y - reference_y, dz = result_z - reference_z,
      ...
      if (difference >= previous_difference) {
         cerr << __FILE__ << ":" << __LINE__ << " New difference (" << difference << ") is larger than previous (" << previous_difference << ")" << endl;
         abort();
      }
   ...
   previous_difference = std::numeric_limits<double>::max(), difference = std::numeric_limits<double>::max();
