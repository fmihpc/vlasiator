...
int main(int argc, char* argv[]) {
   ...
   */
   ...
   if (verbose) {
      cout << "Cube center at " << center[0] / R_E << ", " << center[1] / R_E << ", " << center[2] / R_E
           << " (R_E):" << endl;
   }
   ...
   for (double size = 1 * R_E; size >= 0.01 * R_E; size /= 2) {
      ...
      if (difference >= previous_difference) {
         cerr << __FILE__ << ":" << __LINE__ << " New difference (" << difference << ") is larger than previous ("
              << previous_difference << ")" << endl;
         abort();
      }
