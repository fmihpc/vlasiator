      ...
      for (int offset_vx = -addWidthV; offset_vx <= addWidthV; offset_vx++) {
         for (int offset_vy = -addWidthV; offset_vy <= addWidthV; offset_vy++) {
            for (int offset_vz = -addWidthV; offset_vz <= addWidthV; offset_vz++) {
               const vmesh::GlobalID neighbor_block = get_velocity_block(
                   popID, {{indices[0] + offset_vx, indices[1] + offset_vy, indices[2] + offset_vz}}, refLevel);
      ...
      for (int block_index = this->velocity_block_with_no_content_list.size() - 1; block_index >= 0; --block_index) {
         ...
         if (removeBlock == true) {
            ...
            const Real* block_parameters =
                get_block_parameters(popID) + blockLID * BlockParams::N_VELOCITY_BLOCK_PARAMS;
            const Real DV3 = block_parameters[BlockParams::DVX] * block_parameters[BlockParams::DVY] *
...
void SpatialCell::adjust_velocity_blocks(const std::vector<SpatialCell*>& spatial_neighbors,
   ...
   if (doDeleteEmptyBlocks) {
      for (int block_index = this->velocity_block_with_no_content_list.size() - 1; block_index >= 0; --block_index) {
         ...
         if (neighbors_have_content.find(populations[popID].vmesh.getParent(blockGID)) != neighbors_have_content.end())
            continue;
         ...
         for (size_t c = 0; c < children.size(); ++c) {
            if (neighbors_have_content.find(children[c]) != neighbors_have_content.end())
               ++counter;
         }
         if (counter > 0)
            continue;
   ...
   for (int r = sorted.size() - 1; r >= 0; --r) {
      for (size_t b = 0; b < sorted[r].size(); ++b) {
         ...
         if (r > 0) {
            if (spat_nbr_has_content.find(populations[popID].vmesh.getParent(sorted[r][b])) !=
                spat_nbr_has_content.end()) {
         ...
         if (r > 1) {
            vmesh::GlobalID grandParentGID =
                populations[popID].vmesh.getParent(populations[popID].vmesh.getParent(sorted[r][b]));
   ...
   for (unordered_set<vmesh::GlobalID>::const_iterator it = ghostBlockList.begin(); it != ghostBlockList.end(); ++it) {
      // Parent already exists
      if (populations[popID].vmesh.getLocalID(populations[popID].vmesh.getParent(*it)) !=
          vmesh::VelocityMesh<vmesh::GlobalID, vmesh::LocalID>::invalidLocalID())
         continue;
      ...
      for (size_t c = 0; c < children.size(); ++c) {
         if (populations[popID].vmesh.getLocalID(children[c]) !=
             vmesh::VelocityMesh<vmesh::GlobalID, vmesh::LocalID>::invalidLocalID()) {
            childrensExist = true;
            break;
         }
      ...
      if (childrensExist == true) {
         ...
         for (size_t c = 0; c < children.size(); ++c)
            add_velocity_block(children[c], popID);
         continue;
      }
...
void SpatialCell::coarsen_block(const vmesh::GlobalID& parent,
   ...
   if (add_velocity_block(parent, popID) == false)
      return;
   ...
   for (size_t c = 0; c < children.size(); ++c) {
      ...
      if (childrenLID == SpatialCell::invalid_local_id())
         continue;
      ...
      for (uint k = 0; k < WID; ++k)
         for (uint j = 0; j < WID; ++j)
            for (uint i = 0; i < WID; ++i) {
               parent_data[vblock::index(i_oct * 2 + i / 2, j_oct * 2 + j / 2, k_oct * 2 + k / 2)] +=
                   data[vblock::index(i, j, k)] / 8.0;
            }
   }
...
void SpatialCell::coarsen_blocks(vamr_ref_criteria::Base* refCriterion, const uint popID) {
   ...
   for (size_t r = blocks.size() - 1; r >= 1; --r) {
      ...
      for (size_t b = 0; b < blocks[r].size(); ++b) {
         ...
         if (refCriterion->evaluate(array, popID) < Parameters::vamrCoarsenLimit)
            coarsenList.insert(blockGID);
      }
      ...
      for (unordered_set<vmesh::GlobalID>::const_iterator it = coarsenList.begin(); it != coarsenList.end(); ++it) {
         ...
         if (populations[popID].vmesh.coarsenAllowed(*it) == false)
            continue;
...
std::tuple<void*, int, MPI_Datatype> SpatialCell::get_mpi_datatype(const CellID cellID,
   ...
   if (this->mpiTransferEnabled &&
       ((SpatialCell::mpiTransferAtSysBoundaries == false && SpatialCell::mpiTransferInAMRTranslation == false) ||
        (SpatialCell::mpiTransferAtSysBoundaries == true &&
      ...
      if ((SpatialCell::mpi_transfer_type & Transfer::VEL_BLOCK_DATA) != 0) {
         ...
         block_lengths.push_back(sizeof(Realf) * VELOCITY_BLOCK_LENGTH *
                                 populations[activePopID].blockContainer.size());
      }
   ...
   if (displacements.size() > 0) {
      count = 1;
      MPI_Type_create_hindexed(displacements.size(), &block_lengths[0], &displacements[0], MPI_BYTE, &datatype);
   } else {
      count = 0;
      datatype = MPI_BYTE;
   }
   ...
   if (printMpiDatatype) {
      ...
      cout << myRank << " get_mpi_datatype: " << cellID << " " << sender_rank << " " << receiver_rank << " " << mpiSize
           << ", Nblocks = " << populations[activePopID].N_blocks << ", nbr Nblocks =";
...
#ifdef DEBUG_SPATIAL_CELL
   if (blockGID == invalid_global_id()) {
      cerr << "merge_values_recursive called for GID=" << blockGID << " at r=" << (int)refLevel
           << " parent=" << parentGID << endl;
   }
#endif
...
#warning FIXME activePopID is not correct here (VAMR, hence TBD)
   ...
   for (int k_oct = 0; k_oct < 2; ++k_oct)
      for (int j_oct = 0; j_oct < 2; ++j_oct)
         for (int i_oct = 0; i_oct < 2; ++i_oct) {
            // Copy data belonging to the octant to a temporary array:
            Realf array[WID3];
            for (uint k = 0; k < WID; ++k)
               for (uint j = 0; j < WID; ++j)
                  for (uint i = 0; i < WID; ++i) {
                     array[vblock::index(i, j, k)] =
                         data[vblock::index(i_oct * 2 + i / 2, j_oct * 2 + j / 2, k_oct * 2 + k / 2)];
                  }
...
void SpatialCell::add_values(const vmesh::GlobalID& targetGID,
   ...
   if (it != sourceData.end()) {
      ...
      for (uint k = 0; k < WID; ++k)
         for (uint j = 0; j < WID; ++j)
            for (uint i = 0; i < WID; ++i) {
               targetData[vblock::index(i, j, k)] +=
                   source[vblock::padIndex<1>(parentIndex[0] + i / 2, parentIndex[1] + j / 2, parentIndex[2] + k / 2)];
            }

      for (uint k = 0; k < WID; ++k)
         for (uint j = 0; j < WID; ++j)
            for (uint i = 0; i < WID; ++i)
               source[vblock::padIndex<1>(parentIndex[0] + i / 2, parentIndex[1] + j / 2, parentIndex[2] + k / 2)] = 0;
   }
   ...
   for (int i_off = -1; i_off < 2; i_off += 2) {
      ...
      for (uint k = 0; k < WID; ++k)
         for (uint j = 0; j < WID; ++j) {
            targetData[vblock::index(i_trgt, j, k)] +=
                face_mul * source[vblock::padIndex<1>(i_src, parentIndex[1] + j / 2, parentIndex[2] + k / 2)];
         }
   }
   ...
   for (int j_off = -1; j_off < 2; j_off += 2) {
      ...
      for (uint k = 0; k < WID; ++k)
         for (uint i = 0; i < WID; ++i) {
            targetData[vblock::index(i, j_trgt, k)] +=
                face_mul * source[vblock::padIndex<1>(parentIndex[0] + i / 2, j_src, parentIndex[2] + k / 2)];
         }
   }
   ...
   for (int k_off = -1; k_off < 2; k_off += 2) {
      ...
      for (uint j = 0; j < WID; ++j)
         for (uint i = 0; i < WID; ++i) {
            targetData[vblock::index(i, j, k_trgt)] +=
                face_mul * source[vblock::padIndex<1>(parentIndex[0] + i / 2, parentIndex[1] + j / 2, k_src)];
         }
   }
   ...
   if (parentGID != targetParentGID) {
      ...
      if (it != sourceData.end()) {
         ...
         const uint j_src = max(-j_off, 0) * (WID + 1);
         for (uint k = 0; k < WID; ++k) {
            targetData[vblock::index(i_trgt, j_trgt, k)] +=
                edge_mul * source[vblock::padIndex<1>(i_src, j_src, parentIndex[2] + k / 2)];
         }
      }
   }

   if (octant == 0 || octant == 2) {
      i_off = -1;
      k_off = -1;
   }
   ...
   if (octant == 4 || octant == 6) {
      i_off = -1;
      k_off = +1;
   }
   if (octant == 5 || octant == 7) {
      i_off = +1;
      k_off = +1;
   }
   source = NULL;
   ...
   if (parentGID != targetParentGID) {
      ...
      if (it != sourceData.end()) {
         ...
         const uint k_src = max(-k_off, 0) * (WID + 1);
         for (uint j = 0; j < WID; ++j) {
            targetData[vblock::index(i_trgt, j, k_trgt)] +=
                edge_mul * source[vblock::padIndex<1>(i_src, parentIndex[1] + j / 2, k_src)];
         }
      }
   }
   ...
   if (parentGID != targetParentGID) {
      ...
      if (it != sourceData.end()) {
         ...
         const uint k_src = max(-k_off, 0) * (WID + 1);
         for (uint i = 0; i < WID; ++i) {
            targetData[vblock::index(i, j_trgt, k_trgt)] +=
                edge_mul * source[vblock::padIndex<1>(parentIndex[0] + i / 2, j_src, k_src)];
         }
      }
   }
   ...
   if (parentGID != targetParentGID) {
      ...
      if (it != sourceData.end()) {
         ...
         const int j_src = max(-j_off, 0) * (WID + 1);
         const int k_src = max(-k_off, 0) * (WID + 1);
         targetData[vblock::index(i_trgt, j_trgt, k_trgt)] +=
             corn_mul * source[vblock::padIndex<1>(i_src, j_src, k_src)];
      }
   }
}
...
bool SpatialCell::setCommunicatedSpecies(const uint popID) {
   ...
   if (popID >= getObjectWrapper().particleSpecies.size()) {
      std::cerr << "ERROR, popID " << popID << " exceeds species.size() " << getObjectWrapper().particleSpecies.size()
                << " in ";
      std::cerr << __FILE__ << ":" << __LINE__ << std::endl;
      exit(1);
   }
...
bool SpatialCell::shrink_to_fit() {
   ...
   for (size_t p = 0; p < populations.size(); ++p) {
      const uint64_t amount =
          2 + populations[p].blockContainer.size() * populations[p].blockContainer.getBlockAllocationFactor();
      ...
      if (populations[p].blockContainer.capacity() > amount)
         if (populations[p].blockContainer.recapacitate(amount) == false)
            success = false;
   }
   return success;
}
...
void SpatialCell::printMeshSizes() {
   ...
   for (size_t p = 0; p < populations.size(); ++p) {
      cerr << "\t pop " << p << " " << populations[p].vmesh.size() << ' ' << populations[p].blockContainer.size()
           << endl;
   }
...
void SpatialCell::updateSparseMinValue(const uint popID) {
   ...
   if (population.sparseDynamicAlgorithm == 1 || population.sparseDynamicAlgorithm == 2) {
      ...
      const Real k = (population.sparseDynamicMinValue2 - population.sparseDynamicMinValue1) /
                     (population.sparseDynamicBulkValue2 - population.sparseDynamicBulkValue1);
