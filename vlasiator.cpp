...
#include <fenv.h>
#include <signal.h>
/*! Function used to abort the program upon detecting a floating point exception. Which exceptions are caught is defined
 * using the function feenableexcept.
 */
...
int main(int argn, char* args[]) {
   ...
   if (versionstr.find("Open MPI") != std::string::npos) {
      ...
      mpiioMessage << "We detected OpenMPI but the compilation flag VLASIATOR_ALLOW_MCA_OMPIO was set so we do not "
                      "override the default MCA io flag."
                   << endl;
#else
   ...
   if (!P::writeFullBGB) {
      ...
      // If we are out to write the full background field and derivatives, we don't want to overwrite the existing run's
      // logfile.
   ...
   while (P::tstep <= P::tstep_max && P::t - P::dt <= P::t_max + DT_EPSILON &&
      ...
      if (myRank == MASTER_RANK) {
         // check whether STOP or KILL or SAVE has been passed, should be done by MASTER_RANK only as it can reset
         // P::bailout_write_restart
      ...
      if (myRank == MASTER_RANK) {
         if ((P::saveRestartWalltimeInterval >= 0.0 &&
            ...
            if (globalflags::writeRestart == true) {
               ...
               globalflags::writeRestart = false; // This flag is only used by MASTER_RANK here and it needs to be reset
                                                  // after a restart write has been issued.
            }
         } else {
            doNow[0] = 0;
         }
      ...
      if (((P::tstep % P::rebalanceInterval == 0 && P::tstep > P::tstep_min) || overrideRebalanceNow)) {
         ...
         if (!dtIsChanged && P::adaptRefinement && P::tstep % (P::rebalanceInterval * P::refineMultiplier) == 0 &&
             P::t > P::refineAfter) {
...
#pragma omp parallel for
      ...
      if (SBC::ionosphereGrid.nodes.size() > 0 &&
         ...
         logFile << "tstep = " << P::tstep << " t = " << P::t << " ionosphere iterations = " << nIterations
                 ...
                 << maxPotentialN - minPotentialN << " S potential min " << minPotentialS << " max " << maxPotentialS
                 << " difference " << maxPotentialS - minPotentialS << endl;
