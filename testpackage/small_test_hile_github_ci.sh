...
run_command="srun --mpi=pmix -n $tasks -c $OMP_NUM_THREADS"
small_run_command="srun --mpi=pmix -n 1 -c 1"
run_command_tools="srun --mpi=pmix -n 1 -c 1"
   ...
   do
       ...
       do
           ...
           then
               ...
               echo "test_hile{test=\"${test_name[$run]}\",var=\"${variables[$i]}\",index=\"${indices[$i]}\",diff=\"absolute\"} $absoluteValue" >> $GITHUB_WORKSPACE/metrics.txt
               echo "test_hile{test=\"${test_name[$run]}\",var=\"${variables[$i]}\",index=\"${indices[$i]}\",diff=\"relative\"} $relativeValue" >> $GITHUB_WORKSPACE/metrics.txt
...
if [[ $RUN_ERROR == 0 ]]; then
   cat >> $JUNIT_FILE <<HEADER
   <testsuite name="Testpackage on hile" tests="1" errors="0">
      <testcase classname="vlasiator_hile.${test_name[$run]}" name="${test_name[$run]}" time="$newPerf">
HEADER
else
   cat >> $JUNIT_FILE <<HEADER
