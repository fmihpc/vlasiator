...
public:
   ...
   ~Readparameters();
   static CLI::App* get_app() { return app; }
    ...
    * See CLI11 documentation for more details on the options.
    ...
    */
   template <typename T>
   static CLI::Option* add(const std::string& name, const std::string& desc, T& var,
      ...
      if (cmdNameVec.size() == 2) {
         ...
         if (sub != nullptr) {
            ...
            opt =
                sub->add_option((dashes + namein).c_str(), var, desc.c_str())->capture_default_str();
         ...
         opt = app->add_option(("--" + cmdNameVec.at(0)).c_str(), var, desc.c_str())
                   ->capture_default_str(); 
      }
      if (defval && opt != nullptr) {
         ...
         // however it only applies if the the default value was not given as a variable rather than a literal (if even
         // possible)
   ...
   static void parseComposing() {
      ...
      if (rank == MASTER_RANK) {
         ...
         if (configFile.is_open()) {
            while (getline(configFile, line)) {
               ...
               // this is used for checking whether to pass the handling to CLI11 later(or rather whether to rerun
               // callback)
      ...
      for (std::map<std::string, std::string>::iterator iter = optionsComposing.begin(); iter != optionsComposing.end();
           ++iter) {
         ...
         //Parse the values and run CLI11 callback
         ...
         if (!isOptionParsed[commandName]) {
            ...
            MPI_Bcast(&strSize, 1, MPI_INT, MASTER_RANK, MPI_COMM_WORLD);

            if (rank != MASTER_RANK) {
               optVal.resize(strSize);
            }
            MPI_Bcast(optVal.data(), strSize, MPI_CHAR, MASTER_RANK, MPI_COMM_WORLD);
            ...
            if (optVal != "[]" && n == 0)
               n = 1; // This means we have one argument

            if (opt->get_items_expected() == 0 && optVal == "[]") {
               continue;
            } else if ((opt->get_items_expected_max() < n || opt->get_items_expected_min() > n)) {
               std::cerr << "Option " << commandName << " expected " << opt->get_items_expected()
                         << " argument(s), but found " << n << "." << std::endl;
               isValid = false;
            }
            if (!isValid) {
               abort();
    ...
    */
   template <typename T>
   static CLI::Option* addComposing(const std::string& name, const std::string& desc, T& value,
                                    std::optional<T> defval = std::nullopt) {
    ...
    * Flags are parameters that take no arguments, see CLI11 documentation.
    * @param name The name of the parameter, as given in the input file(s).
    * @param desc Description for the parameter.
    * @param var variable to which the parsed value is assigned.
    * @param defval Default value for variable.
    ...
    */
   static bool isSet(const std::string& name) { return (app->get_option_no_throw(name) != nullptr); }
    ...
    * @param name The name of the option
    */
   static bool removeOption(const std::string& name) {
      if (auto indx = name.find('.'); indx != std::string::npos) {
         std::string subcomName = name.substr(0, indx);
         std::string optName = name.substr(indx + 1, name.size());
         CLI::App* subcom = app->get_subcommand(subcomName);
