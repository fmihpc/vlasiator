...
template <typename GID, typename LID> inline VelocityMesh<GID, LID>::VelocityMesh() {
...
template <typename GID, typename LID> inline GID VelocityMesh<GID, LID>::getGlobalID(const LID& localID) const {
   ...
   if (localID >= localToGlobalMap.size()) {
      std::cerr << "ERROR invalid local id" << std::endl;
      exit(1);
   }
#endif
...
inline GID VelocityMesh<GID, LID>::getGlobalID(const uint8_t& refLevel, const Real* coords) const {
   ...
   const LID indices[3] = {static_cast<LID>(floor((coords[0] - meshParameters[meshID].meshMinLimits[0]) /
                                                  meshParameters[meshID].blockSize[0])),
                           static_cast<LID>(floor((coords[1] - meshParameters[meshID].meshMinLimits[1]) /
                                                  meshParameters[meshID].blockSize[1])),
                           static_cast<LID>(floor((coords[2] - meshParameters[meshID].meshMinLimits[2]) /
...
inline GID VelocityMesh<GID, LID>::getGlobalID(const uint8_t& refLevel, LID indices[3]) const {
   if (indices[0] >= meshParameters[meshID].gridLength[0])
      return invalidGlobalID();
   if (indices[1] >= meshParameters[meshID].gridLength[1])
      return invalidGlobalID();
   if (indices[2] >= meshParameters[meshID].gridLength[2])
      return invalidGlobalID();
   return indices[2] * meshParameters[meshID].gridLength[1] * meshParameters[meshID].gridLength[0] +
          indices[1] * meshParameters[meshID].gridLength[0] + indices[0];
}
...
inline GID VelocityMesh<GID, LID>::getGlobalID(const uint32_t& refLevel, const LID& i, const LID& j,
                                               const LID& k) const {
   if (i >= meshParameters[meshID].gridLength[0] || j >= meshParameters[meshID].gridLength[1] ||
       k >= meshParameters[meshID].gridLength[2]) {
      return invalidGlobalID();
   }
...
template <typename GID, typename LID> inline GID VelocityMesh<GID, LID>::getGlobalIndexOffset(const uint8_t& refLevel) {
   return 0;
}
      ...
      for (int j_off = -1; j_off < 2; ++j_off)
         for (int i_off = -1; i_off < 2; ++i_off) {
            if (i + i_off < Nx_max && (j + j_off < Ny_max && k + k_off < Nz_max))
               neighborIDs[nbr] = getGlobalID(0, i + i_off, j + j_off, k + k_off);
            else
...
inline bool VelocityMesh<GID, LID>::initialize(const size_t& meshID,
   ...
   meshParameters[meshID].max_velocity_blocks = meshParameters[meshID].gridLength[0] *
...
template <typename GID, typename LID> inline bool VelocityMesh<GID, LID>::isInitialized() const {
   return meshParameters[meshID].initialized;
}
...
template <typename GID, typename LID> inline bool VelocityMesh<GID, LID>::push_back(const GID& globalID) {
   ...
   auto position = globalToLocalMap.insert(std::make_pair(globalID, localToGlobalMap.size()));
...
template <typename GID, typename LID> inline bool VelocityMesh<GID, LID>::push_back(const std::vector<GID>& blocks) {
