...
*/
   ...
   GID VelocityMesh<GID,LID>::getMaxVelocityBlocks() const {
      return getObjectWrapper().velocityMeshes[meshID].max_velocity_blocks;
   }
   ...
   void VelocityMesh<GID,LID>::getNeighborsAtSameLevel(const GID& globalID,std::vector<GID>& neighborIDs) const {
      ...
      const LID Nx_max = getObjectWrapper().velocityMeshes[meshID].gridLength[0];
      const LID Ny_max = getObjectWrapper().velocityMeshes[meshID].gridLength[1];
      const LID Nz_max = getObjectWrapper().velocityMeshes[meshID].gridLength[2];
