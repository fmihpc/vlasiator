...
bool createZone(  dccrg::Dccrg<SpatialCell> & mpiGrid,
   ...
   if( !local_zones.empty() ) {
      cerr << "Warning, passed non-empty local_zones to createZone at " << __FILE__ << " " << __LINE__ << endl;
   }
   if( !ghost_zones.empty() ) cerr << "Warning, passed non-empty ghost_zones to createZone at " << __FILE__ << " " << __LINE__ << endl;
   if( local_cells.empty() ) {
      if( !ghost_cells.empty() ) {
         ...
         logFile << "(MAIN) writeGrid: ERROR LOCAL CELLS EMPTY BUT GHOST CELLS NOT AT: " << __FILE__ << " " << __LINE__ << endl << writeVerbose;
         exit( 1 );
      }
   ...
   local_zones.reserve( local_cells.size() );
   ...
   ghost_zones.reserve( ghost_cells.size() );
...
namespace iowrite_new {
   ...
   bool writeVariables( dccrg::Dccrg<SpatialCell>& mpiGrid,
                          DataReducer& dataReducer,
                          Writer & vlsvWriter, 
                          const vector<uint64_t> & cells ) {
...
bool writeGrid(
   ...
   if( newLib ) {
      ...
      if( iowrite_new::writeMeshBoundingBox( vlsvWriter, meshName, masterProcessId, MPI_COMM_WORLD ) == false ) return false;

      //Write the node coordinates: NOTE: master process only
      if( iowrite_new::writeBoundingBoxNodeCoordinates( vlsvWriter, meshName, masterProcessId, MPI_COMM_WORLD ) == false ) return false;

      //Write basic grid variables: NOTE: master process only
      if( iowrite_new::writeCommonGridData(vlsvWriter, mpiGrid, P::systemWrites[index], MPI_COMM_WORLD) == false ) return false;

      //Write zone global id numbers:
      if( iowrite_new::writeZoneGlobalIdNumbers( mpiGrid, vlsvWriter, meshName, local_zones, ghost_zones ) == false ) return false;

      //Write domain sizes:
      if( iowrite_new::writeDomainSizes( vlsvWriter, meshName, local_zones.size(), ghost_zones.size() ) == false ) return false;

      //Write ghost zone domain and local id numbers ( VisIt plugin needs this for MPI )
      if( iowrite_new::writeGhostZoneDomainAndLocalIdNumbers( mpiGrid, vlsvWriter, meshName, ghost_zones ) == false ) return false;

      //Write necessary variables:
      if( iowrite_new::writeVariables( mpiGrid, dataReducer, vlsvWriter, local_cells ) == false ) return false;

      if( writeSmaller == true ) {
         if( iowrite_new::writeVelocitySpace<float>( mpiGrid, vlsvWriter, index, local_cells ) == false ) return false;
      } else {
         if( iowrite_new::writeVelocitySpace<Real>( mpiGrid, vlsvWriter, index, local_cells ) == false ) return false;
      }
...
bool writeRestart(dccrg::Dccrg<SpatialCell>& mpiGrid,
   ...
   if( newLib ) {
      ...
      //Write domain sizes:
      //Note: For write restart, no need to write this
      //if( iowrite_new::writeDomainSizes( vlsvWriter, meshName, local_zones.size(), ghost_zones.size() ) == false ) return false;
      ...
      //Write necessary variables:
      //REMOVE: if( iowrite_new::writeVariables( mpiGrid, dataReducer, vlsvWriter, index, meshName, local_zones ) == false ) return false;
      //O: NOTE: Looking at the original write restart function I think we only need iowrite_new::writeDataReducer and
      //iowrite_new::writeVelocityDistributionData
      for (uint i=0; i<restartReducer.size(); ++i) {
         iowrite_new::writeDataReducer(mpiGrid, local_cells, restartReducer, i, vlsvWriter);
      }

      //write the velocity distribution data -- note: it's expecting a vector of pointers:
      iowrite_new::writeVelocityDistributionData<Real>(vlsvWriter, mpiGrid, local_cells, MPI_COMM_WORLD);
