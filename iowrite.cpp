...
bool updateLocalIds(dccrg::Dccrg<SpatialCell, dccrg::Cartesian_Geometry>& mpiGrid,
                    const std::vector<CellID>& local_cells, MPI_Comm comm) {
   int myRank;
...
/** Writes the velocity distribution into the file.
 ...
 @return Returns true if operation was successful.*/
bool writeVelocityDistributionData(Writer& vlsvWriter, dccrg::Dccrg<SpatialCell, dccrg::Cartesian_Geometry>& mpiGrid,
                                   const vector<CellID>& cells, MPI_Comm comm) {
   bool success = true;
   for (size_t p = 0; p < getObjectWrapper().particleSpecies.size(); ++p) {
      if (writeVelocityDistributionData(p, vlsvWriter, mpiGrid, cells, comm) == false)
         success = false;
   }
   return success;
}
...
bool writeVelocityDistributionData(const uint popID, Writer& vlsvWriter,
   ...
   // Write the array:
   if (vlsvWriter.writeArray("CELLSWITHBLOCKS", attribs, cells.size(), vectorSize, cells.data()) == false)
      success = false;
   if (success == false)
      logFile << "(MAIN) writeGrid: ERROR failed to write CELLSWITHBLOCKS to file!" << endl << writeVerbose;
   // Write blocks per cell, this has to be in the same order as cellswitblocks so that extracting works
   if (vlsvWriter.writeArray("BLOCKSPERCELL", attribs, blocksPerCell.size(), vectorSize, blocksPerCell.data()) == false)
      success = false;
   if (success == false)
      logFile << "(MAIN) writeGrid: ERROR failed to write CELLSWITHBLOCKS to file!" << endl << writeVerbose;
   ...
   if (mpiGrid.get_rank() == MASTER_RANK) {
      if (vlsvWriter.writeArray("MESH_BBOX", attribs, 6, 1, bbox) == false)
         success = false;
      ...
      for (int crd = 0; crd < 3; ++crd) {
         ...
         if (crd == 0) {
            if (vlsvWriter.writeArray("MESH_NODE_CRDS_X", attribs, N_nodes, 1, crds) == false)
               success = false;
         }
         if (crd == 1) {
            if (vlsvWriter.writeArray("MESH_NODE_CRDS_Y", attribs, N_nodes, 1, crds) == false)
               success = false;
         }
         if (crd == 2) {
            if (vlsvWriter.writeArray("MESH_NODE_CRDS_Z", attribs, N_nodes, 1, crds) == false)
               success = false;
         }
         delete[] crds;
         crds = NULL;
      ...
      if (vlsvWriter.writeArray("MESH_BBOX", attribs, 0, 1, bbox) == false)
         success = false;
      Real* crds = NULL;
      if (vlsvWriter.writeArray("MESH_NODE_CRDS_X", attribs, 0, 1, crds) == false)
         success = false;
      if (vlsvWriter.writeArray("MESH_NODE_CRDS_Y", attribs, 0, 1, crds) == false)
         success = false;
      if (vlsvWriter.writeArray("MESH_NODE_CRDS_Z", attribs, 0, 1, crds) == false)
         success = false;
   }
   ...
   if (globalSuccess(success, "(MAIN) writeGrid: ERROR: Failed to fill temporary array velocityBlockIds",
   ...
   attribs["mesh"] = spatMeshName;
   attribs["name"] = popName;
   if (vlsvWriter.writeArray("BLOCKIDS", attribs, totalBlocks, vectorSize, velocityBlockIds.data()) == false)
      success = false;
   if (success == false)
      logFile << "(MAIN) writeGrid: ERROR failed to write BLOCKIDS to file!" << endl << writeVerbose;
   ...
   for (size_t cell = 0; cell < cells.size(); ++cell) {
      ...
      vlsvWriter.addMultiwriteUnit(arrayToWrite,
                                   arrayElements); // Note: We told beforehands that the vectorsize = WID3 = 64
   }
   if (cells.size() == 0) {
   ...
   if (globalSuccess(success, "(MAIN) writeGrid: ERROR: Failed to fill temporary velocityBlockData array",
                     MPI_COMM_WORLD) == false) {
      vlsvWriter.close();
      return false;
   }
...
/*! Writes info received from data reducer. This function writes out the variable arrays into the file
 ...
 \param dataReducerIndex Index in the data reducer (determines which variable to read) Note: size of the data reducer
 can be retrieved with dataReducer.size() \param vlsvWriter Some vlsv writer with a file open \return Returns true if
...
bool writeDataReducer(const dccrg::Dccrg<SpatialCell, dccrg::Cartesian_Geometry>& mpiGrid,
                      ...
                      FsGrid<fsgrids::technical, FS_STENCIL_WIDTH>& technicalGrid, const bool writeAsFloat,
   ...
   if (dataReducer.getMetadata(dataReducerIndex, unitString, unitStringLaTeX, variableStringLaTeX,
                               unitConversionFactor) == false) {
   ...
   // If DRO has a vector size of 0 it means this DRO should not write out anything. This is used e.g. for DROs we want
   ...
   try {
      ...
      logFile << "(MAIN) writeGrid: ERROR FAILED TO ALLOCATE MEMORY AT: " << __FILE__ << " " << __LINE__ << endl
              << writeVerbose;
      return false;
   }
   ...
   for (size_t cell = 0; cell < cells.size(); ++cell) {
      ...
      if (dataReducer.reduceData(mpiGrid[cells[cell]], dataReducerIndex, varBuffer + cell * vectorSize * dataSize) ==
          false) {
         success = false;
   ...
   if (success) {
      ...
      if ((writeAsFloat == true && dataType.compare("float") == 0) && dataSize == sizeof(double)) {
         ...
         if (vlsvWriter.writeArray("VARIABLE", attribs, dataType_smaller, arraySize_smaller, vectorSize_smaller,
                                   dataSize_smaller, varBuffer_smaller_char) == false) {
            success = false;
            logFile << "(MAIN) writeGrid: ERROR failed to write datareductionoperator data to file!" << endl
                    << writeVerbose;
         }
         ...
         phiprof::Timer writeArrayTimer{"writeArray"};
         if (vlsvWriter.writeArray("VARIABLE", attribs, dataType, cells.size(), vectorSize, dataSize, varBuffer) ==
             false) {
            success = false;
            logFile << "(MAIN) writeGrid: ERROR failed to write datareductionoperator data to file!" << endl
                    << writeVerbose;
      ...
      success = dataReducer.writeFsGridData(perBGrid, EGrid, EHallGrid, EGradPeGrid, momentsGrid, dPerBGrid,
                                            dMomentsGrid, BgBGrid, volGrid, technicalGrid, "fsgrid", dataReducerIndex,
                                            vlsvWriter, writeAsFloat);
      writeFsTimer.stop();
...
/*! Writes common grid data such as parameters (time steps, x_min, ..) as well as local cell ids as variables
 ...
 \return Returns true if operation was successful
 */
bool writeCommonGridData(Writer& vlsvWriter, dccrg::Dccrg<SpatialCell, dccrg::Cartesian_Geometry>& mpiGrid,
                         const vector<uint64_t>& local_cells, const uint& fileIndex, MPI_Comm comm) {
   ...
   // Write parameters:
   if (vlsvWriter.writeParameter("time", &P::t) == false) {
      return false;
   }
   if (vlsvWriter.writeParameter("dt", &P::dt) == false) {
      return false;
   }
   if (vlsvWriter.writeParameter("timestep", &P::tstep) == false) {
      return false;
   }
   if (vlsvWriter.writeParameter("fieldSolverSubcycles", &P::fieldSolverSubcycles) == false) {
      return false;
   }
   if (vlsvWriter.writeParameter("fileIndex", &fileIndex) == false) {
      return false;
   }
   if (vlsvWriter.writeParameter("xmin", &P::xmin) == false) {
      return false;
   }
   if (vlsvWriter.writeParameter("xmax", &P::xmax) == false) {
      return false;
   }
   if (vlsvWriter.writeParameter("ymin", &P::ymin) == false) {
      return false;
   }
   if (vlsvWriter.writeParameter("ymax", &P::ymax) == false) {
      return false;
   }
   if (vlsvWriter.writeParameter("zmin", &P::zmin) == false) {
      return false;
   }
   if (vlsvWriter.writeParameter("zmax", &P::zmax) == false) {
      return false;
   }
   if (vlsvWriter.writeParameter("xcells_ini", &P::xcells_ini) == false) {
      return false;
   }
   if (vlsvWriter.writeParameter("ycells_ini", &P::ycells_ini) == false) {
      return false;
   }
   if (vlsvWriter.writeParameter("zcells_ini", &P::zcells_ini) == false) {
      return false;
   }
   if (FieldTracing::fieldTracingParameters.doTraceFullBox) {
      if (vlsvWriter.writeParameter("fieldTracingFluxRopeMaxDistance",
   ...
   float version = 3.00;
   if (vlsvWriter.writeParameter("version", &version) == false) {
      return false;
   }
   return true;
}
...
bool writeGhostZoneDomainAndLocalIdNumbers(dccrg::Dccrg<SpatialCell, dccrg::Cartesian_Geometry>& mpiGrid,
                                           Writer& vlsvWriter, const string& meshName,
   ...
   for (it = ghost_cells.begin(); it != ghost_cells.end(); ++it) {
      ...
      // Now if we're in process 1. and our ghost zone is D, its domainId would be 2. because process 2. has D as local
      // zone In process 2, the local id of D is 0, so that's the local id we want now The local id is being saved in
      // createZone function
   ...
   if (vlsvWriter.writeArray("MESH_GHOST_DOMAINS", xmlAttributes, numberOfGhosts, vectorSize, ghostDomainIds.data()) ==
       false) {
      cerr << "Error, failed to write MEST_GHOST_DOMAINS at: " << __FILE__ << " " << __LINE__ << endl;
      logFile << "(MAIN) writeGrid: ERROR failed to write MEST_GHOST_DOMAINS at: " << __FILE__ << " " << __LINE__
              << endl
              << writeVerbose;
      return false;
   }
   // Write the in the number of ghost local ids: (Returns false if writing fails)
   if (vlsvWriter.writeArray("MESH_GHOST_LOCALIDS", xmlAttributes, numberOfGhosts, vectorSize, ghostLocalIds.data()) ==
       false) {
      cerr << "Error, failed to write MEST_GHOST_LOCALIDS at: " << __FILE__ << " " << __LINE__ << endl;
      logFile << "(MAIN) writeGrid: ERROR failed to write MEST_GHOST_LOCALIDS at: " << __FILE__ << " " << __LINE__
              << endl
              << writeVerbose;
      return false;
   }
...
/*! Writes domain sizes into the vlsv file, so the number of ghost and local cell ids in this process
 ...
 \return Returns true if operation was successful
 */
bool writeDomainSizes(Writer& vlsvWriter, const string& meshName, const unsigned int& numberOfLocalZones,
   ...
   // Write (writeArray does the writing) Note: Here the important part is "MESH_DOMAIN_SIZES" -- visit plugin needs
   // this
   if (vlsvWriter.writeArray("MESH_DOMAIN_SIZES", xmlAttributes, arraySize, vectorSize, domainSize) == false) {
      ...
      logFile << "(MAIN) writeGrid: ERROR FAILED TO WRITE MESH_DOMAIN_SIZES AT: " << __FILE__ << " " << __LINE__ << endl
              << writeVerbose;
      return false;
   }
   return true;
...
/*! Writes the zone global id numbers into the file. The vlsv file needs to know in which order the local cells + ghost
 cells are written. Local cells are first appended to a vector called global ids, after which the ghost cells are
 appended. The global ids vector will then be saved into a vlsv file \param mpiGrid Vlasiator's MPI grid \param
 vlsvWriter Some vlsv writer with a file open \param meshName Name of the mesh ("SpatialGrid" used in the writeGrid
 function and it should be the default) \param local_cells Vector containing the local cells of this process \param
 ghost_cells Vector containing the ghost cells of this process ( The cells on process boundary ) \return Returns true if
 the operation was successful
 */
bool writeZoneGlobalIdNumbers(const dccrg::Dccrg<SpatialCell, dccrg::Cartesian_Geometry>& mpiGrid, Writer& vlsvWriter,
                              const string& meshName, const vector<uint64_t>& local_cells,
                              const vector<uint64_t>& ghost_cells) {
   ...
   // Set periodicity:
   if (mpiGrid.topology.is_periodic(0)) {
      xmlAttributes["xperiodic"] = "yes";
   } else {
      xmlAttributes["xperiodic"] = "no";
   }
   if (mpiGrid.topology.is_periodic(1)) {
      xmlAttributes["yperiodic"] = "yes";
   } else {
      xmlAttributes["yperiodic"] = "no";
   }
   if (mpiGrid.topology.is_periodic(2)) {
      xmlAttributes["zperiodic"] = "yes";
   } else {
...
/*! Writes the node coordinates. This means basically every cell's node (Corner of each cell). Note: The grid is a
 structured one, so writing the nodes means starting from the corner of the grid and writing coordinates per every cell
 length until reaching the other corner of the grid \param vlsvWriter Some vlsv writer with a file open \param meshName
 Name of the mesh (SpatialGrid used in writeGrid and should be the default) \param masterRank The master rank (Vlasiator
 uses 0) \param comm The MPI comm \return Returns true if the operation was successful
 */
bool writeBoundingBoxNodeCoordinates(Writer& vlsvWriter, const string& meshName, const int masterRank, MPI_Comm comm) {
   ...
   xmlAttributes["mesh"] = meshName;
   //"success"'s value will be returned. By default it's true but if some of the vlsvWriter operations fail it will be
   // false:
   bool success = true;
   // Depending on whether our rank is master rank or not the operation is slightly different, so let's get out rank
   // from the MPI_Comm comm:
   int myRank;
   // Input myRank:
   MPI_Comm_rank(comm, &myRank);
   ...
   if (myRank == masterRank) {
      ...
      arraySize = xCells + 1;
      if (vlsvWriter.writeArray("MESH_NODE_CRDS_X", xmlAttributes, arraySize, vectorSize, xNodeCoordinates.data()) ==
          false)
         success = false;
      arraySize = yCells + 1;
      if (vlsvWriter.writeArray("MESH_NODE_CRDS_Y", xmlAttributes, arraySize, vectorSize, yNodeCoordinates.data()) ==
          false)
         success = false;
      arraySize = zCells + 1;
      if (vlsvWriter.writeArray("MESH_NODE_CRDS_Z", xmlAttributes, arraySize, vectorSize, zNodeCoordinates.data()) ==
          false)
         success = false;
   } else {
...
/*! Function for writing the bounding box. This writes only if the process running it is the master rank. This array
 contains info on the boundaries of the grid so for example the number of cells in x, y, z direction.
 * Note that this is *not* the physical coordinate bounding box, but the computational domain bounding box, measured in
 blocks and cells. \param vlsvWriter Some vlsv writer with a file open \param meshName Name of the mesh to write
 ("SpatialGrid" is used in writeGrid and it should be the default) \param masterRank The master process' id. Vlasiator
 uses 0 as the master process id, so by default should be 0 \param comm MPI comm \return Returns true if operation was
 successful
 */
bool writeMeshBoundingBox(Writer& vlsvWriter, const string& meshName, const int masterRank, MPI_Comm comm) {
   ...
   // Write an array (NOTE: success will be returned and writeArray will return true or false depending on whether or
   // not the write is successful)
   bool success;
   if (myRank == masterRank) {
...
bool writeFsGridMetadata(FsGrid<fsgrids::technical, FS_STENCIL_WIDTH>& technicalGrid, vlsv::Writer& vlsvWriter) {
   ...
   std::array<int64_t, 6> boundaryBox({globalSize[0], globalSize[1], globalSize[2], 1, 1, 1});
   ...
   for (int z = 0; z < localSize[2]; z++) {
      for (int y = 0; y < localSize[1]; y++) {
         for (int x = 0; x < localSize[0]; x++) {
            ...
            globalIds[i++] =
                globalIndex[2] * globalSize[0] * globalSize[1] + globalIndex[1] * globalSize[0] + globalIndex[0];
         }
      }
   }
...
bool writeIonosphereGridMetadata(vlsv::Writer& vlsvWriter) {
   ...
   // write DomainSizes
   std::array<uint64_t, 4> meshDomainSize(
       {SBC::ionosphereGrid.elements.size(), 0, SBC::ionosphereGrid.nodes.size(), 0});
   if (rank == 0) {
   ...
   if (rank == 0) {
      // Write this data only on rank 0
      vlsvWriter.writeArray("MESH_NODE_CRDS", xmlAttributes, SBC::ionosphereGrid.nodes.size(), 3,
   ...
   if (rank == 0) {
      // Write this data only on rank 0
      vlsvWriter.writeArray("MESH", xmlAttributes, ionosphereGridElementsAndCorners.size(), 1,
                            ionosphereGridElementsAndCorners.data());
   } else {
...
bool writeVelocitySpace(dccrg::Dccrg<SpatialCell, dccrg::Cartesian_Geometry>& mpiGrid, Writer& vlsvWriter, int index,
   ...
   for (uint i = 0; i < cells.size(); i++) {
      ...
      for (int AMR = 0; AMR <= P::amrMaxSpatialRefLevel; AMR++) {
         ...
         if ((cells[i] >= startindex) && (cells[i] < endindex)) {
            ...
            lineZ =
                ((cells[i] - startindex) / ((AMRm * P::xcells_ini) * (AMRm * P::ycells_ini))) % (AMRm * P::zcells_ini);
      ...
      if (mpiGrid[cells[i]]->parameters[CellParams::ISCELLSAVINGF] > 0)
         continue;
      ...
      for (uint ishell = 0; ishell < P::systemWriteDistributionWriteShellRadius.size(); ishell++) {
         shellRadiusSquare =
             P::systemWriteDistributionWriteShellRadius[ishell] * P::systemWriteDistributionWriteShellRadius[ishell];
         ...
         dz_rp = cellZ < 0 ? 0 : DZ;
         rsquare_minus =
             (cellX + dx_rm) * (cellX + dx_rm) + (cellY + dy_rm) * (cellY + dy_rm) + (cellZ + dz_rm) * (cellZ + dz_rm);
         rsquare_plus =
             (cellX + dx_rp) * (cellX + dx_rp) + (cellY + dy_rp) * (cellY + dy_rp) + (cellZ + dz_rp) * (cellZ + dz_rp);
         ...
         if (withinshell) {
            ...
            if ((P::xcells_ini == 1) || (P::ycells_ini == 1) || (P::zcells_ini == 1)) {
               ...
               if ((P::ycells_ini == 1) &&
                   (((cellX > -1.1 * DX) && (cellX < 0)) || ((cellZ > -1.1 * DZ) && (cellZ < 0))))
                  stridecheck = false;
               if ((P::zcells_ini == 1) &&
                   (((cellX > -1.1 * DX) && (cellX < 0)) || ((cellY > -1.1 * DY) && (cellY < 0))))
                  stridecheck = false;
               ...
               // Clock angle
               bool clockcheck = (T2A >= s[1] - 0.5 * DX) && (T2A < s[1] + 0.5 * DX) && (T2B >= s[0] - 0.5 * DX) &&
                                 (T2B < s[0] + 0.5 * DX);
               ...
               if (((cellX > -1.1 * DX) && (cellX < 0)) || ((cellY > -1.1 * DY) && (cellY < 0)) ||
                   ((cellZ > -1.1 * DZ) && (cellZ < 0)))
                  stridecheck = false;
            }
...
/*! This function makes sure that local cells and ghost cells do not have any identical members (used for error
 checking) \param local_cells List of local cells within this process \param ghost_cells List of ghost cells within this
 process (cells on the process boundary)
 */
...
bool writeGrid(dccrg::Dccrg<SpatialCell, dccrg::Cartesian_Geometry>& mpiGrid,
               ...
               const std::string& configInfo, DataReducer* dataReducer, const uint& outputFileTypeIndex,
   ...
   if (writeCommonGridData(vlsvWriter, mpiGrid, local_cells, P::systemWrites[outputFileTypeIndex], MPI_COMM_WORLD) ==
       false) {
      return false;
   }
   ...
   if (dataReducer != NULL)
      for (uint i = 0; i < dataReducer->size(); ++i) {
         if (writeDataReducer(mpiGrid, local_cells, perBGrid, EGrid, EHallGrid, EGradPeGrid, momentsGrid, dPerBGrid,
                              dMomentsGrid, BgBGrid, volGrid, technicalGrid, (P::writeAsFloat == 1),
   ...
   if (bytesWritten > 1.0e9)
      logFile << bytesWritten / 1.0e9 << " GB in ";
   else if (bytesWritten > 1e6)
      logFile << bytesWritten / 1.0e6 << " MB in ";
   else if (bytesWritten > 1e3)
      logFile << bytesWritten / 1.0e3 << " kB in ";
   else
      logFile << bytesWritten << " B in ";

   logFile << writeTime << " seconds, approximate data rate is ";

   if (bytesWritten / writeTime > 1e9)
      logFile << bytesWritten / writeTime / 1e9 << " GB/s";
   else if (bytesWritten / writeTime > 1e6)
      logFile << bytesWritten / writeTime / 1e6 << " MB/s";
   else if (bytesWritten / writeTime > 1e3)
      logFile << bytesWritten / writeTime / 1e3 << " kB/s";
   else
      logFile << bytesWritten / writeTime << " B/s";
   logFile << endl;
...
bool writeRestart(dccrg::Dccrg<SpatialCell, dccrg::Cartesian_Geometry>& mpiGrid,
   ...
   restartReducer.addOperator(
       new DRO::DataReductionOperatorCellParams("vg_bulk_forcing_flag", CellParams::FORCING_CELL_NUM, 1));
   ...
   if (SBC::ionosphereGrid.nodes.size() > 0) {
      restartReducer.addOperator(
          new DRO::DataReductionOperatorIonosphereNode("ig_fac", [](SBC::SphericalTriGrid& grid) -> std::vector<Real> {
      ...
      restartReducer.addOperator(
          new DRO::DataReductionOperatorIonosphereNode("ig_rhon", [](SBC::SphericalTriGrid& grid) -> std::vector<Real> {
             std::vector<Real> retval(grid.nodes.size());
      ...
      restartReducer.addOperator(new DRO::DataReductionOperatorIonosphereNode(
   ...
   for (uint i = 0; i < restartReducer.size(); ++i) {
      writeDataReducer(mpiGrid, local_cells, perBGrid, EGrid, EHallGrid, EGradPeGrid, momentsGrid, dPerBGrid,
                       dMomentsGrid, BgBGrid, volGrid, technicalGrid, writeAsFloat, true, restartReducer, i,
                       vlsvWriter);
   }
   reducedTimer.stop();
...
bool writeDiagnostic(const dccrg::Dccrg<SpatialCell, dccrg::Cartesian_Geometry>& mpiGrid, DataReducer& dataReducer) {
   ...
   vector<Real> localMin(nOps), localMax(nOps), localSum(nOps + 1), localAvg(nOps), globalMin(nOps), globalMax(nOps),
       globalSum(nOps + 1), globalAvg(nOps);
   ...
   if (printDiagnosticHeader == true && myRank == MASTER_RANK) {
      ...
      for (uint i = 0; i < nOps; ++i) {
         diagnostic << "# Columns " << 4 + i * 4 << " to " << 7 + i * 4 << ": " << dataReducer.getName(i)
   ...
   for (uint i = 0; i < nOps; ++i) {
      ...
      for (uint64_t cell = 0; cell < nCells; ++cell) {
         success = true;
         if (dataReducer.reduceDiagnostic(mpiGrid[cells[cell]], i, &buffer) == false)
            success = false;
      ...
      if (success == false)
         logFile << "(MAIN) writeDiagnostic: ERROR datareductionoperator '" << dataReducer.getName(i)
                 << "' returned false!" << endl
                 << writeVerbose;
   }
