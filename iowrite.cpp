...
bool writeVelocityDistributionData(const uint popID, Writer& vlsvWriter, dccrg::Dccrg<SpatialCell, dccrg::Cartesian_Geometry>& mpiGrid, const std::vector<CellID>& cells, MPI_Comm comm);
...
/** Writes the velocity distribution into the file.
 ...
 @return Returns true if operation was successful.*/
bool writeVelocityDistributionData(Writer& vlsvWriter, dccrg::Dccrg<SpatialCell, dccrg::Cartesian_Geometry>& mpiGrid, const vector<CellID>& cells, MPI_Comm comm) {
   bool success = true;
   for (uint popID = 0; popID < getObjectWrapper().particleSpecies.size(); ++popID) {
      if (writeVelocityDistributionData(popID, vlsvWriter, mpiGrid, cells, comm) == false)
         success = false;
   }
   return success;
}
...
/*! Writes info received from data reducer. This function writes out the variable arrays into the file
 ...
 \return Returns true if operation was successful
 */
bool writeDataReducer(const dccrg::Dccrg<SpatialCell, dccrg::Cartesian_Geometry>& mpiGrid, const std::vector<CellID>& cells,
                      ...
                      FsGrid<std::array<Real, fsgrids::volfields::N_VOL>, FS_STENCIL_WIDTH>& volGrid, FsGrid<fsgrids::technical, FS_STENCIL_WIDTH>& technicalGrid, const bool writeAsFloat,
   ...
   if (dataReducer.getName(dataReducerIndex).find("fg_", 0) == 0) {
      ...
      success = dataReducer.writeFsGridData(
          perBGrid, EGrid, EHallGrid, EGradPeGrid, momentsGrid, dPerBGrid, dMomentsGrid, BgBGrid, volGrid, technicalGrid, "fsgrid", dataReducerIndex, vlsvWriter, writeAsFloat);
      writeFsTimer.stop();
...
/*! Writes common grid data such as parameters (time steps, x_min, ..) as well as local cell ids as variables
 ...
 \return Returns true if operation was successful
 */
bool writeCommonGridData(Writer& vlsvWriter, dccrg::Dccrg<SpatialCell, dccrg::Cartesian_Geometry>& mpiGrid, const vector<uint64_t>& local_cells, const uint& fileIndex, MPI_Comm comm) {
   ...
   if (vlsvWriter.writeParameter("velocity_block_width", &writewid) == false) {
      return false;
   }
...
/*! Writes ghost cell ids into the file
 ...
 \sa updateLocalIds
 */
bool writeGhostZoneDomainAndLocalIdNumbers(dccrg::Dccrg<SpatialCell, dccrg::Cartesian_Geometry>& mpiGrid, Writer& vlsvWriter, const string& meshName, const vector<uint64_t>& ghost_cells) {
...
/*! Writes the zone global id numbers into the file. The vlsv file needs to know in which order the local cells + ghost cells are written. Local cells are first appended to a vector called global ids,
 after which the ghost cells are appended. The global ids vector will then be saved into a vlsv file
 \param mpiGrid Vlasiator's MPI grid
 \param vlsvWriter Some vlsv writer with a file open
 ...
 */
bool writeZoneGlobalIdNumbers(const dccrg::Dccrg<SpatialCell, dccrg::Cartesian_Geometry>& mpiGrid, Writer& vlsvWriter, const string& meshName, const vector<uint64_t>& local_cells,
...
/*! Writes the node coordinates. This means basically every cell's node (Corner of each cell). Note: The grid is a structured one, so writing the nodes means starting from the corner of the grid and
 writing coordinates per every cell length until reaching the other corner of the grid
 \param vlsvWriter Some vlsv writer with a file open
...
bool writeBoundingBoxNodeCoordinates(Writer& vlsvWriter, const string& meshName, const int masterRank, MPI_Comm comm) {
   ...
   if (myRank == masterRank) {
      ...
      arraySize = xCells + 1;
      if (vlsvWriter.writeArray("MESH_NODE_CRDS_X", xmlAttributes, arraySize, vectorSize, xNodeCoordinates.data()) == false)
         success = false;
      arraySize = yCells + 1;
      if (vlsvWriter.writeArray("MESH_NODE_CRDS_Y", xmlAttributes, arraySize, vectorSize, yNodeCoordinates.data()) == false)
         success = false;
      arraySize = zCells + 1;
      if (vlsvWriter.writeArray("MESH_NODE_CRDS_Z", xmlAttributes, arraySize, vectorSize, zNodeCoordinates.data()) == false)
         success = false;
   } else {
...
/*! Function for writing the bounding box. This writes only if the process running it is the master rank. This array contains info on the boundaries of the grid so for example the number of cells in
 x, y, z direction.
...
bool writeMeshBoundingBox(Writer& vlsvWriter, const string& meshName, const int masterRank, MPI_Comm comm) {
   ...
   uint64_t boundaryBox[box_size] = {numberOfXCells, numberOfYCells, numberOfZCells, notBlockBasedMesh, notBlockBasedMesh, notBlockBasedMesh};
 ...
 */
bool writeFsGridMetadata(FsGrid<fsgrids::technical, FS_STENCIL_WIDTH>& technicalGrid, vlsv::Writer& vlsvWriter, bool writeIDs = false) {
   ...
   std::array<FsGridTools::FsSize_t, 6> boundaryBox({globalSize[0], globalSize[1], globalSize[2], 1, 1, 1});
      ...
      for (int z = 0; z < localSize[2]; z++) {
         for (int y = 0; y < localSize[1]; y++) {
            for (int x = 0; x < localSize[0]; x++) {
               ...
               globalIds[i++] = globalIndex[2] * globalSize[0] * globalSize[1] + globalIndex[1] * globalSize[0] + globalIndex[0];
            }
         }
      }
...
bool writeIonosphereGridMetadata(vlsv::Writer& vlsvWriter) {
   ...
   if (vlsvWriter.writeParameter("ionosphere_radius", &SBC::Ionosphere::innerRadius) == false) {
      return false;
   }
   if (vlsvWriter.writeParameter("ionosphere_downmapping_radius", &SBC::Ionosphere::downmapRadius) == false) {
      return false;
   }
   if (vlsvWriter.writeParameter("ionosphere_time_smoothing_constant", &SBC::Ionosphere::couplingTimescale) == false) {
      return false;
   }
   if (vlsvWriter.writeParameter("ionosphere_time_interval", &SBC::Ionosphere::couplingInterval) == false) {
      return false;
   }
   ...
   for (uint i = 0; i < cells.size(); i++) {
      ...
      for (int AMR = 0; AMR <= P::amrMaxSpatialRefLevel; AMR++) {
         ...
         if ((cells[i] >= startindex) && (cells[i] < endindex)) {
            ...
            if ((P::systemWriteDistributionWriteXlineStride[index] > 0 && P::systemWriteDistributionWriteYlineStride[index] > 0 && lineX % P::systemWriteDistributionWriteXlineStride[index] == 0 &&
                 lineY % P::systemWriteDistributionWriteYlineStride[index] == 0) &&
                (P::systemWriteDistributionWriteYlineStride[index] > 0 && P::systemWriteDistributionWriteZlineStride[index] > 0 && lineY % P::systemWriteDistributionWriteYlineStride[index] == 0 &&
                 lineZ % P::systemWriteDistributionWriteZlineStride[index] == 0) &&
                (P::systemWriteDistributionWriteZlineStride[index] > 0 && P::systemWriteDistributionWriteXlineStride[index] > 0 && lineZ % P::systemWriteDistributionWriteZlineStride[index] == 0 &&
...
bool writeGrid(dccrg::Dccrg<SpatialCell, dccrg::Cartesian_Geometry>& mpiGrid, FsGrid<std::array<Real, fsgrids::bfield::N_BFIELD>, FS_STENCIL_WIDTH>& perBGrid,
               ...
               FsGrid<fsgrids::technical, FS_STENCIL_WIDTH>& technicalGrid, const std::string& versionInfo, const std::string& configInfo, DataReducer* dataReducer, const uint& outputFileTypeIndex,
   ...
   if (P::systemWriteHints.size() == 0) {
      ...
      for (std::vector<std::pair<std::string, std::string>>::const_iterator it = P::systemWriteHints.begin(); it != P::systemWriteHints.end(); it++) {
...
bool writeRestart(dccrg::Dccrg<SpatialCell, dccrg::Cartesian_Geometry>& mpiGrid, FsGrid<std::array<Real, fsgrids::bfield::N_BFIELD>, FS_STENCIL_WIDTH>& perBGrid,
                  ...
                  FsGrid<std::array<Real, fsgrids::dperb::N_DPERB>, FS_STENCIL_WIDTH>& dPerBGrid, FsGrid<std::array<Real, fsgrids::dmoments::N_DMOMENTS>, FS_STENCIL_WIDTH>& dMomentsGrid,
                  FsGrid<std::array<Real, fsgrids::bgbfield::N_BGB>, FS_STENCIL_WIDTH>& BgBGrid, FsGrid<std::array<Real, fsgrids::volfields::N_VOL>, FS_STENCIL_WIDTH>& volGrid,
                  FsGrid<fsgrids::technical, FS_STENCIL_WIDTH>& technicalGrid, const std::string& versionInfo, const std::string& configInfo, DataReducer& dataReducer, const string& name,
   ...
   if (P::restartWriteHints.size() == 0) {
      ...
      for (std::vector<std::pair<std::string, std::string>>::const_iterator it = P::restartWriteHints.begin(); it != P::restartWriteHints.end(); it++) {
         MPI_Info_set(MPIinfo, it->first.c_str(), it->second.c_str());
      }
   }
   if (stripe < -1) {
   ...
   // Write FSGrid metadata
   if (writeFsGridMetadata(technicalGrid, vlsvWriter, true) == false)
      return false;
   ...
   for (uint i = 0; i < restartReducer.size(); ++i) {
      writeDataReducer(
          mpiGrid, local_cells, perBGrid, EGrid, EHallGrid, EGradPeGrid, momentsGrid, dPerBGrid, dMomentsGrid, BgBGrid, volGrid, technicalGrid, writeAsFloat, true, restartReducer, i, vlsvWriter);
   }
   reducedTimer.stop();
...
bool writeDiagnostic(const dccrg::Dccrg<SpatialCell, dccrg::Cartesian_Geometry>& mpiGrid, DataReducer& dataReducer) {
   ...
   vector<Real> localMin(nOps), localMax(nOps), localSum(nOps + 1), localAvg(nOps), globalMin(nOps), globalMax(nOps), globalSum(nOps + 1), globalAvg(nOps);
