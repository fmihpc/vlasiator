...
bool Copysphere::initSysBoundary(creal& t, Project& project) {
   getParameters();
   isThisDynamic = false;
...
Real getR(creal x, creal y, creal z, uint geometry, Real center[3]) {
   ...
   default:
      std::cerr << __FILE__ << ":" << __LINE__ << ":"
                << "copysphere.geometry has to be 0, 1 or 2." << std::endl;
      abort();
   }
...
bool Copysphere::applyInitialState(const dccrg::Dccrg<SpatialCell, dccrg::Cartesian_Geometry>& mpiGrid,
                                   FsGrid<fsgrids::technical, FS_STENCIL_WIDTH>& technicalGrid,
   ...
   if (Parameters::xcells_ini == 1) {
      if (Parameters::ycells_ini == 1) {
         if (Parameters::zcells_ini == 1) {
            ...
            std::cerr
                << __FILE__ << ":" << __LINE__ << ":"
                << "What do you expect to do with a single-cell simulation of copysphere boundary type? Stop kidding."
                << std::endl;
            abort();
            // end of X,Y,Z
         } else {
