...
/*!\file maxwellian.cpp
 * \brief Implementation of the class SysBoundaryCondition::Maxwellian to handle
 * cells classified as sysboundarytype::MAXWELLIAN.
 */
...
/* Here the while loop iterates from the centre of the maxwellian in blocksize (4*dvx) increments, and looks at the
 * centre of the first velocity cell in the block (+0.5dvx), checking if the phase-space density is large enough to be
 * included due to sparsity threshold. That results in a "blocks radius" vRadiusSquared from the centre of the
 * Maxwellian distribution. Then we iterate through the actual blocks and calculate their radius R2 based on their
 * velocity coordinates and the plasma bulk velocity. Blocks that fullfil R2 < vRadiusSquared are included to
 * blocksToInitialize.
 */
std::vector<vmesh::GlobalID> Maxwellian::findBlocksToInitialize(const uint popID, spatial_cell::SpatialCell& cell,
                                                                ...
                                                                creal& VZ0) {
   std::vector<vmesh::GlobalID> blocksToInitialize;
   bool search = true;
   uint counter = 0;
...
/*!\brief Generate the template cell for the face corresponding to the index passed, which will be used for setting the
 * boundary conditions.
