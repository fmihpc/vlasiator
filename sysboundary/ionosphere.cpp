...
namespace SBC {
   ...
   struct SphericalTriGrid {
      ...
      std::map<std::array<Real, 3>, std::array<std::pair<int, Real>, 3>> vlasovGridCoupling; /*!< Grid coupling information, caching how vlasovGrid coordinate couple to ionosphere data */
      ...
      void updateIonosphereCommunicator(dccrg::Dccrg<SpatialCell, dccrg::Cartesian_Geometry>& mpiGrid,
                                        FsGrid<fsgrids::technical, FS_STENCIL_WIDTH>& technicalGrid); /*!< (Re-)create the subcommunicator for ionosphere-internal communication */
      ...
      void calculateConductivityTensor(
          ...
          const bool refillTensorAtRestart = false); /*!< Update sigma tensor, if last argument is true, just refill the tensor from SIGMAH, SIGMAP and SIGMAPARALLEL from restart data */
      ...
      Real elementArea(uint32_t elementIndex) {
         ...
         std::array<Real, 3> area{e1[1] * e2[2] - e1[2] * e2[1], e1[2] * e2[0] - e1[0] * e2[2], e1[0] * e2[1] - e1[1] * e2[0]};
      ...
      std::array<Real, 3> mappedElementArea(uint32_t elementIndex) {
         ...
         if (sqrt(a[0] * a[0] + a[1] * a[1] + a[2] * a[2]) == 0 || sqrt(b[0] * b[0] + b[1] * b[1] + b[2] * b[2]) == 0 || sqrt(c[0] * c[0] + c[1] * c[1] + c[2] * c[2]) == 0) {
         ...
         std::array<Real, 3> area{HALF * (e1[1] * e2[2] - e1[2] * e2[1]), HALF * (e1[2] * e2[0] - e1[0] * e2[2]), HALF * (e1[0] * e2[1] - e1[1] * e2[0])};

         // By definition, the area is oriented outwards, so if dot(r,A) < 0, flip it.
         std::array<Real, 3> r{(a[0] + b[0] + c[0]) * THIRD, (a[1] + b[1] + c[1]) * THIRD, (a[2] + b[2] + c[2]) * THIRD};
   ...
   public:
      ...
      virtual void assignSysBoundary(dccrg::Dccrg<SpatialCell, dccrg::Cartesian_Geometry>& mpiGrid, FsGrid<fsgrids::technical, FS_STENCIL_WIDTH>& technicalGrid) override;
      ...
      virtual void fieldSolverBoundaryCondElectricField(FsGrid<std::array<Real, fsgrids::efield::N_EFIELD>, FS_STENCIL_WIDTH>& EGrid, cint i, cint j, cint k, cuint component) override;
      virtual void fieldSolverBoundaryCondHallElectricField(FsGrid<std::array<Real, fsgrids::ehall::N_EHALL>, FS_STENCIL_WIDTH>& EHallGrid, cint i, cint j, cint k, cuint component) override;
      virtual void fieldSolverBoundaryCondGradPeElectricField(FsGrid<std::array<Real, fsgrids::egradpe::N_EGRADPE>, FS_STENCIL_WIDTH>& EGradPeGrid, cint i, cint j, cint k, cuint component) override;
      ...
      virtual void fieldSolverBoundaryCondBVOLDerivatives(FsGrid<std::array<Real, fsgrids::volfields::N_VOL>, FS_STENCIL_WIDTH>& volGrid, cint i, cint j, cint k, cuint component) override;
      ...
      virtual void updateState(dccrg::Dccrg<SpatialCell, dccrg::Cartesian_Geometry>& mpiGrid, FsGrid<std::array<Real, fsgrids::bfield::N_BFIELD>, FS_STENCIL_WIDTH>& perBGrid,
                               FsGrid<std::array<Real, fsgrids::bgbfield::N_BGB>, FS_STENCIL_WIDTH>& BgBGrid, creal t) override;
      ...
      std::array<Real, 3> fieldSolverGetNormalDirection(FsGrid<fsgrids::technical, FS_STENCIL_WIDTH>& technicalGrid, cint i, cint j, cint k);
