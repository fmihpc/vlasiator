 ...
 * \brief Implementation of the class SysBoundaryCondition::Conductingsphere to handle cells classified as
 * sysboundarytype::CONDUCTINGSPHERE.
 */
...
void Conductingsphere::addParameters() {
   ...
   Readparameters::add(
       "conductingsphere.geometry",
       "Select the geometry of the conductingsphere, 0: inf-norm (diamond), 1: 1-norm (square), 2: 2-norm "
       "(circle, DEFAULT), 3: 2-norm cylinder aligned with y-axis, use with polar plane/line dipole.",
       2);
   Readparameters::add(
       "conductingsphere.precedence",
       "Precedence value of the conductingsphere system boundary condition (integer), the higher the stronger.", 2);
   Readparameters::add("conductingsphere.reapplyUponRestart",
   ...
   for (uint i = 0; i < getObjectWrapper().particleSpecies.size(); i++) {
      ...
      Readparameters::add(
          pop + "_conductingsphere.taperRadius",
          "Width of the zone with a density tapering from the conducting sphere value to the background (m)", 0.0);
      ...
      Readparameters::add(pop + "_conductingsphere.VX0",
                          "Bulk velocity of conductospheric distribution function in X direction (m/s)", 0.0);
      Readparameters::add(pop + "_conductingsphere.VY0",
                          "Bulk velocity of conductospheric distribution function in X direction (m/s)", 0.0);
      Readparameters::add(pop + "_conductingsphere.VZ0",
                          "Bulk velocity of conductospheric distribution function in X direction (m/s)", 0.0);
      Readparameters::add(pop + "_conductingsphere.fluffiness",
...
void Conductingsphere::initBoundary(creal t, Project &project) {
   getParameters();
   dynamic = false;
...
void Conductingsphere::assignBoundary(dccrg::Dccrg<SpatialCell, dccrg::Cartesian_Geometry> &mpiGrid,
                                FsGrid<fsgrids::technical, FS_STENCIL_WIDTH> &technicalGrid) {
   for (auto& cell : mpiGrid.get_cells()) {
      ...
      if (getR(x, y, z, this->geometry, this->center) < this->radius) {
         mpiGrid[cell]->sysBoundaryFlag = this->getIndex();
      }
   }
...
void Conductingsphere::applyInitialState(const dccrg::Dccrg<SpatialCell, dccrg::Cartesian_Geometry> &mpiGrid,
...
std::array<Real, 3>
Conductingsphere::fieldSolverGetNormalDirection(FsGrid<fsgrids::technical, FS_STENCIL_WIDTH> &technicalGrid, cint i, cint j,
                                          cint k) {
...
void Conductingsphere::fieldSolverBoundaryCondBVOLDerivatives(
   ...
   // conductingsphere cells.
   this->setCellBVOLDerivativesToZero(volGrid, i, j, k, component);
}

void Conductingsphere::vlasovBoundaryCondition(const dccrg::Dccrg<SpatialCell, dccrg::Cartesian_Geometry> &mpiGrid,
...
Real Conductingsphere::shiftedMaxwellianDistribution(const uint popID, creal &vx, creal &vy, creal &vz) {
