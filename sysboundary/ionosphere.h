...
struct SphericalTriGrid {
   ...
   struct Node {
      ...
      Real deltaPhi() { // Field aligned potential drop between i'sphere and m'sphere
         ...
         //    * sqrt(2. * M_PI * physicalconstants::MASS_ELECTRON / (physicalconstants::K_B * electronTemperature()))
         //    - 1.);
   ...
   void resetReconstructionCoefficientsCache() { reconstructionCoefficientsCache.clear(); }
   ...
   std::array<std::array<std::array<Real, productionNumTemperatures>, productionNumAccEnergies>, numAtmosphereLevels>
       productionTable;
   ...
   bool isCouplingOutwards =
       true; // True for any rank that actually couples ionosphere potential information out to the vlasov grid
   FieldFunction dipoleField; // Simulation background field model to trace connections with
   std::map<std::array<Real, 3>, std::array<std::pair<int, Real>, 3>>
       vlasovGridCoupling; // Grid coupling information, caching how vlasovGrid coordinate couple to ionosphere data
   ...
   void updateIonosphereCommunicator(
       dccrg::Dccrg<SpatialCell, dccrg::Cartesian_Geometry>& mpiGrid,
       FsGrid<fsgrids::technical, FS_STENCIL_WIDTH>&
           technicalGrid);                   // (Re-)create the subcommunicator for ionosphere-internal communication
   ...
   void calculateConductivityTensor(const Real F10_7, const Real recombAlpha,
                                    const Real backgroundIonisation); // Update sigma tensor
   void calculateFsgridCoupling(FsGrid<fsgrids::technical, FS_STENCIL_WIDTH>& technicalGrid,
                                FsGrid<std::array<Real, fsgrids::bfield::N_BFIELD>, FS_STENCIL_WIDTH>& perBGrid,
                                FsGrid<std::array<Real, fsgrids::dperb::N_DPERB>, FS_STENCIL_WIDTH>& dPerBGrid,
                                Real radius);                       // Link each element to fsgrid cells for coupling
   ...
   calculateVlasovGridCoupling(std::array<Real, 3> x,
                               Real couplingRadius); // Find coupled ionosphere mesh node for given location
   ...
   void bulirschStoerStep(std::array<Real, 3>& r, std::array<Real, 3>& b, Real& stepsize, Real maxStepsize,
                          TracingFieldFunction& BFieldFunction,
                          bool outwards = true); // Bulrisch Stoer step
   void eulerStep(std::array<Real, 3>& x, std::array<Real, 3>& v, Real& stepsize, TracingFieldFunction& BFieldFunction,
                  bool outwards = true); // Euler step
   void modifiedMidpointMethod(std::array<Real, 3> r, std::array<Real, 3>& r1, Real n, Real stepsize,
                               TracingFieldFunction& BFieldFunction,
                               bool outwards = true); // Modified Midpoint Method used by BS step
   void richardsonExtrapolation(int i, std::vector<Real>& table, Real& maxError,
                                std::array<int, 3> dims); // Richardson extrapolation method used by BS step
   void stepFieldLine(std::array<Real, 3>& x, std::array<Real, 3>& v, Real& stepsize, Real maxStepsize,
                      IonosphereCouplingMethod method, TracingFieldFunction& BFieldFunction,
                      bool outwards = true); // Handler function for field line tracing
   ...
   void mapDownBoundaryData(FsGrid<std::array<Real, fsgrids::bfield::N_BFIELD>, FS_STENCIL_WIDTH>& perBGrid,
                            FsGrid<std::array<Real, fsgrids::dperb::N_DPERB>, FS_STENCIL_WIDTH>& dPerBGrid,
   ...
   std::array<Real, 3> mappedElementArea(uint32_t elementIndex) {
      ...
      if (sqrt(a[0] * a[0] + a[1] * a[1] + a[2] * a[2]) == 0 || sqrt(b[0] * b[0] + b[1] * b[1] + b[2] * b[2]) == 0 ||
      ...
      std::array<Real, 3> r{(a[0] + b[0] + c[0]) / 3., (a[1] + b[1] + c[1]) / 3., (a[2] + b[2] + c[2]) / 3.};
 ...
 *
 * Ionosphere is a class handling cells tagged as sysboundarytype::IONOSPHERE by this system boundary condition. It
 * applies ionospheric boundary conditions.
 *
 * These consist in:
...
public:
   ...
   static bool
       solverToggleMinimumResidualVariant; /*!< Toggle use of the minimum residual variant between solver restarts */
   static Real shieldingLatitude; /*! Latitude (degree) below which the potential is zeroed in the equator gauge fixing
                                     scheme */
