      ...
      for (uint j = 0; j < 6; j++) {
         if (vlasovSysBoundarySchemeName[j] == "None") {
            ...
            if (myRank == MASTER_RANK)
               cerr << __FILE__ << ":" << __LINE__ << " ERROR: " << vlasovSysBoundarySchemeName[j]
                    << " is an invalid Outflow Vlasov scheme!" << endl;
            exit(1);
         }
      }
...
bool Outflow::initSysBoundary(creal& t, Project& project) {
