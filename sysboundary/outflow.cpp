...
/*!\file outflow.cpp
 * \brief Class SysBoundaryCondition::Outflow to handle cells classified as sysboundarytype::OUTFLOW.
 */
...
void Outflow::initBoundary(creal t, Project& project) {
   ...
   for (uint i = 0; i < getObjectWrapper().particleSpecies.size(); i++) {
      ...
      for (auto& it : sP.faceToReapplyUponRestartList) {
         if (it == "x+") {
...
void Outflow::assignBoundary(dccrg::Dccrg<SpatialCell, dccrg::Cartesian_Geometry>& mpiGrid,
   ...
   for (auto& dccrgId : getLocalCells()) {
