...
namespace SBC {
   ...
   void SysBoundaryCondition::vlasovBoundaryCopyFromTheClosestNbr(
      dccrg::Dccrg<SpatialCell, dccrg::Cartesian_Geometry>& mpiGrid, const CellID& cellID, const bool& copyMomentsOnly, const uint popID, const bool copy_V_moments
   ) {
   ...
   void SysBoundaryCondition::vlasovBoundaryFluffyCopyFromAllCloseNbrs(
      dccrg::Dccrg<SpatialCell, dccrg::Cartesian_Geometry>& mpiGrid, const CellID& cellID, const uint popID, const bool copy_V_moments, creal fluffiness
   ) {
   ...
   void averageCellData(
      dccrg::Dccrg<SpatialCell, dccrg::Cartesian_Geometry>& mpiGrid, const vector<CellID> cellList, SpatialCell* to, const uint popID, creal fluffiness /* default =0.0*/
   ) {
   ...
   Real SysBoundaryCondition::fieldBoundaryCopyFromSolvingNbrMagneticField(
      FsGrid<array<Real, fsgrids::bfield::N_BFIELD>, FS_STENCIL_WIDTH>& bGrid, FsGrid<fsgrids::technical, FS_STENCIL_WIDTH>& technicalGrid, cint i, cint j, cint k, cuint component, cuint mask
   ) {
      ...
      for (int kk = -2; kk < 3; kk++) {
         for (int jj = -2; jj < 3; jj++) {
            for (int ii = -2; ii < 3; ii++) {
               if (
                  technicalGrid.get(i + ii, j + jj, k + kk)                                                                // skip invalid cells returning NULL
