 ...
 */
void SysBoundaryCondition::determineFace(bool* isThisCellOnAFace, creal x, creal y, creal z, creal dx, creal dy,
                                         creal dz,
 ...
 *
 * \param isThisCellOnAFace Referemce to an std::array of 6 bool returning of each face whether the cell is on that
 * face. Order: 0 x+; 1 x-; 2 y+; 3 y-; 4 z+; 5 z- \param mpiGrid Reference to grid \param id ID of cell to check \param
 * excludeSlicesAndPeriodicDimensions If true, do not consider a cell to be part of the face if that face has a depth of
 * 1 only (single-cell thick slices/columns) or if that direciton is periodic..
 */
void SysBoundaryCondition::determineFace(std::array<bool, 6>& isThisCellOnAFace,
                                         const dccrg::Dccrg<SpatialCell, dccrg::Cartesian_Geometry>& mpiGrid, CellID id,
                                         const bool excludeSlicesAndPeriodicDimensions //=false (default)
) {
...
/*! SysBoundaryCondition base class instance of the addParameters function. Should not be used, each derived class
 * should have its own.*/
...
/*! Function used to average and copy the distribution and moments from all the closest sysboundarytype::NOT_SYSBOUNDARY
...
/*! Function used to average and copy the distribution and moments from all the close sysboundarytype::NOT_SYSBOUNDARY
 * cells. \param mpiGrid Grid \param cellID The cell's ID.
 */
...
/*! Function used to copy the distribution from (one of) the closest sysboundarytype::NOT_SYSBOUNDARY cell but limiting
 * to values no higher than where it can flow into. Moments are recomputed. \param mpiGrid Grid \param cellID The cell's
 * ID.
 */
 ...
 */
void SysBoundaryCondition::copyCellData(SpatialCell* from, SpatialCell* to, const bool copyMomentsOnly,
                                        const uint popID, const bool calculate_V_moments) {
   if (popID == 0) {
 ...
 */
void averageCellData(const dccrg::Dccrg<SpatialCell, dccrg::Cartesian_Geometry>& mpiGrid, const vector<CellID> cellList,
                     SpatialCell* to, const uint popID, creal fluffiness /* default =0.0*/
) {
   ...
   for (size_t i = 0; i < numberOfCells; i++) {
      ...
      for (vmesh::LocalID incBlockLID = 0; incBlockLID < incomingCell->get_number_of_velocity_blocks(popID);
           ++incBlockLID) {
 ...
 */
void SysBoundaryCondition::vlasovBoundaryReflect(const dccrg::Dccrg<SpatialCell, dccrg::Cartesian_Geometry>& mpiGrid,
               ...
               for (uint ic = 0; ic < WID; ++ic) {
                  ...
                  if (vNormal >= 0.0) {
                     ...
                     cell->increment_value(
                         vxCellCenter, vyCellCenter, vzCellCenter,
                         factor * incomingCell->get_value(vxCellCenter, vyCellCenter, vzCellCenter, popID), popID);
                  } else {
                     // Flowing in, bounce off.
                     cell->increment_value(
                         vxCellCenter - 2.0 * vNormal * nx, vyCellCenter - 2.0 * vNormal * ny,
 ...
 * quench.
 */
void SysBoundaryCondition::vlasovBoundaryAbsorb(const dccrg::Dccrg<SpatialCell, dccrg::Cartesian_Geometry>& mpiGrid,
                                                const CellID& cellID, creal& nx, creal& ny, creal& nz,
                                                creal& quenchingFactor, const uint popID) {
   SpatialCell* cell = mpiGrid[cellID];
   const vector<CellID>& cellList = this->getAllClosestNonsysboundaryCells(cellID);
   const size_t numberOfCells = cellList.size();
         ...
         for (uint kc = 0; kc < WID; ++kc)
            for (uint jc = 0; jc < WID; ++jc)
               for (uint ic = 0; ic < WID; ++ic) {
                  ...
                  if (vNormal >= 0.0) {
                     // Not flowing in, leave as is.
                     cell->increment_value(vxCellCenter, vyCellCenter, vzCellCenter,
                                           factor * incomingCell->get_value(vxCellCenter, vyCellCenter, vzCellCenter),
                                           popID);
                  } else {
                     // Flowing in, bounce off.
                     cell->increment_value(vxCellCenter, vyCellCenter, vzCellCenter,
                                           factor * quenchingFactor *
...
bool SysBoundaryCondition::updateSysBoundaryConditionsAfterLoadBalance(
   ...
   // Loop over cellids
   for (vector<CellID>::const_iterator it = local_cells_on_boundary.begin(); it != local_cells_on_boundary.end();
        ++it) {
      ...
      // int indexstep = pow(2,P::amrMaxSpatialRefLevel -
      // mpiGrid[cellId]->SpatialCell::parameters[CellParams::REFINEMENT_LEVEL]); Note this must be int, not uint, for
      // latter calculations
 ...
 */
array<int, 3>
SysBoundaryCondition::getTheClosestNonsysboundaryCell(FsGrid<fsgrids::technical, FS_STENCIL_WIDTH>& technicalGrid,
 ...
 */
vector<array<int, 3>>
SysBoundaryCondition::getAllClosestNonsysboundaryCells(FsGrid<fsgrids::technical, FS_STENCIL_WIDTH>& technicalGrid,
                                                       cint i, cint j, cint k) {
   ...
   for (int kk = -2; kk < 3; kk++) {
      for (int jj = -2; jj < 3; jj++) {
         for (int ii = -2; ii < 3; ii++) {
            if (technicalGrid.get(i + ii, j + jj, k + kk) &&
                technicalGrid.get(i + ii, j + jj, k + kk)->sysBoundaryFlag == sysboundarytype::NOT_SYSBOUNDARY) {
 ...
 * \sa getAllClosestNonsysboundaryCells
 */
CellID& SysBoundaryCondition::getTheClosestNonsysboundaryCell(const CellID& cellID) {
 ...
 * \sa getTheClosestNonsysboundaryCell
 */
vector<CellID>& SysBoundaryCondition::getAllClosestNonsysboundaryCells(const CellID& cellID) {
   vector<CellID>& closestCells = allClosestNonsysboundaryCells.at(cellID);
   return closestCells;
}
 ...
 * \return The vector of cell indices of those cells
 */
vector<CellID>& SysBoundaryCondition::getAllCloseNonsysboundaryCells(const CellID& cellID) {
 ...
 * NOT_SYSBOUNDARY). \param cellID ID of the cell to start look from. \return The vector of cell indices of those cells
 */
array<SpatialCell*, 27>& SysBoundaryCondition::getFlowtoCells(const CellID& cellID) {
   phiprof::Timer timer{"getFlowtoCells"};
   array<SpatialCell*, 27>& flowtoCells = allFlowtoCells.at(cellID);
   return flowtoCells;
}

array<Realf*, 27> SysBoundaryCondition::getFlowtoCellsBlock(const array<SpatialCell*, 27> flowtoCells,
   ...
   for (uint i = 0; i < 27; i++) {
      if (flowtoCells.at(i)) {
         flowtoCellsBlock.at(i) =
             flowtoCells.at(i)->get_data(flowtoCells.at(i)->get_velocity_block_local_id(blockGID, popID), popID);
      }
   }
      ...
      for (int jj = -2; jj < 3; jj++) {
         for (int ii = -2; ii < 3; ii++) {
            if (technicalGrid.get(i + ii, j + jj, k + kk) // skip invalid cells returning NULL
                ...
                && technicalGrid.get(i + ii, j + jj, k + kk)->sysBoundaryFlag !=
                       sysboundarytype::DO_NOT_COMPUTE // Do not copy from there
            ) {
               distance = min(distance, ii * ii + jj * jj + kk * kk);
            }
         }
...
void SysBoundaryCondition::setPeriodicity(bool isFacePeriodic[3]) {
   for (uint i = 0; i < 3; i++) {
