 ...
 * slices/columns) or if that direction is periodic. Set to false by default.
 */
                     ...
                     for (int dvy = 0; dvy <= 1; dvy++) {
                        for (int dvz = 0; dvz <= 1; dvz++) {
                           ...
                           if (flowtoCells.at(flowToId)) {
                              value = std::min(value, flowtoCellsBlockCache.at(flowToId)[cell]);
                           }
                        }
                     }
                  }
...
void SysBoundaryCondition::updateBoundaryConditionsAfterLoadBalance(
    dccrg::Dccrg<SpatialCell, dccrg::Cartesian_Geometry>& mpiGrid, const std::vector<CellID>& local_cells_on_boundary) {
   // Loop over cellids
   for (std::vector<CellID>::const_iterator it = local_cells_on_boundary.begin(); it != local_cells_on_boundary.end();
        ++it) {
      ...
      uint d2 = std::numeric_limits<uint>::max();
      int indexstep =
      ...
      for (int jj = -2; jj < 3; jj++) {
         for (int ii = -2; ii < 3; ii++) {
            if (technicalGrid.get(i + ii, j + jj, k + kk) &&
               ...
               distance = std::min(distance, ii * ii + jj * jj + kk * kk);
            }
         }
      }
   }
