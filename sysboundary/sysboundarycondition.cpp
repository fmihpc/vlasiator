 ...
 * \brief Implementation of the base class SysBoundaryCondition to handle
 * boundary cells.
 *
 * \sa donotcompute.cpp ionosphere.cpp outflow.cpp user.cpp maxwellian.cpp
 * maxwellian.cpp
 */
...
namespace SBC {
// ************************************************************
// ***** DEFINITIONS FOR SYSBOUNDARYCONDITION BASE CLASS *****
// ************************************************************
...
void SysBoundaryCondition::determineFace(bool* isThisCellOnAFace, creal x, creal y, creal z, creal dx, creal dy,
                                         creal dz, const bool excludeSlicesAndPeriodicDimensions) {
...
/*! Base class default empty constructor.*/
SysBoundaryCondition::SysBoundaryCondition() {}

/*! Base class default empty virtual destructor.*/
SysBoundaryCondition::~SysBoundaryCondition() {}

/*! Base class instance of the addParameters function.
 * Each derived class should have its own implementation.*/
void SysBoundaryCondition::addParameters() {
   abort_mpi("ERROR: SysBoundaryCondition::addParameters called instead of derived class function!");
}
 ...
 * closest sysboundarytype::NOT_SYSBOUNDARY cell.
 * \param copyMomentsOnly If true, do not touch velocity space.
 */
void SysBoundaryCondition::vlasovBoundaryCopyFromClosestNbr(
 ...
 * closest sysboundarytype::NOT_SYSBOUNDARY cells.
...
/*! Function used to average and copy the distribution and moments from all the
 * close sysboundarytype::NOT_SYSBOUNDARY cells.
 */
 ...
 * sysboundarytype::NOT_SYSBOUNDARY cell but limit to values no higher than where it
 * can flow into. Moments are recomputed.
 */
void SysBoundaryCondition::vlasovBoundaryCopyFromClosestNbrAndLimit(
    const dccrg::Dccrg<SpatialCell, dccrg::Cartesian_Geometry>& mpiGrid, const CellID& cellID, const uint popID) {
   const CellID closestCell = getTheClosestNonboundaryCell(cellID);
...
void SysBoundaryCondition::copyCellData(SpatialCell* from, SpatialCell* to, const bool copyMomentsOnly,
                                        const uint popID, const bool doCalcMomentsV) {
   if (popID == 0) {
      if (doCalcMomentsV) {
 ...
 */
void SysBoundaryCondition::averageCellData(const dccrg::Dccrg<SpatialCell, dccrg::Cartesian_Geometry>& mpiGrid,
 ...
 * This is called from e.g. the class SysBoundary.
 * \param mpiGrid Grid
 * \param local_cells_on_boundary Cells within this process
 */
void SysBoundaryCondition::updateBoundaryConditionsAfterLoadBalance(
      ...
      for (auto nbrPair : *Nbrs) {
         if (nbrPair.first != INVALID_CELLID) {
            if (mpiGrid[nbrPair.first]->sysBoundaryFlag == sysboundarytype::NOT_SYSBOUNDARY) {
               ...
               if ((mpiGrid[cellId]->sysBoundaryLayer == 1) && (abs(nbrPair.second[0]) <= indexstep) &&
 ...
 */
std::array<int, 3>
SysBoundaryCondition::getTheClosestNonboundaryCell(FsGrid<fsgrids::technical, FS_STENCIL_WIDTH>& technicalGrid, cint i,
                                                   cint j, cint k) {
...
SysBoundaryCondition::getAllClosestNonboundaryCells(FsGrid<fsgrids::technical, FS_STENCIL_WIDTH>& technicalGrid, cint i,
                                                    cint j, cint k) {
 ...
 * \sa getAllClosestNonboundaryCells
 */
CellID& SysBoundaryCondition::getTheClosestNonboundaryCell(const CellID& cellID) {
 ...
 * \sa getTheClosestNonboundaryCell
 */
std::vector<CellID>& SysBoundaryCondition::getAllClosestNonboundaryCells(const CellID& cellID) {
   std::vector<CellID>& closestCells = allClosestNonboundaryCells.at(cellID);
   return closestCells;
}
 ...
 * \return The vector of cell indices of those cells
 */
std::vector<CellID>& SysBoundaryCondition::getAllCloseNonboundaryCells(const CellID& cellID) {
 ...
 * distribution can flow and which is of type NOT_SYSBOUNDARY).
 * \param cellID ID of the cell to start look from.
 * \return The vector of cell indices of those cells.
 */
std::array<SpatialCell*, 27>& SysBoundaryCondition::getFlowtoCells(const CellID& cellID) {
...
std::array<Realf*, 27> SysBoundaryCondition::getFlowtoCellsBlock(const std::array<SpatialCell*, 27> flowtoCells,
...
void SysBoundaryCondition::getFaces(bool* faces) {
   abort_mpi("ERROR: SysBoundaryCondition::getFaces called instead of derived class function!");
}
...
void abort_mpi(const string str, const int err_type) {
   int myRank;
