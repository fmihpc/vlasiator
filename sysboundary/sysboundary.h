...
namespace SBC {
    ...
    * \param excludeSlicesAndPeriodicDimensions If true, do not consider a cell to be part of the face if that face has a depth of 1 only (single-cell thick slices/columns) or if that direciton is
    * periodic..
    */
   void SysBoundaryCondition::determineFace(bool* isThisCellOnAFace, creal x, creal y, creal z, creal dx, creal dy, creal dz,
    ...
    * periodic..
    */
   void SysBoundaryCondition::determineFace(std::array<bool, 6>& isThisCellOnAFace, const dccrg::Dccrg<SpatialCell, dccrg::Cartesian_Geometry>& mpiGrid, CellID id,
                                            const bool excludeSlicesAndPeriodicDimensions //=false (default)
   ) {
   ...
   void SysBoundaryCondition::addParameters() { cerr << "ERROR: SysBoundaryCondition::addParameters called instead of derived class function!" << endl; }
    ...
    */
   void SysBoundaryCondition::setCellDerivativesToZero(FsGrid<array<Real, fsgrids::dperb::N_DPERB>, FS_STENCIL_WIDTH>& dPerBGrid,
                                                       FsGrid<array<Real, fsgrids::dmoments::N_DMOMENTS>, FS_STENCIL_WIDTH>& dMomentsGrid, cint i, cint j, cint k, cuint component) {
    ...
    */
   void SysBoundaryCondition::setCellBVOLDerivativesToZero(FsGrid<array<Real, fsgrids::volfields::N_VOL>, FS_STENCIL_WIDTH>& volGrid, cint i, cint j, cint k, cuint component) {
    ...
    */
   void SysBoundaryCondition::vlasovBoundaryCopyFromTheClosestNbr(dccrg::Dccrg<SpatialCell, dccrg::Cartesian_Geometry>& mpiGrid, const CellID& cellID, const bool& copyMomentsOnly, const uint popID,
    ...
    * \param copy_V_moments which set of moments to copy (_V or _R)
    */
   void SysBoundaryCondition::vlasovBoundaryCopyFromTheClosestL1OutflowNbr(dccrg::Dccrg<SpatialCell, dccrg::Cartesian_Geometry>& mpiGrid, const CellID& cellID, const bool& copyMomentsOnly,
                                                                           const uint popID, const bool copy_V_moments) {
    ...
    * \param copy_V_moments which set of moments to copy (_V or _R)
    */
   void SysBoundaryCondition::vlasovBoundaryCopyFromAllClosestNbrs(dccrg::Dccrg<SpatialCell, dccrg::Cartesian_Geometry>& mpiGrid, const CellID& cellID, const uint popID, const bool copy_V_moments) {
    ...
    */
   void SysBoundaryCondition::vlasovBoundaryFluffyCopyFromAllCloseNbrs(dccrg::Dccrg<SpatialCell, dccrg::Cartesian_Geometry>& mpiGrid, const CellID& cellID, const uint popID, const bool copy_V_moments,
                                                                       creal fluffiness) {
    ...
    * \param copy_V_moments which set of moments to copy (_V or _R)
    */
   void SysBoundaryCondition::copyCellData(SpatialCell* from, SpatialCell* to, const bool copyMomentsOnly, const uint popID, const bool copy_V_moments) {
      if (popID == 0) {
         if (copy_V_moments) {
    ...
    */
   void averageCellData(dccrg::Dccrg<SpatialCell, dccrg::Cartesian_Geometry>& mpiGrid, const vector<CellID> cellList, SpatialCell* to, const uint popID, creal fluffiness /* default =0.0*/
   ) {
    ...
    */
   bool SysBoundaryCondition::updateSysBoundaryConditionsAfterLoadBalance(dccrg::Dccrg<SpatialCell, dccrg::Cartesian_Geometry>& mpiGrid, const vector<CellID>& local_cells_on_boundary) {
      // Loop over cellids
    ...
    */
   array<int, 3> SysBoundaryCondition::getTheClosestNonsysboundaryCell(FsGrid<fsgrids::technical, FS_STENCIL_WIDTH>& technicalGrid, cint i, cint j, cint k) {
    ...
    */
   vector<array<int, 3>> SysBoundaryCondition::getAllClosestNonsysboundaryCells(FsGrid<fsgrids::technical, FS_STENCIL_WIDTH>& technicalGrid, cint i, cint j, cint k) {
    ...
    * \sa getAllClosestNonsysboundaryCells
    */
   CellID& SysBoundaryCondition::getTheClosestL1OutflowCell(const CellID& cellID) {
   ...
   Real SysBoundaryCondition::fieldBoundaryCopyFromSolvingNbrMagneticField(FsGrid<array<Real, fsgrids::bfield::N_BFIELD>, FS_STENCIL_WIDTH>& bGrid,
   ...
   void SysBoundaryCondition::mapCellPotentialAndGetEXBDrift(std::array<Real, CellParams::N_SPATIAL_CELL_PARAMS>& cellParams) {
      ...
      for (uint kv = 0; kv < vblocks_ini[2]; ++kv) {
         for (uint jv = 0; jv < vblocks_ini[1]; ++jv) {
            for (uint iv = 0; iv < vblocks_ini[0]; ++iv) {
               ...
               Real R2 = ((V_crds[0]) * (V_crds[0]) + (V_crds[1]) * (V_crds[1]) + (V_crds[2]) * (V_crds[2]));
