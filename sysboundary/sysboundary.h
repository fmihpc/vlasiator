 ...
 * If the simulation domain is not fully periodic then the behaviour at the edges or boundaries of the volume has to be
 * properly defined.
 *
...
public:
   ...
   bool addSysBoundary(SBC::SysBoundaryCondition* sbc, Project& project, creal& t);
   bool initSysBoundaries(Project& project, creal& t);
   ...
   bool
   updateSysBoundariesAfterLoadBalance(dccrg::Dccrg<spatial_cell::SpatialCell, dccrg::Cartesian_Geometry>& mpiGrid);
...
/*
   Input a vector of cellIDs (cellList) and compute a new vector with only those cells which are on a sysboundary and
   are to be computed
*/
