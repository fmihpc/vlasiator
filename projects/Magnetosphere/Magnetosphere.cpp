...
void Magnetosphere::getParameters() {
   ...
   if (sysBoundaryContainer.existSysBoundary("Copysphere")) {
      ...
      if (myRank == MASTER_RANK) {
         std::cerr << "Warning in initializing Magnetosphere: Could not find inner boundary (ionosphere or copysphere)!"
                   << std::endl;
      }
   }
   ...
   for (uint i = 0; i < getObjectWrapper().particleSpecies.size(); i++) {
      ...
      // Backward-compatibility: cfgs from before Sep 2021 setting pop_ionosphere.taperRadius will fail with the unknown
      // option. Some fail-safety checks
      if (sP.taperInnerRadius < 0 || sP.taperOuterRadius < 0) {
         if (myRank == MASTER_RANK) {
            cerr << "Error: " << pop << "_Magnetosphere.taperInnerRadius and tapeOuterRadius should be >= 0! Aborting."
                 << endl;
         }
         abort();
      }
      if (sP.taperInnerRadius > sP.taperOuterRadius) {
         if (myRank == MASTER_RANK) {
            cerr << "Error: " << pop << "_Magnetosphere.taperInnerRadius should be <= taperOuterRadius! Aborting."
                 << endl;
         }
         abort();
      }
      if (sP.taperOuterRadius > 0 && sP.taperOuterRadius <= this->ionosphereRadius) {
         if (myRank == MASTER_RANK) {
            cerr << "Error: " << pop
                 << "_Magnetosphere.taperOuterRadius is non-zero yet smaller than ionosphere.radius / "
                    "copysphere.radius! Aborting."
                 << endl;
         }
         abort();
      }
      if (sP.taperInnerRadius == 0 && sP.taperOuterRadius > 0) {
         if (myRank == MASTER_RANK) {
            cerr << "Warning: " << pop
                 << "_Magnetosphere.taperInnerRadius is zero (default), now setting this to the same value as "
                    "ionosphere.radius / copysphere.radius, that is "
                 << this->ionosphereRadius << ". Set/change " << pop
      ...
      if (sP.ionosphereT == 0) {
         if (myRank == MASTER_RANK) {
            if (sysBoundaryContainer.existSysBoundary("Copysphere")) {
               cerr << "Warning: " << pop << "_copysphere.T is zero (default), now setting to the same value as " << pop
                    << "_Magnetosphere.T, that is " << sP.T << ". Set/change " << pop
                    << "_copysphere.T if this is not the expected behavior." << endl;
            } else if (sysBoundaryContainer.existSysBoundary("Ionosphere")) {
               cerr << "Warning: " << pop << "_ionosphere.T is zero (default), now setting to the same value as " << pop
                    << "_Magnetosphere.T, that is " << sP.T << ". Set/change " << pop
      ...
      if (sP.ionosphereRho == 0) {
         if (myRank == MASTER_RANK) {
            if (sysBoundaryContainer.existSysBoundary("Copysphere")) {
               cerr << "Warning: " << pop << "_copysphere.rho is zero (default), now setting to the same value as "
                    << pop << "_Magnetosphere.rho, that is " << sP.rho << ". Set/change " << pop
                    << "_copysphere.rho if this is not the expected behavior." << endl;
            } else if (sysBoundaryContainer.existSysBoundary("Ionosphere")) {
               cerr << "Warning: " << pop << "_ionosphere.rho is zero (default), now setting to the same value as "
                    << pop << "_Magnetosphere.rho, that is " << sP.rho << ". Set/change " << pop
...
Real Magnetosphere::getDistribValue(creal& x, creal& y, creal& z, creal& vx, creal& vy, creal& vz, creal& dvx,
   ...
   case 2:
      ...
      radius = sqrt((x - center[0]) * (x - center[0]) + (y - center[1]) * (y - center[1]) +
   ...
   if (radius < sP.taperOuterRadius) {
      // sine tapering
      initRho =
          sP.rho - (sP.rho - sP.ionosphereRho) * 0.5 *
                       (1.0 + sin(M_PI * (radius - sP.taperInnerRadius) / (sP.taperOuterRadius - sP.taperInnerRadius) +
                                  0.5 * M_PI));
      initT =
          sP.T - (sP.T - sP.ionosphereT) * 0.5 *
                     (1.0 + sin(M_PI * (radius - sP.taperInnerRadius) / (sP.taperOuterRadius - sP.taperInnerRadius) +
                                0.5 * M_PI));
   ...
   return initRho * pow(mass / (2.0 * M_PI * physicalconstants::K_B * initT), 1.5) *
          exp(-mass *
              ((vx - initV0[0]) * (vx - initV0[0]) + (vy - initV0[1]) * (vy - initV0[1]) +
               (vz - initV0[2]) * (vz - initV0[2])) /
              (2.0 * physicalconstants::K_B * initT));
}

vector<std::array<Real, 3>> Magnetosphere::getV0(creal x, creal y, creal z, const uint popID) const {
   const MagnetosphereSpeciesParameters& sP = this->speciesParams[popID];
   ...
   if (radius < sP.taperOuterRadius) {
      ...
      Real q =
          0.5 *
          (1.0 - sin(M_PI * (radius - sP.taperInnerRadius) / (sP.taperOuterRadius - sP.taperInnerRadius) + 0.5 * M_PI));
...
bool Magnetosphere::canRefine(spatial_cell::SpatialCell* cell) const {
   return cell->sysBoundaryFlag == sysboundarytype::NOT_SYSBOUNDARY &&
          (cell->sysBoundaryLayer == 0 || cell->sysBoundaryLayer > 2);
}

bool Magnetosphere::refineSpatialCells(
    dccrg::Dccrg<spatial_cell::SpatialCell, dccrg::Cartesian_Geometry>& mpiGrid) const {
   ...
   if (P::amrMaxSpatialRefLevel > 2 && P::amrMaxAllowedSpatialRefLevel > 2) {
      ...
      for (uint i = 0; i < cells.size(); ++i) {
         ...
         bool inTail = (xyz[0] > refine_L3tailxmin) && (xyz[0] < refine_L3tailxmax) &&
                       (fabs(xyz[1]) < refine_L3tailwidth) && (fabs(xyz[2]) < refine_L3tailheight);
...
int Magnetosphere::adaptRefinement(dccrg::Dccrg<spatial_cell::SpatialCell, dccrg::Cartesian_Geometry>& mpiGrid) const {
   ...
   for (CellID id : cells) {
      ...
      if (!canRefine(mpiGrid[id])) {
         ...
         const Real beta{P::useJPerB ? std::log2(cell->parameters[CellParams::AMR_JPERB] + EPS) + logDx +
                                           P::JPerBModifier + refLevel
                                     : 0.0};
         ...
         for (const auto& [neighbor, dir] : mpiGrid.get_face_neighbors_of(id)) {
            ...
            const Real neighborBeta{P::useJPerB
                                        ? std::log2(mpiGrid[neighbor]->parameters[CellParams::AMR_JPERB] + EPS) +
                                              logDx + P::JPerBModifier + neighborRef
                                        : 0.0};
            if (neighborRef > refLevel) {
               ++refined_neighbors;
            } else if (neighborRef < refLevel) {
               ++coarser_neighbors;
            } else if (mpiGrid[neighbor]->parameters[CellParams::AMR_ALPHA] > P::refineThreshold ||
                       neighborBeta > 0.5) {
               refined_neighbors += 4;
            } else if (mpiGrid[neighbor]->parameters[CellParams::AMR_ALPHA] < P::unrefineThreshold ||
                       neighborBeta < -0.5) {
               ++coarser_neighbors;
            }
         }
...
bool Magnetosphere::forceRefinement(dccrg::Dccrg<spatial_cell::SpatialCell, dccrg::Cartesian_Geometry>& mpiGrid,
                                    int n) const {
