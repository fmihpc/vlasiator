...
Real calcPhaseSpaceDensity(creal& x, creal& y, creal& z, creal& dx, creal& dy, creal& dz, creal& vx, creal& vy,
   ...
   creal cell2dID = (int)(X / dx) + (int)(Y / dy) * Parameters::xcells_ini;
   creal vel2dID = (int)(vX / dvx) + (int)(vY / dvy) * Parameters::vxblocks_ini * 4;

   if (cell2dID == vel2dID && vz + 0.5 * dvz >= -0.4 * dvz && vz + 0.5 * dvz < 0.4 * dvz &&
       radius <= TTP::radLimitSup && radius >= TTP::radLimitInf) {
      return 1.0;
   }
   return 0.0;
}
