...
#pragma omp threadprivate(isSet)
   ...
   if (vx < Parameters::vxmin + 0.5 * dvx || vy < Parameters::vymin + 0.5 * dvy || vz < Parameters::vzmin + 0.5 * dvz ||
       vx > Parameters::vxmax - 1.5 * dvx || vy > Parameters::vymax - 1.5 * dvy || vz > Parameters::vzmax - 1.5 * dvz)
      return 0.0;

   if (isSet)
      return 0.0; // exactly one value to be set

   if (fabs(vx - this->VX0) < dvx && fabs(vy - this->VY0) < dvy && fabs(vz - this->VZ0) < dvz &&
       fabs(x - this->X0) < dx && fabs(y - this->Y0) < dy && fabs(z - this->Z0) < dz) {
      isSet = true;
