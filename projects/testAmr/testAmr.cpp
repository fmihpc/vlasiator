...
bool testAmr::initialize(void) { return Project::initialize(); }
...
void testAmr::getParameters() {
   ...
   for (uint i = 0; i < getObjectWrapper().particleSpecies.size(); i++) {
      ...
      if (!sP.isConsistent()) {
         cerr << "You should define all parameters (testAmr.rho, testAmr.Tx, testAmr.Ty, testAmr.Tz, testAmr.Vx, "
                 "testAmr.Vy, testAmr.Vz, testAmr.rhoPertAbsAmp) for all "
...
Real testAmr::getDistribValue(creal& vx, creal& vy, creal& vz, creal& dvx, creal& dvy, creal& dvz, const uint popID)
   ...
   for (uint i = 0; i < sP.numberOfPeaks; ++i) {
      value += (sP.rho[i] + sP.rhoPertAbsAmp[i] * rhoRnd) * pow(mass / (2.0 * M_PI * kb), 1.5) * 1.0 /
               sqrt(sP.Tx[i] * sP.Ty[i] * sP.Tz[i]) *
               exp(-mass *
                   (pow(vx - sP.Vx[i], 2.0) / (2.0 * kb * sP.Tx[i]) + pow(vy - sP.Vy[i], 2.0) / (2.0 * kb * sP.Ty[i]) +
...
Real testAmr::calcPhaseSpaceDensity(creal& x,
   ...
   do {
      ...
      case TestCase:
         rhoFactor = 1.0;
         if (x < P::xmin + 0.31 * (P::xmax - P::xmin) && y < P::ymin + 0.31 * (P::ymax - P::ymin)) {
            rhoFactor = 3.0;
         }
         break;
      default:
      ...
      if (fabs(avgCurrent - avgAccum) / (avgAccum + eps) < 0.01)
         ok = true;
      else if (avg < avgLimit)
         ok = true;
      else if (N > 10) {
         ok = true;
      }
...
std::vector<std::array<Real, 3>> testAmr::getV0(creal x, creal y, creal z, const uint popID) const {
   const testAmrSpeciesParameters& sP = speciesParams[popID];
