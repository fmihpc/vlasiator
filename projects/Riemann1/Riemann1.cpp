...
Real Riemann1::getDistribValue(creal& x,
   ...
   return this->rho[side] *
          pow(physicalconstants::MASS_PROTON / (2.0 * M_PI * physicalconstants::K_B * this->T[side]), 1.5) *
          exp(-physicalconstants::MASS_PROTON *
              (pow(vx - this->Vx[side], 2.0) + pow(vy - this->Vy[side], 2.0) + pow(vz - this->Vz[side], 2.0)) /
