...
namespace projects {
   ...
   void Magnetosphere::getParameters() {
      ...
      for (uint i = 0; i < getObjectWrapper().particleSpecies.size(); i++) {
         ...
         if (sP.taperInnerRadius == 0 && sP.taperOuterRadius > 0) {
            if (myRank == MASTER_RANK) {
               cerr << "Warning: " << pop << "_Magnetosphere.taperInnerRadius is zero (default), now setting this to the same value as ionosphere.radius / copysphere.radius, that is "
                    << this->ionosphereRadius << ". Set/change " << pop << "_Magnetosphere.taperInnerRadius if this is not the expected behavior." << endl;
            }
         ...
         if (sP.ionosphereT == 0) {
            if (myRank == MASTER_RANK) {
               if (sysBoundaryContainer.existSysBoundary("Copysphere")) {
                  cerr << "Warning: " << pop << "_copysphere.T is zero (default), now setting to the same value as " << pop << "_Magnetosphere.T, that is " << sP.T << ". Set/change " << pop
                       << "_copysphere.T if this is not the expected behavior." << endl;
               } else if (sysBoundaryContainer.existSysBoundary("Ionosphere")) {
                  cerr << "Warning: " << pop << "_ionosphere.T is zero (default), now setting to the same value as " << pop << "_Magnetosphere.T, that is " << sP.T << ". Set/change " << pop
         ...
         if (sP.ionosphereRho == 0) {
            if (myRank == MASTER_RANK) {
               if (sysBoundaryContainer.existSysBoundary("Copysphere")) {
                  cerr << "Warning: " << pop << "_copysphere.rho is zero (default), now setting to the same value as " << pop << "_Magnetosphere.rho, that is " << sP.rho << ". Set/change " << pop
                       << "_copysphere.rho if this is not the expected behavior." << endl;
               } else if (sysBoundaryContainer.existSysBoundary("Ionosphere")) {
                  cerr << "Warning: " << pop << "_ionosphere.rho is zero (default), now setting to the same value as " << pop << "_Magnetosphere.rho, that is " << sP.rho << ". Set/change " << pop
   ...
   void Magnetosphere::setProjectBField(FsGrid<std::array<Real, fsgrids::bfield::N_BFIELD>, FS_STENCIL_WIDTH>& perBGrid, FsGrid<std::array<Real, fsgrids::bgbfield::N_BGB>, FS_STENCIL_WIDTH>& BgBGrid,
...
#pragma omp for collapse(2)
   ...
   /* Evaluates local SpatialCell properties for the project and population,
      ...
      and populates them
   */
   Realf Magnetosphere::fillPhaseSpace(spatial_cell::SpatialCell* cell, const uint popID, const uint nRequested) const {
   ...
   */
   Realf Magnetosphere::probePhaseSpace(spatial_cell::SpatialCell* cell, const uint popID, Real vx_in, Real vy_in, Real vz_in) const {
      const MagnetosphereSpeciesParameters& sP = this->speciesParams[popID];
