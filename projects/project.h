...
public:
    ...
    * \param BgBGrid Grid on which values for the background field can be set if needed, e.g. using the background field
    * functions. \param technicalGrid Technical fsgrid, available if some of its data is necessary.
    *
   ...
   /*!\brief Adapts refinement by one level according to the project. Returns true if any cells were refined, false if
    * not. \param mpiGrid grid to refine
    ...
    * \param n Static refinement pass. 0th pass refines level 0 cells and unrefines max level cells, 1st pass refines
    * level 1 and unrefines max level -1 etc.
    */
   virtual bool forceRefinement(dccrg::Dccrg<spatial_cell::SpatialCell, dccrg::Cartesian_Geometry>& mpiGrid,
                                int n) const;
...
protected:
    ...
    *
    * Uses the function findBlocksToInitialize and loops through the list returned by it to initialize the cells'
    * velocity space. NOTE: This function is called inside parallel region so it must be declared as const.
    *
