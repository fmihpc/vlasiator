...
namespace projects {
    ...
    */
   ARCH_HOSTDEV inline Realf MaxwellianPhaseSpaceDensity(creal& vx, creal& vy, creal& vz, creal& T, creal& rho, creal& mass) {
      return rho * pow(mass / (2.0 * M_PI * physicalconstants::K_B * T), 1.5) * exp(-mass * (vx * vx + vy * vy + vz * vz) / (2.0 * physicalconstants::K_B * T));
   }
    ...
    * The physical unit of this quantity is 1 / (m^3 (m/s)^3).
    */
   ARCH_HOSTDEV inline Realf TriMaxwellianPhaseSpaceDensity(creal& vx, creal& vy, creal& vz, creal& Tx, creal& Ty, creal& Tz, creal& rho, creal& mass) {
      return rho * pow(mass / (2.0 * M_PI * physicalconstants::K_B), 1.5) * exp(-mass * (vx * vx / Tx + vy * vy / Ty + vz * vz / Tz) / (2.0 * physicalconstants::K_B)) / sqrt(Tx * Ty * Tz);
   }
   ...
   public:
      ...
      virtual void
      hook(cuint& stage, const dccrg::Dccrg<spatial_cell::SpatialCell, dccrg::Cartesian_Geometry>& mpiGrid, FsGrid<std::array<Real, fsgrids::bfield::N_BFIELD>, FS_STENCIL_WIDTH>& perBGrid) const;
