...
Real Distributions::getDistribValue(creal& x, creal& y, creal& z, creal& vx, creal& vy, creal& vz,
   ...
   value += this->rhoRnd[0] * pow(physicalconstants::MASS_PROTON / (2.0 * M_PI * physicalconstants::K_B), 1.5) * 1.0 /
            sqrt(this->Tx[0] * this->Ty[0] * this->Tz[0]) *
            exp(-physicalconstants::MASS_PROTON *
                (pow(vx - this->Vx[0], 2.0) / (2.0 * physicalconstants::K_B * this->Tx[0]) +
                 pow(vy - this->Vy[0], 2.0) / (2.0 * physicalconstants::K_B * this->Ty[0]) +
                 pow(vz - this->Vz[0], 2.0) / (2.0 * physicalconstants::K_B * this->Tz[0])));

   value +=
       this->rhoRnd[1] * pow(physicalconstants::MASS_PROTON / (2.0 * M_PI * physicalconstants::K_B), 1.5) * 1.0 /
       sqrt(this->Tx[1] * this->Ty[1] * this->Tz[1]) *
       exp(-physicalconstants::MASS_PROTON * (pow(vx - scaledVx1, 2.0) / (2.0 * physicalconstants::K_B * this->Tx[1]) +
                                              pow(vy - scaledVy1, 2.0) / (2.0 * physicalconstants::K_B * this->Ty[1]) +
                                              pow(vz - scaledVz1, 2.0) / (2.0 * physicalconstants::K_B * this->Tz[1])));
