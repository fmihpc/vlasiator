...
void IPShock::getParameters() {
   ...
   */
   ...
   for (auto& sP : speciesParams) {
      ...
      if ((abs(this->Bucosphi) - abs(sP.Vucosphi) > 1e-10) ||
          (this->Byusign * this->Bzusign != sP.Vyusign * sP.Vzusign)) {
         if (myRank == MASTER_RANK) {
            std::cout << " Warning: Upstream B and V not parallel" << std::endl;
            std::cout << " Bucosphi " << Bucosphi << " Vucosphi " << sP.Vucosphi << " Byusign " << Byusign
                      << " Bzusign " << Bzusign << " Vyusign " << sP.Vyusign << " Vzusign " << sP.Vzusign << std::endl;
         }
      }
      if ((abs(this->Bdcosphi) - abs(sP.Vdcosphi) > 1e-10) ||
          (this->Bydsign * this->Bzdsign != sP.Vydsign * sP.Vzdsign)) {
         if (myRank == MASTER_RANK) {
            std::cout << " Warning: Downstream B and V not parallel" << std::endl;
            std::cout << " Bdcosphi " << Bdcosphi << " Vdcosphi " << sP.Vdcosphi << " Bydsign " << Bydsign
                      << " Bzdsign " << Bzdsign << " Vydsign " << sP.Vydsign << " Vzdsign " << sP.Vzdsign << std::endl;
         }
      }
      /* Verify that upstream and downstream flows are in a plane */
      if ((abs(this->Bdcosphi) - abs(this->Bucosphi) > 1e-10) &&
          (this->Bydsign * this->Bzdsign != this->Byusign * this->Bzusign)) {
         if (myRank == MASTER_RANK) {
            std::cout << " Warning: Upstream and downstream B_tangentials not in same plane" << std::endl;
            std::cout << " Bdcosphi " << Bdcosphi << " Bucosphi " << Bucosphi << " Bydsign " << Bydsign << " Bzdsign "
                      << Bzdsign << " Byusign " << Byusign << " Bzusign " << Bzusign << std::endl;
         }
      }
      if ((abs(sP.Vdcosphi) - abs(sP.Vucosphi) > 1e-10) && (sP.Vydsign * sP.Vzdsign != sP.Vyusign * sP.Vzusign)) {
         if (myRank == MASTER_RANK) {
            ...
            std::cout << " Vdcosphi " << sP.Vdcosphi << " Vucosphi " << sP.Vucosphi << " Vydsign " << sP.Vydsign
                      << " Vzdsign " << sP.Vzdsign << " Vyusign " << sP.Vyusign << " Vzusign " << sP.Vzusign
                      << std::endl;
         }
      }
   }
...
Real IPShock::getDistribValue(creal& x, creal& y, creal& z, creal& vx, creal& vy, creal& vz, creal& dvx, creal& dvy,
   ...
   result = DENSITY * std::pow(mass / (2.0 * M_PI * KB * TEMPERATURE), 1.5) *
            exp(-mass *
                ((vx - pertV0[0]) * (vx - pertV0[0]) + (vy - pertV0[1]) * (vy - pertV0[1]) +
                 (vz - pertV0[2]) * (vz - pertV0[2])) /
                (2.0 * KB * TEMPERATURE));
