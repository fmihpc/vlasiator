...
void Project::setProjectBField(FsGrid<std::array<Real, fsgrids::bfield::N_BFIELD>, FS_STENCIL_WIDTH>& perBGrid,
   ...
   for (uint kc = 0; kc < WID_VZ; ++kc)
      for (uint jc = 0; jc < WID_VY; ++jc)
         for (uint ic = 0; ic < WID_VX; ++ic) {
            ...
            creal average =
                calcPhaseSpaceDensity(x, y, z, dx, dy, dz, vxCell, vyCell, vzCell, dvxCell, dvyCell, dvzCell, popID);
            if (average != 0.0) {
...
void Project::setVelocitySpace(const uint popID, SpatialCell* cell) const {
   ...
   vamr_ref_criteria::Base* refCriterion =
       getObjectWrapper().vamrVelRefCriteria.create(Parameters::vamrVelRefCriterion);
   ...
   for (size_t b = 0; b < removeList.size(); ++b)
      cell->remove_velocity_block(removeList[b], popID);
   ...
   bool refine = true;
   uint currentLevel = 0;
   if (currentLevel == Parameters::vamrMaxVelocityRefLevel)
      refine = false;
   while (refine == true) {
      removeList.clear();
      ...
      for (map<vmesh::GlobalID, vmesh::LocalID>::const_iterator it = insertedBlocks.begin(); it != insertedBlocks.end();
           ++it) {
...
void Project::calcCellParameters(SpatialCell* cell, creal& t) {
   ...
   MPI_Comm_rank(MPI_COMM_WORLD, &rank);
   if (rank == MASTER_RANK) {
      cerr << "(Project.cpp) WARNING: Base class 'calcCellParameters' in " << __FILE__ << ":" << __LINE__ << " called."
           << endl;
   }
   exit(1);
}
...
bool Project::refineSpatialCells(dccrg::Dccrg<spatial_cell::SpatialCell, dccrg::Cartesian_Geometry>& mpiGrid) const {
   ...
   if (myRank == MASTER_RANK) {
      cerr << "(Project.cpp) Base class 'refineSpatialCells' in " << __FILE__ << ":" << __LINE__
           << " called. Make sure that this is correct." << endl;
   }
...
int Project::adaptRefinement(dccrg::Dccrg<spatial_cell::SpatialCell, dccrg::Cartesian_Geometry>& mpiGrid) const {
   ...
   MPI_Comm_rank(MPI_COMM_WORLD, &myRank);
   if (myRank == MASTER_RANK) {
      cerr << "(Project.cpp) Base class 'adaptRefinement' in " << __FILE__ << ":" << __LINE__
...
bool Project::forceRefinement(dccrg::Dccrg<spatial_cell::SpatialCell, dccrg::Cartesian_Geometry>& mpiGrid,
                              int n) const {
   int myRank;
   MPI_Comm_rank(MPI_COMM_WORLD, &myRank);
   if (myRank == MASTER_RANK) {
      cerr << "(Project.cpp) Base class 'forceRefinement' in " << __FILE__ << ":" << __LINE__
           << " called. Function is not implemented for project." << endl;
   }
...
bool Project::filterRefined(dccrg::Dccrg<spatial_cell::SpatialCell, dccrg::Cartesian_Geometry>& mpiGrid) const {
   ...
   for (auto cellPair : cellsMap) {
      ...
      for (auto& neighbour : *mpiGrid.get_neighbors_of(id, NEAREST_NEIGHBORHOOD_ID)) {
         if (mpiGrid[neighbour.first]->parameters[CellParams::RECENTLY_REFINED] &&
             mpiGrid.get_refinement_level(neighbour.first) == refLevel) {
      ...
      // In boxcar filter, we take the average of each of the neighbours and the cell itself. For each missing
      // neighbour, add the cell one more time
