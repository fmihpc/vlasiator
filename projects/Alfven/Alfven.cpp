         ...
         for (int x = 0; x < localSize[0]; ++x) {
            for (int y = 0; y < localSize[1]; ++y) {
               for (int z = 0; z < localSize[2]; ++z) {
                  ...
                  cuint nPts = pow(this->nSpaceSamples, 3.0);
                  cell[fsgrids::bfield::PERBX] = this->B0 * cos(this->ALPHA) - this->A_MAG * this->B0 * sin(this->ALPHA) * dBxavg / nPts;
                  cell[fsgrids::bfield::PERBY] = this->B0 * sin(this->ALPHA) + this->A_MAG * this->B0 * cos(this->ALPHA) * dByavg / nPts;
                  cell[fsgrids::bfield::PERBZ] = this->B0 * this->A_MAG * dBzavg / nPts;
