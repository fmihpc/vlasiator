...
Real Dispersion::getDistribValue(creal& vx, creal& vy, creal& vz, const uint popID) const {
   ...
   return exp(-mass * ((vx - sP.VX0) * (vx - sP.VX0) + (vy - sP.VY0) * (vy - sP.VY0) + (vz - sP.VZ0) * (vz - sP.VZ0)) /
...
void Dispersion::calcCellParameters(spatial_cell::SpatialCell* cell, creal& t) {
   ...
   CellID cellID = (int)((x - Parameters::xmin) / dx) + (int)((y - Parameters::ymin) / dy) * Parameters::xcells_ini +
