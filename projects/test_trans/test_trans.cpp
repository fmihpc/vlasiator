...
bool test_trans::initialize(void) { return Project::initialize(); }
...
Real test_trans::calcPhaseSpaceDensity(creal& x, creal& y, creal& z, creal& dx, creal& dy, creal& dz, creal& vx,
   ...
   for (int box = 0; box < 8; box++) {
      ...
      for (int i = 0; i < 3; i++) {
         if (xyz[i] < (box_real[box][i] - 0.1) || xyz[i] > (box_real[box][i] + 0.1) ||
             vxyz[i] < (box_vel[box][i] - 0.1) || vxyz[i] > (box_vel[box][i] + 0.1)) {
            outsideBox = true;
            break;
         }
      }
