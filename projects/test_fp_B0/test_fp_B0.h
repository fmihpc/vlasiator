...
template<typename T> T velocityFluxX(const T& j,const T& k,const T& avg_neg,const T& avg_pos,const T* const cellParams,const T* const blockParams) {
   ...
   const T BY = cellParams[CellParams::BY] + cellParams[CellParams::BYFACEY0];
   const T BZ = cellParams[CellParams::BZ] + cellParams[CellParams::BZFACEZ0];
...
template<typename CELLID,typename UINT,typename REAL>
void fieldSolverBoundaryCondDerivZ(const CELLID& cellID,REAL* const array,const UINT& existingCells,const UINT& nonExistingCells,Real* const derivatives,const
#ifdef PARGRID
ParGrid<SpatialCell>
#else
dccrg::Dccrg<SpatialCell>
...
void vlasovBoundaryCondition(const CELLID& cellID,const UINT& existingCells,const UINT& nonExistingCells,const
#ifdef PARGRID
ParGrid<SpatialCell>
#else
dccrg::Dccrg<SpatialCell>
