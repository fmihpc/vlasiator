...
{
  return ((j) / ( VECL / WID)) * WID * ( num_k_blocks + 2) + (k) + ( k_block + 1 ) * WID;
}
            ...
            //printf("CUDA 4\n");
             ...
             // k + WID is the index where we have stored k index, WID amount of padding.
              ...
              //{
                //Vec a[2];
                Vec *a = new Vec[2];
                compute_plm_coeff(values + dev_columns[column].valuesOffset + i_pcolumnv(j, 0, -1, nblocks), k + WID, a, minValue);
              //}
              /*
             ...
             int minGk = min(int(lagrangian_gk_l[minGkIndex]), int(dev_columns[column].minBlockK * WID));
             int maxGk = max(int(lagrangian_gk_r[maxGkIndex]), int((dev_columns[column].maxBlockK + 1) * WID - 1));
             ...
             for(int gk = dev_columns[column].minBlockK * WID; gk <= dev_columns[column].maxBlockK * WID; gk++)
             {
                ...
                for (int target_i=0; target_i < VECL; ++target_i)
                {
                  ...
                  printf("CUDA 9\n");
                  Realf *b = &dev_blockData[a];
                  b[tcell] = b[tcell] + tval;
