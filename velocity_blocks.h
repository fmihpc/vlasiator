...
template <int PAD, int METHOD> inline void accum_xyz(Realf* array, const Real* pos, const Realf& value) {
   switch (METHOD) {
      ...
      for (int k_off = -1; k_off < 2; ++k_off)
         for (int j_off = -1; j_off < 2; ++j_off)
            for (int i_off = -1; i_off < 2; ++i_off) {
               ...
               array[padIndex<PAD>(indices[0] + i_off + 1, indices[1] + j_off + 1, indices[2] + k_off + 1)] +=
                   value * shapeFactor;
            }
   } break;
   default:
...
addToFine_x(const T& j_fine, const T& k_fine, const T* coarseOffset, Realf* fineArray, const Realf* coarseArray) {
   ...
   for (T i_fine = 0; i_fine < WID; ++i_fine) {
      const T coarseIndex =
          padIndex<PAD>(coarseOffset[0] + i_fine / 2, coarseOffset[1] + j_fine / 2, coarseOffset[2] + k_fine / 2);
      fineArray[index(i_fine, j_fine, k_fine)] += coarseArray[coarseIndex];
      // fineArray[index(i_fine,j_fine,k_fine)] +=
      // coarseArray[padIndex<PAD>(i_coarse+i_fine/2,j_coarse+j_fine/2,k_coarse+k_fine/2)];
   }
}

template <typename T> inline T nbrIndex(const T& i_off, const T& j_off, const T& k_off) {
   return (k_off + 1) * 9 + (j_off + 1) * 3 + i_off + 1;
}

template <typename T> inline T index(const T& i, const T& j, const T& k) { return k * WID2 + j * WID + i; }

template <int METHOD, typename REAL> inline Realf interp_xy(const REAL* pos, const Realf* data) {
   switch (METHOD) {
   case interpmethod::NGP: {
      const int i = static_cast<int>(pos[0]);
      const int j = static_cast<int>(pos[1]);
   ...
   case interpmethod::CIC: {
      ...
      return data[index(i + 0, j + 0, k)] * (1 - W_x) * (1 - W_y) + data[index(i + 1, j + 0, k)] * (W_x) * (1 - W_y) +
             data[index(i + 0, j + 1, k)] * (1 - W_x) * (W_y) + data[index(i + 1, j + 1, k)] * (W_x) * (W_y);
   } break;
   default:
...
template <int METHOD, typename REAL> inline Realf interp_xz(const REAL* pos, const Realf* data) {
   switch (METHOD) {
   case interpmethod::NGP: {
      const int i = static_cast<int>(pos[0]);
      const int j = static_cast<int>(pos[1]);
   ...
   case interpmethod::CIC: {
      ...
      return data[index(i + 0, j, k + 0)] * (1 - W_x) * (1 - W_z) + data[index(i + 1, j, k + 0)] * (W_x) * (1 - W_z) +
             data[index(i + 0, j, k + 1)] * (1 - W_x) * (W_z) + data[index(i + 1, j, k + 1)] * (W_x) * (W_z);
   } break;
   default:
      std::cerr << "Unknown interpolation method in " << __FILE__ << ' ' << __LINE__ << std::endl;
      exit(1);
...
template <int METHOD, typename REAL> inline Realf interp_yz(const REAL* pos, const Realf* data) {
   switch (METHOD) {
   case interpmethod::NGP: {
      const int i = static_cast<int>(pos[0]);
      const int j = static_cast<int>(pos[1]);
   ...
   case interpmethod::CIC: {
      ...
      return data[index(i, j + 0, k + 0)] * (1 - W_y) * (1 - W_z) + data[index(i, j + 1, k + 0)] * (W_y) * (1 - W_z) +
             data[index(i, j + 0, k + 1)] * (1 - W_y) * (W_z) + data[index(i, j + 1, k + 1)] * (W_y) * (W_z);
   } break;
   default:
      std::cerr << "Unknown interpolation method in " << __FILE__ << ' ' << __LINE__ << std::endl;
      exit(1);
...
template <int METHOD, typename REAL> inline Realf interp_xyz(const REAL* pos, const Realf* data) {
   switch (METHOD) {
   case interpmethod::NGP: {
      const int i = static_cast<int>(pos[0]);
      const int j = static_cast<int>(pos[1]);
...
template <int PADDING, typename T> inline T padIndex(const T& i, const T& j, const T& k) {
...
template <typename T> inline T refIndex(const T& i, const T& j, const T& k, T& i_trgt, T& j_trgt, T& k_trgt) {
   i_trgt = (i % 2) * 2;
   j_trgt = (j % 2) * 2;
   k_trgt = (k % 2) * 2;
