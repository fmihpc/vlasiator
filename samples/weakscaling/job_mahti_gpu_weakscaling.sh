...
#echo "zlength ${zlength} actualntasks ${actualntasks} ntasks ${SLURM_NTASKS} SLURM_NNODES ${SLURM_NNODES}"

srun -n $actualntasks --cpu-bind=${CPU_BIND} ./select_gpu_${SLURM_JOB_ID} --run_config ${CFG} --gridbuilder.z_length=${zlength} --gridbuilder.z_min=-${zlength}e7 --gridbuilder.z_max=${zlength}e7
