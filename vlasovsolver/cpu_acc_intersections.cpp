...
/*!
Computes the first intersection data; this is z~ in section 2.4 in Zerroukat et al (2012). We assume all velocity cells
have the same dimensions. Intersection z coordinate for (i,j,k) is: intersection + i * intersection_di + j *
...
backward in time, used to compute the lagrangian departure gri \param dimension Along which dimension is this
intersection/mapping computation done. It is assumed the three mappings are in order 012, 120 or 201 \param refLevel
Refinement level at which intersections are computed \param intersection Intersection z coordinate at i,j,k=0 \param
intersection_di Change in z-coordinate for a change in i index of 1 \param intersection_dj Change in z-coordinate for a
...
void compute_intersections_1st(const vmesh::VelocityMesh<vmesh::GlobalID, vmesh::LocalID>& vmesh,
   ...
   if (dimension == 1) { // Prepare intersections for mapping along Y first (mapping order Y-Z-X)
      ...
      const Eigen::Matrix<Real, 3, 1> line_point(0.5 * vmesh.getCellSize(refLevel)[0] + vmesh.getMeshMinLimits()[0],
                                                 0.0,
                                                 0.5 * vmesh.getCellSize(refLevel)[2] + vmesh.getMeshMinLimits()[2]);
   ...
   if (dimension == 2) {
      ...
      const Eigen::Matrix<Real, 3, 1> line_point(0.5 * vmesh.getCellSize(refLevel)[0] + vmesh.getMeshMinLimits()[0],
                                                 0.5 * vmesh.getCellSize(refLevel)[1] + vmesh.getMeshMinLimits()[1],
                                                 0.0);
      const Eigen::Matrix<Real, 3, 1> euclidian_di = Eigen::Matrix<Real, 3, 1>(vmesh.getCellSize(refLevel)[0], 0, 0.0);
      const Eigen::Matrix<Real, 3, 1> euclidian_dj = Eigen::Matrix<Real, 3, 1>(0, vmesh.getCellSize(refLevel)[1], 0.0);
...
/*!
  ...
  intersection/mapping computation done. It is assumed the three mappings are in order 012, 120 or 201 \param refLevel
  Refinement level at which the intersections are computed \param intersection Intersection x-coordinate at i,j,k=0
...
void compute_intersections_2nd(const vmesh::VelocityMesh<vmesh::GlobalID, vmesh::LocalID>& vmesh,
   ...
   if (dimension == 0) { // Prepare intersections for mapping along X second (mapping order Z-X-Y)
      ...
      const Eigen::Matrix<Real, 3, 1> line_point =
          bwd_transform * Eigen::Matrix<Real, 3, 1>(vmesh.getMeshMinLimits()[0],
                                                    0.5 * vmesh.getCellSize(refLevel)[1] + vmesh.getMeshMinLimits()[1],
                                                    0.5 * vmesh.getCellSize(refLevel)[2] + vmesh.getMeshMinLimits()[2]);
   ...
   if (dimension == 1) { // Prepare intersections for mapping along Y second (mapping order X-Y-Z)
      ...
      const Eigen::Matrix<Real, 3, 1> line_point =
          bwd_transform * Eigen::Matrix<Real, 3, 1>(0.5 * vmesh.getCellSize(refLevel)[0] + vmesh.getMeshMinLimits()[0],
   ...
   if (dimension == 2) { // Prepare intersections for mapping along Z second (mapping order Y-Z-X)
      ...
      const Eigen::Matrix<Real, 3, 1> line_direction =
          bwd_transform.linear() * Eigen::Matrix<Real, 3, 1>(1.0, 0.0, 0.0);
      const Eigen::Matrix<Real, 3, 1> line_point =
          bwd_transform * Eigen::Matrix<Real, 3, 1>(0.5 * vmesh.getCellSize(refLevel)[0] + vmesh.getMeshMinLimits()[0],
                                                    0.5 * vmesh.getCellSize(refLevel)[1] + vmesh.getMeshMinLimits()[1],
...
/*!
  Computes the third intersection data; this is y intersections in Zerroukat et al (2012). We assume all velocity cells
  have the same dimensions. Intersection y-coordinate for (i,j,k) is: intersection + i * intersection_di + j *
  intersection_dj + k * intersection_dk \param spatial_cell spatial cell that is accelerated \param fwd_transform
  Transform that describes acceleration forward in time \param bwd_transform Transform that describes acceleration
  backward in time, used to compute the lagrangian departure grid \param dimension Along which dimension is this
  intersection/mapping computation done. It is assumed the three mappings are in order 012, 120 or 201 \param refLevel
  Refinement level at which intersections are computed \param intersection Intersection y-coordinate at i,j,k=0 \param
  intersection_di Change in y-coordinate for a change in i index of 1 \param intersection_dj Change in y-coordinate for
  a change in j index of 1 \param intersection_dk Change in y-coordinate for a change in k index of 1
...
void compute_intersections_3rd(const vmesh::VelocityMesh<vmesh::GlobalID, vmesh::LocalID>& vmesh,
   ...
   if (dimension == 0) { // Prepare intersections for mapping along X third (mapping order Y-Z-X)
      const Eigen::Matrix<Real, 3, 1> point_0_0_0 =
          bwd_transform * Eigen::Matrix<Real, 3, 1>(0.0 * vmesh.getCellSize(refLevel)[0] + vmesh.getMeshMinLimits()[0],
                                                    0.5 * vmesh.getCellSize(refLevel)[1] + vmesh.getMeshMinLimits()[1],
                                                    0.5 * vmesh.getCellSize(refLevel)[2] + vmesh.getMeshMinLimits()[2]);
      const Eigen::Matrix<Real, 3, 1> point_1_0_0 =
          bwd_transform * Eigen::Matrix<Real, 3, 1>(1.0 * vmesh.getCellSize(refLevel)[0] + vmesh.getMeshMinLimits()[0],
                                                    0.5 * vmesh.getCellSize(refLevel)[1] + vmesh.getMeshMinLimits()[1],
                                                    0.5 * vmesh.getCellSize(refLevel)[2] + vmesh.getMeshMinLimits()[2]);
   ...
   if (dimension == 1) { // Prepare intersections for mapping along Y third (mapping order Z-X-Y)
      ...
      const Eigen::Matrix<Real, 3, 1> point_0_0_0 =
          bwd_transform * Eigen::Matrix<Real, 3, 1>(0.5 * vmesh.getCellSize(refLevel)[0] + vmesh.getMeshMinLimits()[0],
                                                    ...
                                                    0.5 * vmesh.getCellSize(refLevel)[2] + vmesh.getMeshMinLimits()[2]);
      const Eigen::Matrix<Real, 3, 1> point_1_0_0 =
          bwd_transform * Eigen::Matrix<Real, 3, 1>(1.5 * vmesh.getCellSize(refLevel)[0] + vmesh.getMeshMinLimits()[0],
                                                    vmesh.getMeshMinLimits()[1],
                                                    0.5 * vmesh.getCellSize(refLevel)[2] + vmesh.getMeshMinLimits()[2]);
      const Eigen::Matrix<Real, 3, 1> point_0_1_0 =
          bwd_transform * Eigen::Matrix<Real, 3, 1>(0.5 * vmesh.getCellSize(refLevel)[0] + vmesh.getMeshMinLimits()[0],
