...
inline bool paircomparator(const std::pair<uint, uint>& l, const std::pair<uint, uint>& r) { return l.first < r.first; }
...
void sortBlocklistByDimension( // const spatial_cell::SpatialCell* spatial_cell,
   ...
   for (vmesh::LocalID i = 0; i < nBlocks; ++i) {
      ...
      case 1: {
         ...
         // const uint y_indice =
         // (block/SpatialCell::get_velocity_grid_length()[0])%SpatialCell::SpatialCell::get_velocity_grid_length()[1];
         ...
         const vmesh::GlobalID blockId_mapped = block - (x_index + y_index * vmesh.getGridLength(REFLEVEL)[0]) +
                                                y_index + x_index * vmesh.getGridLength(REFLEVEL)[1];
         block_pairs[i] = std::make_pair(blockId_mapped, block);
      } break;
      case 2: {
         // Do operation:
         ...
         const vmesh::GlobalID blockId_mapped =
             z_index + y_index * vmesh.getGridLength(REFLEVEL)[2] +
   ...
   for (vmesh::LocalID i = 0; i < nBlocks; ++i) {
      ...
      if (i > 0 && (column_id != prev_column_id || dimension_id != (prev_dimension_id + 1))) {
         ...
         columnNumBlocks.push_back(columnBlockOffsets[columnBlockOffsets.size() - 1] -
                                   columnBlockOffsets[columnBlockOffsets.size() - 2]);
         ...
         if (column_id != prev_column_id) {
            ...
            setNumColumns.push_back(setColumnOffsets[setColumnOffsets.size() - 1] -
                                    setColumnOffsets[setColumnOffsets.size() - 2]);
         }
      }
