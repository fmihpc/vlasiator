...
inline void compute_h8_left_face_value(const Vec* const values, uint k, Vec& fv_l) {
   fv_l = 1.0 / 840.0 *
          (-3.0 * values[k - 4] + 29.0 * values[k - 3] - 139.0 * values[k - 2] + 533.0 * values[k - 1] + 533.0 * values[k] - 139.0 * values[k + 1] + 29.0 * values[k + 2] - 3.0 * values[k + 3]);
}
...
inline void compute_h7_left_face_derivative(const Vec* const values, uint k, Vec& fd_l) {
   fd_l = 1.0 / 5040.0 *
          (+9.0 * values[k - 4] - 119.0 * values[k - 3] + 889.0 * values[k - 2] - 7175.0 * values[k - 1] + 7175.0 * values[k] - 889.0 * values[k + 1] + 119.0 * values[k + 2] - 9.0 * values[k + 3]);
}
...
inline void compute_h6_left_face_value(const Vec* const values, uint k, Vec& fv_l) {
   // compute left value
   fv_l = 1.0 / 60.0 * (values[k - 3] - 8.0 * values[k - 2] + 37.0 * values[k - 1] + 37.0 * values[k] - 8.0 * values[k + 1] + values[k + 2]);
}
...
inline void compute_h5_left_face_derivative(const Vec* const values, uint k, Vec& fd_l) {
   fd_l = 1.0 / 180.0 * (245 * (values[k] - values[k - 1]) - 25 * (values[k + 1] - values[k - 2]) + 2 * (values[k + 2] - values[k - 3]));
}
...
inline void compute_h5_face_values(const Vec* const values, uint k, Vec& fv_l, Vec& fv_r) {
   // compute left values
   fv_l = 1.0 / 60.0 * (-3.0 * values[k - 2] + 27.0 * values[k - 1] + 47.0 * values[k] - 13.0 * values[k + 1] + 2.0 * values[k + 2]);
   fv_r = 1.0 / 60.0 * (2.0 * values[k - 2] - 13.0 * values[k - 1] + 47.0 * values[k] + 27.0 * values[k + 1] - 3.0 * values[k + 2]);
}
/*!
  ...
  \param fd_l Face derivative on left face of cell i
*/
inline void compute_h4_left_face_derivative(const Vec* const values, uint k, Vec& fd_l) { fd_l = 1.0 / 12.0 * (15.0 * (values[k] - values[k - 1]) - (values[k + 1] - values[k - 2])); }
...
inline void compute_h4_left_face_value(const Vec* const values, uint k, Vec& fv_l) {
   // compute left value
   fv_l = 1.0 / 12.0 * (-1.0 * values[k - 2] + 7.0 * values[k - 1] + 7.0 * values[k] - 1.0 * values[k + 1]);
}
...
inline void compute_h4_left_face_value_nonuniform(const Realf* const h, const Vec* const u, uint k, Vec& fv_l) {
   ...
   fv_l = (1.0 / (h[k - 2] + h[k - 1] + h[k] + h[k + 1]) *
           ((h[k - 2] + h[k - 1]) * (h[k] + h[k + 1]) / (h[k - 1] + h[k]) * (u[k - 1] * h[k] + u[k] * h[k - 1]) * (1.0 / (h[k - 2] + h[k - 1] + h[k]) + 1.0 / (h[k - 1] + h[k] + h[k + 1])) +
            (h[k] * (h[k] + h[k + 1])) / ((h[k - 2] + h[k - 1] + h[k]) * (h[k - 2] + h[k - 1])) * (u[k - 1] * (h[k - 2] + 2.0 * h[k - 1]) - (u[k - 2] * h[k - 1])) +
            h[k - 1] * (h[k - 2] + h[k - 1]) / ((h[k - 1] + h[k] + h[k + 1]) * (h[k] + h[k + 1])) * (u[k] * (2.0 * h[k] + h[k + 1]) - u[k + 1] * h[k])));
}
...
inline Vec get_D2aLim(const Realf* h, const Vec* values, uint k, const Vec C, Vec& fv) {
   ...
   Vec d2aLim;
   if ((horizontal_or(d2a * d2aL >= 0)) && (horizontal_or(d2a * d2aR >= 0)) && horizontal_and(d2a != 0)) {
...
inline void constrain_face_values(const Realf* h, const Vec* values, uint k, Vec& fv_l, Vec& fv_r) {
   ...
   Vec p_face = 0.5 * (values[k] + values[k + 1]) - h[k] * h[k] / 3.0 * get_D2aLim(h, values, k, C, fv_r);
   Vec m_face = 0.5 * (values[k - 1] + values[k]) - h[k - 1] * h[k - 1] / 3.0 * get_D2aLim(h, values, k - 1, C, fv_l);
   ...
   if ((horizontal_or(d2a * d2aL >= 0)) && (horizontal_or(d2a * d2aR >= 0)) && (horizontal_or(d2a * d2aC >= 0)) && horizontal_and(d2a != 0)) {
...
inline void compute_filtered_face_values_nonuniform_conserving(const Realf* const dv, const Vec* const values, uint k, face_estimate_order order, Vec& fv_l, Vec& fv_r, const Realf threshold) {
   ...
   if (horizontal_or((fv_r - values[k]) * (values[k] - fv_l) <= Vec(0.0)) && horizontal_or((values[k - 1] - values[k]) * (values[k] - values[k + 1]) <= Vec(0.0))) {
