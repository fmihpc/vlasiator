...
__global__ void __launch_bounds__(WID3) translation_kernel(const uint dimension, const Realf dt, const uint* __restrict__ pencilLengths, const uint* __restrict__ pencilStarts,
                                                           ...
                                                           uint* dev_pencilsInBin, uint* dev_binStart, uint* dev_binSize, const uint numberOfBins) {
...
// Now using warp accessor.
            ...
            if (blockLID != vmesh->invalidLocalID()) {
               ...
               thisPencilOrderedSource[celli * WID3 + ti] = (cellContainer->getData(blockLID))[ti];
            } else {
               if (ti == 0) {
      ...
      for (uint pencilIndex = 0; pencilIndex < dev_binSize[nBin]; pencilIndex++) {
         ...
         for (uint i = VLASOV_STENCIL_WIDTH; i < lengthOfPencil - VLASOV_STENCIL_WIDTH; i++) {
            ...
            if (!check_skip_blocks(pencilBlockData, pencilBlockDataOffset + start + i)) {
               ...
               compute_ppm_coeff_nonuniform(pencilDZ + start + i - VLASOV_STENCIL_WIDTH, thisPencilOrderedSource + (i - VLASOV_STENCIL_WIDTH) * WID3, h4, VLASOV_STENCIL_WIDTH, a, threshold, ti, WID3);
               // Compute integral
               const Realf ngbr_target_density = z_2 * (a[0] + z_2 * (a[1] + z_2 * a[2])) - z_1 * (a[0] + z_1 * (a[1] + z_1 * a[2]));
               ...
               if (areaRatio_p1 && block_data_p1) {
                  const Realf p1Contribution = (positiveTranslationDirection ? ngbr_target_density * pencilDZ[start + i] / pencilDZ[start + i + 1] : (Realf)(0.0)) * areaRatio_p1;
                  // atomicAdd(&block_data_p1[ti],p1Contribution);
                  block_data_p1[ti] += p1Contribution;
               }
               if (areaRatio_m1 && block_data_m1) {
                  const Realf m1Contribution = (!positiveTranslationDirection ? ngbr_target_density * pencilDZ[start + i] / pencilDZ[start + i - 1] : (Realf)(0.0)) * areaRatio_m1;
...
__global__ void __launch_bounds__(GPUTHREADS* WARPSPERBLOCK)
    gather_union_of_blocks_kernel_WA(Hashinator::Hashmap<vmesh::GlobalID, vmesh::LocalID>* unionOfBlocksSet, const vmesh::VelocityMesh* __restrict__ const* dev_vmeshes, const uint nAllCells) {
...
#else
__global__ void __launch_bounds__(GPUTHREADS* WARPSPERBLOCK)
    gather_union_of_blocks_kernel(Hashinator::Hashmap<vmesh::GlobalID, vmesh::LocalID>* unionOfBlocksSet, const vmesh::VelocityMesh* __restrict__ const* dev_vmeshes, const uint nAllCells) {
 ...
 */
bool trans_map_1d_amr(const dccrg::Dccrg<SpatialCell, dccrg::Cartesian_Geometry>& mpiGrid, const vector<CellID>& localPropagatedCells, const vector<CellID>& remoteTargetCells,
                      std::vector<uint>& nPencilsLB, const uint dimension, const Realf dt, const uint popID) {
...
#endif
       dev_unionOfBlocksSet, dev_vmeshes, nAllCells);
   ...
   translation_kernel<<<grid, block, 0, bgStream>>>(dimension,
                                                    dt,
                                                    pencilLengths,
                                                    pencilStarts,
...
*/
__global__ static void remote_increment_kernel(Realf* blockData, Realf* neighborData, vmesh::LocalID nBlocks) {
...
// Initialize local cells
   ...
   for (auto c : local_cells) {
      ...
      if (!all_of(n_nbrs.begin(), n_nbrs.end(), [&mpiGrid](CellID i) { return mpiGrid.is_local(i); })) {
         ...
         for (const auto nbr : n_nbrs) {
            if (nbr != INVALID_CELLID && !mpiGrid.is_local(nbr) && ccell->sysBoundaryFlag == sysboundarytype::NOT_SYSBOUNDARY) {
               ...
               if (mpiGrid.get_refinement_level(nbr) >= mpiGrid.get_refinement_level(c)) {
                  ...
                  for (uint i_sib = 0; i_sib < MAX_NEIGHBORS_PER_DIM; ++i_sib) {
                     ...
                     if (sibling != INVALID_CELLID && scell && mpiGrid.get_process(sibling) != mpiGrid.get_process(nbr) && myIndices.at(dimension) == sibIndices.at(dimension) &&
                         ncell->neighbor_number_of_blocks.at(i_sib) != scell->get_number_of_velocity_blocks(popID) && scell->sysBoundaryFlag == sysboundarytype::NOT_SYSBOUNDARY) {
   ...
   if (receive_cells.size() != 0) {
      ...
      for (size_t c = 0; c < receive_cells.size(); ++c) {
         ...
         if (nBlocks > 0) {
            dim3 block(WID, WID, WID);
            remote_increment_kernel<<<nBlocks, block, 0, cellStream>>>(blockData, neighborData, nBlocks);
            CHK_ERR(gpuPeekAtLastError());
