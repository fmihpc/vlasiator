...
#pragma omp critical
   ...
   #pragma omp parallel
   {
      ...
      for(uint pencili = 0; pencili < DimensionPencils[dimension].N; ++pencili) {
         ...
         const uint sourcestart = DimensionPencils[dimension].sourceIdsStart[pencili];
         for(uint i = 0; i < sourceLength; ++i) {
            sourceCells[i] = mpiGrid[DimensionPencils[dimension].sourceIds[sourcestart + i]];
...
//             for ( auto pencili : unionOfBlocksMapToPencilIds.at(blockGID) ) {
            ...
            unordered_set<CellID> :: iterator targetID;
            for (targetID = DimensionTargetCells[dimension].begin(); targetID != DimensionTargetCells[dimension].end(); ++targetID) {
               SpatialCell* spatial_cell = mpiGrid[(*targetID)];
            ...
            for(uint pencili = 0; pencili < DimensionPencils[dimension].N; pencili++){
               ...
               const uint targetLength = DimensionPencils[dimension].lengthOfPencils[pencili] + 2 * nTargetNeighborsPerPencil;
               ...
               for ( uint celli = 0; celli < targetLength; celli++ ) {
                  ...
                  const uint GID = celli + totalTargetLength;
                  //const CellID TID = DimensionPencils[dimension].targetIds[GID];
                  const uint TIDstart = DimensionPencils[dimension].targetIdsStart[pencili];
                  const CellID TID = DimensionPencils[dimension].targetIds[TIDstart+celli];

                  if (TID) { // this check also skips sysboundary cells
                     SpatialCell* targetCell = mpiGrid[TID];
