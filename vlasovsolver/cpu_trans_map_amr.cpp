...
#define i_trans_pt_blockv(planeVectorIndex, planeIndex, blockIndex)                                                    \
   (planeVectorIndex + planeIndex * VEC_PER_PLANE + (blockIndex + 1) * VEC_PER_BLOCK)

#define i_trans_ps_blockv_pencil(planeVectorIndex, planeIndex, blockIndex, lengthOfPencil)                             \
   ((blockIndex) + VLASOV_STENCIL_WIDTH +                                                                              \
    ((planeVectorIndex) + (planeIndex)*VEC_PER_PLANE) * (lengthOfPencil + 2 * VLASOV_STENCIL_WIDTH))
...
void flagSpatialCellsForAmrCommunication(const dccrg::Dccrg<SpatialCell, dccrg::Cartesian_Geometry>& mpiGrid,
   ...
   if (mpiGrid.get_maximum_refinement_level() == 0)
      return;
...
#pragma omp parallel for
      for (uint i = 0; i < localPropagatedCells.size(); i++) {
         ...
         // In dimension, check iteratively if any neighbors up to VLASOV_STENCIL_WIDTH distance away are on a different
         // process
         ...
         */
         ...
         for (auto it = distancesplus.begin(); it != distancesplus.end(); ++it) {
            if (ccell->SpatialCell::parameters[CellParams::AMR_TRANSLATE_COMM_X + dimension] == true)
               iSrc = -1;
            if (iSrc < 0)
...
void computeSpatialSourceCellsForPencil(const dccrg::Dccrg<SpatialCell, dccrg::Cartesian_Geometry>& mpiGrid,
   ...
   for (auto it = distances.begin(); it != distances.end(); ++it) {
      ...
      if (neighbors.size() == 1) {
         if (sourceCells[iSrc + 1] == mpiGrid[neighbors.at(0)])
         ...
         if (sourceCells[iSrc + 1] == mpiGrid[neighbors.at(pencils.path[iPencil][refLvl])])
   ...
   for (auto it = distances.begin(); it != distances.end(); ++it) {
      if (iSrc >= L + 2 * VLASOV_STENCIL_WIDTH)
      ...
      if (frontNeighbors.size() > 0) {
         ...
         if (frontNeighborIds.size() == 0) {
            ...
            for (const auto& nbrPair : frontNeighbors) {
               std::cerr << ids.front() << " dim " << dimension << " " << nbrPair.first << " " << nbrPair.second
                         << std::endl;
            }
         }
      ...
      if (backNeighbors.size() > 0) {
         ...
         if (backNeighborIds.size() == 0) {
            ...
            for (const auto& nbrPair : backNeighbors) {
               std::cerr << ids.back() << " dim " << dimension << " " << nbrPair.first << " " << nbrPair.second
                         << std::endl;
            }
         }
 ...
 * @param [in] dimension Spatial dimension
 * @param [in] path Integer value that determines which neighbor is added to the pencil when a higher refinement level
 * is met
 * @param [in] endIds Prescribed end conditions for the pencil. If any of these cell ids is about to be added to the
   ...
   for (uint i = 0; i < lengthOfPencil; i++) {
      ...
      for (uint k = 0; k < WID; ++k) {
         ...
         for (uint planeVector = 0; planeVector < VEC_PER_PLANE; planeVector++) {
            ...
            if (check_skip_remapping(
                    values + i_trans_ps_blockv_pencil(planeVector, k, i - VLASOV_STENCIL_WIDTH, lengthOfPencil)))
               continue;

            // Compute polynomial coefficients
            Vec a[3];
            // Dz: is a padded array, pointer can point to the beginning, i + VLASOV_STENCIL_WIDTH will get the right
            ...
            const Vec ngbr_target_density =
                z_2 * (a[0] + z_2 * (a[1] + z_2 * a[2])) - z_1 * (a[0] + z_1 * (a[1] + z_1 * a[2]));
            ...
            targetValues[i_trans_pt_blockv(planeVector, k, i + 1)] +=
                select(positiveTranslationDirection, ngbr_target_density * dz[i_source] / dz[i_source + 1], Vec(0.0));
            targetValues[i_trans_pt_blockv(planeVector, k, i - 1)] +=
                select(!positiveTranslationDirection, ngbr_target_density * dz[i_source] / dz[i_source - 1], Vec(0.0));
      ...
      for (const auto& [neighbor, dir] : mpiGrid.get_face_neighbors_of(celli)) {
         if (dir == -((int)dimension + 1)) {
            ...
            if (abs((int64_t)(myIndices[dimension] - nbrIndices[dimension])) >
                ...
                !mpiGrid.is_local(neighbor) || !do_translate_cell(mpiGrid[neighbor])) {
               addToSeedIds = true;
               break;
            }
         }
...
#pragma omp critical
   ...
   std::vector<SpatialCell*> targetCells(DimensionPencils[dimension].sumOfLengths +
                                         DimensionPencils[dimension].N * 2 * nTargetNeighborsPerPencil);
...
#pragma omp parallel
   {
      ...
      std::vector<Realf, aligned_allocator<Realf, WID3>> targetBlockData(
          (DimensionPencils[dimension].sumOfLengths + 2 * nTargetNeighborsPerPencil * DimensionPencils[dimension].N) *
          WID3);
      std::vector<std::vector<SpatialCell*>> pencilSourceCells;

      // Allocate aligned vectors which are needed once per pencil to avoid reallocating once per block loop + pencil
      // loop iteration
            ...
            for (uint icell = 0; icell < targetLength; icell++) {
               ...
               for (uint k = 0; k < WID; k++) {
                  ...
                  for (uint planeVector = 0; planeVector < VEC_PER_PLANE; planeVector++) {
                     ...
                     Realf vector[VECL];
                     // pencilSourceVecData[pencili][i_trans_ps_blockv_pencil(planeVector, k, icell - 1,
                     // L)].store(vector);
                     ...
                     for (uint iv = 0; iv < VECL; iv++) {
                        ...
                        targetBlockData[(totalTargetLength + icell) * WID3 +
                                        cellid_transpose[iv + planeVector * VECL + k * WID2]] = vector[iv];
                     }
                  }
               }
            }
         ...
         for (uint pencili = 0; pencili < DimensionPencils[dimension].N; pencili++) {
            ...
            for (uint celli = 0; celli < targetLength; celli++) {
               ...
               if (targetCell) { // this check also skips sysboundary cells
                  ...
                  // areaRatio is the reatio of the cross-section of the spatial cell to the cross-section of the
                  // pencil.
                  int diff = targetCell->SpatialCell::parameters[CellParams::REFINEMENT_LEVEL] -
                             DimensionPencils[dimension].path[pencili].size();
                  int ratio;
                  Realf areaRatio;
                  if (diff >= 0) {
                     ratio = 1 << diff;
...
void update_remote_mapping_contribution_amr(dccrg::Dccrg<SpatialCell, dccrg::Cartesian_Geometry>& mpiGrid,
   ...
   // MPI_Barrier(MPI_COMM_WORLD);
   // cout << "begin update_remote_mapping_contribution_amr, dimension = " << dimension << ", direction = " << direction
   // << endl; MPI_Barrier(MPI_COMM_WORLD);
            ...
            if (nbr != INVALID_CELLID && do_translate_cell(ccell) && !mpiGrid.is_local(nbr)) {
               ...
               if (pcell && pcell->sysBoundaryFlag == sysboundarytype::NOT_SYSBOUNDARY) {
                  ...
                  if (send_cells.find(nbr) == send_cells.end()) {
                     ...
                     ccell->neighbor_block_data.at(sendIndex) = (Realf*)aligned_malloc(
                         ccell->neighbor_number_of_blocks.at(sendIndex) * WID3 * sizeof(Realf), 64);
         ...
         for (const auto nbr : n_nbrs) {
            ...
            if (nbr != INVALID_CELLID && !mpiGrid.is_local(nbr) &&
               ...
               if (mpiGrid.get_refinement_level(nbr) >= mpiGrid.get_refinement_level(c)) {
                  ...
                  // std::array<uint64_t, 8> siblingarr =
                  // mpiGrid.mapping.get_all_children(mpiGrid.mapping.get_parent(c)); vector<CellID>
                  // mySiblings(siblingarr.begin(), siblingarr.end());
                  ...
                  for (uint i_sib = 0; i_sib < MAX_NEIGHBORS_PER_DIM; ++i_sib) {
                     ...
                     if (sibling != INVALID_CELLID && scell &&
                        ...
                        ncell->neighbor_number_of_blocks.at(i_sib) = scell->get_number_of_velocity_blocks(popID);
                        ncell->neighbor_block_data.at(i_sib) = (Realf*)aligned_malloc(
                            ncell->neighbor_number_of_blocks.at(i_sib) * WID3 * sizeof(Realf), 64);
   ...
   //#pragma omp parallel
   {
      for (size_t c = 0; c < receive_cells.size(); ++c) {
         ...
         //#pragma omp for
         for (uint vCell = 0; vCell < VELOCITY_BLOCK_LENGTH * receive_cell->get_number_of_velocity_blocks(popID);
              ++vCell) {
   ...
   // MPI_Barrier(MPI_COMM_WORLD);
   // cout << "end update_remote_mapping_contribution_amr, dimension = " << dimension << ", direction = " << direction
   // << endl; MPI_Barrier(MPI_COMM_WORLD);
}
