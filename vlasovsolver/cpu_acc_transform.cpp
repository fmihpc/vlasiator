...
compute_acceleration_transformation(SpatialCell* spatial_cell, const uint popID, const Real& dt) {
   ...
   unsigned int bulk_velocity_substeps; // in this many substeps we iterate forward bulk velocity when the complete
                                        // transformation is computed (0.1 deg per substep).
   bulk_velocity_substeps = fabs(dt) / fabs(gyro_period * (0.1 / 360.0));
   if (bulk_velocity_substeps < 1)
      bulk_velocity_substeps = 1;

   const Real substeps_radians =
       -(2.0 * M_PI * dt / gyro_period) / bulk_velocity_substeps; // how many radians each substep is.
   const Real substeps_dt = dt / bulk_velocity_substeps;          /*!< how many s each substep is*/
   Eigen::Matrix<Real, 3, 1> EgradPe(spatial_cell->parameters[CellParams::EXGRADPE],
   ...
   if (globalflags::ionosphereJustSolved && spatial_cell->parameters[CellParams::FORCING_CELL_NUM] == 0 &&
       SBC::boundaryVDFmode == SBC::ForceL2EXB) {
      getObjectWrapper()
          .sysBoundaryContainer.getSysBoundary(sysboundarytype::IONOSPHERE)
          ->mapCellPotentialAndGetEXBDrift(spatial_cell->parameters); // This sets the FORCING_CELL_NUM to 1
   }
