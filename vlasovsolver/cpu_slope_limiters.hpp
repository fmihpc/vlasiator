 ...
 */
CellID get_spatial_neighbor(const dccrg::Dccrg<SpatialCell, dccrg::Cartesian_Geometry>& mpiGrid, const CellID& cellID, const bool include_first_boundary_layer, const int spatial_di,
                            const int spatial_dj, const int spatial_dk) {
   ...
   if (include_first_boundary_layer && mpiGrid[nbrID]->sysBoundaryFlag != sysboundarytype::NOT_SYSBOUNDARY && mpiGrid[nbrID]->sysBoundaryLayer != 1) {
      return INVALID_CELLID;
   }
...
SpatialCell* get_spatial_neighbor_pointer(const dccrg::Dccrg<SpatialCell, dccrg::Cartesian_Geometry>& mpiGrid, const CellID& cellID, const bool include_first_boundary_layer, const int spatial_di,
                                          const int spatial_dj, const int spatial_dk) {
 ...
 */
void copy_trans_block_data(SpatialCell** source_neighbors, const vmesh::GlobalID blockGID, Vec* values, const unsigned char* const cellid_transpose, const uint popID) {
...
bool trans_map_1d(const dccrg::Dccrg<SpatialCell, dccrg::Cartesian_Geometry>& mpiGrid, const vector<CellID>& localPropagatedCells, const vector<CellID>& remoteTargetCells, const uint dimension,
                  const Realv dt, const uint popID) {
...
#pragma omp for schedule(guided)
      for (uint blocki = 0; blocki < unionOfBlocks.size(); blocki++) {
         ...
         for (uint celli = 0; celli < localPropagatedCells.size(); celli++) {
            ...
            if (blockLID == vmesh::VelocityMesh<vmesh::GlobalID, vmesh::LocalID>::invalidLocalID() || get_spatial_neighbor(mpiGrid, cellID, true, 0, 0, 0) == INVALID_CELLID) {
            ...
            for (uint k = 0; k < WID; ++k) {
               ...
               for (uint planeVector = 0; planeVector < VEC_PER_PLANE; planeVector++) {
...
#ifdef TRANS_SEMILAG_PLM
                  const Vec ngbr_target_density = z_2 * (a[0] + z_2 * a[1]) - z_1 * (a[0] + z_1 * a[1]);
#endif
#ifdef TRANS_SEMILAG_PPM
                  const Vec ngbr_target_density = z_2 * (a[0] + z_2 * (a[1] + z_2 * a[2])) - z_1 * (a[0] + z_1 * (a[1] + z_1 * a[2]));
#endif
#ifdef TRANS_SEMILAG_PQM
                  const Vec ngbr_target_density = z_2 * (a[0] + z_2 * (a[1] + z_2 * (a[2] + z_2 * (a[3] + z_2 * a[4])))) - z_1 * (a[0] + z_1 * (a[1] + z_1 * (a[2] + z_1 * (a[3] + z_1 * a[4]))));
#endif
                  ...
                  targetVecValues[i_trans_pt_blockv(planeVector, k, 0)] +=
                      values[i_trans_ps_blockv(planeVector, k, 0)] - ngbr_target_density; // in the current original cells we will put the rest of the original density
               }
            }
...
#pragma omp simd
                     for (uint i = 0; i < VECL; i++) {
                        ...
                        // cell_indices_to_id
                        targetBlockData[(celli * 3 + b + 1) * WID3 + cellid_transpose[i + planeVector * VECL + k * WID2]] = vector[i];
                     }
                  }
               }
            }
...
void update_remote_mapping_contribution(dccrg::Dccrg<SpatialCell, dccrg::Cartesian_Geometry>& mpiGrid, const uint dimension, int direction, const uint popID) {
   ...
   case 0:
      if (direction > 0)
         mpiGrid.update_copies_of_remote_neighbors(Neighborhoods::SHIFT_P_X);
      if (direction < 0)
         mpiGrid.update_copies_of_remote_neighbors(Neighborhoods::SHIFT_M_X);
      break;
   case 1:
      if (direction > 0)
         mpiGrid.update_copies_of_remote_neighbors(Neighborhoods::SHIFT_P_Y);
      if (direction < 0)
         mpiGrid.update_copies_of_remote_neighbors(Neighborhoods::SHIFT_M_Y);
      break;
   case 2:
      if (direction > 0)
         mpiGrid.update_copies_of_remote_neighbors(Neighborhoods::SHIFT_P_Z);
      if (direction < 0)
         mpiGrid.update_copies_of_remote_neighbors(Neighborhoods::SHIFT_M_Z);
      break;
   }
