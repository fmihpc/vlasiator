...
ARCH_DEV inline void compute_h5_left_face_derivative(const Realf* const values, int k, Realf& fd_l, const int index, const int stride) {
   fd_l =
      (Realf)(1.0 / 180.0) * ((Realf)(245.0) * (values[k * stride + index] - values[(k - 1) * stride + index]) - (Realf)(25.0) * (values[(k + 1) * stride + index] - values[(k - 2) * stride + index]) +
   ...
   fv_l =
      ((Realf)(1.0) / (hkMinus2 + hkMinus1 + hk + hkPlus1) *
       ((hkMinus2 + hkMinus1) * (hk + hkPlus1) / (hkMinus1 + hk) * (u[(k - 1) * stride + index] * hk + u[k * stride + index] * hkMinus1) *
        ...
        (hk * (hk + hkPlus1)) / ((hkMinus2 + hkMinus1 + hk) * (hkMinus2 + hkMinus1)) * (u[(k - 1) * stride + index] * (hkMinus2 + (Realf)(2.0) * hkMinus1) - (u[(k - 2) * stride + index] * hkMinus1)) +
...
ARCH_DEV inline void compute_filtered_face_values_derivatives(
   const Realf* const values, int k, face_estimate_order order, Realf& fv_l, Realf& fv_r, Realf& fd_l, Realf& fd_r, const Realf threshold, const int index, const int stride
) {
   switch (order) {
   case h4:
...
ARCH_DEV inline void compute_filtered_face_values_nonuniform(
   const Realf* const dv, const Realf* const values, int k, face_estimate_order order, Realf& fv_l, Realf& fv_r, const Realf threshold, const int index, const int stride
) {
   switch (order) {
   case h4:
...
ARCH_DEV inline void compute_filtered_face_values_nonuniform_conserving(
   const Realf* const dv, const Realf* const values, int k, face_estimate_order order, Realf& fv_l, Realf& fv_r, const Realf threshold, const int index, const int stride
) {
   switch (order) {
   case h4:
