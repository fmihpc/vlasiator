...
static ARCH_DEV inline void filter_pqm_monotonicity(const Realf* __restrict__ values, int k, Realf& fv_l, Realf& fv_r, Realf& fd_l, Realf& fd_r, const int index, const int stride) {
   ...
   const Realf sqrt_val = (val_to_sqrt < (Realf)(0.0)) ? b1 + (Realf)(200.0) * b2 : sqrt(val_to_sqrt);
   ...
   // sign
   const Realf root1_slope = (root1 >= (Realf)(0.0) && root1 <= (Realf)(1.0)) ? c0 + root1 * (c1 + root1 * (c2 + root1 * c3)) : slope_sign;
   const Realf root2_slope = (root2 >= (Realf)(0.0) && root2 <= (Realf)(1.0)) ? c0 + root2 * (c1 + root2 * (c2 + root2 * c3)) : slope_sign;
