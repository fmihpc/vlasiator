...
inline void filter_pqm_monotonicity(Vec *values, uint k, Vec &fv_l, Vec &fv_r, Vec &fd_l, Vec &fd_r){   
   const Vec root_outside = Vec(100); //fixed values give to roots clearly outside [0,1], or nonexisting ones*/
   /*second derivative coefficients, eq 23 in white et al.*/
   Vec b0 =   60 * values[k] - 24 * fv_r - 36 * fv_l + 3 * (fd_r - 3 * fd_l);
   Vec b1 = -360 * values[k] + 36 * fd_l - 24 * fd_r + 168 * fv_r + 192 * fv_l;
   Vec b2 =  360 * values[k] + 30 * (fd_r - fd_l) - 180 * (fv_l + fv_r);
   ...
   const Vec sqrt_val = select(b1 * b1 - 4 * b0 * b2 < 0, 
                                b1 + 200 * b2,
                                sqrt(max(b1 * b1- 4 * b0 * b2, 0))); 
   ...
   Vec plm_slope_l = 2 * (values[k] - values[k - 1]);
   Vec plm_slope_r = 2 * (values[k + 1] - values[k]);
   ...
   //sign
   Vec root1_slope = select(root1 >= 0 && root1 <= 1,
                             c0  + root1 * ( c1 + root1 * (c2 + root1 * c3 ) ),
                             slope_sign);
   Vec root2_slope = select(root2 >= 0 && root2 <= 1,
                            c0  + root2 * ( c1 + root2 * (c2 + root2 * c3 ) ),
                            slope_sign);
   Vecb fixInflexion = root1_slope * slope_sign < 0 || root2_slope * slope_sign < 0;
      ...
      for(uint i = 0;i < VECL; i++) {
         if(fixInflexion[i]){
            ...
            if(fabs(plm_slope_l[i]) <= fabs(plm_slope_r[i])) {
               ...
               fda_l[i] =  ( 10 * valuesa[i] - 2 * fva_r[i] - 8 * fva_l[i]) / 3;
               fda_r[i] =  -10 * valuesa[i] + 6 * fva_r[i] + 4 * fva_l[i];
               ...
               if (slope_signa[i] * fda_l[i] < 0) {
                  ...
                  fda_l[i] =  10 * (-valuesa[i] + fva_r[i]) / 3;
               }
            }
            else {
               //collapse to right edge (eq 21)
               fda_l[i] =  10 * valuesa[i] - 6 * fva_l[i] - 4 * fva_r[i];
               fda_r[i] =  ( - 10 * valuesa[i] + 2 * fva_l[i] + 8 * fva_r[i]) / 3;
               //check if PLM slope is consistent (eq 28 & 29)
               if (slope_signa[i] * fda_l[i] < 0) {
                  fda_l[i] =  0;
                  fva_r[i] =  0.5 * ( 5 * valuesa[i] - 3 * fva_l[i]);
                  fda_r[i] =  10 * (valuesa[i] - fva_l[i]) / 3;
               }
               else if (slope_signa[i] * fda_r[i] < 0) {
                  fda_r[i] =  0;
                  fva_l[i] =  5 * valuesa[i] - 4 * fva_r[i];
                  fda_l[i] =  20 * ( - valuesa[i] + fva_r[i]);
               }
            }
         }
      }      
...
inline void compute_pqm_coeff(Vec *values, face_estimate_order order, uint k, Vec a[5]){
   ...
   a[0] = fv_l;
   a[1] =  0.5 * fd_l;
   a[2] =  10 * values[k] - 4 * fv_r - 6 * fv_l + 0.5 * (fd_r - 3 * fd_l);
   a[3] = -15 * values[k] + 1.5 * fd_l - fd_r + 7 * fv_r + 8 * fv_l;
   a[4] =   6 * values[k] + 0.5 * (fd_r - fd_l) - 3 * (fv_l + fv_r);
}
