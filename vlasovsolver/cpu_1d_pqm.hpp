...
inline void filter_pqm_monotonicity(Vec* values, uint k, Vec& fv_l, Vec& fv_r, Vec& fd_l, Vec& fd_r) {
   ...
   const Vec sqrt_val = select(val_to_sqrt < 0.0, b1 + 200.0 * b2, val_to_sqrt * approx_rsqrt(val_to_sqrt));
#else
   const Vec sqrt_val = select(val_to_sqrt < 0.0, b1 + 200.0 * b2, sqrt(val_to_sqrt));
#endif
   ...
   // sign
   Vec root1_slope = select(root1 >= 0.0 && root1 <= 1.0, c0 + root1 * (c1 + root1 * (c2 + root1 * c3)), slope_sign);
   Vec root2_slope = select(root2 >= 0.0 && root2 <= 1.0, c0 + root2 * (c1 + root2 * (c2 + root2 * c3)), slope_sign);
...
//   White, Laurent, and Alistair Adcroft. “A High-Order Finite Volume Remapping Scheme for Nonuniform Grids: The
//   Piecewise Quartic Method (PQM).” Journal of Computational Physics 227, no. 15 (July 2008): 7394–7422.
