         ...
         while (dtTotalDiff < Parameters::dt) { // Substep loop
            ...
            for (vmesh::LocalID n = 0; n < cell.get_number_of_velocity_blocks(popID); n++) { // Iterate through velocity blocks
               ...
               loop_over_block([&](Veci i_indices, Veci j_indices, int k) -> void { // Lambda function processor
                  ...
                  const Vec VX(
                     parameters[n * BlockParams::N_VELOCITY_BLOCK_PARAMS + BlockParams::VXCRD] + (to_realf(i_indices) + 0.5) * parameters[n * BlockParams::N_VELOCITY_BLOCK_PARAMS + BlockParams::DVX]
                  );
                  const Vec VY(
                     parameters[n * BlockParams::N_VELOCITY_BLOCK_PARAMS + BlockParams::VYCRD] + (to_realf(j_indices) + 0.5) * parameters[n * BlockParams::N_VELOCITY_BLOCK_PARAMS + BlockParams::DVY]
                  );
