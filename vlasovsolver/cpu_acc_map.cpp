   ...
   */
   ...
   for (uint setIndex = 0; setIndex < setColumnOffsets.size(); ++setIndex) {
      ...
      for (uint columnIndex = setColumnOffsets[setIndex];
         ...
         if (firstBlockIndexK < wallmargin || firstBlockIndexK >= max_v_length - wallmargin ||
             lastBlockIndexK < wallmargin || lastBlockIndexK >= max_v_length - wallmargin) {
      ...
      for (uint blockK = 0; blockK < MAX_BLOCKS_PER_DIM; blockK++) {
         if (isTargetBlock[blockK] && !isSourceBlock[blockK]) {
            const int targetBlock = setFirstBlockIndices[0] * block_indices_to_id[0] +
                                    setFirstBlockIndices[1] * block_indices_to_id[1] + blockK * block_indices_to_id[2];
...
// create vectors with the i and j indices in the vector position on the plane.
            ...
            const Veci j_indices =
                Veci(j, j, j, j, j + 1, j + 1, j + 1, j + 1, j + 2, j + 2, j + 2, j + 2, j + 3, j + 3, j + 3, j + 3);
#endif
            ...
            */
            ...
            const Vec intersection_min = intersection +
...
// k + WID is the index where we have stored k index, WID amount of padding.
               ...
               int maxGk =
                   std::min(int(lagrangian_gk_r[maxGkIndex]), int((columnMaxBlockK[columnIndex] + 1) * WID - 1));
               ...
               for (int gk = minGk; gk <= maxGk; gk++) {
                  ...
                  // For vector elements where gk is already larger than needed (lagrangian_gk_r), v_2=v_1=v_r and thus
                  // the value is zero.
...
// compute right integrand
                  ...
                  if (dimension == 2) {
                     Realf* targetDataPointer =
                         blockIndexToBlockData[blockK] + j * cell_indices_to_id[1] + gk_mod_WID * cell_indices_to_id[2];
                     Vec targetData;
