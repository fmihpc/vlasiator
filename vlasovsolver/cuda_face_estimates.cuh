...
__device__ void compute_h8_left_face_value(const Realf * const values, uint k, Realf &fv_l)
{
   fv_l = 1.0/840.0 * (
              - 3.0 * values[k - 4]
              + 29.0 * values[k - 3]
...
__device__ void compute_h7_left_face_derivative(const Realf * const values, uint k, Realf &fd_l){
    fd_l = 1.0/5040.0 * (
       + 9.0 * values[k - 4]
       - 119.0 * values[k - 3]
       + 889.0 * values[k - 2]
...
__device__ void compute_h6_left_face_value(const Realf * const values, uint k, Realf &fv_l)
{
  //compute left value
   fv_l = 1.0/60.0 * (values[k - 3]
            - 8.0 * values[k - 2]
...
__device__ void compute_h5_left_face_derivative(const Realf * const values, uint k, Realf &fd_l)
{
  fd_l = 1.0/180.0 * (245 * (values[k] - values[k - 1])
                     - 25 * (values[k + 1] - values[k - 2])
                     + 2 * (values[k + 2] - values[k - 3]));
}
__device__ void compute_h5_face_values(const Realf * const values, uint k, Realf &fv_l, Realf &fv_r)
{
  //compute left values
  fv_l = 1.0/60.0 * (- 3.0 * values[k - 2]
                     + 27.0 * values[k - 1]
...
__device__ void compute_h4_left_face_derivative(const Realf * const values, uint k, Realf &fd_l)
{
  fd_l = 1.0/12.0 * (15.0 * (values[k] - values[k - 1]) - (values[k + 1] - values[k - 2]));
}
__device__ void compute_h4_left_face_value(const Realf * const values, uint k, Realf &fv_l)
{
  //compute left value
  fv_l = 1.0/12.0 * ( - 1.0 * values[k - 2]
                      + 7.0 * values[k - 1]
...
__device__ void compute_h3_left_face_derivative(const Realf * const values, uint k, Realf &fv_l)
{
  /*compute left value*/
  fv_l = 1.0/12.0 * (15 * (values[k] - values[k - 1]) - (values[k + 1] - values[k - 2]));
}
__device__ void compute_filtered_face_values_derivatives(const Realf * const values,uint k, face_estimate_order order, Realf &fv_l, Realf &fv_r, Realf &fd_l, Realf &fd_r, const Realv threshold)
{
   switch(order)
   {
       case h4:
   ...
   if(is_extrema)
   {
      fv_r = (is_extrema) ? values[k] : fv_r;
      fv_l = (is_extrema) ? values[k] : fv_l;
      fd_l = (is_extrema) ? 0.0 : fd_l;
      fd_r = (is_extrema) ? 0.0 : fd_r;
   }
   //Fix left face if needed; boundary value is not bounded or slope is not consistent
   bool filter = (values[k -1] - fv_l) * (fv_l - values[k]) < 0 || slope_sign * fd_l < 0.0;
   if(filter)
   {
      //Go to linear (PLM) estimates if not ok (this is always ok!)
      fv_l= (filter) ? values[k ] - slope_sign * 0.5 * slope_abs : fv_l;
   ...
   {
      //Go to linear (PLM) estimates if not ok (this is always ok!)
      fv_r= (filter) ? values[k] + slope_sign * 0.5 * slope_abs : fv_r;
...
/*Filters in section 2.6.1 of white et al. to be used for PPM
  ...
  3) Makes sure face slopes are consistent with PLM slope
*/
__device__ void compute_filtered_face_values(const Realf * const values, uint k, face_estimate_order order, Realf &fv_l, Realf &fv_r, const Realv threshold)
{
  switch(order)
  {
      case h4:
  ...
  //Fix left face if needed; boundary value is not bounded
  bool filter = (values[k -1] - fv_l) * (fv_l - values[k]) < 0 ;
  if(filter)
  {
     //Go to linear (PLM) estimates if not ok (this is always ok!)
     fv_l = (filter) ? values[k ] - slope_sign * 0.5 * slope_abs : fv_l;
