...
__global__ void __launch_bounds__(WID3)
    build2dArrayOfFvmu_kernel(size_t* dev_cellIdxArray, size_t* dev_velocityIdxArray, const vmesh::VelocityBlockContainer* __restrict__ const* dev_velocityBlockContainer, Real* dev_bulkVX,
                              Real* dev_bulkVY, Real* dev_bulkVZ, Real* dev_bValues, Realf* dev_fmu, int* dev_fcount, Real dVbins, const Real dmubins, int nbins_v, int nbins_mu) {
...
__global__ void __launch_bounds__(WID3) computeNewCellValues_kernel(size_t* dev_cellIdxArray, size_t* dev_remappedCellIdxArray, size_t* dev_velocityIdxArray,
                                                                    vmesh::VelocityBlockContainer* __restrict__* dev_velocityBlockContainer, Real* dev_bulkVX, Real* dev_bulkVY, Real* dev_bulkVZ,
                                                                    Real* dev_bValues, Realf* dev_dfdt_mu, Real* dev_Ddt, Real dVbins, const Real dmubins, int nbins_v, int nbins_mu) {
...
__global__ void computeDerivativesCFLDdt_kernel(size_t* dev_smallCellIdxArray, int* dev_fcount, Realf* dev_fmu, Real* dev_nu0Values, Realf* dev_dfdt_mu, int* dev_cellIdxKeys,
                                                Real* dev_potentialDdtValues, Realf* dev_sparsity, Real dVbins, const Real dmubins, const Real epsilon, Realf PADCFL, int nbins_v, int nbins_mu,
                                                int blocksPerSpatialCell, int lastBlockSize) {
   ...
   if (nextIndexInsideBlock != 0 || threadIndex < lastBlockSize) {
      ...
      if ((cRight != 0) || (cLeft != 0)) {
         dfdmu = (GPUCELLMUSPACE(dev_fmu, cellIdx, indv, indmu + cRight) - GPUCELLMUSPACE(dev_fmu, cellIdx, indv, indmu - cLeft)) / ((cRight + cLeft) * dmubins);
      }
      if ((cRight != 0) && (cLeft != 0)) {
         dfdmu2 = ((GPUCELLMUSPACE(dev_fmu, cellIdx, indv, indmu + cRight) - GPUCELLMUSPACE(dev_fmu, cellIdx, indv, indmu)) / (cRight * dmubins) -
                   (GPUCELLMUSPACE(dev_fmu, cellIdx, indv, indmu) - GPUCELLMUSPACE(dev_fmu, cellIdx, indv, indmu - cLeft)) / (cLeft * dmubins)) /
...
__global__ void __launch_bounds__(Hashinator::defaults::MAX_BLOCKSIZE / 2)
    dividefByCount_kernel(size_t* dev_smallCellIdxArray, Realf* dev_fmu, int* dev_fcount, int nbins_v, int nbins_mu, int maxThreadIndex) {

   int idx = blockIdx.x * blockDim.x + threadIdx.x;

   if (idx >= maxThreadIndex) {
      return;
   }
...
__global__ void __launch_bounds__(Hashinator::defaults::MAX_BLOCKSIZE / 2)
    getCellIndexArray_kernel(size_t* dev_cellIdxArray, size_t* dev_velocityIdxArray, size_t* dev_cellIdxStartCutoff, int numberOfComputedVelocityBlocks, int maxCellIndex) {

   size_t totalBlockIndex = blockIdx.x * blockDim.x + threadIdx.x;

   if (totalBlockIndex >= (size_t)numberOfComputedVelocityBlocks) {
      return;
   }
...
__global__ void __launch_bounds__(WID3) calculateDensity_kernel(Realf* dev_density, const vmesh::VelocityBlockContainer* __restrict__ const* dev_velocityBlockContainer) {
...
__global__ void __launch_bounds__(WID3) conserveMass_kernel(Realf* dev_densityPreAdjust, Realf* dev_densityPostAdjust, vmesh::VelocityBlockContainer* __restrict__* dev_velocityBlockContainer) {
   ...
   if (dev_densityPostAdjust[cellIdx] == 0.0 || dev_densityPreAdjust[cellIdx] == dev_densityPostAdjust[cellIdx]) {
      return;
   }
...
// Compute parameters
   ...
   if (getObjectWrapper().particleSpecies[popID].sparse_conserve_mass) {
      ...
      calculateDensity_kernel<<<blocksPerGrid_massConservation, threadsPerBlock_massConservation>>>(dev_densityPreAdjust, dev_VBCs);
   ...
   while (!allSpatialCellTimeLoopsComplete) { // Substep loop
      ...
      int totalThreadsPerBlock_getCellIndexArray =
          Hashinator::defaults::MAX_BLOCKSIZE / 2; // Using Hashinator::defaults::MAX_BLOCKSIZE/2 = 512 blocks can lead to better streaming multiprocessor occupancy
      ...
      getCellIndexArray_kernel<<<blocksPerGrid_getCellIndexArray, totalThreadsPerBlock_getCellIndexArray>>>(
          dev_cellIdxArray, dev_velocityIdxArray, dev_cellIdxStartCutoff, numberOfComputedVelocityBlocks, maxCellIndex);
      ...
      build2dArrayOfFvmu_kernel<<<blocksPerGrid_build2dArrayOfFvmu, threadsPerBlock_build2dArrayOfFvmu>>>(
          dev_cellIdxArray, dev_velocityIdxArray, dev_VBCs, dev_bulkVX, dev_bulkVY, dev_bulkVZ, dev_bValues, dev_fmu, dev_fcount, dVbins, dmubins, nbins_v, nbins_mu);

      CHK_ERR(gpuPeekAtLastError());
      builFvmuTimer.stop();

      int totalThreadsPerBlock_dividefByCount =
          Hashinator::defaults::MAX_BLOCKSIZE / 2; // Using Hashinator::defaults::MAX_BLOCKSIZE/2 = 512 blocks can lead to better streaming multiprocessor occupancy
      ...
      dividefByCount_kernel<<<blocksPerGrid_dividefByCount, totalThreadsPerBlock_dividefByCount>>>(dev_smallCellIdxArray, dev_fmu, dev_fcount, nbins_v, nbins_mu, maxThreadIndex_dividefByCount);
      ...
      computeDerivativesCFLDdt_kernel<<<blocksPerGrid_computeDerivativesCFLDdt, totalThreadsPerBlock_computeDerivativesCFLDdt, sharedMemorySize>>>(dev_smallCellIdxArray,
                                                                                                                                                   dev_fcount,
                                                                                                                                                   dev_fmu,
                                                                                                                                                   dev_nu0Values,
                                                                                                                                                   dev_dfdt_mu,
      ...
      reduceDdtValues_kernel<<<blocksPerGrid_reduceDdtValues, totalThreadsPerBlock_reduceDdtValues, sharedMemorySize_reduceDdtValues>>>(
          dev_cellIdxKeys, dev_potentialDdtValues, dev_Ddt, blocksPerSpatialCell);
      ...
      computeNewCellValues_kernel<<<blocksPerGrid_computeNewCellValues, threadsPerBlock_computeNewCellValues>>>(
          dev_cellIdxArray, dev_remappedCellIdxArray, dev_velocityIdxArray, dev_VBCs, dev_bulkVX, dev_bulkVY, dev_bulkVZ, dev_bValues, dev_dfdt_mu, dev_Ddt, dVbins, dmubins, nbins_v, nbins_mu);
   ...
   if (getObjectWrapper().particleSpecies[popID].sparse_conserve_mass) {
      ...
      // Ensure mass conservation
      calculateDensity_kernel<<<blocksPerGrid_massConservation, threadsPerBlock_massConservation>>>(dev_densityPostAdjust, dev_VBCs);

      CHK_ERR(gpuPeekAtLastError());

      conserveMass_kernel<<<blocksPerGrid_massConservation, threadsPerBlock_massConservation>>>(dev_densityPreAdjust, dev_densityPostAdjust, dev_VBCs);
