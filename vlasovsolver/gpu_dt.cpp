 ...
 */
__global__ void
__launch_bounds__(GPUTHREADS* WARPSPERBLOCK) reduce_v_dt_kernel(const vmesh::VelocityMesh* __restrict__ const* dev_vmeshes, Real* dev_max_dt, const Real* dev_dxdydz, const uint nAllCells) {
...
// Gather vmeshes
   ...
   reduce_v_dt_kernel<<<nAllCells, GPUTHREADS * WARPSPERBLOCK, 0, 0>>>(dev_vmeshes, dev_max_dt, dev_dxdydz, nAllCells * nPOP);
   ...
   for (vector<CellID>::const_iterator cell_id = cells.begin(); cell_id != cells.end(); ++cell_id) {
      ...
      if (cell->parameters[CellParams::MAXVDT] != 0 &&
          (cell->sysBoundaryFlag == sysboundarytype::NOT_SYSBOUNDARY || (P::vlasovAccelerateMaxwellianBoundaries && cell->sysBoundaryFlag == sysboundarytype::MAXWELLIAN))) {
