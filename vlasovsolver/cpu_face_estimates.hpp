...
inline void compute_h8_left_face_value(const Vec* const values, uint k, Vec& fv_l) {
   fv_l = 1.0 / 840.0 *
          (-3.0 * values[k - 4] + 29.0 * values[k - 3] - 139.0 * values[k - 2] + 533.0 * values[k - 1] +
           533.0 * values[k] - 139.0 * values[k + 1] + 29.0 * values[k + 2] - 3.0 * values[k + 3]);
}
...
inline void compute_h7_left_face_derivative(const Vec* const values, uint k, Vec& fd_l) {
   fd_l = 1.0 / 5040.0 *
          (+9.0 * values[k - 4] - 119.0 * values[k - 3] + 889.0 * values[k - 2] - 7175.0 * values[k - 1] +
           7175.0 * values[k] - 889.0 * values[k + 1] + 119.0 * values[k + 2] - 9.0 * values[k + 3]);
}
...
inline void compute_h6_left_face_value(const Vec* const values, uint k, Vec& fv_l) {
   /*compute left value*/
   fv_l = 1.0 / 60.0 *
          (values[k - 3] - 8.0 * values[k - 2] + 37.0 * values[k - 1] + 37.0 * values[k] - 8.0 * values[k + 1] +
           values[k + 2]);
}
...
inline void compute_h5_left_face_derivative(const Vec* const values, uint k, Vec& fd_l) {
   fd_l =
       1.0 / 180.0 *
       (245 * (values[k] - values[k - 1]) - 25 * (values[k + 1] - values[k - 2]) + 2 * (values[k + 2] - values[k - 3]));
}
...
inline void compute_h5_face_values(const Vec* const values, uint k, Vec& fv_l, Vec& fv_r) {
   /*compute left values*/
   fv_l = 1.0 / 60.0 *
          (-3.0 * values[k - 2] + 27.0 * values[k - 1] + 47.0 * values[k] - 13.0 * values[k + 1] + 2.0 * values[k + 2]);
   fv_r = 1.0 / 60.0 *
          (2.0 * values[k - 2] - 13.0 * values[k - 1] + 47.0 * values[k] + 27.0 * values[k + 1] - 3.0 * values[k + 2]);
}
/*!
...
inline void compute_h4_left_face_value_nonuniform(const Vec* const h, const Vec* const u, uint k, Vec& fv_l) {
   ...
   fv_l = (1.0 / (h[k - 2] + h[k - 1] + h[k] + h[k + 1]) *
           ((h[k - 2] + h[k - 1]) * (h[k] + h[k + 1]) / (h[k - 1] + h[k]) * (u[k - 1] * h[k] + u[k] * h[k - 1]) *
...
/*Filters in section 2.6.1 of white et al. to be used for PPM
  ...
  3) Makes sure face slopes are consistent with PLM slope
*/
inline void compute_filtered_face_values(const Vec* const values, uint k, face_estimate_order order, Vec& fv_l,
                                         Vec& fv_r, const Realv threshold) {
   switch (order) {
   case h4:
      compute_h4_left_face_value(values, k, fv_l);
      compute_h4_left_face_value(values, k + 1, fv_r);
...
inline void compute_filtered_face_values_nonuniform(const Vec* const dv, const Vec* const values, uint k,
                                                    face_estimate_order order, Vec& fv_l, Vec& fv_r,
                                                    const Realv threshold) {
   switch (order) {
   case h4:
...
inline pair<Vec, Vec> constrain_face_values(const Vec* h, const Vec* values, uint k, Vec& fv_l, Vec& fv_r) {
   ...
   if ((horizontal_or(d2a * d2aL >= 0)) && (horizontal_or(d2a * d2aR >= 0)) && (horizontal_or(d2a * d2aC >= 0)) &&
   ...
   //   std::cout << "k, a-, a+, <a>: " << k << ", " << p_face_interpolant[0] << ", " << m_face_interpolant[0] << ", "
   //   << values[k][0] << endl;

   return faceInterpolants;
}

inline void compute_filtered_face_values_nonuniform_conserving(const Vec* const dv, const Vec* const values, uint k,
                                                               face_estimate_order order, Vec& fv_l, Vec& fv_r,
                                                               const Realv threshold) {
   switch (order) {
   case h4:
