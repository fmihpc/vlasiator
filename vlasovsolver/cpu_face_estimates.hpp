...
inline pair<Vec,Vec> constrain_face_values(const Vec * h,const Vec * values,uint k,Vec & fv_l, Vec & fv_r) {
  ...
  Vec d2a =  invh2 * 3.0 * (values[k]     - 2.0 * fv_r          + values[k + 1]);
  ...
  if ( (horizontal_or(d2a * d2aL >= 0)) && (horizontal_or(d2a * d2aR >= 0)) &&
   ...
   Vec p_face = 0.5 * (values[k] + values[k + 1]) - h[k] * h[k] / 3.0 * d2aLim;

   // Calculate negative face value. TODO: Fix copy-paste
   uint l = k - 1;
   invh2 = 1.0 / (h[l] * h[l]);
   d2a =  invh2 * 3.0 * (values[l]     - 2.0 * fv_l          + values[l + 1]);
   d2aL = invh2       * (values[l - 1] - 2.0 * values[l]     + values[l + 1]);
   d2aR = invh2       * (values[l]     - 2.0 * values[l + 1] + values[l + 2]);
   if ( (horizontal_or(d2a * d2aL >= 0)) && (horizontal_or(d2a * d2aR >= 0)) &&
	horizontal_and(d2a != 0)) {
     d2aLim = d2a / abs(d2a) * min(abs(d2a),min(C*abs(d2aL),C*abs(d2aR)));
   } else {
   ...
   Vec m_face = 0.5 * (values[l] + values[l + 1]) - h[l] * h[l] / 3.0 * d2aLim;

   // Colella & Sekora, eq 22
   d2a = -2.0 * invh2 * 6.0 * (values[k] - 3.0 * (m_face + p_face)); // a6,j from eq. 7 
   Vec d2aC = invh2 * (values[k - 1] - 2.0 * values[k] + values[k + 1]);
   //d2aL = invh2 * (values[k-2] - 2.0 * values[k] + values[k]);
   d2aL = invh2 * (values[k-2] - 2.0 * values[k-1] + values[k]);
   d2aR = invh2 * (values[k] - 2.0 * values[k+1] + values[k+2]);
   if ( (horizontal_or(d2a * d2aL >= 0)) && (horizontal_or(d2a * d2aR >= 0)) &&
	(horizontal_or(d2a * d2aC >= 0)) && horizontal_and(d2a != 0)) {
     ...
     if( horizontal_or(d2a == 0.0)) {
       // Set a non-zero value for the divison in eq. 23.
       // According to the paper the ratio should be 0.0
       d2a = 1.0;
     }
   }
