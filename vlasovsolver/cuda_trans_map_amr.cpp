 ...
 * are target cells. Boundary cells are not included.
...
void computeSpatialTargetCellsForPencilsWithFaces(const dccrg::Dccrg<SpatialCell,dccrg::Cartesian_Geometry>& mpiGrid,
                                                  setOfPencils& pencils,
                                                  const uint dimension,
                                                  SpatialCell **targetCells // splitvector data accessor
   ){
...
#pragma omp parallel
   {
      ...
      for(uint pencili = 0; pencili < DimensionPencils[dimension].N; ++pencili) {
         ...
         if (cuda_vlasov_allocatedSize < sourceLength) {
            // If it's large enough for the source, it's large enough for the target as well.
            std::cerr<<"Warning! Too small data container in __FILE__:__LINE__"<<std::endl;
         }
...
//             for ( auto pencili : unionOfBlocksMapToPencilIds.at(blockGID) ) {
               ...
               for (uint icell = 0; icell < targetLength; icell++) {
                  ...
                  for (uint k=0; k<WID; k++) {
                     ...
                     for(uint planeVector = 0; planeVector < VEC_PER_PLANE; planeVector++){
                        ...
                        Realf vector[VECL];
                        dev_blockDataTarget[cpuThreadID][i_trans_pt_blockv(planeVector, k, icell - 1)].store(vector);
            ...
            for(uint pencili = 0; pencili < DimensionPencils[dimension].N; pencili++){
               ...
               for ( uint celli = 0; celli < targetLength; celli++ ) {
                  ...
                  SpatialCell* targetCell = targetCells->at(GID);
