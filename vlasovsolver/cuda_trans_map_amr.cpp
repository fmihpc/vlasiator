...
/* Copy the data to the temporary values array. Does not bother with
 * dimension swapping because the GPU version assumes a whole block
 * is always handled at once.
 */
/*__global__ void copy_trans_block_data_amr_kernel(
   ...
   // Expecting warpSize to be equal to WID3
...
#pragma omp critical
   ...
   // split::SplitVector<SpatialCell*> *targetCells; // In unified memory
   // targetCells = new split::SplitVector<SpatialCell*>(DimensionPencils[dimension].sumOfLengths + DimensionPencils[dimension].N * 2 * nTargetNeighborsPerPencil );
...
#pragma omp parallel
   {
      ...
      split::SplitVector<Realf> targetBlockData((DimensionPencils[dimension].sumOfLengths + 2 * nTargetNeighborsPerPencil * DimensionPencils[dimension].N) * WID3);
      ...
      //    //split::SplitVector<Vec> targetValues((L + 2 * nTargetNeighborsPerPencil) * WID3 / VECL);
      //    //pencilTargetValues.push_back(targetValues);
      //    // Allocate source data: sourcedata<length of pencil * WID3)
      //    //if (cuda_vlasov_allocatedSize < sourceLength) { / need to compare total length, not per pencil
      ...
      //    split::SplitVector<Vec> dz(sourceLength);
      //    for(uint i = 0; i < sourceCells.size(); ++i) {
      //       dz[i] = mpiGrid[DimensionPencils[dimension].sourceIds[sourceIdsStart[pencili]+i]]->parameters[CellParams::DX+dimension];
      ...
      for(uint blocki = 0; blocki < unionOfBlocks.size(); blocki++) {
         ...
         for(uint pencili = 0; pencili < DimensionPencils[dimension].N; pencili++){
            ...
            for ( uint i = 0; i < targetLength; i++ ) {
               ...
               uint targetID = DimensionPencils[dimension].targetIds[targetIndex];
               ...
               if(targetCell) { // this check also skips sysboundary cells
                  ...
                  for(int i = 0; i < WID3 ; i++) {
                     blockData[i] += targetBlockData[targetIndex * WID3 + i] * areaRatio;
                  }
               }
            }
         }
