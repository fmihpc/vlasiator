...
#define BANK_OFFSET(n) ((n) >> (LOG_BANKS) + (n) >> (2 * LOG_BANKS))
...
__global__ void prefill_probe_kernel(vmesh::VelocityMesh** __restrict__ vmeshes,
                                     ...
                                     const uint flatExtent, const size_t Dacc, const size_t Dother, const vmesh::LocalID invalidLID,
                                     // Pass these for emptying
                                     split::SplitVector<vmesh::GlobalID>** lists_with_replace_new, split::SplitVector<Hashinator::hash_pair<vmesh::GlobalID, vmesh::LocalID>>** lists_delete,
                                     ...
                                     split::SplitVector<vmesh::GlobalID>** dev_vbwcl_vec, const uint cumulativeOffset) {
...
__global__ void fill_probe_ordered(vmesh::VelocityMesh** __restrict__ vmeshes,
                                   Realf** dev_blockDataOrdered, // recast to vmesh::LocalID *probeCube
                                   const uint flatExtent, const uint* __restrict__ gpu_block_indices_to_probe, const uint cumulativeOffset) {
   ...
   const int target = indices[0] * gpu_block_indices_to_probe[0] + indices[1] * gpu_block_indices_to_probe[1] + indices[2] * gpu_block_indices_to_probe[2];
...
/*!
   ...
   @param invalidLID value to be used as the value for invalid vmesh::LocalID
*/
__global__ void flatten_probe_cube(Realf** dev_blockDataOrdered, // recast to vmesh::LocalID *probeCube, *probeFlattened
                                   const vmesh::LocalID Dacc, const vmesh::LocalID Dother, const size_t flatExtent, const vmesh::LocalID invalidLID) {
...
__global__ void scan_probe(vmesh::VelocityMesh** __restrict__ vmeshes,
                           ...
                           const vmesh::LocalID Dacc, const vmesh::LocalID Dother, const size_t flatExtent, vmesh::LocalID* dev_numCols, vmesh::LocalID* dev_numColSets,
                           vmesh::LocalID* dev_resizeSuccess, ColumnOffsets* dev_columnOffsetData, const uint cumulativeOffset) {
   const uint parallelOffsetIndex = blockIdx.y;
   const uint cellOffset = parallelOffsetIndex + cumulativeOffset;
   vmesh::LocalID* probeFlattened = reinterpret_cast<vmesh::LocalID*>(dev_blockDataOrdered[parallelOffsetIndex]);
   // vmesh::LocalID* probeCube = probeFlattened + flatExtent*GPU_PROBEFLAT_N;
   ...
   if (ti == 0) {
      ...
      if ((columnData->dev_capacityCols() < numCols) || (columnData->dev_capacityColSets() < numColSets)) {
...
__global__ void build_column_offsets(vmesh::VelocityMesh** __restrict__ vmeshes,
                                     ...
                                     const vmesh::LocalID D0, const vmesh::LocalID D1, const vmesh::LocalID D2, const int dimension, const size_t flatExtent, const vmesh::LocalID invalidLID,
...
__global__ void __launch_bounds__(WID3)
    reorder_blocks_by_dimension_kernel(vmesh::VelocityBlockContainer** __restrict__ blockContainers, Realf** dev_blockDataOrdered, const uint* __restrict__ gpu_cell_indices_to_id,
                                       split::SplitVector<vmesh::GlobalID>** dev_vbwcl_vec, // use as LIDlist
                                       ColumnOffsets* dev_columnOffsetData, vmesh::LocalID* dev_nColumns, const uint cumulativeOffset) {
   // This is launched with block size (WID,WID,WID)
   const uint ti = threadIdx.z * blockDim.x * blockDim.y + threadIdx.y * blockDim.x + threadIdx.x;
   // Acceleration direction becomes "z"
   const uint sourcei = threadIdx.x * gpu_cell_indices_to_id[0] + threadIdx.y * gpu_cell_indices_to_id[1] + threadIdx.z * gpu_cell_indices_to_id[2];
...
/*!
   ...
   @param cumulativeOffset the current cumulative offset at which to index the aforementioned buffers, using
    the grid index of this kernel on top of this provided offset.
*/
__global__ void __launch_bounds__(GPUTHREADS, 4)
    evaluate_column_extents_kernel(const uint dimension, vmesh::VelocityMesh** __restrict__ vmeshes, ColumnOffsets* dev_columnOffsetData, split::SplitVector<vmesh::GlobalID>** lists_with_replace_new,
                                   Hashinator::Hashmap<vmesh::GlobalID, vmesh::LocalID>** allMaps, const uint* __restrict__ gpu_block_indices_to_id, const Realf* dev_intersections,
                                   const int bailout_velocity_space_wall_margin, const int max_v_length, const Realf v_min, const Realf dv,
   ...
   if (setIndex < columnData->setColumnOffsets.size()) {
      ...
      */
      ...
      intersectionMins[0] = intersection + (set_i * WID + 0) * intersection_di + (set_j * WID + 0) * intersection_dj;
      intersectionMins[1] = intersection + (set_i * WID + 0) * intersection_di + (set_j * WID + WID - 1) * intersection_dj;
      intersectionMins[2] = intersection + (set_i * WID + WID - 1) * intersection_di + (set_j * WID + 0) * intersection_dj;
      intersectionMins[3] = intersection + (set_i * WID + WID - 1) * intersection_di + (set_j * WID + WID - 1) * intersection_dj;

      Realf min_intersectionMin = std::min(std::min(intersectionMins[0], intersectionMins[1]), std::min(intersectionMins[2], intersectionMins[3]));
      Realf max_intersectionMin = std::max(std::max(intersectionMins[0], intersectionMins[1]), std::max(intersectionMins[2], intersectionMins[3]));
      ...
      for (uint columnIndex = columnData->setColumnOffsets[setIndex]; columnIndex < columnData->setColumnOffsets[setIndex] + columnData->setNumColumns[setIndex]; ++columnIndex) {
         ...
         if (firstBlockIndexK < bailout_velocity_space_wall_margin || firstBlockIndexK >= max_v_length - bailout_velocity_space_wall_margin || lastBlockIndexK < bailout_velocity_space_wall_margin ||
      ...
      for (uint blockT = 0; blockT < MAX_BLOCKS_PER_DIM; blockT += warpSize) {
         ...
         if (blockK < MAX_BLOCKS_PER_DIM) {
            if (isTargetBlock[blockK] != 0) {
               const int targetBlock = set_i * gpu_block_indices_to_id[0] + set_j * gpu_block_indices_to_id[1] + blockK * gpu_block_indices_to_id[2];
...
/*!
   ...
   @param cumulativeOffset the current cumulative offset at which to index the aforementioned buffers, using
    the grid index of this kernel on top of this provided offset.
 */
__global__ void __launch_bounds__(WID3, ACCELERATION_KERNEl_MIN_BLOCKS) acceleration_kernel(vmesh::VelocityMesh** __restrict__ vmeshes,      // indexing: cellOffset
                                                                                            ...
                                                                                            const uint* __restrict__ gpu_cell_indices_to_id, const uint* __restrict__ gpu_block_indices_to_id,
                                                                                            ColumnOffsets* __restrict__ dev_columnOffsetData, // indexing: blockIdx.y
                                                                                            const Realf* dev_intersections,                   // indexing: cellOffset
                                                                                            const Realf v_min, const Realf i_dv, const Realf dv,
                                                                                            const Real* dev_minValues, // indexing: cellOffset
                                                                                            const size_t invalidLID, const uint cumulativeOffset) {
   ...
   for (uint columnIndex = 0; columnIndex < numColumns; ++columnIndex) {
      ...
      const Realf intersection_min = intersection + intersection_di * (Realf)(col_i * WID + i) + intersection_dj * (Realf)(col_j * WID + j);
      // Pre-computed constant target offset contribution
      const int target_cell_index_common = i * gpu_cell_indices_to_id[0] + j * gpu_cell_indices_to_id[1];
...
*/
__host__ bool gpu_acc_map_1d(dccrg::Dccrg<spatial_cell::SpatialCell, dccrg::Cartesian_Geometry>& mpiGrid, vector<CellID>& launchCells, const uint popID, const uint dimension,
   ...
   */
   ...
   prefill_probe_kernel<<<grid_prefill_probe, Hashinator::defaults::MAX_BLOCKSIZE, 0, baseStream>>>(dev_vmeshes,
   ...
   fill_probe_ordered<<<grid_fill_ord, Hashinator::defaults::MAX_BLOCKSIZE, 0, baseStream>>>(dev_vmeshes,
                                                                                             dev_blockDataOrdered, // recast to vmesh::LocalID *probeCube
                                                                                             flatExtent,
   ...
   flatten_probe_cube<<<grid_cube, Hashinator::defaults::MAX_BLOCKSIZE, 0, baseStream>>>(dev_blockDataOrdered, // recast to vmesh::LocalID *probeCube, *probeFlattened
                                                                                         Dacc,
                                                                                         Dother,
                                                                                         flatExtent,
                                                                                         invalidLocalID);
   ...
   */
   ...
   scan_probe<<<grid_scan, Hashinator::defaults::MAX_BLOCKSIZE, 0, baseStream>>>(dev_vmeshes,
   ...
   build_column_offsets<<<grid_cube, Hashinator::defaults::MAX_BLOCKSIZE, 0, baseStream>>>(dev_vmeshes,
   ...
   reorder_blocks_by_dimension_kernel<<<grid_reorder, block_reorder, 0, baseStream>>>(dev_VBCs,
   ...
   evaluate_column_extents_kernel<<<grid_column_extents, GPUTHREADS, 0, baseStream>>>(dimension,
                                                                                      dev_vmeshes,
                                                                                      dev_columnOffsetData,
                                                                                      dev_lists_with_replace_new,
                                                                                      dev_allMaps,
   ...
   if (needSecondLaunchColumnExtents) {
      ...
      CHK_ERR(gpuMemcpyAsync(dev_lists_with_replace_new + cumulativeOffset,
                             host_lists_with_replace_new + cumulativeOffset,
                             nLaunchCells * sizeof(split::SplitVector<vmesh::GlobalID>*),
   ...
   */
   ...
   extract_to_delete_or_move_caller(dev_allMaps + 2 * cumulativeOffset,            // dev_has_content_maps, // input maps
   ...
   extract_to_delete_or_move_caller(dev_allMaps + 2 * cumulativeOffset + 1,        // dev_has_no_content_maps, // input maps
   ...
   extract_to_replace_caller(dev_allMaps + 2 * cumulativeOffset + 1,        // dev_has_no_content_maps, // input maps
   ...
   batch_adjust_blocks_caller(mpiGrid, launchCells, cumulativeOffset, largestBlocksToChange, largestBlocksBeforeOrAfter, popID);
   ...
   fill_VBC_zero_kernel<<<grid_fill_VBC_zero, Hashinator::defaults::MAX_BLOCKSIZE, 0, baseStream>>>(dev_VBCs, // indexing: cellOffset
                                                                                                    cumulativeOffset);
   CHK_ERR(gpuPeekAtLastError());
   CHK_ERR(gpuStreamSynchronize(baseStream));
   zeroTimer.stop();
   ...
   acceleration_kernel<<<grid_acc, block_acc, 0, baseStream>>>(dev_vmeshes,          // indexing: cellOffset
