...
//#endif
   ...
   // sign
   const Vec root1_slope = select(root1 >= 0.0 && root1 <= 1.0, c0 + root1 * (c1 + root1 * (c2 + root1 * c3)), slope_sign);
   const Vec root2_slope = select(root2 >= 0.0 && root2 <= 1.0, c0 + root2 * (c1 + root2 * (c2 + root2 * c3)), slope_sign);
...
//   White, Laurent, and Alistair Adcroft. “A High-Order Finite Volume Remapping Scheme for Nonuniform Grids: The Piecewise Quartic Method (PQM).” Journal of Computational Physics 227, no. 15 (July
//   2008): 7394–7422. doi:10.1016/j.jcp.2008.04.026.
// */
