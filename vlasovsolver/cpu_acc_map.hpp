...
inline void load_column_block_data(SpatialCell* spatial_cell, uint* blocks, uint n_blocks, Vec4 * __restrict__ values, int dimension){
   ...
   for(uint block_k = 0; block_k < n_blocks; block_k++){
      Velocity_Block *block=spatial_cell->at(blocks[block_k]);
...
#endif
#ifdef ACC_SEMILAG_PPM
	   compute_ppm_coeff_explicit(values + i_pcolumnv(n_cblocks, -1, j, 0), k + WID, a);
#endif
#ifdef ACC_SEMILAG_PQM
	   compute_pqm_coeff_explicit(values + i_pcolumnv(n_cblocks, -1, j, 0), k + WID, a);
#endif
