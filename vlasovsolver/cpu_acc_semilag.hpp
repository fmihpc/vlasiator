...
void sortBlocklistByDimension( // const spatial_cell::SpatialCell* spatial_cell,
    const vmesh::VelocityMesh* vmesh, const uint dimension, uint* blocks, std::vector<uint>& columnBlockOffsets, std::vector<uint>& columnNumBlocks, std::vector<uint>& setColumnOffsets,
   ...
   for (vmesh::LocalID i = 0; i < nBlocks; ++i) {
      ...
      case 1: {
         ...
         const vmesh::GlobalID blockId_mapped = block - (x_index + y_index * vmesh->getGridLength()[0]) + y_index + x_index * vmesh->getGridLength()[1];
         block_pairs[i] = std::make_pair(blockId_mapped, block);
      } break;
      case 2: {
         // Do operation:
         ...
         const vmesh::GlobalID blockId_mapped = z_index + y_index * vmesh->getGridLength()[2] + x_index * vmesh->getGridLength()[1] * vmesh->getGridLength()[2];
         block_pairs[i] = std::make_pair(blockId_mapped, block);
      } break;
      }
   }
