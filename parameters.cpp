...
static map<string,bool>   isParsed;
...
/** Get the value of the given parameter. This may be called after having called Parse, and it may be called
...
bool Readparameters::parse() {
   ...
   /*
     ...
     Separate bcasts not optimal from performance pow, but parse is normally just
   ...
   */
   ...
   if(rank==0){
       numoptions=0;
       for( map<string,bool>::iterator ip=isParsed.begin(); ip!=isParsed.end();++ip){
           if(! ip->second) numoptions++;
       }
   }

   MPI_Bcast(&numoptions,1,MPI_INT,0,MPI_COMM_WORLD);
   if(rank==0){
       //iterate through map and bcast cstrings of key/val pairs not parsed before
       for( map<string,string>::iterator p=options.begin(); p!=options.end();++p){
           
           if(! isParsed[p->first]) {
               strncpy(name,p->first.c_str(),maxchar-1);
               strncpy(val,p->second.c_str(),maxchar-1);
               MPI_Bcast(name,maxchar,MPI_CHAR,0,MPI_COMM_WORLD);
               MPI_Bcast(val,maxchar,MPI_CHAR,0,MPI_COMM_WORLD);
               isParsed[p->first]=true; 
           }
       }
   }
   else{
