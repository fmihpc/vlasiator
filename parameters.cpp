...
bool P::addParameters() {
   ...
   RP::addComposing("io.system_write_t_interval", "Save the simulation every arg simulated seconds. Negative values disable writes. [Define for all groups.]");
   RP::addComposing("io.system_write_file_name", "Save the simulation to this file name series. [Define for all groups.]");
   RP::addComposing("io.system_write_path", "Save this series in this location. Default is ./ [Define for all groups or none.]");
   RP::addComposing("io.system_write_distribution_stride", "Every this many cells write out their velocity space. 0 is none. [Define for all groups.]");
   RP::addComposing(
   ...
   RP::addComposing("io.system_write_distribution_shell_stride", "Every this many cells for those on selected shells write out their velocity space. 0 is none.");
   RP::addComposing("io.system_write_fsgrid_variables", "If 0 don't write fsgrid DROs, if 1 do write them.");
   RP::addComposing("io.system_write_mpiio_hint_key", "MPI-IO hint key passed to the non-restart IO. Has to be matched by io.system_write_mpiio_hint_value.");
   RP::addComposing("io.system_write_mpiio_hint_value", "MPI-IO hint value passed to the non-restart IO. Has to be matched by io.system_write_mpiio_hint_key.");
   RP::addComposing("io.restart_write_mpiio_hint_key", "MPI-IO hint key passed to the restart IO. Has to be matched by io.restart_write_mpiio_hint_value.");
   RP::addComposing("io.restart_write_mpiio_hint_value", "MPI-IO hint value passed to the restart IO. Has to be matched by io.restart_write_mpiio_hint_key.");
   RP::addComposing("io.restart_read_mpiio_hint_key", "MPI-IO hint key passed to the restart IO. Has to be matched by io.restart_read_mpiio_hint_value.");
   RP::addComposing("io.restart_read_mpiio_hint_value", "MPI-IO hint value passed to the restart IO. Has to be matched by io.restart_read_mpiio_hint_key.");

   RP::add("io.write_initial_state", "Write initial state, not even the 0.5 dt propagation is done. Do not use for restarting. ", false);

   RP::add("io.write_full_bgb_data", "Write a dedicated file containing all BGB components and first derivatives, then exit.", false);

   RP::add("io.restart_walltime_interval", "Save the complete simulation in given walltime intervals. Negative values disable writes.", -1.0);
   RP::add("io.number_of_restarts", "Exit the simulation after certain number of walltime-based restarts.", numeric_limits<uint>::max());
   RP::add("io.recover_tstep_interval", "Save the complete simulation in given tstep intervals. 0 disables writes.", 0);
   RP::add("io.number_of_recovers", "Overwrite recovers cyclically after this number of recovers written.", 2);
   RP::add("io.vlsv_buffer_size", "Buffer size passed to VLSV writer (bytes, up to uint64_t), default 0 as this is sensible on sisu", 0);
   ...
   RP::add("restart.overrideReadFsGridDecompositionX", "Manual FsGridDecomposition for field solver grid stored in a restart file.", 0);
   RP::add("restart.overrideReadFsGridDecompositionY", "Manual FsGridDecomposition for field solver grid stored in a restart file.", 0);
   RP::add("restart.overrideReadFsGridDecompositionZ", "Manual FsGridDecomposition for field solver grid stored in a restart file.", 0);
   ...
   RP::add("gridbuilder.t_max", "Maximum simulation time, in seconds. If timestep_max limit is hit first this time will never be reached", LARGE_REAL);
   RP::add("gridbuilder.timestep_max", "Max. value for timesteps. If t_max limit is hit first, this step will never be reached", numeric_limits<uint>::max());
   RP::add("gridbuilder.dt_ceil", "Maximum simulation dt in seconds.", -1.0);

   // Field solver parameters
   RP::add("fieldsolver.maxWaveVelocity", "Maximum wave velocity allowed in the fastest velocity determination in m/s, default unlimited", LARGE_REAL);
   ...
   RP::add("fieldsolver.ohmHallTerm", "Enable/choose spatial order of the Hall term in Ohm's law. 0: off, 1: 1st spatial order, 2: 2nd spatial order", 0);
   RP::add("fieldsolver.ohmGradPeTerm", "Enable/choose spatial order of the electron pressure gradient term in Ohm's law. 0: off, 1: 1st spatial order.", 0);
   RP::add("fieldsolver.electronTemperature", "Upstream (anchor point) electron temperature to be used for the electron pressure gradient term (K).", 0.0);
   RP::add("fieldsolver.electronDensity", "Upstream (anchor point) electron density to be used for the electron pressure gradient term (m^-3).", 0.0);
   RP::add(
   ...
   RP::add("fieldsolver.maxCFL", "The maximum CFL limit for field propagation. Used to set timestep if dynamic_timestep is true.", 0.5);
   RP::add("fieldsolver.minCFL", "The minimum CFL limit for field propagation. Used to set timestep if dynamic_timestep is true.", 0.4);

   RP::add("fieldsolver.manualFsGridDecompositionX", "Manual FsGridDecomposition for field solver grid.", 0);
   RP::add("fieldsolver.manualFsGridDecompositionY", "Manual FsGridDecomposition for field solver grid.", 0);
   RP::add("fieldsolver.manualFsGridDecompositionZ", "Manual FsGridDecomposition for field solver grid.", 0);

   // Vlasov solver parameters
   RP::add("vlasovsolver.maxSlAccelerationRotation", "Maximum rotation angle (degrees) allowed by the Semi-Lagrangian solver (Use >25 values with care)", 25.0);
   ...
   RP::add("vlasovsolver.accelerateMaxwellianBoundaries", "Propagate maxwellian boundary cell contents in velocity space. Default false.", false);
   ...
   RP::addComposing(
      ...
      string() +
         ...
         "Available (20250413): " + "fg_b fg_b_background fg_b_perturbed fg_b_background_vol fg_derivs_b_background fg_e " + "vg_rhom vg_rhoq populations_vg_rho " + "fg_rhom fg_rhoq " +
         "vg_v fg_v populations_vg_v " + "populations_vg_moments_thermal populations_vg_moments_nonthermal " + "populations_vg_effectivesparsitythreshold populations_vg_rho_loss_adjust " +
         "populations_vg_energydensity populations_vg_precipitationdifferentialflux " + "populations_vg_heatflux " + "populations_vg_1dmuspace " + "populations_vg_nonmaxwellianity " +
         "vg_maxdt_acceleration vg_maxdt_translation populations_vg_maxdt_acceleration " + "populations_vg_maxdt_translation " + "fg_maxdt_fieldsolver " +
         "vg_rank fg_rank fg_amr_level vg_loadbalance_weight " + "vg_boundarytype fg_boundarytype vg_boundarylayer fg_boundarylayer " + "populations_vg_blocks vg_f_saved " +
         "populations_vg_acceleration_subcycles " + "vg_e_vol fg_e_vol " + "fg_e_hall vg_e_gradpe fg_b_vol vg_b_vol vg_b_background_vol vg_b_perturbed_vol " +
         ...
         "ig_electrontemp ig_solverinternals ig_upmappednodecoords ig_upmappedb ig_openclosed ig_potential " + "ig_precipitation ig_deltaphi " +
         "ig_inplanecurrent ig_b ig_e vg_drift vg_ionospherecoupling vg_connection vg_fluxrope fg_curvature " +
         "vg_amr_drho vg_amr_du vg_amr_dpsq vg_amr_dbsq vg_amr_db vg_amr_alpha1 vg_amr_reflevel vg_amr_alpha2 " + "vg_gridcoordinates fg_gridcoordinates vg_pressure_anisotropy vg_amr_vorticity"
   );

   RP::addComposing(
      "variables_deprecated.output",
      string() + "List of deprecated names for data reduction operators (DROs). Names are case insensitive. " + "Available (20250413): " + "B BackgroundB fg_BackgroundB PerturbedB fg_PerturbedB " +
         "E " + "Rhom Rhoq populations_Rho " + "V populations_V " + "populations_moments_Backstream populations_moments_NonBackstream " +
         ...
         "LBweight vg_lbweight vg_loadbalanceweight MaxVdt MaxRdt populations_MaxVdt populations_MaxRdt " + "populations_maxdt_acceleration populations_maxdt_translation MaxFieldsdt fg_maxfieldsdt" +
         "MPIrank FsGridRank " + "FsGridBoundaryType BoundaryType FsGridBoundaryLayer BoundaryLayer " + "populations_Blocks fSaved vg_fsaved" +
         "populations_accSubcycles populations_acceleration_subcycles" + "VolE vg_VolE Evol E_vol fg_VolE fg_Evol " +
         "HallE fg_HallE GradPeE e_gradpe VolB vg_VolB fg_VolB B_vol Bvol vg_Bvol fg_volB fg_Bvol " + "BackgroundVolB PerturbedVolB " + "Pressure vg_Pressure fg_Pressure populations_PTensor " +
   ...
   RP::addComposing(
      ...
      string() +
         ...
         "Available (20250130): " + "populations_vg_blocks " + "vg_rhom populations_vg_rho_loss_adjust " + "vg_loadbalance_weight " + "vg_maxdt_acceleration vg_maxdt_translation " +
         "fg_maxdt_fieldsolver " + "populations_vg_maxdt_acceleration populations_vg_maxdt_translation "
   );
   ...
   RP::addComposing(
      ...
      string() +
         ...
         "Available (20250130): " + "rhom populations_rholossadjust populations_rho_loss_adjust " + "populations_blocks lbweight loadbalance_weight " + "vg_lbweight vg_loadbalanceweight " +
         "maxvdt maxdt_acceleration " + "maxrdt maxdt_translation " + "populations_maxvdt populations_maxrdt " + "populations_maxdt_acceleration populations_maxdt_translation " +
         "maxfieldsdt maxdt_fieldsolver fg_maxfieldsdt"
   );

   // bailout parameters
   RP::add("bailout.write_restart", "If 1, write a restart file on bailout. Gets reset when sending a STOP (1) or a KILL (0).", true);
   RP::add("bailout.min_dt", "Minimum time step below which bailout occurs (s).", 1e-6);
   RP::add("bailout.max_memory", "Maximum amount of memory used per node (in GiB) over which bailout occurs.", 1073741824.);
   RP::add(
   ...
   RP::add(
      ...
      "How many boxes outside which to suppress refinement, that number of box edges have to then be defined as well. If more than 1 box is defined, refinement is suppressed outside the union of the "
   ...
   RP::add(
      ...
      "Maximum absolute distance in m to trace along the field line before ending. Defaults to the sum of the simulation box edge lengths LX+LY+LZ if set <= 0.",
      -1
   );
   RP::add(
      "fieldtracing.fullbox_max_incomplete_cells",
      "Maximum fraction of cells left incomplete when stopping tracing loop for full box tracing. Defaults to zero to process all, will be slow at scale! Both fluxrope_max_incomplete_cells and "
      "fullbox_max_incomplete_cells will be achieved.",
      0
   );
   RP::add(
      "fieldtracing.fluxrope_max_incomplete_cells",
      "Maximum fraction of cells left incomplete when stopping loop for flux rope tracing. Defaults to zero to process all, will be slow at scale! Both fluxrope_max_incomplete_cells and "
      "fullbox_max_incomplete_cells will be achieved.",
      0
   );
...
void Parameters::getParameters() {
   ...
   if (access(&(P::recoverWritePath[0]), W_OK) != 0) {
      if (myRank == MASTER_RANK) {
         cerr << "ERROR recover write path " << P::recoverWritePath << " not writeable, defaulting to local directory." << endl;
      }
   ...
   for (uint i = 0; i < maxSize; i++) {
      if (P::systemWriteDistributionWriteStride[i] != 0 || P::systemWriteDistributionWriteXlineStride[i] > 0 || P::systemWriteDistributionWriteYlineStride[i] > 0 ||
   ...
   if (P::amrBoxNumber != (int)P::amrBoxHalfWidthX.size() || P::amrBoxNumber != (int)P::amrBoxHalfWidthY.size() || P::amrBoxNumber != (int)P::amrBoxHalfWidthZ.size() ||
       P::amrBoxNumber != (int)P::amrBoxCenterX.size() || P::amrBoxNumber != (int)P::amrBoxCenterY.size() || P::amrBoxNumber != (int)P::amrBoxCenterZ.size() ||
   ...
   if (P::refineBoxNumber != (int)P::refinementMinX.size() || P::refineBoxNumber != (int)P::refinementMinY.size() || P::refineBoxNumber != (int)P::refinementMinZ.size() ||
       P::refineBoxNumber != (int)P::refinementMaxX.size() || P::refineBoxNumber != (int)P::refinementMaxY.size() || P::refineBoxNumber != (int)P::refinementMaxZ.size()) {
