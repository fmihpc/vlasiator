...
Parameters::Parameters(int argc, char* argv[]) {
   ...
   ("run_config", boost::program_options::value<std::string>(&run_config_file_name)->default_value(""), "read options from the run's configuration file arg (relative to the current working directory). Options given in this file override options given in the user's and global configuration files. Options given in this file are overridden by options given in environment variables (prefixed with MAIN_) and the command line")
    ...
    ("q", boost::program_options::value<Real>(&P::q)->default_value(1.0), "Charge (C) of simulated particle species")
    ("m", boost::program_options::value<Real>(&P::m)->default_value(1.0), "Mass (kg) of simulated particle species")
...
bool Parameters::add(const string& name,const string& desc,std::string& var,const std::string& defValue) {
   if (initialized == false) return false;
...
bool Parameters::parse() {
   ...
   if (run_config_file_name.size() > 0) {
	 ...
	 PO::store(PO::parse_config_file(run_config_file, *descriptions), *variables);
	 PO::notify(*variables);
      }
      run_config_file.close();
   }
   ...
   if (user_config_file_name.size() > 0) {
	 ...
	 PO::store(PO::parse_config_file(user_config_file, *descriptions), *variables);
	 PO::notify(*variables);
      }
      user_config_file.close();
   }
   ...
   if (global_config_file_name.size() > 0) {
	 ...
	 PO::store(PO::parse_config_file(global_config_file, *descriptions), *variables);
	 PO::notify(*variables);
      }
      global_config_file.close();
   }
