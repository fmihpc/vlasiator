...
namespace NbrsSpa {
const uint INNER =
    0; /*!< The cell is an inner cell, i.e. all its neighbours are located on the same computation node.*/
...
enum {
   STATE, /*!< Contains the neighbour information of this cell, i.e. whether it is an inner cell or a boundary cell in
             one or more coordinate directions.*/
...
enum {
   ...
   ISCELLSAVINGF,   /*!< Value telling whether a cell is saving its distribution function when partial f data is written
                       out. */
...
enum ionosphereParameters {
   ...
   SIGMA21,
   SIGMA22,
   SIGMA23, /*!< Overall conductivity tensor */
   SIGMA31,
   SIGMA32,
   SIGMA33,
   ...
   NODE_BX,
   NODE_BY,
   NODE_BZ, /*!< Magnetic field at the node */
   UPMAPPED_BX,
   UPMAPPED_BY,
   UPMAPPED_BZ,   /*!< Magnetic field at the upper and of the mapping fieldline */
 ...
 * it is stored in SpatialCell::sysBoundaryFlag and used by the BoundaryCondition class' functions to determine what
 * type of BC to apply to a cell. At least for the workings of vlasovmover_leveque.cpp the order of the first two
...
enum {
   RK_ORDER1,       /*!< First order method, one step (and initialisation) */
   RK_ORDER2_STEP1, /*!< Two-step second order method, first step */
   RK_ORDER2_STEP2  /*!< Two-step second order method, second step */
};

const int WID = 4; /*!< Number of cells per coordinate in a velocity block. Only a value of 4 supported by vectorized
                      Leveque solver */
 ...
 */
struct globalflags {
   static int bailingOut; /*!< Global flag raised to true if a run bailout (write restart if requested/set and stop the
                             simulation peacefully) is needed. */
   static bool writeRestart;   /*!< Global flag raised to true if a restart writing is needed (without bailout). NOTE:
                                  used only by MASTER_RANK in vlasiator.cpp. */
   static bool balanceLoad;    /*!< Global flag raised to true if a load balancing is needed. NOTE: used only by
                                  MASTER_RANK in vlasiator.cpp. */
   static int AMRstencilWidth; /*!< Global variable used for the extended AMR stencil width */
   static bool ionosphereJustSolved; /*!< Flag used to notify that the ionosphere has been freshly solved, used to check
                                        whether the Vlasov boundary/bulk forcing need updating. */
};
