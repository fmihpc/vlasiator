...
class DeviceGrid {
 public:
   ...
   /** Query if the DeviceGrid is initialized. 
 ...
 private:
   ...
   void* array;                            /**< Pointer to array in GPU memory.*/
 ...
 */
template<typename T> T* DeviceGrid::getArray() const {
 ...
 * @return Pointer to an array on device.
 */
template<typename T> T* DeviceGrid::getArray(cuint& offset) const {
   T* ptr = reinterpret_cast<T*>(array);
   return ptr + offset;
}
