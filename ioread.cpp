...
bool readBlockData(
   ...
   for (uint popID=0; popID<getObjectWrapper().particleSpecies.size(); ++popID) {
      ...
      const vmesh::MeshParameters& ourMeshParams = vmesh::getMeshWrapper()->at(meshID);
      if(fileMeshBBox[0] != ourMeshParams.gridLength[0] ||
         ...
         const Real dVx = vmesh::getMeshWrapper()->at(meshID).getCellDx(0);
         for(const auto& c : fileVelCoordsX) {
            Real cellindex = (c - vmesh::getMeshWrapper()->at(meshID).meshMinLimits[0]) / dVx;
            if(fabs(nearbyint(cellindex) - cellindex) > 1./10000.) {
               logFile << "(RESTART) ERROR: Can't resize velocity space as cell coordinates don't match." << endl
                  << "          (X coordinate " << c << " = " << cellindex <<" * " << dVx << " + " << vmesh::getMeshWrapper()->at(meshID).meshMinLimits[0] << endl
         ...
         for(const auto& c : fileVelCoordsY) {
            ...
            if(fabs(nearbyint(cellindex) - cellindex) > 1./10000.) {
               logFile << "(RESTART) ERROR: Can't resize velocity space as cell coordinates don't match." << endl
                  << "           (Y coordinate " << c << " = " << cellindex <<" * " << dVy << " + " << vmesh::getMeshWrapper()->at(meshID).meshMinLimits[1] << endl
                  << "           coordinate  = cellindex *   dV  +  meshMinLimits)" << endl << write;
               abort();
            }
         }

         const Real dVz = vmesh::getMeshWrapper()->at(meshID).getCellDx(2);
         for(const auto& c : fileVelCoordsY) {
            Real cellindex = (c - vmesh::getMeshWrapper()->at(meshID).meshMinLimits[2]) / dVz;
            if(fabs(nearbyint(cellindex) - cellindex) > 1./10000.) {
               logFile << "(RESTART) ERROR: Can't resize velocity space as cell coordinates don't match." << endl
                  << "           (Z coordinate " << c << " = " << cellindex <<" * " << dVz << " + " << vmesh::getMeshWrapper()->at(meshID).meshMinLimits[2] << endl
                  << "           coordinate  = cellindex *   dV  +  meshMinLimits)" << endl << write;
               abort();
            }
         }
         ...
         velGridOffset[0] = (fileVelCoordsX[0] - vmesh::getMeshWrapper()->at(meshID).meshMinLimits[0]) / dVx;
         velGridOffset[1] = (fileVelCoordsY[0] - vmesh::getMeshWrapper()->at(meshID).meshMinLimits[1]) / dVy;
         velGridOffset[2] = (fileVelCoordsZ[0] - vmesh::getMeshWrapper()->at(meshID).meshMinLimits[2]) / dVz;
