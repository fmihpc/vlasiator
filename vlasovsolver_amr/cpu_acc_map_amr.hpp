...
bool map_1d(SpatialCell* spatial_cell,
   ...
   for (vmesh::LocalID block = 0; block < vmesh.size(); ++block) {
      uint8_t refLevel =
          vmesh::VelocityMesh<vmesh::GlobalID, vmesh::LocalID>::getRefinementLevel(vmesh.getGlobalID(block));
   ...
   case 2:
      ...
      break;
   default:
      std::cerr << "error in map1d" << std::endl;
      exit(1);
      break;
   }
   ...
   propagParams.k_cell_global_target_max =
       SpatialCell::get_velocity_grid_length(propagParams.maxRefLevel)[dimension] * WID;
   switch (dimension) {
   case 0: {
      ...
      for (int k = 0; k < 2; ++k)
         for (int j = 0; j < 2; ++j)
            for (int i = 0; i < 2; ++i) {
               ...
               Realf intersection_min = params.intersection + (sourceIndex[0] + i_cell) * params.intersection_di +
               ...
               vmesh::GlobalID lagrangian_gk_top =
                   static_cast<vmesh::GlobalID>((v_top - intersection_min) / params.intersection_dk);
               if (lagrangian_gk_top >= params.k_cell_global_target_max)
                  continue;
               if (lagrangian_gk_top < k_trgt_min)
                  k_trgt_min = lagrangian_gk_top;
               if (lagrangian_gk_top > k_trgt_max)
                  k_trgt_max = lagrangian_gk_top;
            }
         ...
         for (int j = 0; j < WID; ++j)
            for (int i = 0; i < WID; ++i) {
               ...
               Real intersection_min =
                   params.intersection + i_cell_trgt * params.intersection_di + j_cell_trgt * params.intersection_dj;
               ...
               // the correct value.
               vmesh::GlobalID k_cell_src_top =
                   std::min(params.Nx * WID - 1, static_cast<vmesh::GlobalID>(v_src_top_l));
            ...
            while (k_cell_src <= k_cell_src_max) {
               ...
               for (int j = 0; j < WID; ++j)
                  for (int i = 0; i < WID; ++i) {
                     ...
                     Real v_top = std::min(v_src_tops[j * WID + i],
                                           (Real)(k_block_src * (WID * srcRefMul) + (k_cell_bot + 1) * srcRefMul));
                     Real v_bot = std::max(v_src_bots[j * WID + i],
                                           (Real)(k_block_src * (WID * srcRefMul) + (k_cell_bot)*srcRefMul));
                     ...
                     int srcCellIndex[3];
                     // srcCellIndex[params.i_mapped] = (srcIndex[params.i_mapped] - i_block_src*(WID*srcRefMul)) /
                     // srcRefMul; srcCellIndex[params.j_mapped] = (srcIndex[params.j_mapped] -
                     // j_block_src*(WID*srcRefMul)) / srcRefMul; srcCellIndex[params.k_mapped] = k_cell_bot;
                     srcCellIndex[params.i_mapped] =
                         PAD + (srcIndex[params.i_mapped] - i_block_src * (WID * srcRefMul)) / srcRefMul;
                     srcCellIndex[params.j_mapped] =
                         PAD + (srcIndex[params.j_mapped] - j_block_src * (WID * srcRefMul)) / srcRefMul;
                     ...
                     // data_trgt[trgtCell] += data_src[vblock::index(srcCellIndex[0],srcCellIndex[1],srcCellIndex[2])]
                     // * (v_int_max-v_int_min) * factor;
                     data_trgt[trgtCell] +=
                         array1[vblock::padIndex<PAD>(srcCellIndex[0], srcCellIndex[1], srcCellIndex[2])] *
                         (v_int_max - v_int_min) * factor;

                     // accum += data_src[vblock::index(srcCellIndex[0],srcCellIndex[1],srcCellIndex[2])] *
                     // (v_int_max-v_int_min) * factor;
                     accum += array1[vblock::padIndex<PAD>(srcCellIndex[0], srcCellIndex[1], srcCellIndex[2])] *
                              (v_int_max - v_int_min) * factor;
                  }
...
#warning DEPRECATED use per-species sparseMinValue
   ...
   for (size_t b = 0; b < removeList.size(); ++b)
      spatial_cell->remove_velocity_block(removeList[b]);
}
