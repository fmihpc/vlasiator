...
/**
 * Computes the first intersection data; this is z~ in section 2.4 in Zerroukat et al (2012). We assume all velocity
 * cells have the same dimensions. Intersection z coordinate for (i,j,k) is: intersection + i * intersection_di + j *
 * intersection_dj + k * intersection_dk
 ...
 * @param bwd_transform Transform that describes acceleration backward in time, used to compute the lagrangian departure
 * grid
 * @param dimension Along which dimension is this intersection/mapping computation done. It is assumed the three
 * mappings are in order 012, 120 or 201
...
void compute_intersections_1st(const SpatialCell* spatial_cell,
   ...
   if (dimension == 0) { // Prepare intersections for mapping along X first (mapping order X-Y-Z)
      ...
      const Eigen::Matrix<Real, 3, 1> plane_point =
          bwd_transform * Eigen::Matrix<Real, 3, 1>(SpatialCell::get_velocity_grid_min_limits()[0], 0.0, 0.0);
      ...
      const Eigen::Matrix<Real, 3, 1> lagrangian_di =
          bwd_transform.linear() *
          Eigen::Matrix<Real, 3, 1>(SpatialCell::get_velocity_grid_cell_size(refLevel)[0], 0, 0.0);
      const Eigen::Matrix<Real, 3, 1> euclidian_dj =
          Eigen::Matrix<Real, 3, 1>(0, SpatialCell::get_velocity_grid_cell_size(refLevel)[1], 0.0);
      const Eigen::Matrix<Real, 3, 1> euclidian_dk =
          Eigen::Matrix<Real, 3, 1>(0.0, 0.0, SpatialCell::get_velocity_grid_cell_size(refLevel)[2]);

      // compute intersections, varying lines and plane in i,j,k
      const Eigen::Matrix<Real, 3, 1> intersection_0_0_0 =
          line_plane_intersection(line_point, line_direction, plane_point, plane_normal);
      const Eigen::Matrix<Real, 3, 1> intersection_1_0_0 =
          line_plane_intersection(line_point, line_direction, plane_point + lagrangian_di, plane_normal);
      const Eigen::Matrix<Real, 3, 1> intersection_0_1_0 =
          line_plane_intersection(line_point + euclidian_dj, line_direction, plane_point, plane_normal);
      const Eigen::Matrix<Real, 3, 1> intersection_0_0_1 =
      ...
      // Point on lowest potential lagrangian plane
      const Eigen::Matrix<Real, 3, 1> plane_point =
          bwd_transform * Eigen::Matrix<Real, 3, 1>(0.0, SpatialCell::get_velocity_grid_min_limits()[1], 0.0);
      // line along euclidian y direction, unit vector
      const Eigen::Matrix<Real, 3, 1> line_direction = Eigen::Matrix<Real, 3, 1>(0.0, 1.0, 0.0);

      const Eigen::Matrix<Real, 3, 1> line_point(
          0.5 * SpatialCell::get_velocity_grid_cell_size(refLevel)[0] + SpatialCell::get_velocity_grid_min_limits()[0],
          0.0,
          0.5 * SpatialCell::get_velocity_grid_cell_size(refLevel)[2] + SpatialCell::get_velocity_grid_min_limits()[2]);

      const Eigen::Matrix<Real, 3, 1> euclidian_di =
          Eigen::Matrix<Real, 3, 1>(SpatialCell::get_velocity_grid_cell_size(refLevel)[0], 0.0, 0.0);
      const Eigen::Matrix<Real, 3, 1> lagrangian_dj =
          bwd_transform.linear() *
          Eigen::Matrix<Real, 3, 1>(0.0, SpatialCell::get_velocity_grid_cell_size(refLevel)[1], 0.0);
      const Eigen::Matrix<Real, 3, 1> euclidian_dk =
          Eigen::Matrix<Real, 3, 1>(0.0, 0.0, SpatialCell::get_velocity_grid_cell_size(refLevel)[2]);
      ...
      const Eigen::Matrix<Real, 3, 1> plane_normal =
          bwd_transform.linear() * Eigen::Matrix<Real, 3, 1>(0, 0, 1.0); // Normal of lagrangian planes
      const Eigen::Matrix<Real, 3, 1> plane_point =
          bwd_transform *
...
/*!
  Computes the second intersection data; this is x~ in section 2.4 in Zerroukat et al (2012). We assume all velocity
  cells have the same dimensions. Intersection x coordinate for (i,j,k) is: intersection + i * intersection_di + j *
  intersection_dj + k * intersection_dk \param spatial_cell spatial cell that is accelerated \param fwd_transform
  Transform that describes acceleration forward in time \param bwd_transform Transform that describes acceleration
  backward in time, used to compute the lagrangian departure grid \param dimension Along which dimension is this
  intersection/mapping computation done. It is assumed the three mappings are in order 012, 120 or 201 \param
  intersection Intersection x-coordinate at i,j,k=0 \param intersection_di Change in x-coordinate for a change in i
  index of 1 \param intersection_dj Change in x-coordinate for a change in j index of 1 \param intersection_dk Change in
...
void compute_intersections_2nd(const SpatialCell* spatial_cell,
   ...
   if (dimension == 0) {
      ...
      const Eigen::Matrix<Real, 3, 1> plane_normal =
          Eigen::Matrix<Real, 3, 1>(0.0, 1.0, 0.0); // Normal of Euclidian y-plane
      Eigen::Matrix<Real, 3, 1> plane_point = Eigen::Matrix<Real, 3, 1>(
          0,
          SpatialCell::get_velocity_grid_min_limits()[1] + SpatialCell::get_velocity_grid_cell_size(refLevel)[1] * 0.5,
          0); // Point on lowest euclidian y-plane through middle of cells
      const Eigen::Matrix<Real, 3, 1> lagrangian_di =
          bwd_transform.linear() *
          Eigen::Matrix<Real, 3, 1>(SpatialCell::get_velocity_grid_cell_size(refLevel)[0], 0, 0.0);
      const Eigen::Matrix<Real, 3, 1> euclidian_dj = Eigen::Matrix<Real, 3, 1>(
          0, SpatialCell::get_velocity_grid_cell_size(refLevel)[1], 0.0); // Distance between euclidian planes
      const Eigen::Matrix<Real, 3, 1> lagrangian_dk =
          bwd_transform.linear() *
          Eigen::Matrix<Real, 3, 1>(0.0, 0.0, SpatialCell::get_velocity_grid_cell_size(refLevel)[2]);
      ...
      const Eigen::Matrix<Real, 3, 1> line_direction =
          bwd_transform.linear() *
          Eigen::Matrix<Real, 3, 1>(
              0, 1.0, 0.0); // line along lagrangian y line, unit vector. Only rotation here, not translation
      const Eigen::Matrix<Real, 3, 1> line_point =
          bwd_transform * Eigen::Matrix<Real, 3, 1>(SpatialCell::get_velocity_grid_min_limits()[0],
                                                    0.5 * SpatialCell::get_velocity_grid_cell_size(refLevel)[1] +
                                                        SpatialCell::get_velocity_grid_min_limits()[1],
                                                    0.5 * SpatialCell::get_velocity_grid_cell_size(refLevel)[2] +
                                                        SpatialCell::get_velocity_grid_min_limits()[2]);
      /*Compute two intersections between lagrangian line (absolute position does not matter so set to 0,0,0, and two
       * euclidian planes*/
      Eigen::Matrix<Real, 3, 1> intersect_0_0_0 =
          line_plane_intersection(line_point, line_direction, plane_point, plane_normal);
      Eigen::Matrix<Real, 3, 1> intersect_1_0_0 =
          line_plane_intersection(line_point + lagrangian_di, line_direction, plane_point, plane_normal);
      Eigen::Matrix<Real, 3, 1> intersect_0_1_0 =
          line_plane_intersection(line_point, line_direction, plane_point + euclidian_dj, plane_normal);
      Eigen::Matrix<Real, 3, 1> intersect_0_0_1 =
   ...
   if (dimension == 1) {
      ...
      const Eigen::Matrix<Real, 3, 1> lagrangian_dj =
          bwd_transform.linear() *
          Eigen::Matrix<Real, 3, 1>(0.0, SpatialCell::get_velocity_grid_cell_size(refLevel)[1], 0.0);
      const Eigen::Matrix<Real, 3, 1> euclidian_dk = Eigen::Matrix<Real, 3, 1>(
          0.0, 0.0, SpatialCell::get_velocity_grid_cell_size(refLevel)[2]); // Distance between euclidian planes

      const Eigen::Matrix<Real, 3, 1> line_direction =
          bwd_transform.linear() *
          Eigen::Matrix<Real, 3, 1>(
              0.0, 0.0, 1.0); // line along lagrangian z line, unit vector. Only rotation here, not translation
      const Eigen::Matrix<Real, 3, 1> line_point =
          bwd_transform * Eigen::Matrix<Real, 3, 1>(0.5 * SpatialCell::get_velocity_grid_cell_size(refLevel)[0] +
                                                        SpatialCell::get_velocity_grid_min_limits()[0],
                                                    SpatialCell::get_velocity_grid_min_limits()[1],
                                                    0.5 * SpatialCell::get_velocity_grid_cell_size(refLevel)[2] +
                                                        SpatialCell::get_velocity_grid_min_limits()[2]);
      /*Compute two intersections between lagrangian line (absolute position does not matter so set to 0,0,0, and two
   ...
   if (dimension == 2) {
      ...
      const Eigen::Matrix<Real, 3, 1> line_direction =
          bwd_transform.linear() *
          Eigen::Matrix<Real, 3, 1>(
              1.0, 0.0, 0.0); // line along lagrangian x line, unit vector. Only rotation here, not translation
      const Eigen::Matrix<Real, 3, 1> line_point =
          bwd_transform * Eigen::Matrix<Real, 3, 1>(0.5 * SpatialCell::get_velocity_grid_cell_size(refLevel)[0] +
                                                        SpatialCell::get_velocity_grid_min_limits()[0],
                                                    0.5 * SpatialCell::get_velocity_grid_cell_size(refLevel)[1] +
...
/*!
  Computes the third intersection data; this is y intersesctions in Zerroukat et al (2012). We assume all velocity cells
  have the same dimensions. Intersection y-coordinate for (i,j,k) is: intersection + i * intersection_di + j *
  intersection_dj + k * intersection_dk \param spatial_cell spatial cell that is accelerated \param fwd_transform
  Transform that describes acceleration forward in time \param bwd_transform Transform that describes acceleration
  backward in time, used to compute the lagrangian departure grid \param dimension Along which dimension is this
  intersection/mapping computation done. It is assumed the three mappings are in order 012, 120 or 201 \param
  intersection Intersection y-coordinate at i,j,k=0 \param intersection_di Change in y-coordinate for a change in i
  index of 1 \param intersection_dj Change in y-coordinate for a change in j index of 1 \param intersection_dk Change in
  y-coordinate for a change in k index of 1
...
void compute_intersections_3rd(const SpatialCell* spatial_cell,
   ...
   if (dimension == 0) {
      ...
      const Eigen::Matrix<Real, 3, 1> point_0_0_0 =
          bwd_transform * Eigen::Matrix<Real, 3, 1>(0.0 * SpatialCell::get_velocity_grid_cell_size(refLevel)[0] +
                                                        SpatialCell::get_velocity_grid_min_limits()[0],
                                                    0.5 * SpatialCell::get_velocity_grid_cell_size(refLevel)[1] +
                                                        SpatialCell::get_velocity_grid_min_limits()[1],
                                                    0.5 * SpatialCell::get_velocity_grid_cell_size(refLevel)[2] +
      ...
      const Eigen::Matrix<Real, 3, 1> point_1_0_0 =
          bwd_transform * Eigen::Matrix<Real, 3, 1>(1.0 * SpatialCell::get_velocity_grid_cell_size(refLevel)[0] +
                                                        SpatialCell::get_velocity_grid_min_limits()[0],
                                                    0.5 * SpatialCell::get_velocity_grid_cell_size(refLevel)[1] +
                                                        SpatialCell::get_velocity_grid_min_limits()[1],
                                                    0.5 * SpatialCell::get_velocity_grid_cell_size(refLevel)[2] +
                                                        SpatialCell::get_velocity_grid_min_limits()[2]);
      const Eigen::Matrix<Real, 3, 1> point_0_1_0 =
          bwd_transform * Eigen::Matrix<Real, 3, 1>(0.0 * SpatialCell::get_velocity_grid_cell_size(refLevel)[0] +
                                                        SpatialCell::get_velocity_grid_min_limits()[0],
                                                    1.5 * SpatialCell::get_velocity_grid_cell_size(refLevel)[1] +
                                                        SpatialCell::get_velocity_grid_min_limits()[1],
                                                    0.5 * SpatialCell::get_velocity_grid_cell_size(refLevel)[2] +
                                                        SpatialCell::get_velocity_grid_min_limits()[2]);
      const Eigen::Matrix<Real, 3, 1> point_0_0_1 =
          bwd_transform * Eigen::Matrix<Real, 3, 1>(0.0 * SpatialCell::get_velocity_grid_cell_size(refLevel)[0] +
                                                    ...
                                                    0.5 * SpatialCell::get_velocity_grid_cell_size(refLevel)[1] +
                                                        SpatialCell::get_velocity_grid_min_limits()[1],
                                                    1.5 * SpatialCell::get_velocity_grid_cell_size(refLevel)[2] +
                                                        SpatialCell::get_velocity_grid_min_limits()[2]);
   ...
   if (dimension == 1) {
      ...
      const Eigen::Matrix<Real, 3, 1> point_0_0_0 =
          bwd_transform * Eigen::Matrix<Real, 3, 1>(0.5 * SpatialCell::get_velocity_grid_cell_size(refLevel)[0] +
                                                    ...
                                                    0.5 * SpatialCell::get_velocity_grid_cell_size(refLevel)[2] +
                                                        SpatialCell::get_velocity_grid_min_limits()[2]);
      const Eigen::Matrix<Real, 3, 1> point_1_0_0 =
          bwd_transform * Eigen::Matrix<Real, 3, 1>(1.5 * SpatialCell::get_velocity_grid_cell_size(refLevel)[0] +
                                                        SpatialCell::get_velocity_grid_min_limits()[0],
                                                    SpatialCell::get_velocity_grid_min_limits()[1],
                                                    0.5 * SpatialCell::get_velocity_grid_cell_size(refLevel)[2] +
                                                        SpatialCell::get_velocity_grid_min_limits()[2]);
      const Eigen::Matrix<Real, 3, 1> point_0_1_0 =
          bwd_transform * Eigen::Matrix<Real, 3, 1>(0.5 * SpatialCell::get_velocity_grid_cell_size(refLevel)[0] +
                                                        SpatialCell::get_velocity_grid_min_limits()[0],
                                                    1.0 * SpatialCell::get_velocity_grid_cell_size(refLevel)[1] +
                                                        SpatialCell::get_velocity_grid_min_limits()[1],
                                                    0.5 * SpatialCell::get_velocity_grid_cell_size(refLevel)[2] +
                                                        SpatialCell::get_velocity_grid_min_limits()[2]);
      const Eigen::Matrix<Real, 3, 1> point_0_0_1 =
   ...
   if (dimension == 2) {
      ...
      const Eigen::Matrix<Real, 3, 1> point_0_0_0 =
          bwd_transform * Eigen::Matrix<Real, 3, 1>(0.5 * SpatialCell::get_velocity_grid_cell_size(refLevel)[0] +
                                                    ...
                                                    0.5 * SpatialCell::get_velocity_grid_cell_size(refLevel)[1] +
                                                        SpatialCell::get_velocity_grid_min_limits()[1],
                                                    0.0 * SpatialCell::get_velocity_grid_cell_size(refLevel)[2] +
                                                        SpatialCell::get_velocity_grid_min_limits()[2]);
      const Eigen::Matrix<Real, 3, 1> point_1_0_0 =
          bwd_transform * Eigen::Matrix<Real, 3, 1>(1.5 * SpatialCell::get_velocity_grid_cell_size(refLevel)[0] +
                                                        SpatialCell::get_velocity_grid_min_limits()[0],
      ...
      const Eigen::Matrix<Real, 3, 1> point_0_0_1 =
          bwd_transform * Eigen::Matrix<Real, 3, 1>(0.5 * SpatialCell::get_velocity_grid_cell_size(refLevel)[0] +
                                                    ...
                                                    0.5 * SpatialCell::get_velocity_grid_cell_size(refLevel)[1] +
                                                        SpatialCell::get_velocity_grid_min_limits()[1],
                                                    1.0 * SpatialCell::get_velocity_grid_cell_size(refLevel)[2] +
                                                        SpatialCell::get_velocity_grid_min_limits()[2]);
      intersection = point_0_0_0[dimension];
      intersection_di = point_1_0_0[dimension] - point_0_0_0[dimension];
      intersection_dj = point_0_1_0[dimension] - point_0_0_0[dimension];
