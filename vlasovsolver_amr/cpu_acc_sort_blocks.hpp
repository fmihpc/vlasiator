...
inline bool paircomparator(const pair<uint, uint>& l, const pair<uint, uint>& r) { return l.first < r.first; }
...
static void sort_blocklist_by_dimension(const SpatialCell* spatial_cell,
   ...
   for (vmesh::LocalID i = 0; i < nBlocks; ++i) {
      ...
      case 0: {
         const vmesh::GlobalID blockId_mapped =
             block; // Mapping the block id to different coordinate system if dimension is not zero:
      ...
      case 1: {
         ...
         const uint y_indice = (block / SpatialCell::get_velocity_grid_length()[0]) %
                               SpatialCell::SpatialCell::get_velocity_grid_length()[1];
         // Mapping the block id to different coordinate system if dimension is not zero:
         const uint blockId_mapped = block - (x_indice + y_indice * SpatialCell::get_velocity_grid_length()[0]) +
                                     y_indice + x_indice * SpatialCell::SpatialCell::get_velocity_grid_length()[1];
         block_pairs[i] = make_pair(blockId_mapped, block);
      } break;
      case 2: {
         // Do operation:
         ...
         const uint y_indice = (block / SpatialCell::get_velocity_grid_length()[0]) %
                               SpatialCell::SpatialCell::get_velocity_grid_length()[1];
         const uint z_indice = (block / (SpatialCell::get_velocity_grid_length()[0] *
                                         SpatialCell::SpatialCell::get_velocity_grid_length()[1]));
         // Mapping the block id to different coordinate system if dimension is not zero:
         const uint blockId_mapped = z_indice + y_indice * SpatialCell::SpatialCell::get_velocity_grid_length()[2] +
                                     x_indice * SpatialCell::SpatialCell::get_velocity_grid_length()[1] *
                                         SpatialCell::SpatialCell::get_velocity_grid_length()[2];
         block_pairs[i] = make_pair(blockId_mapped, block);
      } break;
      }
   }
   ...
   for (vmesh::LocalID i = 0; i < nBlocks; ++i) {
      ...
      if (i > 0 && (column_id != prev_column_id || dimension_id != (prev_dimension_id + 1))) {
         ...
         block_column_lengths.push_back(block_column_offsets[block_column_offsets.size() - 1] -
                                        block_column_offsets[block_column_offsets.size() - 2]);
      }
