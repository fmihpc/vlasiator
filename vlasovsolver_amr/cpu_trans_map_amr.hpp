...
// Is cell translated? It is not translated if DO_NO_COMPUTE or if it is sysboundary cell and not in first
// sysboundarylayer
...
CellID get_spatial_neighbor(const dccrg::Dccrg<SpatialCell, dccrg::Cartesian_Geometry>& mpiGrid, const CellID& cellID,
   ...
   if (nbrID == dccrg::error_cell) {
      std::cerr << __FILE__ << ":" << __LINE__ << " No neighbor for cell?" << cellID << " at offsets " << spatial_di
                << ", " << spatial_dj << ", " << spatial_dk << std::endl;
   ...
   if (include_first_boundary_layer && mpiGrid[nbrID]->sysBoundaryFlag != sysboundarytype::NOT_SYSBOUNDARY &&
...
inline void addUpstreamBlocks(dccrg::Dccrg<SpatialCell, dccrg::Cartesian_Geometry>& mpiGrid, CellID nbrID, int dim,
   ...
   for (vmesh::LocalID blockLID = 0; blockLID < cellNbr->get_number_of_velocity_blocks(); ++blockLID) {
      ...
      if (blockParams == NULL) {
         std::cerr << "ERROR, cell " << cellID << " got NULL blockParams in " << __FILE__ << ' ' << __LINE__
                   << std::endl;
      ...
      default:
         std::cerr << "ERROR, invalid DIR in " << __FILE__ << ' ' << __LINE__ << std::endl;
         exit(1);
         break;
      }
...
void createTargetMesh(dccrg::Dccrg<SpatialCell, dccrg::Cartesian_Geometry>& mpiGrid, CellID cellID, int dim,
   ...
   if (isRemoteCell == true) {
      if (mpiGrid.is_local(cells[0]) == false)
         cells[0] = INVALID_CELLID;
      if (mpiGrid.is_local(cells[2]) == false)
         cells[2] = INVALID_CELLID;
   }
...
bool trans_map_1d(const dccrg::Dccrg<SpatialCell, dccrg::Cartesian_Geometry>& mpiGrid, const CellID cellID,
   ...
   for (int i = 0; i < 3; ++i)
      targetCells[i] = mpiGrid[targetCellIDs[i]];
   ...
   for (vmesh::LocalID blockLID = 0; blockLID < vmesh.size(); ++blockLID) {
      ...
      if (targetBlocks[0].size() > 1)
         reconstruct = true;
      if (targetBlocks[1].size() > 1)
         reconstruct = true;
      if (targetBlocks[2].size() > 1)
         reconstruct = true;
         ...
         for (int k = 0; k < WID; ++k)
            for (int j = 0; j < WID; ++j)
               for (int i = 0; i < WID; ++i) {
                  ...
                  /*
                                Real V_bot = (blockParams[dimension] + 0.25*blockParams[BlockParams::DVX+dimension])*dt
                     / DZ; if (V_bot < 0) depositToNeighbor(targetBlocks[0],avgsRec,-V_bot,i,j,k); else
                     depositToNeighbor(targetBlocks[2],avgsRec, V_bot,i,j,k);

                                Real V_top = (blockParams[dimension] + 0.75*blockParams[BlockParams::DVX+dimension])*dt
                     / DZ; if (V_top < 0) depositToNeighbor(targetBlocks[0],avgsRec,-V_top,i,j,k); else
                     depositToNeighbor(targetBlocks[2],avgsRec, V_top,i,j,k);
...
*/
/*
void update_remote_mapping_contribution(dccrg::Dccrg<SpatialCell,dccrg::Cartesian_Geometry>& mpiGrid, const uint
dimension, int direction) { const vector<CellID>& local_cells = getLocalCells(); const vector<CellID> remote_cells =
mpiGrid.get_remote_cells_on_process_boundary(VLASOV_SOLVER_NEIGHBORHOOD_ID); vector<CellID> receive_cells;
   ...
   for (size_t c=0; c<local_cells.size(); ++c) {
      ...
      switch(dimension) {
       case 0:
         p_ngbr=get_spatial_neighbor(mpiGrid, local_cells[c], false,  direction, 0, 0); //p_ngbr is target, if in
boundaries then it is not updated m_ngbr=get_spatial_neighbor(mpiGrid, local_cells[c], true, -direction, 0, 0); //m_ngbr
is source, first boundary layer is propagated so that it flows into system break; case 1:
         p_ngbr=get_spatial_neighbor(mpiGrid, local_cells[c], false, 0, direction, 0); //p_ngbr is target, if in
boundaries then it is not update m_ngbr=get_spatial_neighbor(mpiGrid, local_cells[c], true, 0, -direction, 0); //m_ngbr
is source, first boundary layer is propagated so that it flows into system break; case 2:
         p_ngbr=get_spatial_neighbor(mpiGrid, local_cells[c], false, 0, 0, direction); //p_ngbr is target, if in
boundaries then it is not update m_ngbr=get_spatial_neighbor(mpiGrid, local_cells[c], true,  0, 0, -direction); //m_ngbr
is source, first boundary layer is propagated so that it flows into system break; default: cerr << "Dimension wrong at
(impossible!) "<< __FILE__ <<":" << __LINE__<<endl; exit(1);
      }
      ...
      if (m_ngbr != INVALID_CELLID && !mpiGrid.is_local(m_ngbr) && ccell->sysBoundaryFlag ==
sysboundarytype::NOT_SYSBOUNDARY) {
   ...
   #pragma omp parallel
   {
      ...
      for (size_t c=0; c < receive_cells.size(); ++c) {
         ...
         #pragma omp for nowait
         for(unsigned int cell = 0; cell < VELOCITY_BLOCK_LENGTH * spatial_cell->get_number_of_velocity_blocks();
cell++) {
            //copy data to fx for solvers, and set data to zero as we will map new values there
            spatial_cell->get_data()[cell] += spatial_cell->get_fx()[cell];
         }
      ...
      for (size_t c=0; c < send_cells.size(); ++c) {
         ...
         #pragma omp for nowait
         for(unsigned int cell = 0; cell < VELOCITY_BLOCK_LENGTH * spatial_cell->get_number_of_velocity_blocks();
cell++) { spatial_cell->get_data()[cell] = 0.0;
         }
      }
   }
}
