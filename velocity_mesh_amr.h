...
template <typename GID, typename LID> class VelocityMesh {
public:
   // VelocityMesh();
   //~VelocityMesh();
...
private:
   ...
   // std::vector<GID> offsets;                                          /**< Block global ID offsets for each
   // refinement level.*/ Real* blockSizes; Real* cellSizes; LID* gridLengths;
...
template <typename GID, typename LID> inline size_t VelocityMesh<GID, LID>::capacityInBytes() const {
   return localToGlobalMap.capacity() * sizeof(GID) + globalToLocalMap.bucket_count() * (sizeof(GID) + sizeof(LID));
}

template <typename GID, typename LID> inline bool VelocityMesh<GID, LID>::check() const {
   bool ok = true;

   if (localToGlobalMap.size() != globalToLocalMap.size()) {
      std::cerr << "VMO ERROR: sizes differ, " << localToGlobalMap.size() << " vs " << globalToLocalMap.size()
                << std::endl;
      ok = false;
   }
   ...
   for (size_t b = 0; b < size(); ++b) {
      ...
      for (size_t c = 0; c < children.size(); ++c) {
         if (children[c] == invalidGlobalID())
            continue;
         if (checkChildren(children[c]) == true)
            hasChildren = true;
      }
      if (hasChildren == true) {
         std::cerr << "VM AMR ERROR: block " << globalID << " at refinement level "
                   << (int)getRefinementLevel(globalID);
...
template <typename GID, typename LID> inline bool VelocityMesh<GID, LID>::checkChildren(const GID& globalID) const {
...
template <typename GID, typename LID> inline bool VelocityMesh<GID, LID>::checkParent(const GID& globalID) const {
   GID parent = getParent(globalID);
   if (parent == globalID)
      return false;
   if (getLocalID(parent) != invalidLocalID())
      return true;
   return checkParent(parent);
}

template <typename GID, typename LID> inline void VelocityMesh<GID, LID>::clear() {
   std::vector<GID>().swap(localToGlobalMap);
   std::unordered_map<GID, LID>().swap(globalToLocalMap);
}

template <typename GID, typename LID> inline bool VelocityMesh<GID, LID>::coarsenAllowed(const GID& globalID) const {
   if (globalToLocalMap.count(globalID) == 0)
      return false;
   ...
   for (size_t s = 0; s < siblings.size(); ++s) {
      ...
      for (size_t c = 0; c < children.size(); ++c) {
         if (globalToLocalMap.count(children[c]) > 0)
            return false;
      }
   }
...
inline bool VelocityMesh<GID, LID>::copy(const LID& sourceLID, const LID& targetLID) {
#ifdef DEBUG_VAMR_MESH
   bool ok = true;
   if (sourceLID >= localToGlobalMap.size())
      ok = false;
   if (targetLID >= localToGlobalMap.size())
      ok = false;
   if (ok == false) {
...
#ifdef DEBUG_VAMR_MESH
   if (globalToLocalMap.find(sourceGID) == globalToLocalMap.end())
      ok = false;
   if (globalToLocalMap.find(targetGID) == globalToLocalMap.end())
      ok = false;
   if (ok == false) {
      std::cerr << "ERROR in copy, source/target LIDs are " << sourceLID << ' ' << targetLID << std::endl;
...
template <typename GID, typename LID> inline size_t VelocityMesh<GID, LID>::count(const GID& globalID) const {
   return globalToLocalMap.count(globalID);
}

template <typename GID, typename LID>
inline GID VelocityMesh<GID, LID>::findBlockDown(uint8_t& refLevel, GID cellIndices[3]) const {
...
template <typename GID, typename LID>
inline GID VelocityMesh<GID, LID>::findBlock(uint8_t& refLevel, GID cellIndices[3]) const {
   GID rvalue = invalidGlobalID();
...
template <typename GID, typename LID>
inline bool VelocityMesh<GID, LID>::getBlockCoordinates(const GID& globalID, Real coords[3]) const {
...
template <typename GID, typename LID>
inline void VelocityMesh<GID, LID>::getBlockInfo(const GID& globalID, Real* array) const {
#ifdef DEBUG_VAMR_MESH
   if (globalID == invalidGlobalID()) {
      for (int i = 0; i < 6; ++i)
...
template <typename GID, typename LID>
inline const Real* VelocityMesh<GID, LID>::getBlockSize(const uint8_t& refLevel) const {
   return meshParameters[meshID].blockSizes.data() + refLevel * 3;
}

template <typename GID, typename LID>
inline bool VelocityMesh<GID, LID>::getBlockSize(const GID& globalID, Real size[3]) const {
   // Calculate block's refinement level and (i,j,k) indices:
   uint8_t refLevel;
   LID indices[3];
   getIndices(globalID, refLevel, indices[0], indices[1], indices[2]);
...
template <typename GID, typename LID>
inline const Real* VelocityMesh<GID, LID>::getCellSize(const uint8_t& refLevel) const {
   return meshParameters[meshID].cellSizes.data() + refLevel * 3;
}

template <typename GID, typename LID>
inline bool VelocityMesh<GID, LID>::getCellSize(const GID& globalID, Real size[3]) const {
...
template <typename GID, typename LID>
inline void VelocityMesh<GID, LID>::getChildren(const GID& globalID, std::vector<GID>& children) const {
   children.clear();
...
template <typename GID, typename LID> inline GID VelocityMesh<GID, LID>::getGlobalID(const LID& localID) const {
...
template <typename GID, typename LID>
inline GID VelocityMesh<GID, LID>::getGlobalID(const uint8_t& refLevel, LID indices[3]) const {
   const uint8_t multiplier = std::pow(2, refLevel);
   if (indices[0] >= meshParameters[meshID].gridLength[0] * multiplier)
      return invalidGlobalID();
   if (indices[1] >= meshParameters[meshID].gridLength[1] * multiplier)
      return invalidGlobalID();
   if (indices[2] >= meshParameters[meshID].gridLength[2] * multiplier)
      return invalidGlobalID();
   return meshParameters[meshID].offsets[refLevel] +
          indices[2] * meshParameters[meshID].gridLength[1] * meshParameters[meshID].gridLength[0] * multiplier *
              multiplier +
...
inline GID VelocityMesh<GID, LID>::getGlobalID(const uint32_t& refLevel, const LID& i, const LID& j,
   ...
   if (i >= meshParameters[meshID].gridLength[0] * multiplier)
      return invalidGlobalID();
   if (j >= meshParameters[meshID].gridLength[1] * multiplier)
      return invalidGlobalID();
   if (k >= meshParameters[meshID].gridLength[2] * multiplier)
      return invalidGlobalID();
   return meshParameters[meshID].offsets[refLevel] +
...
template <typename GID, typename LID>
inline GID VelocityMesh<GID, LID>::getGlobalID(const uint8_t& refLevel, const Real* coords) const {
   ...
   const LID indices[3] = {static_cast<LID>(floor((coords[0] - meshParameters[meshID].meshMinLimits[0]) * multiplier /
                                                  meshParameters[meshID].blockSize[0])),
                           static_cast<LID>(floor((coords[1] - meshParameters[meshID].meshMinLimits[1]) * multiplier /
                                                  meshParameters[meshID].blockSize[1])),
                           static_cast<LID>(floor((coords[2] - meshParameters[meshID].meshMinLimits[2]) * multiplier /
                                                  meshParameters[meshID].blockSize[2]))};
...
template <typename GID, typename LID>
inline GID VelocityMesh<GID, LID>::getGlobalIndexOffset(const uint8_t& refLevel) const {
   return meshParameters[meshID].offsets[refLevel];
}

template <typename GID, typename LID> inline std::vector<GID>& VelocityMesh<GID, LID>::getGrid() {
   return localToGlobalMap;
}

template <typename GID, typename LID>
inline const LID* VelocityMesh<GID, LID>::getGridLength(const uint8_t& refLevel) const {
   return meshParameters[meshID].gridLengths.data() + refLevel * 3;
}

template <typename GID, typename LID>
inline void VelocityMesh<GID, LID>::getIndices(const GID& globalID, uint8_t& refLevel, LID& i, LID& j, LID& k) const {
...
template <typename GID, typename LID> inline LID VelocityMesh<GID, LID>::getLocalID(const GID& globalID) const {
   typename std::unordered_map<GID, LID>::const_iterator it = globalToLocalMap.find(globalID);
...
template <typename GID, typename LID> inline uint8_t VelocityMesh<GID, LID>::getMaxAllowedRefinementLevel() const {
   return meshParameters[meshID].refLevelMaxAllowed;
}

template <typename GID, typename LID> inline GID VelocityMesh<GID, LID>::getMaxVelocityBlocks() const {
   return meshParameters[meshID].max_velocity_blocks;
}

template <typename GID, typename LID> inline size_t VelocityMesh<GID, LID>::getMesh() const { return meshID; }

template <typename GID, typename LID> inline const Real* VelocityMesh<GID, LID>::getMeshMaxLimits() const {
   return meshParameters[meshID].meshMaxLimits;
}

template <typename GID, typename LID> inline const Real* VelocityMesh<GID, LID>::getMeshMinLimits() const {
   return meshParameters[meshID].meshMinLimits;
}

template <typename GID, typename LID>
inline void VelocityMesh<GID, LID>::getNeighborsAtSameLevel(const GID& globalID, std::vector<GID>& neighborIDs) const {
   ...
   for (int k_off = -1; k_off < 2; ++k_off)
      for (int j_off = -1; j_off < 2; ++j_off)
         for (int i_off = -1; i_off < 2; ++i_off) {
            if (i + i_off < Nx_max && (j + j_off < Ny_max && k + k_off < Nz_max))
               neighborIDs[nbr] = getGlobalID(refLevel, i + i_off, j + j_off, k + k_off);
            else
               neighborIDs[nbr] = invalidGlobalID();
            ++nbr;
         }
}

template <typename GID, typename LID>
inline void VelocityMesh<GID, LID>::getNeighborsExistingAtSameLevel(const GID& globalID,
                                                                    std::vector<GID>& neighborIDs) const {
   neighborIDs.clear();
...
inline void VelocityMesh<GID, LID>::getNeighborsExistingAtOffset(const GID& globalID, const int& i_off,
   ...
   if (abs(i_off) > 1 || (abs(j_off) > 1 || abs(k_off) > 1)) {
      std::stringstream ss;
      ss << "VelocityMesh ERROR: invalid offsets in getNeighborsExistingAtOffset " << i_off << ' ' << j_off << ' '
         << k_off << std::endl;
      std::cerr << ss.str();
      exit(1);
   }
   ...
   if (nbrGlobalID == invalidGlobalID())
      return;
   ...
   for (size_t n = 0; n < 4; ++n)
      neighborLocalIDs[n] = invalidLocalID();

   const int nbrTypeID = (k_off + 1) * 9 + (j_off + 1) * 3 + (i_off + 1);
   switch (nbrTypeID) {
   case 4: // -z face neighbor
      nbr = globalToLocalMap.find(nbrChildren[4]);
      if (nbr != globalToLocalMap.end()) {
         neighborLocalIDs[0] = nbr->second;
      }
      nbr = globalToLocalMap.find(nbrChildren[5]);
      if (nbr != globalToLocalMap.end()) {
         neighborLocalIDs[1] = nbr->second;
      }
      nbr = globalToLocalMap.find(nbrChildren[6]);
      if (nbr != globalToLocalMap.end()) {
         neighborLocalIDs[2] = nbr->second;
      }
      nbr = globalToLocalMap.find(nbrChildren[7]);
      if (nbr != globalToLocalMap.end()) {
         neighborLocalIDs[3] = nbr->second;
      }
      break;
   case 10: // -y face neighbor
      nbr = globalToLocalMap.find(nbrChildren[2]);
      if (nbr != globalToLocalMap.end()) {
         neighborLocalIDs[0] = nbr->second;
      }
      nbr = globalToLocalMap.find(nbrChildren[3]);
      if (nbr != globalToLocalMap.end()) {
         neighborLocalIDs[1] = nbr->second;
      }
      nbr = globalToLocalMap.find(nbrChildren[6]);
   ...
   case 12: // -x face neighbor
      nbr = globalToLocalMap.find(nbrChildren[1]);
      if (nbr != globalToLocalMap.end()) {
         neighborLocalIDs[0] = nbr->second;
      }
      nbr = globalToLocalMap.find(nbrChildren[3]);
   ...
   case 14: // +x face neighbor
      nbr = globalToLocalMap.find(nbrChildren[0]);
      if (nbr != globalToLocalMap.end()) {
         neighborLocalIDs[0] = nbr->second;
      }
      nbr = globalToLocalMap.find(nbrChildren[2]);
...
template <typename GID, typename LID> inline int VelocityMesh<GID, LID>::getOctant(const GID& globalID) const {
...
template <typename GID, typename LID> inline GID VelocityMesh<GID, LID>::getParent(const GID& globalID) const {
   // Calculate block indices and refinement level
   uint8_t refLevel;
   LID i, j, k;
   getIndices(globalID, refLevel, i, j, k);
...
template <typename GID, typename LID>
inline uint8_t VelocityMesh<GID, LID>::getRefinementLevel(const GID& globalID) const {
   for (size_t i = 1; i < meshParameters[meshID].refLevelMaxAllowed + 1; ++i) {
      if (globalID < meshParameters[meshID].offsets[i])
         return i - 1;
   }
   return meshParameters[meshID].refLevelMaxAllowed;
}
...
template <typename GID, typename LID>
inline void VelocityMesh<GID, LID>::getSiblingNeighbors(const GID& globalID, std::vector<GID>& nbrs) const {
   nbrs.clear();
...
template <typename GID, typename LID>
inline void VelocityMesh<GID, LID>::getSiblings(const GID& globalID, std::vector<GID>& siblings) const {
   uint8_t refLevel;
   LID i, j, k;
   getIndices(globalID, refLevel, i, j, k);
...
template <typename GID, typename LID> inline bool VelocityMesh<GID, LID>::hasChildren(const GID& globalID) const {
   std::vector<GID> children;
   getChildren(globalID, children);
...
template <typename GID, typename LID> inline GID VelocityMesh<GID, LID>::hasGrandParent(const GID& globalID) const {
   ...
   do {
      ...
      if (getLocalID(parentGID) != invalidLocalID())
         return parentGID;
...
template <typename GID, typename LID> inline bool VelocityMesh<GID, LID>::initialize(const size_t& meshID) {
   this->meshID = meshID;
   return true;
}

template <typename GID, typename LID>
inline bool VelocityMesh<GID, LID>::initialize(const size_t& meshID,
                                               std::vector<vmesh::MeshParameters>& meshParameters) {
   ...
   meshParameters[meshID].gridSize[0] =
       meshParameters[meshID].meshMaxLimits[0] - meshParameters[meshID].meshMinLimits[0];
   meshParameters[meshID].gridSize[1] =
       meshParameters[meshID].meshMaxLimits[1] - meshParameters[meshID].meshMinLimits[1];
   meshParameters[meshID].gridSize[2] =
       meshParameters[meshID].meshMaxLimits[2] - meshParameters[meshID].meshMinLimits[2];
   ...
   const GID N_blocks0 = meshParameters[meshID].gridLength[0] * meshParameters[meshID].gridLength[1] *
                         meshParameters[meshID].gridLength[2];
   ...
   for (uint8_t r = 0; r < meshParameters[meshID].refLevelMaxAllowed + 1; ++r) {
      for (int i = 0; i < 3; ++i)
         meshParameters[meshID].gridLengths[3 * r + i] = meshParameters[meshID].gridLength[i] * mul;
      for (int i = 0; i < 3; ++i)
         meshParameters[meshID].blockSizes[3 * r + i] = meshParameters[meshID].blockSize[i] / mul;
      for (int i = 0; i < 3; ++i)
         meshParameters[meshID].cellSizes[3 * r + i] =
             meshParameters[meshID].blockSize[i] / (mul * meshParameters[meshID].blockLength[i]);
      mul *= 2;
   }
...
template <typename GID, typename LID> inline LID VelocityMesh<GID, LID>::invalidBlockIndex() {
   return INVALID_VEL_BLOCK_INDEX;
}

template <typename GID, typename LID> inline GID VelocityMesh<GID, LID>::invalidGlobalID() { return INVALID_GLOBALID; }

template <typename GID, typename LID> inline LID VelocityMesh<GID, LID>::invalidLocalID() { return INVALID_LOCALID; }

template <typename GID, typename LID> inline bool VelocityMesh<GID, LID>::isInitialized() const { return initialized; }

template <typename GID, typename LID> inline void VelocityMesh<GID, LID>::pop() {
   if (size() == 0)
      return;
...
#ifdef DEBUG_VAMR_MESH
   bool ok = true;
   if (globalToLocalMap.find(lastGID) == globalToLocalMap.end())
      ok = false;
   if (ok == false) {
...
template <typename GID, typename LID> inline bool VelocityMesh<GID, LID>::push_back(const GID& globalID) {
   if (globalID == invalidGlobalID())
      return false;
   if (size() >= meshParameters[meshID].max_velocity_blocks) {
      std::cerr << "vmesh-amr: too many blocks, current size is " << size() << " max "
                << meshParameters[meshID].max_velocity_blocks << std::endl;
      return false;
   }
...
template <typename GID, typename LID> inline uint8_t VelocityMesh<GID, LID>::push_back(const std::vector<GID>& blocks) {
...
template <typename GID, typename LID> inline void VelocityMesh<GID, LID>::setGrid() {
...
template <typename GID, typename LID> inline bool VelocityMesh<GID, LID>::setGrid(const std::vector<GID>& globalIDs) {
   globalToLocalMap.clear();
...
template <typename GID, typename LID> inline bool VelocityMesh<GID, LID>::setMesh(const size_t& meshID) {
   if (meshID >= meshParameters.size())
      return false;
   this->meshID = meshID;
   return true;
}

template <typename GID, typename LID> inline void VelocityMesh<GID, LID>::setNewSize(const LID& newSize) {
   localToGlobalMap.resize(newSize);
}

template <typename GID, typename LID> inline size_t VelocityMesh<GID, LID>::size() const {
   return localToGlobalMap.size();
}

template <typename GID, typename LID> inline size_t VelocityMesh<GID, LID>::sizeInBytes() const {
   return globalToLocalMap.size() * sizeof(GID) + localToGlobalMap.size() * (sizeof(GID) + sizeof(LID));
}

template <typename GID, typename LID> inline void VelocityMesh<GID, LID>::swap(VelocityMesh& vm) {
