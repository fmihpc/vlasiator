...
#ifdef ACC_SEMILAG_PPM
            Vec target = a[0] + 2.0 * r_norm * a[1] + 3.0 * r_norm * r_norm * a[2];
#endif
...
int main(void) {
   ...
   for (uint i = 0; i < blocks_per_dim * WID; i++) {
      ...
      r = r + 0.5 * dr[i + WID][0];
      values[i + WID] =
          rho * pow(physicalconstants::MASS_PROTON / (2.0 * M_PI * physicalconstants::K_B * T), 1.5) * exp(-physicalconstants::MASS_PROTON * (r - r1) * (r - r1) / (2.0 * physicalconstants::K_B * T));
