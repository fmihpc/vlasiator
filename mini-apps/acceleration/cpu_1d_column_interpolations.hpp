...
inline void compute_h6_face_values(Real* values, uint n_cblocks, Real* fv_l, Real* fv_r) {
   ...
   for (int k = 0; k < n_cblocks * WID + 1; k++) {
      /*compute left values*/
      fv_l[k] = 1.0 / 60.0 * (values[k - 3 + WID] - 8.0 * values[k - 2 + WID] + 37.0 * values[k - 1 + WID] + 37.0 * values[k + WID] - 8.0 * values[k + 1 + WID] + values[k + 2 + WID]);
      /*set right value*/
      if (k > 0)
         fv_r[k - 1] = fv_l[k];
   }
...
inline void filter_boundedness(Real* values, uint n_cblocks, Real* fv_l, Real* fv_r) {
   /*First Eq. 19 & 20*/
   for (int k = 0; k < n_cblocks * WID; k++) {
      bool do_fix_bounds = (values[k - 1 + WID] - fv_l[k]) * (fv_l[k] - values[k + WID]) < 0 || (values[k + 1 + WID] - fv_r[k]) * (fv_r[k] - values[k + WID]) < 0;
      if (do_fix_bounds) {
         ...
         fv_l[k] = (values[k - 1 + WID] - fv_l[k]) * (fv_l[k] - values[k + WID]) < 0 ? values[k + WID] - slope_sign * min((Real)0.5 * slope_abs, abs(fv_l[k] - values[k + WID])) : fv_l[k];
         fv_r[k] = (values[k + 1 + WID] - fv_r[k]) * (fv_r[k] - values[k + WID]) < 0 ? values[k + WID] + slope_sign * min((Real)0.5 * slope_abs, abs(fv_r[k] - values[k + WID])) : fv_r[k];
      }
   }
}
...
inline void compute_ppm_coeff_explicit_column(Real* values, uint n_cblocks, Real a[][RECONSTRUCTION_ORDER + 1]) {
   ...
   for (uint k = 0; k < n_cblocks * WID; k++) {
      ...
      m_face = (p_face - m_face) * (values[k + WID] - 0.5 * (m_face + p_face)) > (p_face - m_face) * (p_face - m_face) / 6.0 ? 3 * values[k + WID] - 2 * p_face : m_face;
      p_face = -(p_face - m_face) * (p_face - m_face) / 6.0 > (p_face - m_face) * (values[k + WID] - 0.5 * (m_face + p_face)) ? 3 * values[k + WID] - 2 * m_face : p_face;
