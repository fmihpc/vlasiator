...
int main(void) {
   ...
   for (int i = 0; i < blocks_per_dim * WID; i++) {
      Real v = v_min + i * dv;
      if (v > v_min + 0.8 * (blocks_per_dim * WID * dv) && v < v_min + 0.9 * (blocks_per_dim * WID * dv)) {
         values[i + WID] = 1.0;
      }
   }
   ...
   for (int step = 0; step < iterations; step++) {
      ...
      propagate(values, blocks_per_dim, v_min, dv, i_block, i_cell, j_block, j_cell, intersection, intersection_di, intersection_dj, intersection_dk);
   }
}
