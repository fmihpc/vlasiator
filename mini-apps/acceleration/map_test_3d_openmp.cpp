...
void propagate(const Real* const values_in, Real* values_out, uint blocks_per_dim_x, uint blocks_per_dim_y, uint blocks_per_dim_z, Real v_min, Real dv, Real intersection, Real intersection_di,
      ...
      for (int j = 0; j < blocks_per_dim_y * WID; j++) {
         for (uint k = 0; k < blocks_per_dim_z * WID; k++) {
            ...
            /* intersection_min is the intersection z coordinate (z after
               ...
               index
            */
            const Real intersection_min = intersection + (i_block * WID + i_cell) * intersection_di + (j_block * WID + j_cell) * intersection_dj;
...
#ifdef ACC_SEMILAG_PLM
               Real target_density_l = v_int_norm_l * a[0] + v_int_norm_l * v_int_norm_l * a[1];
               Real target_density_r = v_int_norm_r * a[0] + v_int_norm_r * v_int_norm_r * a[1];
#endif
#ifdef ACC_SEMILAG_PPM
               Real target_density_l = v_int_norm_l * a[0] + v_int_norm_l * v_int_norm_l * a[1] + v_int_norm_l * v_int_norm_l * v_int_norm_l * a[2];
               Real target_density_r = v_int_norm_r * a[0] + v_int_norm_r * v_int_norm_r * a[1] + v_int_norm_r * v_int_norm_r * v_int_norm_r * a[2];
#endif
...
int main(void) {
   ...
   for (int i = 0; i < blocks_per_dim_x * WID; i++) {
      for (int j = 0; j < blocks_per_dim_y * WID; j++) {
         for (int k = 0; k < blocks_per_dim_z * WID; k++) {
            Real v = v_min + k * dv;
            if (v > v_min + 0.8 * (blocks_per_dim_z * WID * dv) && v < v_min + 0.9 * (blocks_per_dim_z * WID * dv))
   ...
   for (int step = 0; step < iterations; step += 2) {
      ...
      propagate(values_a, values_b, blocks_per_dim_x, blocks_per_dim_y, blocks_per_dim_z, v_min, dv, intersection, intersection_di, intersection_dj, intersection_dk);
      propagate(values_b, values_a, blocks_per_dim_x, blocks_per_dim_y, blocks_per_dim_z, v_min, dv, intersection, intersection_di, intersection_dj, intersection_dk);
   }
}
