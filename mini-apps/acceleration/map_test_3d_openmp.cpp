      ...
      for (int j = 0; j < blocks_per_dim_y * WID; j++) {
         for (uint k = 0; k < blocks_per_dim_z * WID; k++) {
            ...
            /* intersection_min is the intersection z coordinate (z after
               ...
               index
            */
            const Real intersection_min =
                intersection + (i_block * WID + i_cell) * intersection_di + (j_block * WID + j_cell) * intersection_dj;
...
int main(void) {
   ...
   /*intersection values define the acceleration transformation. These would be obtained from other routines, but are
    * here fixed*/
