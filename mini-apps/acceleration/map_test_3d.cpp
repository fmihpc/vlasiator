...
void propagate(
   Real* values, uint blocks_per_dim, Real v_min, Real dv, uint i_block, uint i_cell, uint j_block, uint j_cell, Real intersection, Real intersection_di, Real intersection_dj, Real intersection_dk
) {
...
#ifdef ACC_SEMILAG_PLM
            Real target_density_l = v_int_norm_l * a[k_block * WID + k_cell][0] + v_int_norm_l * v_int_norm_l * a[k_block * WID + k_cell][1];
            Real target_density_r = v_int_norm_r * a[k_block * WID + k_cell][0] + v_int_norm_r * v_int_norm_r * a[k_block * WID + k_cell][1];
#endif
#ifdef ACC_SEMILAG_PPM
            Real target_density_l =
               v_int_norm_l * a[k_block * WID + k_cell][0] + v_int_norm_l * v_int_norm_l * a[k_block * WID + k_cell][1] + v_int_norm_l * v_int_norm_l * v_int_norm_l * a[k_block * WID + k_cell][2];
            Real target_density_r =
               v_int_norm_r * a[k_block * WID + k_cell][0] + v_int_norm_r * v_int_norm_r * a[k_block * WID + k_cell][1] + v_int_norm_r * v_int_norm_r * v_int_norm_r * a[k_block * WID + k_cell][2];
#endif
   ...
   for (int step = 0; step < iterations; step++) {
      ...
      for (int i = 0; i < blocks_per_dim_x * WID; i++) {
         for (int j = 0; j < blocks_per_dim_y * WID; j++) {
            ...
            propagate(values + colindex(i, j), blocks_per_dim_z, v_min, dv, i_block, i_cell, j_block, j_cell, intersection, intersection_di, intersection_dj, intersection_dk);
         }
      }
   }
}
