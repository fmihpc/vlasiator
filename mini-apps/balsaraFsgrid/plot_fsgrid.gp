...
template <typename ID> inline bool paircomparator(const std::pair<ID, uint>& l, const std::pair<ID, uint>& r) { return l.first < r.first; }

template <typename ID, typename LENGTH> inline void sortIds(const uint dimension, const LENGTH meshSize, const std::vector<ID>& ids, std::map<ID, ID>& mapping) {
...
void sortIdlistByDimension(const uint dimension, const LENGTH meshSize, std::vector<ID>& ids, std::vector<uint>& columnIdOffsets, std::vector<uint>& columnNumIds, std::vector<uint>& setColumnOffsets,
