...
void propagate(Vec values[], uint blocks_per_dim, Real v_min, Real dv, uint i_block, uint j_block, uint j_cell, Real intersection, Real intersection_di, Real intersection_dj, Real intersection_dk) {
   ...
   /* intersection_min is the intersection z coordinate (z after
      ...
      index (i in vector)
   */
   const Real intersection_min_base = intersection + (i_block * WID) * intersection_di + (j_block * WID + j_cell) * intersection_dj;

   // const Vec intersection_min(intersection_min_base);
   const Vec intersection_min(
       intersection_min_base + 0 * intersection_di, intersection_min_base + 1 * intersection_di, intersection_min_base + 2 * intersection_di, intersection_min_base + 3 * intersection_di);
      ...
      for (uint k_cell = 0; k_cell < WID; ++k_cell) {
         ...
         while (horizontal_or(gk <= target_gk_r)) {
...
#ifdef ACC_SEMILAG_PLM
            Vec target_density_l = v_int_norm_l * a[0] + v_int_norm_l * v_int_norm_l * a[1];
            Vec target_density_r = v_int_norm_r * a[0] + v_int_norm_r * v_int_norm_r * a[1];
#endif
#ifdef ACC_SEMILAG_PPM
            Vec target_density_l = v_int_norm_l * a[0] + v_int_norm_l * v_int_norm_l * a[1] + v_int_norm_l * v_int_norm_l * v_int_norm_l * a[2];
            Vec target_density_r = v_int_norm_r * a[0] + v_int_norm_r * v_int_norm_r * a[1] + v_int_norm_r * v_int_norm_r * v_int_norm_r * a[2];
#endif
#ifdef ACC_SEMILAG_PQM
            Vec target_density_l = v_int_norm_l * a[0] + v_int_norm_l * v_int_norm_l * a[1] + v_int_norm_l * v_int_norm_l * v_int_norm_l * a[2] +
                                   v_int_norm_l * v_int_norm_l * v_int_norm_l * v_int_norm_l * a[3] + v_int_norm_l * v_int_norm_l * v_int_norm_l * v_int_norm_l * v_int_norm_l * a[4];

            Vec target_density_r = v_int_norm_r * a[0] + v_int_norm_r * v_int_norm_r * a[1] + v_int_norm_r * v_int_norm_r * v_int_norm_r * a[2] +
                                   v_int_norm_r * v_int_norm_r * v_int_norm_r * v_int_norm_r * a[3] + v_int_norm_r * v_int_norm_r * v_int_norm_r * v_int_norm_r * v_int_norm_r * a[4];
            ...
            for (uint elem = 0; elem < 4; elem++) {
               ...
               if (k_in_target >= 0 && k_in_target < blocks_per_dim * WID) {
...
void print_reconstruction(int step, Vec values[], uint blocks_per_dim, Real v_min, Real dv, uint i_block, uint j_block, uint j_cell, Real intersection, Real intersection_di, Real intersection_dj,
...
#ifdef ACC_SEMILAG_PLM
            Vec target = a[0] + 2.0 * v_norm * a[1];
#endif
#ifdef ACC_SEMILAG_PPM
            Vec target = a[0] + 2.0 * v_norm * a[1] + 3.0 * v_norm * v_norm * a[2];
#endif
#ifdef ACC_SEMILAG_PQM
            Vec target = a[0] + 2.0 * v_norm * a[1] + 3.0 * v_norm * v_norm * a[2] + 4.0 * v_norm * v_norm * v_norm * a[3] + 5.0 * v_norm * v_norm * v_norm * v_norm * a[4];
#endif
 ...
 */
   ...
   for (int i = 0; i < blocks_per_dim * WID; i++) {
      Real v = v_min + i * dv;
      values[i + WID] = rho * pow(physicalconstants::MASS_PROTON / (2.0 * M_PI * physicalconstants::K_B * T), 1.5) * exp(-physicalconstants::MASS_PROTON * v * v / (2.0 * physicalconstants::K_B * T));
   }

   // print_values(0,values,blocks_per_dim, v_min, dv);
   print_reconstruction(0, values, blocks_per_dim, v_min, dv, i_block, j_block, j_cell, intersection, intersection_di, intersection_dj, intersection_dk);

   clock_t t = clock();
   /*loop over propagations*/
   for (int step = 0; step <= iterations; step++) {
      propagate(values, blocks_per_dim, v_min, dv, i_block, j_block, j_cell, intersection, intersection_di, intersection_dj, intersection_dk);
      if (step % 10 == 0)
         print_reconstruction(step, values, blocks_per_dim, v_min, dv, i_block, j_block, j_cell, intersection, intersection_di, intersection_dj, intersection_dk);
   }
