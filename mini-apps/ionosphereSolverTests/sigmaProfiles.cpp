...
int main(int argc, char** argv) {
   ...
   for (int e = 0; e < productionNumParticleEnergies; e++) {
      ...
      // const Real electronRange = 1.64e-5 * pow(particle_energy[e], 1.67) * (1. + 9.48e-2 * pow(particle_energy[e],
      // -1.57));
      Real rho_R = 0.;
      ...
      for (int h = 0; h < numAtmosphereLevels; h++) {
         ...
         // const Real rate = particle_energy[e] / (electronRange / rho_R) / eps_ion_keV *   lambda   *
         // atmosphere[h].density / integratedDensity;
         ...
         // const Real rate = atmosphere[h].density / eps_ion_keV * particle_energy[e] * lambda / electronRange; //
         // TODO: Albedo flux?
   ...
   for (int p = 0; p < productionNumParticleEnergies; p++) {
      ...
      if (particle_energy[p] > accenergy) {
         ...
         differentialFlux[p] = sqrt(1. / (2. * M_PI * MASS_ELECTRON)) * particle_energy[p] / tempenergy /
                               sqrt(tempenergy * 1e3 * CHARGE) * deltaE * exp(-energyparam);
   ...
   std::cout
       << "# Altitude (m)\tn_e (1/m³)\tsigmaP (mho)\tsigmaH (mho)\tsigmaParallel (mho)\tProduction rate (m^-3 s^-1)"
       << std::endl;
   ...
   for (int h = 1; h < numAtmosphereLevels; h++) {
      ...
      // Real sigmap = halfdx * 0.5 * (electronDensity.at(h)*atmosphere.at(h).pedersencoeff +
      // electronDensity.at(h-1)*atmosphere.at(h-1).pedersencoeff); Real sigmah = halfdx * 0.5 *
      // (electronDensity.at(h)*atmosphere.at(h).hallcoeff + electronDensity.at(h-1)*atmosphere.at(h-1).hallcoeff); Real
      // sigmaParallel = halfdx * 0.5 * (electronDensity.at(h)*atmosphere.at(h).parallelcoeff +
      // electronDensity.at(h-1)*atmosphere.at(h-1).parallelcoeff);

      SigmaP += sigmap;
      SigmaH += sigmah;
      SigmaParallel += sigmaParallel;

      std::cout << atmosphere[h].altitude << "\t" << 0.5 * (electronDensity[h] + electronDensity[h - 1]) << "\t"
                << sigmap / (2. * halfdx) << "\t" << sigmah / (2. * halfdx) << "\t" << sigmaParallel / (2. * halfdx)
                << "\t" << qref << std::endl;
   }
   std::cerr << std::endl;
