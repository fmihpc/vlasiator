   ...
   while (i < length) {
      ...
      if (hasChildren) {
         ...
         if (path.size() > grid.get_refinement_level(id)) {
            ...
            vector<CellID> myChildren = getMyChildren(children, dimension, path[grid.get_refinement_level(id)].first, path[grid.get_refinement_level(id)].second);
...
int main(int argc, char* argv[]) {
   ...
   for (const auto& cell : cells) {
      ...
      if (parent > 0 && !(std::find(ids.begin(), ids.end(), parent) != ids.end())) {
