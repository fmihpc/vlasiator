...
if [[ $PLATFORM == "-pioneer" ]]; then
   PARALLEL=64
elif [[ $PLATFORM == "-hile_cpu" || $PLATFORM == "-hile_gpu" || $PLATFORM == "-lumi_2503" || $PLATFORM == "-carrington_gcc_openmpi" || $PLATFORM == "-frankenstein_hopper2_cuda" || $PLATFORM != "-roihu_gpu" || $PLATFORM == "-maahinen" ]]; then
   PARALLEL=128
else
