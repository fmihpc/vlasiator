...
size_t *host_cellIdxStartCutoff = nullptr, *host_smallCellIdxArray = nullptr,
       *host_remappedCellIdxArray = nullptr; // remappedCellIdxArray tells the position of the cell index in the sequence instead of the actual index
// Device pointers
Real *dev_bValues = nullptr, *dev_nu0Values = nullptr, *dev_bulkVX = nullptr, *dev_bulkVY = nullptr, *dev_bulkVZ = nullptr, *dev_Ddt = nullptr, *dev_potentialDdtValues = nullptr;
...
__host__ gpuStream_t gpu_getStream() { return gpuStreamList[gpu_getThread()]; }
...
__host__ uint gpu_getAllocationCount() { return allocationCount; }
...
int gpu_reportMemory(const size_t local_cells_capacity, const size_t ghost_cells_capacity, const size_t local_cells_size, const size_t ghost_cells_size) {
   ...
   */
   ...
   size_t miniBuffers = maxNThreads * (8 * sizeof(Real)             // returnReal
   ...
   for (uint i = 0; i < allocationCount; ++i) {
      vlasovBuffers += gpu_vlasov_allocatedSize[i] * WID3 * sizeof(Realf); // gpu_blockDataOrdered[cpuThreadID] // sizes of actual buffers
 ...
 */
__host__ void gpu_vlasov_allocate(const uint maxBlockCount // Largest found vmesh size
) {
...
__host__ void gpu_vlasov_allocate_perthread(uint allocID, uint blockAllocationCount) {
   ...
   if ((gpu_vlasov_allocatedSize[allocID] > blockAllocationCount * BLOCK_ALLOCATION_FACTOR) && (gpu_vlasov_allocatedSize[allocID] * WID3 * sizeof(Realf) >= probeRequirement)) {
      return;
   }
...
__host__ void gpu_vlasov_deallocate_perthread(uint allocID) {
...
__host__ void gpu_acc_allocate_perthread(uint allocID,
   ...
   if ((columnAllocationCount > host_columnOffsetData[allocID].capacityCols()) || (columnSetAllocationCount > host_columnOffsetData[allocID].capacityColSets())) {
...
/*
   ...
   This is called from within non-threaded regions so does not perform async.
 */
__host__ void gpu_trans_allocate(cuint nAllCells, cuint sumOfLengths, cuint largestVmesh, cuint unionSetSize, cuint transGpuBlocks, cuint nPencils) {
   gpuStream_t stream = gpu_getStream();
