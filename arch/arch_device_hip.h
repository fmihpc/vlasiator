...
namespace arch {
   ...
   public:
      void syncDeviceData(void) { CHK_ERR(hipMemcpyAsync(d_ptr, ptr, bytes, hipMemcpyHostToDevice, gpuStreamList[thread_id])); }

      void syncHostData(void) { CHK_ERR(hipMemcpyAsync(ptr, d_ptr, bytes, hipMemcpyDeviceToHost, gpuStreamList[thread_id])); }
      ...
      __host__ __device__ buf(const buf& u) : ptr(u.ptr), d_ptr(u.d_ptr), bytes(u.bytes), is_copy(1), thread_id(u.thread_id) {}
   ...
   template <typename T> __host__ __forceinline__ static void free(T* ptr, hipStream_t stream) { CHK_ERR(hipFreeAsync(ptr, stream)); }
   ...
   template <typename T> __forceinline__ static void memcpy_h2d(T* dst, T* src, size_t bytes, hipStream_t stream) { CHK_ERR(hipMemcpyAsync(dst, src, bytes, hipMemcpyHostToDevice, stream)); }
   ...
   template <typename T> __forceinline__ static void memcpy_d2h(T* dst, T* src, size_t bytes, hipStream_t stream) { CHK_ERR(hipMemcpyAsync(dst, src, bytes, hipMemcpyDeviceToHost, stream)); }

   /* Register, ie, page-lock existing host allocations */
   template <typename T> __forceinline__ static void host_register(T* ptr, size_t bytes) { CHK_ERR(hipHostRegister(ptr, bytes, hipHostRegisterDefault)); }
   ...
   template <typename Lambda, typename T> __device__ __forceinline__ static void lambda_eval(const uint (&idx)[1], T* __restrict__ thread_data, Lambda loop_body) { loop_body(idx[0], thread_data); }

   template <typename Lambda, typename T> __device__ __forceinline__ static void lambda_eval(const uint (&idx)[2], T* __restrict__ thread_data, Lambda loop_body) {
      loop_body(idx[0], idx[1], thread_data);
   }

   template <typename Lambda, typename T> __device__ __forceinline__ static void lambda_eval(const uint (&idx)[3], T* __restrict__ thread_data, Lambda loop_body) {
      loop_body(idx[0], idx[1], idx[2], thread_data);
   }

   template <typename Lambda, typename T> __device__ __forceinline__ static void lambda_eval(const uint (&idx)[4], T* __restrict__ thread_data, Lambda loop_body) {
      ...
      if (Op == reduce_op::null) {
         ...
         if (gridsize > 0) {
            reduction_kernel<ARCH_BLOCKSIZE_R, Op, NDim, NReduStatic>
                <<<gridsize, blocksize, 0, gpuStreamList[thread_id]>>>(loop_body, d_const_buf, d_buf, d_limits, n_total, n_reductions, d_thread_data_dynamic);
         }
         ...
         if (gridsize > 0) {
            if (blocksize == ARCH_BLOCKSIZE_R) {
               reduction_kernel<ARCH_BLOCKSIZE_R, Op, NDim, 0>
                   <<<gridsize, blocksize, shared_mem_bytes_per_block_request, gpuStreamList[thread_id]>>>(loop_body, d_const_buf, d_buf, d_limits, n_total, n_reductions, d_thread_data_dynamic);
