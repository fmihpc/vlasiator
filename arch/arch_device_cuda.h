...
namespace arch {
   ...
   public:
      ...
      void syncHostData(void) { CHK_ERR(cudaMemcpyAsync(ptr, d_ptr, bytes, cudaMemcpyDeviceToHost, gpuStreamList[thread_id])); }
   ...
   template <typename T> __host__ __forceinline__ static void free(T* ptr, cudaStream_t stream) { CHK_ERR(cudaFreeAsync(ptr, stream)); }
   ...
   template <typename T> __forceinline__ static void memcpy_h2d(T* dst, T* src, size_t bytes, cudaStream_t stream) { CHK_ERR(cudaMemcpyAsync(dst, src, bytes, cudaMemcpyHostToDevice, stream)); }
   ...
   template <typename T> __forceinline__ static void memcpy_d2h(T* dst, T* src, size_t bytes, cudaStream_t stream) { CHK_ERR(cudaMemcpyAsync(dst, src, bytes, cudaMemcpyDeviceToHost, stream)); }

   /* Register, ie, page-lock existing host allocations */
   template <typename T> __forceinline__ static void host_register(T* ptr, size_t bytes) { CHK_ERR(cudaHostRegister(ptr, bytes, cudaHostRegisterDefault)); }
