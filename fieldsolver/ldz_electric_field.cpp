 ...
 */
Real calculateCflSpeed(const Real& v0, const Real& v1, const Real& vA, const Real& vS, const Real& vW) {
 ...
 */
void calculateWaveSpeedYZ(FsGrid<std::array<Real, fsgrids::bfield::N_BFIELD>, FS_STENCIL_WIDTH>& perBGrid, FsGrid<std::array<Real, fsgrids::moments::N_MOMENTS>, FS_STENCIL_WIDTH>& momentsGrid,
                          ...
                          FsGrid<std::array<Real, fsgrids::bgbfield::N_BGB>, FS_STENCIL_WIDTH>& BgBGrid, cint i, cint j, cint k, cint nbi, cint nbj, cint nbk, const Real& By, const Real& Bz,
                          const Real& dBydx, const Real& dBydz, const Real& dBzdx, const Real& dBzdy, const Real& ydir, const Real& zdir, const Real& minRhom, const Real& maxRhom, Real& ret_vA,
 ...
 * \param ret_vW Whistler speed returned
 */
void calculateWaveSpeedXZ(FsGrid<std::array<Real, fsgrids::bfield::N_BFIELD>, FS_STENCIL_WIDTH>& perBGrid, FsGrid<std::array<Real, fsgrids::moments::N_MOMENTS>, FS_STENCIL_WIDTH>& momentsGrid,
                          FsGrid<std::array<Real, fsgrids::dperb::N_DPERB>, FS_STENCIL_WIDTH>& dPerBGrid, FsGrid<std::array<Real, fsgrids::dmoments::N_DMOMENTS>, FS_STENCIL_WIDTH>& dMomentsGrid,
                          FsGrid<std::array<Real, fsgrids::bgbfield::N_BGB>, FS_STENCIL_WIDTH>& BgBGrid, cint i, cint j, cint k, cint nbi, cint nbj, cint nbk, const Real& Bx, const Real& Bz,
                          const Real& dBxdy, const Real& dBxdz, const Real& dBzdx, const Real& dBzdy, const Real& xdir, const Real& zdir, const Real& minRhom, const Real& maxRhom, Real& ret_vA,
                          Real& ret_vS, Real& ret_vW) {
   std::array<Real, fsgrids::bfield::N_BFIELD>* perb = perBGrid.get(i, j, k);
   std::array<Real, fsgrids::bfield::N_BFIELD>* nbr_perb = perBGrid.get(nbi, nbj, nbk);
   std::array<Real, fsgrids::moments::N_MOMENTS>* moments = momentsGrid.get(i, j, k);
 ...
 * \param ret_vW Whistler speed returned
 */
void calculateWaveSpeedXY(FsGrid<std::array<Real, fsgrids::bfield::N_BFIELD>, FS_STENCIL_WIDTH>& perBGrid, FsGrid<std::array<Real, fsgrids::moments::N_MOMENTS>, FS_STENCIL_WIDTH>& momentsGrid,
                          FsGrid<std::array<Real, fsgrids::dperb::N_DPERB>, FS_STENCIL_WIDTH>& dPerBGrid, FsGrid<std::array<Real, fsgrids::dmoments::N_DMOMENTS>, FS_STENCIL_WIDTH>& dMomentsGrid,
                          FsGrid<std::array<Real, fsgrids::bgbfield::N_BGB>, FS_STENCIL_WIDTH>& BgBGrid, cint i, cint j, cint k, cint nbi, cint nbj, cint nbk, const Real& Bx, const Real& By,
                          const Real& dBxdy, const Real& dBxdz, const Real& dBydx, const Real& dBydz, const Real& xdir, const Real& ydir, const Real& minRhom, const Real& maxRhom, Real& ret_vA,
                          Real& ret_vS, Real& ret_vW) {
   std::array<Real, fsgrids::bfield::N_BFIELD>* perb = perBGrid.get(i, j, k);
   std::array<Real, fsgrids::bfield::N_BFIELD>* nbr_perb = perBGrid.get(nbi, nbj, nbk);
   std::array<Real, fsgrids::moments::N_MOMENTS>* moments = momentsGrid.get(i, j, k);
 ...
 *
 * Computes the upwinded electric field X component along the cell's corresponding edge as the cross product of B and V in the YZ plane. Also includes the calculation of the maximally allowed time
 * step.
 *
 * Expects that the correct RHO and B fields are being passed, depending on the stage of the Runge-Kutta time stepping method.
 *
 ...
 */
void calculateEdgeElectricFieldX(FsGrid<std::array<Real, fsgrids::bfield::N_BFIELD>, FS_STENCIL_WIDTH>& perBGrid, FsGrid<std::array<Real, fsgrids::efield::N_EFIELD>, FS_STENCIL_WIDTH>& EGrid,
                                 ...
                                 FsGrid<fsgrids::technical, FS_STENCIL_WIDTH>& technicalGrid, cint i, cint j, cint k, cint& RKCase) {
   #ifdef DEBUG_FSOLVER
   bool ok = true;
   ...
   minRhom =
       min(minRhom, min(moments_SW->at(fsgrids::moments::RHOM), min(moments_SE->at(fsgrids::moments::RHOM), min(moments_NW->at(fsgrids::moments::RHOM), moments_NE->at(fsgrids::moments::RHOM)))));
   maxRhom =
       max(maxRhom, max(moments_SW->at(fsgrids::moments::RHOM), max(moments_SE->at(fsgrids::moments::RHOM), max(moments_NW->at(fsgrids::moments::RHOM), moments_NE->at(fsgrids::moments::RHOM)))));
   ...
   if (Parameters::resistivity > 0) {
      Ex_SW += Parameters::resistivity *
               sqrt((bgb_SW->at(fsgrids::bgbfield::BGBX) + perb_SW->at(fsgrids::bfield::PERBX)) * (bgb_SW->at(fsgrids::bgbfield::BGBX) + perb_SW->at(fsgrids::bfield::PERBX)) +
                    (bgb_SW->at(fsgrids::bgbfield::BGBY) + perb_SW->at(fsgrids::bfield::PERBY)) * (bgb_SW->at(fsgrids::bgbfield::BGBY) + perb_SW->at(fsgrids::bfield::PERBY)) +
                    (bgb_SW->at(fsgrids::bgbfield::BGBZ) + perb_SW->at(fsgrids::bfield::PERBZ)) * (bgb_SW->at(fsgrids::bgbfield::BGBZ) + perb_SW->at(fsgrids::bfield::PERBZ))) /
   ...
   // 2nd order terms:
   Ex_SW += +HALF * ((By_S - HALF * dBydz_S) * (-dmoments_SW->at(fsgrids::dmoments::dVzdy) - dmoments_SW->at(fsgrids::dmoments::dVzdz)) - dBydz_S * Vz0 +
                     SIXTH * dBydx_S * dmoments_SW->at(fsgrids::dmoments::dVzdx));
   Ex_SW += -HALF * ((Bz_W - HALF * dBzdy_W) * (-dmoments_SW->at(fsgrids::dmoments::dVydy) - dmoments_SW->at(fsgrids::dmoments::dVydz)) - dBzdy_W * Vy0 +
                     SIXTH * dBzdx_W * dmoments_SW->at(fsgrids::dmoments::dVydx));
   #endif
   calculateWaveSpeedYZ(perBGrid, momentsGrid, dPerBGrid, dMomentsGrid, BgBGrid, i, j, k, i + 1, j, k, By_S, Bz_W, dBydx_S, dBydz_S, dBzdx_W, dBzdy_W, MINUS, MINUS, minRhom, maxRhom, vA, vS, vW);
   ...
   if (Parameters::resistivity > 0) {
      Ex_SE += Parameters::resistivity *
               sqrt((bgb_SE->at(fsgrids::bgbfield::BGBX) + perb_SE->at(fsgrids::bfield::PERBX)) * (bgb_SE->at(fsgrids::bgbfield::BGBX) + perb_SE->at(fsgrids::bfield::PERBX)) +
                    (bgb_SE->at(fsgrids::bgbfield::BGBY) + perb_SE->at(fsgrids::bfield::PERBY)) * (bgb_SE->at(fsgrids::bgbfield::BGBY) + perb_SE->at(fsgrids::bfield::PERBY)) +
                    (bgb_SE->at(fsgrids::bgbfield::BGBZ) + perb_SE->at(fsgrids::bfield::PERBZ)) * (bgb_SE->at(fsgrids::bgbfield::BGBZ) + perb_SE->at(fsgrids::bfield::PERBZ))) /
   ...
   #ifndef FS_1ST_ORDER_SPACE
   // 2nd order terms:
   Ex_SE += +HALF * ((By_S - HALF * dBydz_S) * (+dmoments_SE->at(fsgrids::dmoments::dVzdy) - dmoments_SE->at(fsgrids::dmoments::dVzdz)) - dBydz_S * Vz0 +
                     SIXTH * dBydx_S * dmoments_SE->at(fsgrids::dmoments::dVzdx));
   Ex_SE += -HALF * ((Bz_E + HALF * dBzdy_E) * (+dmoments_SE->at(fsgrids::dmoments::dVydy) - dmoments_SE->at(fsgrids::dmoments::dVydz)) + dBzdy_E * Vy0 +
                     SIXTH * dBzdx_E * dmoments_SE->at(fsgrids::dmoments::dVydx));
   #endif

   calculateWaveSpeedYZ(
       perBGrid, momentsGrid, dPerBGrid, dMomentsGrid, BgBGrid, i, j - 1, k, i + 1, j - 1, k, By_S, Bz_E, dBydx_S, dBydz_S, dBzdx_E, dBzdy_E, PLUS, MINUS, minRhom, maxRhom, vA, vS, vW);
   c_y = min(Parameters::maxWaveVelocity, sqrt(vA * vA + vS * vS) + vW);
   c_z = c_y;
   ...
   if (Parameters::resistivity > 0) {
      Ex_NW += Parameters::resistivity *
               sqrt((bgb_NW->at(fsgrids::bgbfield::BGBX) + perb_NW->at(fsgrids::bfield::PERBX)) * (bgb_NW->at(fsgrids::bgbfield::BGBX) + perb_NW->at(fsgrids::bfield::PERBX)) +
                    (bgb_NW->at(fsgrids::bgbfield::BGBY) + perb_NW->at(fsgrids::bfield::PERBY)) * (bgb_NW->at(fsgrids::bgbfield::BGBY) + perb_NW->at(fsgrids::bfield::PERBY)) +
                    (bgb_NW->at(fsgrids::bgbfield::BGBZ) + perb_NW->at(fsgrids::bfield::PERBZ)) * (bgb_NW->at(fsgrids::bgbfield::BGBZ) + perb_NW->at(fsgrids::bfield::PERBZ))) /
   ...
   #ifndef FS_1ST_ORDER_SPACE
   // 2nd order terms:
   Ex_NW += +HALF * ((By_N + HALF * dBydz_N) * (-dmoments_NW->at(fsgrids::dmoments::dVzdy) + dmoments_NW->at(fsgrids::dmoments::dVzdz)) + dBydz_N * Vz0 +
                     SIXTH * dBydx_N * dmoments_NW->at(fsgrids::dmoments::dVzdx));
   Ex_NW += -HALF * ((Bz_W - HALF * dBzdy_W) * (-dmoments_NW->at(fsgrids::dmoments::dVydy) + dmoments_NW->at(fsgrids::dmoments::dVydz)) - dBzdy_W * Vy0 +
                     SIXTH * dBzdx_W * dmoments_NW->at(fsgrids::dmoments::dVydx));
   #endif

   calculateWaveSpeedYZ(
       perBGrid, momentsGrid, dPerBGrid, dMomentsGrid, BgBGrid, i, j, k - 1, i + 1, j, k - 1, By_N, Bz_W, dBydx_N, dBydz_N, dBzdx_W, dBzdy_W, MINUS, PLUS, minRhom, maxRhom, vA, vS, vW);
   c_y = min(Parameters::maxWaveVelocity, sqrt(vA * vA + vS * vS) + vW);
   c_z = c_y;
   ay_neg = max(ay_neg, -Vy0 + c_y);
   ay_pos = max(ay_pos, +Vy0 + c_y);
   ...
   if (Parameters::resistivity > 0) {
      Ex_NE += Parameters::resistivity *
               sqrt((bgb_NE->at(fsgrids::bgbfield::BGBX) + perb_NE->at(fsgrids::bfield::PERBX)) * (bgb_NE->at(fsgrids::bgbfield::BGBX) + perb_NE->at(fsgrids::bfield::PERBX)) +
                    (bgb_NE->at(fsgrids::bgbfield::BGBY) + perb_NE->at(fsgrids::bfield::PERBY)) * (bgb_NE->at(fsgrids::bgbfield::BGBY) + perb_NE->at(fsgrids::bfield::PERBY)) +
                    (bgb_NE->at(fsgrids::bgbfield::BGBZ) + perb_NE->at(fsgrids::bfield::PERBZ)) * (bgb_NE->at(fsgrids::bgbfield::BGBZ) + perb_NE->at(fsgrids::bfield::PERBZ))) /
   ...
   #ifndef FS_1ST_ORDER_SPACE
   // 2nd order terms:
   Ex_NE += +HALF * ((By_N + HALF * dBydz_N) * (+dmoments_NE->at(fsgrids::dmoments::dVzdy) + dmoments_NE->at(fsgrids::dmoments::dVzdz)) + dBydz_N * Vz0 +
                     SIXTH * dBydx_N * dmoments_NE->at(fsgrids::dmoments::dVzdx));
   Ex_NE += -HALF * ((Bz_E + HALF * dBzdy_E) * (+dmoments_NE->at(fsgrids::dmoments::dVydy) + dmoments_NE->at(fsgrids::dmoments::dVydz)) + dBzdy_E * Vy0 +
                     SIXTH * dBzdx_E * dmoments_NE->at(fsgrids::dmoments::dVydx));
   #endif

   calculateWaveSpeedYZ(
       perBGrid, momentsGrid, dPerBGrid, dMomentsGrid, BgBGrid, i, j - 1, k - 1, i + 1, j - 1, k - 1, By_N, Bz_E, dBydx_N, dBydz_N, dBzdx_E, dBzdy_E, PLUS, PLUS, minRhom, maxRhom, vA, vS, vW);
   c_y = min(Parameters::maxWaveVelocity, sqrt(vA * vA + vS * vS) + vW);
   c_z = c_y;
   ay_neg = max(ay_neg, -Vy0 + c_y);
   ay_pos = max(ay_pos, +Vy0 + c_y);
...
/*! \brief Low-level electric field propagation function.
 *
 * Computes the upwinded electric field Y component along the cell's corresponding edge as the cross product of B and V in the XZ plane. Also includes the calculation of the maximally allowed time
 * step.
 *
 * Expects that the correct RHO and B fields are being passed, depending on the stage of the Runge-Kutta time stepping method.
 *
 ...
 * \param RKCase Element in the enum defining the Runge-Kutta method steps
 */
void calculateEdgeElectricFieldY(FsGrid<std::array<Real, fsgrids::bfield::N_BFIELD>, FS_STENCIL_WIDTH>& perBGrid, FsGrid<std::array<Real, fsgrids::efield::N_EFIELD>, FS_STENCIL_WIDTH>& EGrid,
                                 FsGrid<std::array<Real, fsgrids::ehall::N_EHALL>, FS_STENCIL_WIDTH>& EHallGrid, FsGrid<std::array<Real, fsgrids::egradpe::N_EGRADPE>, FS_STENCIL_WIDTH>& EGradPeGrid,
                                 FsGrid<std::array<Real, fsgrids::moments::N_MOMENTS>, FS_STENCIL_WIDTH>& momentsGrid, FsGrid<std::array<Real, fsgrids::dperb::N_DPERB>, FS_STENCIL_WIDTH>& dPerBGrid,
                                 FsGrid<std::array<Real, fsgrids::dmoments::N_DMOMENTS>, FS_STENCIL_WIDTH>& dMomentsGrid, FsGrid<std::array<Real, fsgrids::bgbfield::N_BGB>, FS_STENCIL_WIDTH>& BgBGrid,
                                 FsGrid<fsgrids::technical, FS_STENCIL_WIDTH>& technicalGrid, cint i, cint j, cint k, cint& RKCase) {
   ...
   #ifndef FS_1ST_ORDER_SPACE
   // 2nd order terms
   Ey_SW += +HALF * ((Bz_S - HALF * dBzdx_S) * (-dmoments_SW->at(fsgrids::dmoments::dVxdx) - dmoments_SW->at(fsgrids::dmoments::dVxdz)) - dBzdx_S * Vx0 +
                     SIXTH * dBzdy_S * dmoments_SW->at(fsgrids::dmoments::dVxdy));
   Ey_SW += -HALF * ((Bx_W - HALF * dBxdz_W) * (-dmoments_SW->at(fsgrids::dmoments::dVzdx) - dmoments_SW->at(fsgrids::dmoments::dVzdz)) - dBxdz_W * Vz0 +
                     SIXTH * dBxdy_W * dmoments_SW->at(fsgrids::dmoments::dVzdy));
   #endif

   calculateWaveSpeedXZ(perBGrid, momentsGrid, dPerBGrid, dMomentsGrid, BgBGrid, i, j, k, i, j + 1, k, Bx_W, Bz_S, dBxdy_W, dBxdz_W, dBzdx_S, dBzdy_S, MINUS, MINUS, minRhom, maxRhom, vA, vS, vW);
   ...
   #ifndef FS_1ST_ORDER_SPACE
   // 2nd order terms:
   Ey_SE += +HALF * ((Bz_S - HALF * dBzdx_S) * (-dmoments_SE->at(fsgrids::dmoments::dVxdx) + dmoments_SE->at(fsgrids::dmoments::dVxdz)) - dBzdx_S * Vx0 +
                     SIXTH * dBzdy_S * dmoments_SE->at(fsgrids::dmoments::dVxdy));
   Ey_SE += -HALF * ((Bx_E + HALF * dBxdz_E) * (-dmoments_SE->at(fsgrids::dmoments::dVzdx) + dmoments_SE->at(fsgrids::dmoments::dVzdz)) + dBxdz_E * Vz0 +
                     SIXTH * dBxdy_E * dmoments_SE->at(fsgrids::dmoments::dVzdy));
   #endif

   calculateWaveSpeedXZ(
       perBGrid, momentsGrid, dPerBGrid, dMomentsGrid, BgBGrid, i, j, k - 1, i, j + 1, k - 1, Bx_E, Bz_S, dBxdy_E, dBxdz_E, dBzdx_S, dBzdy_S, MINUS, PLUS, minRhom, maxRhom, vA, vS, vW);
   c_z = min(Parameters::maxWaveVelocity, sqrt(vA * vA + vS * vS) + vW);
   c_x = c_z;
   az_neg = max(az_neg, -Vz0 + c_z);
   az_pos = max(az_pos, +Vz0 + c_z);
   ...
   #ifndef FS_1ST_ORDER_SPACE
   // 2nd order terms:
   Ey_NW += +HALF * ((Bz_N + HALF * dBzdx_N) * (+dmoments_NW->at(fsgrids::dmoments::dVxdx) - dmoments_NW->at(fsgrids::dmoments::dVxdz)) + dBzdx_N * Vx0 +
                     SIXTH * dBzdy_N * dmoments_NW->at(fsgrids::dmoments::dVxdy));
   Ey_NW += -HALF * ((Bx_W - HALF * dBxdz_W) * (+dmoments_NW->at(fsgrids::dmoments::dVzdx) - dmoments_NW->at(fsgrids::dmoments::dVzdz)) - dBxdz_W * Vz0 +
                     SIXTH * dBxdy_W * dmoments_NW->at(fsgrids::dmoments::dVzdy));
   #endif

   calculateWaveSpeedXZ(
       perBGrid, momentsGrid, dPerBGrid, dMomentsGrid, BgBGrid, i - 1, j, k, i - 1, j + 1, k, Bx_W, Bz_N, dBxdy_W, dBxdz_W, dBzdx_N, dBzdy_N, PLUS, MINUS, minRhom, maxRhom, vA, vS, vW);
   c_z = min(Parameters::maxWaveVelocity, sqrt(vA * vA + vS * vS) + vW);
   c_x = c_z;
   az_neg = max(az_neg, -Vz0 + c_z);
   az_pos = max(az_pos, +Vz0 + c_z);
   ...
   #ifndef FS_1ST_ORDER_SPACE
   // 2nd order terms:
   Ey_NE += +HALF * ((Bz_N + HALF * dBzdx_N) * (+dmoments_NE->at(fsgrids::dmoments::dVxdx) + dmoments_NE->at(fsgrids::dmoments::dVxdz)) + dBzdx_N * Vx0 +
                     SIXTH * dBzdy_N * dmoments_NE->at(fsgrids::dmoments::dVxdy));
   Ey_NE += -HALF * ((Bx_E + HALF * dBxdz_E) * (+dmoments_NE->at(fsgrids::dmoments::dVzdx) + dmoments_NE->at(fsgrids::dmoments::dVzdz)) + dBxdz_E * Vz0 +
                     SIXTH * dBxdy_E * dmoments_NE->at(fsgrids::dmoments::dVzdy));
   #endif

   calculateWaveSpeedXZ(
       perBGrid, momentsGrid, dPerBGrid, dMomentsGrid, BgBGrid, i - 1, j, k - 1, i - 1, j + 1, k - 1, Bx_E, Bz_N, dBxdy_E, dBxdz_E, dBzdx_N, dBzdy_N, PLUS, PLUS, minRhom, maxRhom, vA, vS, vW);
   c_z = min(Parameters::maxWaveVelocity, sqrt(vA * vA + vS * vS) + vW);
   c_x = c_z;
   az_neg = max(az_neg, -Vz0 + c_z);
   az_pos = max(az_pos, +Vz0 + c_z);
...
/*! \brief Low-level electric field propagation function.
 *
 * Computes the upwinded electric field Z component along the cell's corresponding edge as the cross product of B and V in the XY plane. Also includes the calculation of the maximally allowed time
 * step.
 *
 * Expects that the correct RHO and B fields are being passed, depending on the stage of the Runge-Kutta time stepping method.
 *
 ...
 * \param RKCase Element in the enum defining the Runge-Kutta method steps
 */
void calculateEdgeElectricFieldZ(FsGrid<std::array<Real, fsgrids::bfield::N_BFIELD>, FS_STENCIL_WIDTH>& perBGrid, FsGrid<std::array<Real, fsgrids::efield::N_EFIELD>, FS_STENCIL_WIDTH>& EGrid,
                                 FsGrid<std::array<Real, fsgrids::ehall::N_EHALL>, FS_STENCIL_WIDTH>& EHallGrid, FsGrid<std::array<Real, fsgrids::egradpe::N_EGRADPE>, FS_STENCIL_WIDTH>& EGradPeGrid,
                                 FsGrid<std::array<Real, fsgrids::moments::N_MOMENTS>, FS_STENCIL_WIDTH>& momentsGrid, FsGrid<std::array<Real, fsgrids::dperb::N_DPERB>, FS_STENCIL_WIDTH>& dPerBGrid,
                                 FsGrid<std::array<Real, fsgrids::dmoments::N_DMOMENTS>, FS_STENCIL_WIDTH>& dMomentsGrid, FsGrid<std::array<Real, fsgrids::bgbfield::N_BGB>, FS_STENCIL_WIDTH>& BgBGrid,
                                 FsGrid<fsgrids::technical, FS_STENCIL_WIDTH>& technicalGrid, cint i, cint j, cint k, cint& RKCase) {
   ...
   #ifndef FS_1ST_ORDER_SPACE
   // 2nd order terms:
   Ez_SW += +HALF * ((Bx_S - HALF * dBxdy_S) * (-dmoments_SW->at(fsgrids::dmoments::dVydx) - dmoments_SW->at(fsgrids::dmoments::dVydy)) - dBxdy_S * Vy0 +
                     SIXTH * dBxdz_S * dmoments_SW->at(fsgrids::dmoments::dVydz));
   Ez_SW += -HALF * ((By_W - HALF * dBydx_W) * (-dmoments_SW->at(fsgrids::dmoments::dVxdx) - dmoments_SW->at(fsgrids::dmoments::dVxdy)) - dBydx_W * Vx0 +
                     SIXTH * dBydz_W * dmoments_SW->at(fsgrids::dmoments::dVxdz));
   #endif
   ...
   calculateWaveSpeedXY(perBGrid, momentsGrid, dPerBGrid, dMomentsGrid, BgBGrid, i, j, k, i, j, k + 1, Bx_S, By_W, dBxdy_S, dBxdz_S, dBydx_W, dBydz_W, MINUS, MINUS, minRhom, maxRhom, vA, vS, vW);
   ...
   #ifndef FS_1ST_ORDER_SPACE
   // 2nd order terms:
   Ez_SE += +HALF * ((Bx_S - HALF * dBxdy_S) * (+dmoments_SE->at(fsgrids::dmoments::dVydx) - dmoments_SE->at(fsgrids::dmoments::dVydy)) - dBxdy_S * Vy0 +
                     SIXTH * dBxdz_S * dmoments_SE->at(fsgrids::dmoments::dVydz));
   Ez_SE += -HALF * ((By_E + HALF * dBydx_E) * (+dmoments_SE->at(fsgrids::dmoments::dVxdx) - dmoments_SE->at(fsgrids::dmoments::dVxdy)) + dBydx_E * Vx0 +
                     SIXTH * dBydz_E * dmoments_SE->at(fsgrids::dmoments::dVxdz));
   #endif

   calculateWaveSpeedXY(
       perBGrid, momentsGrid, dPerBGrid, dMomentsGrid, BgBGrid, i - 1, j, k, i - 1, j, k + 1, Bx_S, By_E, dBxdy_S, dBxdz_S, dBydx_E, dBydz_E, PLUS, MINUS, minRhom, maxRhom, vA, vS, vW);
   c_x = min(Parameters::maxWaveVelocity, sqrt(vA * vA + vS * vS) + vW);
   c_y = c_x;
   ax_neg = max(ax_neg, -Vx0 + c_x);
   ax_pos = max(ax_pos, +Vx0 + c_x);
   ...
   #ifndef FS_1ST_ORDER_SPACE
   // 2nd order terms:
   Ez_NW += +HALF * ((Bx_N + HALF * dBxdy_N) * (-dmoments_NW->at(fsgrids::dmoments::dVydx) + dmoments_NW->at(fsgrids::dmoments::dVydy)) + dBxdy_N * Vy0 +
                     SIXTH * dBxdz_N * dmoments_NW->at(fsgrids::dmoments::dVydz));
   Ez_NW += -HALF * ((By_W - HALF * dBydx_W) * (-dmoments_NW->at(fsgrids::dmoments::dVxdx) + dmoments_NW->at(fsgrids::dmoments::dVxdy)) - dBydx_W * Vx0 +
                     SIXTH * dBydz_W * dmoments_NW->at(fsgrids::dmoments::dVxdz));
   #endif

   calculateWaveSpeedXY(
       perBGrid, momentsGrid, dPerBGrid, dMomentsGrid, BgBGrid, i, j - 1, k, i, j - 1, k + 1, Bx_N, By_W, dBxdy_N, dBxdz_N, dBydx_W, dBydz_W, MINUS, PLUS, minRhom, maxRhom, vA, vS, vW);
   c_x = min(Parameters::maxWaveVelocity, sqrt(vA * vA + vS * vS) + vW);
   c_y = c_x;
   ax_neg = max(ax_neg, -Vx0 + c_x);
   ax_pos = max(ax_pos, +Vx0 + c_x);
   ...
   #ifndef FS_1ST_ORDER_SPACE
   // 2nd order terms:
   Ez_NE += +HALF * ((Bx_N + HALF * dBxdy_N) * (+dmoments_NE->at(fsgrids::dmoments::dVydx) + dmoments_NE->at(fsgrids::dmoments::dVydy)) + dBxdy_N * Vy0 +
                     SIXTH * dBxdz_N * dmoments_NE->at(fsgrids::dmoments::dVydz));
   Ez_NE += -HALF * ((By_E + HALF * dBydx_E) * (+dmoments_NE->at(fsgrids::dmoments::dVxdx) + dmoments_NE->at(fsgrids::dmoments::dVxdy)) + dBydx_E * Vx0 +
                     SIXTH * dBydz_E * dmoments_NE->at(fsgrids::dmoments::dVxdz));
   #endif

   calculateWaveSpeedXY(
       perBGrid, momentsGrid, dPerBGrid, dMomentsGrid, BgBGrid, i - 1, j - 1, k, i - 1, j - 1, k + 1, Bx_N, By_E, dBxdy_N, dBxdz_N, dBydx_E, dBydz_E, PLUS, PLUS, minRhom, maxRhom, vA, vS, vW);
   c_x = min(Parameters::maxWaveVelocity, sqrt(vA * vA + vS * vS) + vW);
   c_y = c_x;
   ax_neg = max(ax_neg, -Vx0 + c_x);
   ax_pos = max(ax_pos, +Vx0 + c_x);
 ...
 *
 */
void calculateElectricField(FsGrid<std::array<Real, fsgrids::bfield::N_BFIELD>, FS_STENCIL_WIDTH>& perBGrid, FsGrid<std::array<Real, fsgrids::efield::N_EFIELD>, FS_STENCIL_WIDTH>& EGrid,
                            FsGrid<std::array<Real, fsgrids::ehall::N_EHALL>, FS_STENCIL_WIDTH>& EHallGrid, FsGrid<std::array<Real, fsgrids::egradpe::N_EGRADPE>, FS_STENCIL_WIDTH>& EGradPeGrid,
                            FsGrid<std::array<Real, fsgrids::moments::N_MOMENTS>, FS_STENCIL_WIDTH>& momentsGrid, FsGrid<std::array<Real, fsgrids::dperb::N_DPERB>, FS_STENCIL_WIDTH>& dPerBGrid,
                            FsGrid<std::array<Real, fsgrids::dmoments::N_DMOMENTS>, FS_STENCIL_WIDTH>& dMomentsGrid, FsGrid<std::array<Real, fsgrids::bgbfield::N_BGB>, FS_STENCIL_WIDTH>& BgBGrid,
                            FsGrid<fsgrids::technical, FS_STENCIL_WIDTH>& technicalGrid, cint i, cint j, cint k, SysBoundary& sysBoundaries, cint& RKCase) {
   ...
   if ((bitfield & compute::EX) == compute::EX) {
      calculateEdgeElectricFieldX(perBGrid, EGrid, EHallGrid, EGradPeGrid, momentsGrid, dPerBGrid, dMomentsGrid, BgBGrid, technicalGrid, i, j, k, RKCase);
   } else {
   ...
   if ((bitfield & compute::EY) == compute::EY) {
      calculateEdgeElectricFieldY(perBGrid, EGrid, EHallGrid, EGradPeGrid, momentsGrid, dPerBGrid, dMomentsGrid, BgBGrid, technicalGrid, i, j, k, RKCase);
   } else {
   ...
   if ((bitfield & compute::EZ) == compute::EZ) {
      calculateEdgeElectricFieldZ(perBGrid, EGrid, EHallGrid, EGradPeGrid, momentsGrid, dPerBGrid, dMomentsGrid, BgBGrid, technicalGrid, i, j, k, RKCase);
   } else {
...
void calculateUpwindedElectricFieldSimple(
    FsGrid<std::array<Real, fsgrids::bfield::N_BFIELD>, FS_STENCIL_WIDTH>& perBGrid, FsGrid<std::array<Real, fsgrids::bfield::N_BFIELD>, FS_STENCIL_WIDTH>& perBDt2Grid,
    FsGrid<std::array<Real, fsgrids::efield::N_EFIELD>, FS_STENCIL_WIDTH>& EGrid, FsGrid<std::array<Real, fsgrids::efield::N_EFIELD>, FS_STENCIL_WIDTH>& EDt2Grid,
    FsGrid<std::array<Real, fsgrids::ehall::N_EHALL>, FS_STENCIL_WIDTH>& EHallGrid, FsGrid<std::array<Real, fsgrids::egradpe::N_EGRADPE>, FS_STENCIL_WIDTH>& EGradPeGrid,
    FsGrid<std::array<Real, fsgrids::egradpe::N_EGRADPE>, FS_STENCIL_WIDTH>& EGradPeDt2Grid, FsGrid<std::array<Real, fsgrids::moments::N_MOMENTS>, FS_STENCIL_WIDTH>& momentsGrid,
    ...
    FsGrid<std::array<Real, fsgrids::dmoments::N_DMOMENTS>, FS_STENCIL_WIDTH>& dMomentsGrid, FsGrid<std::array<Real, fsgrids::dmoments::N_DMOMENTS>, FS_STENCIL_WIDTH>& dMomentsDt2Grid,
    FsGrid<std::array<Real, fsgrids::bgbfield::N_BGB>, FS_STENCIL_WIDTH>& BgBGrid, FsGrid<fsgrids::technical, FS_STENCIL_WIDTH>& technicalGrid, SysBoundary& sysBoundaries, cint& RKCase,
      ...
      for (FsGridTools::FsIndex_t k = 0; k < gridDims[2]; k++) {
         for (FsGridTools::FsIndex_t j = 0; j < gridDims[1]; j++) {
            for (FsGridTools::FsIndex_t i = 0; i < gridDims[0]; i++) {
               if (RKCase == RK_ORDER1 || RKCase == RK_ORDER2_STEP2) {
                  calculateElectricField(perBGrid, EGrid, EHallGrid, EGradPeGrid, momentsGrid, dPerBGrid, dMomentsGrid, BgBGrid, technicalGrid, i, j, k, sysBoundaries, RKCase);
               } else { // RKCase == RK_ORDER2_STEP1
                  calculateElectricField(perBDt2Grid, EDt2Grid, EHallGrid, EGradPeDt2Grid, momentsDt2Grid, dPerBGrid, dMomentsDt2Grid, BgBGrid, technicalGrid, i, j, k, sysBoundaries, RKCase);
               }
            }
         }
      }
