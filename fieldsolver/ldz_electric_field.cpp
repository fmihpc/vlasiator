 ...
 */
void calculateWaveSpeedYZ(FsGrid<std::array<Real, fsgrids::bfield::N_BFIELD>, FS_STENCIL_WIDTH>& perBGrid,
                          FsGrid<std::array<Real, fsgrids::moments::N_MOMENTS>, FS_STENCIL_WIDTH>& momentsGrid,
                          FsGrid<std::array<Real, fsgrids::dperb::N_DPERB>, FS_STENCIL_WIDTH>& dPerBGrid,
                          FsGrid<std::array<Real, fsgrids::dmoments::N_DMOMENTS>, FS_STENCIL_WIDTH>& dMomentsGrid,
                          FsGrid<std::array<Real, fsgrids::bgbfield::N_BGB>, FS_STENCIL_WIDTH>& BgBGrid, cint i, cint j,
                          cint k, cint nbi, cint nbj, cint nbk, const Real& By, const Real& Bz, const Real& dBydx,
                          const Real& dBydz, const Real& dBzdx, const Real& dBzdy, const Real& ydir, const Real& zdir,
                          const Real& minRhom, const Real& maxRhom, Real& ret_vA, Real& ret_vS, Real& ret_vW) {
   ...
   A_0 = HALF * (nbr_perb->at(fsgrids::bfield::PERBX) + nbr_bgb->at(fsgrids::bgbfield::BGBX) +
                 perb->at(fsgrids::bfield::PERBX) + bgb->at(fsgrids::bgbfield::BGBX));
   A_X = (nbr_perb->at(fsgrids::bfield::PERBX) + nbr_bgb->at(fsgrids::bgbfield::BGBX)) -
         (perb->at(fsgrids::bfield::PERBX) + bgb->at(fsgrids::bgbfield::BGBX));
   rhom = moments->at(fsgrids::moments::RHOM) + ydir * HALF * dmoments->at(fsgrids::dmoments::drhomdy) +
          zdir * HALF * dmoments->at(fsgrids::dmoments::drhomdz);
   p11 = moments->at(fsgrids::moments::P_11) + ydir * HALF * dmoments->at(fsgrids::dmoments::dp11dy) +
         zdir * HALF * dmoments->at(fsgrids::dmoments::dp11dz);
   p22 = moments->at(fsgrids::moments::P_22) + ydir * HALF * dmoments->at(fsgrids::dmoments::dp22dy) +
         zdir * HALF * dmoments->at(fsgrids::dmoments::dp22dz);
   p33 = moments->at(fsgrids::moments::P_33) + ydir * HALF * dmoments->at(fsgrids::dmoments::dp33dy) +
   ...
   //   perBGrid.DX*pc::CHARGE*sqrt(Bmag2)) : 0.0; // whistler speed
   const Real vW =
       Parameters::ohmHallTerm > 0
           ? sqrt(vA2) * (1 + divideIfNonZero(2 * M_PI * M_PI * pc::MASS_PROTON * pc::MASS_PROTON,
                                              perBGrid.DX * perBGrid.DX * rhom * pc::CHARGE * pc::CHARGE * pc::MU_0) /
                                  sqrt(1 + divideIfNonZero(M_PI * M_PI * pc::MASS_PROTON * pc::MASS_PROTON,
                                                           perBGrid.DX * perBGrid.DX * rhom * pc::CHARGE * pc::CHARGE *
                                                               pc::MU_0)))
           : 0.0; // whistler speed
 ...
 * \param ret_vW Whistler speed returned
 */
void calculateWaveSpeedXZ(FsGrid<std::array<Real, fsgrids::bfield::N_BFIELD>, FS_STENCIL_WIDTH>& perBGrid,
                          FsGrid<std::array<Real, fsgrids::moments::N_MOMENTS>, FS_STENCIL_WIDTH>& momentsGrid,
                          FsGrid<std::array<Real, fsgrids::dperb::N_DPERB>, FS_STENCIL_WIDTH>& dPerBGrid,
                          FsGrid<std::array<Real, fsgrids::dmoments::N_DMOMENTS>, FS_STENCIL_WIDTH>& dMomentsGrid,
                          FsGrid<std::array<Real, fsgrids::bgbfield::N_BGB>, FS_STENCIL_WIDTH>& BgBGrid, cint i, cint j,
                          cint k, cint nbi, cint nbj, cint nbk, const Real& Bx, const Real& Bz, const Real& dBxdy,
                          const Real& dBxdz, const Real& dBzdx, const Real& dBzdy, const Real& xdir, const Real& zdir,
                          const Real& minRhom, const Real& maxRhom, Real& ret_vA, Real& ret_vS, Real& ret_vW) {
   std::array<Real, fsgrids::bfield::N_BFIELD>* perb = perBGrid.get(i, j, k);
   std::array<Real, fsgrids::bfield::N_BFIELD>* nbr_perb = perBGrid.get(nbi, nbj, nbk);
   std::array<Real, fsgrids::moments::N_MOMENTS>* moments = momentsGrid.get(i, j, k);
   ...
   B_0 = HALF * (nbr_perb->at(fsgrids::bfield::PERBY) + nbr_bgb->at(fsgrids::bgbfield::BGBY) +
                 perb->at(fsgrids::bfield::PERBY) + bgb->at(fsgrids::bgbfield::BGBY));
   B_Y = (nbr_perb->at(fsgrids::bfield::PERBY) + nbr_bgb->at(fsgrids::bgbfield::BGBY)) -
         (perb->at(fsgrids::bfield::PERBY) + bgb->at(fsgrids::bgbfield::BGBY));
   rhom = moments->at(fsgrids::moments::RHOM) + xdir * HALF * dmoments->at(fsgrids::dmoments::drhomdx) +
          zdir * HALF * dmoments->at(fsgrids::dmoments::drhomdz);
   p11 = moments->at(fsgrids::moments::P_11) + xdir * HALF * dmoments->at(fsgrids::dmoments::dp11dx) +
         zdir * HALF * dmoments->at(fsgrids::dmoments::dp11dz);
   p22 = moments->at(fsgrids::moments::P_22) + xdir * HALF * dmoments->at(fsgrids::dmoments::dp22dx) +
         zdir * HALF * dmoments->at(fsgrids::dmoments::dp22dz);
   p33 = moments->at(fsgrids::moments::P_33) + xdir * HALF * dmoments->at(fsgrids::dmoments::dp33dx) +
         zdir * HALF * dmoments->at(fsgrids::dmoments::dp33dz);
 ...
 * \param ret_vW Whistler speed returned
 */
void calculateWaveSpeedXY(FsGrid<std::array<Real, fsgrids::bfield::N_BFIELD>, FS_STENCIL_WIDTH>& perBGrid,
                          FsGrid<std::array<Real, fsgrids::moments::N_MOMENTS>, FS_STENCIL_WIDTH>& momentsGrid,
                          FsGrid<std::array<Real, fsgrids::dperb::N_DPERB>, FS_STENCIL_WIDTH>& dPerBGrid,
                          FsGrid<std::array<Real, fsgrids::dmoments::N_DMOMENTS>, FS_STENCIL_WIDTH>& dMomentsGrid,
                          FsGrid<std::array<Real, fsgrids::bgbfield::N_BGB>, FS_STENCIL_WIDTH>& BgBGrid, cint i, cint j,
                          cint k, cint nbi, cint nbj, cint nbk, const Real& Bx, const Real& By, const Real& dBxdy,
                          const Real& dBxdz, const Real& dBydx, const Real& dBydz, const Real& xdir, const Real& ydir,
                          const Real& minRhom, const Real& maxRhom, Real& ret_vA, Real& ret_vS, Real& ret_vW) {
   std::array<Real, fsgrids::bfield::N_BFIELD>* perb = perBGrid.get(i, j, k);
   std::array<Real, fsgrids::bfield::N_BFIELD>* nbr_perb = perBGrid.get(nbi, nbj, nbk);
   std::array<Real, fsgrids::moments::N_MOMENTS>* moments = momentsGrid.get(i, j, k);
   ...
   C_0 = HALF * (nbr_perb->at(fsgrids::bfield::PERBZ) + nbr_bgb->at(fsgrids::bgbfield::BGBZ) +
                 perb->at(fsgrids::bfield::PERBZ) + bgb->at(fsgrids::bgbfield::BGBZ));
   C_Z = (nbr_perb->at(fsgrids::bfield::PERBZ) + nbr_bgb->at(fsgrids::bgbfield::BGBZ)) -
         (perb->at(fsgrids::bfield::PERBZ) + bgb->at(fsgrids::bgbfield::BGBZ));
   rhom = moments->at(fsgrids::moments::RHOM) + xdir * HALF * dmoments->at(fsgrids::dmoments::drhomdx) +
...
void calculateEdgeElectricFieldX(
   ...
   minRhom =
       min(minRhom, min(moments_SW->at(fsgrids::moments::RHOM),
                        min(moments_SE->at(fsgrids::moments::RHOM),
                            min(moments_NW->at(fsgrids::moments::RHOM), moments_NE->at(fsgrids::moments::RHOM)))));
   maxRhom =
       max(maxRhom, max(moments_SW->at(fsgrids::moments::RHOM),
                        max(moments_SE->at(fsgrids::moments::RHOM),
                            max(moments_NW->at(fsgrids::moments::RHOM), moments_NE->at(fsgrids::moments::RHOM)))));
   ...
   if (Parameters::resistivity > 0) {
      Ex_SW += Parameters::resistivity *
               ...
               (dperb_SW->at(fsgrids::dperb::dPERBzdy) / technicalGrid.DY -
                dperb_SW->at(fsgrids::dperb::dPERBydz) / technicalGrid.DZ);
   }
...
#endif
   calculateWaveSpeedYZ(perBGrid, momentsGrid, dPerBGrid, dMomentsGrid, BgBGrid, i, j, k, i + 1, j, k, By_S, Bz_W,
                        dBydx_S, dBydz_S, dBzdx_W, dBzdy_W, MINUS, MINUS, minRhom, maxRhom, vA, vS, vW);
   ...
   if (Parameters::resistivity > 0) {
      Ex_SE += Parameters::resistivity *
               ...
               (dperb_SE->at(fsgrids::dperb::dPERBzdy) / technicalGrid.DY -
                dperb_SE->at(fsgrids::dperb::dPERBydz) / technicalGrid.DZ);
   }
   ...
   calculateWaveSpeedYZ(perBGrid, momentsGrid, dPerBGrid, dMomentsGrid, BgBGrid, i, j - 1, k, i + 1, j - 1, k, By_S,
                        Bz_E, dBydx_S, dBydz_S, dBzdx_E, dBzdy_E, PLUS, MINUS, minRhom, maxRhom, vA, vS, vW);
   c_y = min(Parameters::maxWaveVelocity, sqrt(vA * vA + vS * vS) + vW);
   c_z = c_y;
   ...
   if (Parameters::resistivity > 0) {
      Ex_NW += Parameters::resistivity *
               ...
               (dperb_NW->at(fsgrids::dperb::dPERBzdy) / technicalGrid.DY -
                dperb_NW->at(fsgrids::dperb::dPERBydz) / technicalGrid.DZ);
   }
   ...
   calculateWaveSpeedYZ(perBGrid, momentsGrid, dPerBGrid, dMomentsGrid, BgBGrid, i, j, k - 1, i + 1, j, k - 1, By_N,
                        Bz_W, dBydx_N, dBydz_N, dBzdx_W, dBzdy_W, MINUS, PLUS, minRhom, maxRhom, vA, vS, vW);
   c_y = min(Parameters::maxWaveVelocity, sqrt(vA * vA + vS * vS) + vW);
   c_z = c_y;
   ay_neg = max(ay_neg, -Vy0 + c_y);
   ay_pos = max(ay_pos, +Vy0 + c_y);
   ...
   if (Parameters::resistivity > 0) {
      Ex_NE += Parameters::resistivity *
               ...
               (dperb_NE->at(fsgrids::dperb::dPERBzdy) / technicalGrid.DY -
                dperb_NE->at(fsgrids::dperb::dPERBydz) / technicalGrid.DZ);
   }
   ...
   calculateWaveSpeedYZ(perBGrid, momentsGrid, dPerBGrid, dMomentsGrid, BgBGrid, i, j - 1, k - 1, i + 1, j - 1, k - 1,
...
void calculateEdgeElectricFieldY(
      ...
      Ey_SW += Parameters::resistivity *
               sqrt((bgb_SW->at(fsgrids::bgbfield::BGBX) + perb_SW->at(fsgrids::bfield::PERBX)) *
                    ...
                    (bgb_SW->at(fsgrids::bgbfield::BGBZ) + perb_SW->at(fsgrids::bfield::PERBZ)) *
                        (bgb_SW->at(fsgrids::bgbfield::BGBZ) + perb_SW->at(fsgrids::bfield::PERBZ))) /
               moments_SW->at(fsgrids::moments::RHOQ) / physicalconstants::MU_0 *
               (dperb_SW->at(fsgrids::dperb::dPERBxdz) / technicalGrid.DZ -
                dperb_SW->at(fsgrids::dperb::dPERBzdx) / technicalGrid.DX);
   }
   ...
   calculateWaveSpeedXZ(perBGrid, momentsGrid, dPerBGrid, dMomentsGrid, BgBGrid, i, j, k, i, j + 1, k, Bx_W, Bz_S,
                        dBxdy_W, dBxdz_W, dBzdx_S, dBzdy_S, MINUS, MINUS, minRhom, maxRhom, vA, vS, vW);
      ...
      Ey_SE += Parameters::resistivity *
               sqrt((bgb_SE->at(fsgrids::bgbfield::BGBX) + perb_SE->at(fsgrids::bfield::PERBX)) *
                    ...
                    (bgb_SE->at(fsgrids::bgbfield::BGBZ) + perb_SE->at(fsgrids::bfield::PERBZ)) *
                        (bgb_SE->at(fsgrids::bgbfield::BGBZ) + perb_SE->at(fsgrids::bfield::PERBZ))) /
               moments_SE->at(fsgrids::moments::RHOQ) / physicalconstants::MU_0 *
               (dperb_SE->at(fsgrids::dperb::dPERBxdz) / technicalGrid.DZ -
                dperb_SE->at(fsgrids::dperb::dPERBzdx) / technicalGrid.DX);
   }
   ...
   calculateWaveSpeedXZ(perBGrid, momentsGrid, dPerBGrid, dMomentsGrid, BgBGrid, i, j, k - 1, i, j + 1, k - 1, Bx_E,
                        Bz_S, dBxdy_E, dBxdz_E, dBzdx_S, dBzdy_S, MINUS, PLUS, minRhom, maxRhom, vA, vS, vW);
   c_z = min(Parameters::maxWaveVelocity, sqrt(vA * vA + vS * vS) + vW);
   c_x = c_z;
   az_neg = max(az_neg, -Vz0 + c_z);
   az_pos = max(az_pos, +Vz0 + c_z);
      ...
      Ey_NW += Parameters::resistivity *
               sqrt((bgb_NW->at(fsgrids::bgbfield::BGBX) + perb_NW->at(fsgrids::bfield::PERBX)) *
                    ...
                    (bgb_NW->at(fsgrids::bgbfield::BGBZ) + perb_NW->at(fsgrids::bfield::PERBZ)) *
                        (bgb_NW->at(fsgrids::bgbfield::BGBZ) + perb_NW->at(fsgrids::bfield::PERBZ))) /
               moments_NW->at(fsgrids::moments::RHOQ) / physicalconstants::MU_0 *
               (dperb_NW->at(fsgrids::dperb::dPERBxdz) / technicalGrid.DZ -
                dperb_NW->at(fsgrids::dperb::dPERBzdx) / technicalGrid.DX);
   }
   ...
   calculateWaveSpeedXZ(perBGrid, momentsGrid, dPerBGrid, dMomentsGrid, BgBGrid, i - 1, j, k, i - 1, j + 1, k, Bx_W,
                        Bz_N, dBxdy_W, dBxdz_W, dBzdx_N, dBzdy_N, PLUS, MINUS, minRhom, maxRhom, vA, vS, vW);
   c_z = min(Parameters::maxWaveVelocity, sqrt(vA * vA + vS * vS) + vW);
   c_x = c_z;
   az_neg = max(az_neg, -Vz0 + c_z);
   az_pos = max(az_pos, +Vz0 + c_z);
      ...
      Ey_NE += Parameters::resistivity *
               sqrt((bgb_NE->at(fsgrids::bgbfield::BGBX) + perb_NE->at(fsgrids::bfield::PERBX)) *
                    ...
                    (bgb_NE->at(fsgrids::bgbfield::BGBZ) + perb_NE->at(fsgrids::bfield::PERBZ)) *
                        (bgb_NE->at(fsgrids::bgbfield::BGBZ) + perb_NE->at(fsgrids::bfield::PERBZ))) /
               moments_NE->at(fsgrids::moments::RHOQ) / physicalconstants::MU_0 *
               (dperb_NE->at(fsgrids::dperb::dPERBxdz) / technicalGrid.DZ -
                dperb_NE->at(fsgrids::dperb::dPERBzdx) / technicalGrid.DX);
   }
   ...
   calculateWaveSpeedXZ(perBGrid, momentsGrid, dPerBGrid, dMomentsGrid, BgBGrid, i - 1, j, k - 1, i - 1, j + 1, k - 1,
...
void calculateEdgeElectricFieldZ(
      ...
      Ez_SW += Parameters::resistivity *
               sqrt((bgb_SW->at(fsgrids::bgbfield::BGBX) + perb_SW->at(fsgrids::bfield::PERBX)) *
                    ...
                    (bgb_SW->at(fsgrids::bgbfield::BGBZ) + perb_SW->at(fsgrids::bfield::PERBZ)) *
                        (bgb_SW->at(fsgrids::bgbfield::BGBZ) + perb_SW->at(fsgrids::bfield::PERBZ))) /
               moments_SW->at(fsgrids::moments::RHOQ) / physicalconstants::MU_0 *
               (dperb_SW->at(fsgrids::dperb::dPERBydx) / technicalGrid.DX -
                dperb_SW->at(fsgrids::dperb::dPERBxdy) / technicalGrid.DY);
   }
   ...
   calculateWaveSpeedXY(perBGrid, momentsGrid, dPerBGrid, dMomentsGrid, BgBGrid, i, j, k, i, j, k + 1, Bx_S, By_W,
                        dBxdy_S, dBxdz_S, dBydx_W, dBydz_W, MINUS, MINUS, minRhom, maxRhom, vA, vS, vW);
      ...
      Ez_SE += Parameters::resistivity *
               sqrt((bgb_SE->at(fsgrids::bgbfield::BGBX) + perb_SE->at(fsgrids::bfield::PERBX)) *
                    ...
                    (bgb_SE->at(fsgrids::bgbfield::BGBZ) + perb_SE->at(fsgrids::bfield::PERBZ)) *
                        (bgb_SE->at(fsgrids::bgbfield::BGBZ) + perb_SE->at(fsgrids::bfield::PERBZ))) /
               moments_SE->at(fsgrids::moments::RHOQ) / physicalconstants::MU_0 *
               (dperb_SE->at(fsgrids::dperb::dPERBydx) / technicalGrid.DX -
                dperb_SE->at(fsgrids::dperb::dPERBxdy) / technicalGrid.DY);
   }
   ...
   calculateWaveSpeedXY(perBGrid, momentsGrid, dPerBGrid, dMomentsGrid, BgBGrid, i - 1, j, k, i - 1, j, k + 1, Bx_S,
                        By_E, dBxdy_S, dBxdz_S, dBydx_E, dBydz_E, PLUS, MINUS, minRhom, maxRhom, vA, vS, vW);
   c_x = min(Parameters::maxWaveVelocity, sqrt(vA * vA + vS * vS) + vW);
   c_y = c_x;
   ax_neg = max(ax_neg, -Vx0 + c_x);
   ax_pos = max(ax_pos, +Vx0 + c_x);
      ...
      Ez_NW += Parameters::resistivity *
               sqrt((bgb_NW->at(fsgrids::bgbfield::BGBX) + perb_NW->at(fsgrids::bfield::PERBX)) *
                    ...
                    (bgb_NW->at(fsgrids::bgbfield::BGBZ) + perb_NW->at(fsgrids::bfield::PERBZ)) *
                        (bgb_NW->at(fsgrids::bgbfield::BGBZ) + perb_NW->at(fsgrids::bfield::PERBZ))) /
               moments_NW->at(fsgrids::moments::RHOQ) / physicalconstants::MU_0 *
               (dperb_NW->at(fsgrids::dperb::dPERBydx) / technicalGrid.DX -
                dperb_NW->at(fsgrids::dperb::dPERBxdy) / technicalGrid.DY);
   }
   ...
   calculateWaveSpeedXY(perBGrid, momentsGrid, dPerBGrid, dMomentsGrid, BgBGrid, i, j - 1, k, i, j - 1, k + 1, Bx_N,
                        By_W, dBxdy_N, dBxdz_N, dBydx_W, dBydz_W, MINUS, PLUS, minRhom, maxRhom, vA, vS, vW);
   c_x = min(Parameters::maxWaveVelocity, sqrt(vA * vA + vS * vS) + vW);
   c_y = c_x;
   ax_neg = max(ax_neg, -Vx0 + c_x);
   ax_pos = max(ax_pos, +Vx0 + c_x);
      ...
      Ez_NE += Parameters::resistivity *
               sqrt((bgb_NE->at(fsgrids::bgbfield::BGBX) + perb_NE->at(fsgrids::bfield::PERBX)) *
                    ...
                    (bgb_NE->at(fsgrids::bgbfield::BGBZ) + perb_NE->at(fsgrids::bfield::PERBZ)) *
                        (bgb_NE->at(fsgrids::bgbfield::BGBZ) + perb_NE->at(fsgrids::bfield::PERBZ))) /
               moments_NE->at(fsgrids::moments::RHOQ) / physicalconstants::MU_0 *
               (dperb_NE->at(fsgrids::dperb::dPERBydx) / technicalGrid.DX -
                dperb_NE->at(fsgrids::dperb::dPERBxdy) / technicalGrid.DY);
   }
   ...
   calculateWaveSpeedXY(perBGrid, momentsGrid, dPerBGrid, dMomentsGrid, BgBGrid, i - 1, j - 1, k, i - 1, j - 1, k + 1,
 ...
 *
 */
void calculateElectricField(FsGrid<std::array<Real, fsgrids::bfield::N_BFIELD>, FS_STENCIL_WIDTH>& perBGrid,
                            FsGrid<std::array<Real, fsgrids::efield::N_EFIELD>, FS_STENCIL_WIDTH>& EGrid,
                            FsGrid<std::array<Real, fsgrids::ehall::N_EHALL>, FS_STENCIL_WIDTH>& EHallGrid,
                            FsGrid<std::array<Real, fsgrids::egradpe::N_EGRADPE>, FS_STENCIL_WIDTH>& EGradPeGrid,
                            FsGrid<std::array<Real, fsgrids::moments::N_MOMENTS>, FS_STENCIL_WIDTH>& momentsGrid,
         ...
         for (int j = 0; j < gridDims[1]; j++) {
            for (int i = 0; i < gridDims[0]; i++) {
               if (RKCase == RK_ORDER1 || RKCase == RK_ORDER2_STEP2) {
                  ...
                  calculateElectricField(perBDt2Grid, EDt2Grid, EHallGrid, EGradPeGrid, momentsDt2Grid, dPerBGrid,
                                         dMomentsGrid, BgBGrid, technicalGrid, i, j, k, sysBoundaries, RKCase);
               }
            }
         }
      }
