 ...
 */
void reconstructionCoefficients(FsGrid<std::array<Real, fsgrids::bfield::N_BFIELD>, FS_STENCIL_WIDTH>& perBGrid, FsGrid<std::array<Real, fsgrids::dperb::N_DPERB>, FS_STENCIL_WIDTH>& dPerBGrid,
                                std::array<Real, Rec::N_REC_COEFFICIENTS>& perturbedResult, cint i, cint j, cint k, creal& reconstructionOrder) {
 ...
 */
std::array<Real, 3> interpolatePerturbedB(FsGrid<std::array<Real, fsgrids::bfield::N_BFIELD>, FS_STENCIL_WIDTH>& perBGrid,
                                          ...
                                          std::map<std::array<int, 3>, std::array<Real, Rec::N_REC_COEFFICIENTS>>& reconstructionCoefficientsCache, cint i, cint j, cint k,
   ...
   // Eq. (7) Balsara 2009
   interpolatedB[0] = rc[Rec::a_0] + rc[Rec::a_x] * xLocal[0] + rc[Rec::a_y] * xLocal[1] + rc[Rec::a_z] * xLocal[2] + rc[Rec::a_xx] * (xLocal[0] * xLocal[0] - TWELWTH) +
                      rc[Rec::a_xy] * xLocal[0] * xLocal[1] + rc[Rec::a_xz] * xLocal[0] * xLocal[2];
   // Eq. (8) Balsara 2009
   interpolatedB[1] = rc[Rec::b_0] + rc[Rec::b_x] * xLocal[0] + rc[Rec::b_y] * xLocal[1] + rc[Rec::b_z] * xLocal[2] + rc[Rec::b_yy] * (xLocal[1] * xLocal[1] - TWELWTH) +
                      rc[Rec::b_xy] * xLocal[0] * xLocal[1] + rc[Rec::b_yz] * xLocal[1] * xLocal[2];
   // Eq. (9) Balsara 2009
   interpolatedB[2] = rc[Rec::c_0] + rc[Rec::c_x] * xLocal[0] + rc[Rec::c_y] * xLocal[1] + rc[Rec::c_z] * xLocal[2] + rc[Rec::c_zz] * (xLocal[2] * xLocal[2] - TWELWTH) +
                      rc[Rec::c_xz] * xLocal[0] * xLocal[2] + rc[Rec::c_yz] * xLocal[1] * xLocal[2];
 ...
 * \param x 3D global simulation x,y,z coordinates of point to interpolate to
 */
std::array<Real, 3> interpolateCurlB(FsGrid<std::array<Real, fsgrids::bfield::N_BFIELD>, FS_STENCIL_WIDTH>& perBGrid, FsGrid<std::array<Real, fsgrids::dperb::N_DPERB>, FS_STENCIL_WIDTH>& dPerBGrid,
                                     FsGrid<fsgrids::technical, FS_STENCIL_WIDTH>& technicalGrid,
                                     std::map<std::array<int, 3>, std::array<Real, Rec::N_REC_COEFFICIENTS>>& reconstructionCoefficientsCache, cint i, cint j, cint k, const std::array<Real, 3> x) {
   cuint cellSysBoundaryFlag = technicalGrid.get(i, j, k)->sysBoundaryFlag;
   if (cellSysBoundaryFlag != sysboundarytype::NOT_SYSBOUNDARY) {
      std::array<Real, 3> zero = {0, 0, 0};
      return zero;
...
#define BALSARA_CURLB_IMPLEMENTATION
   ...
   interpolatedCurlB[0] =
       (12 * rc[Rec::c_yzz] * xLocal[2] * xLocal[2] + 24 * rc[Rec::c_yyz] * xLocal[1] * xLocal[2] - 24 * rc[Rec::b_yzz] * xLocal[1] * xLocal[2] + 12 * rc[Rec::c_xyz] * xLocal[0] * xLocal[2] +
        12 * rc[Rec::c_yz] * xLocal[2] - 24 * rc[Rec::b_zz] * xLocal[2] - 12 * rc[Rec::b_yyz] * xLocal[1] * xLocal[1] - 12 * rc[Rec::b_xyz] * xLocal[0] * xLocal[1] + 24 * rc[Rec::c_yy] * xLocal[1] -
        12 * rc[Rec::b_yz] * xLocal[1] + 12 * rc[Rec::c_xy] * xLocal[0] - 12 * rc[Rec::b_xz] * xLocal[0] - rc[Rec::c_yzz] + 12 * rc[Rec::c_y] - 12 * rc[Rec::b_z] + rc[Rec::b_yyz]) /
       12;
   // See that minus if you ever copy again from wxMaxima!
   interpolatedCurlB[1] =
       -(12 * rc[Rec::c_xzz] * xLocal[2] * xLocal[2] + 12 * rc[Rec::c_xyz] * xLocal[1] * xLocal[2] + 24 * rc[Rec::c_xxz] * xLocal[0] * xLocal[2] - 24 * rc[Rec::a_xzz] * xLocal[0] * xLocal[2] +
         12 * rc[Rec::c_xz] * xLocal[2] - 24 * rc[Rec::a_zz] * xLocal[2] - 12 * rc[Rec::a_xyz] * xLocal[0] * xLocal[1] + 12 * rc[Rec::c_xy] * xLocal[1] - 12 * rc[Rec::a_yz] * xLocal[1] -
         12 * rc[Rec::a_xxz] * xLocal[0] * xLocal[0] + 24 * rc[Rec::c_xx] * xLocal[0] - 12 * rc[Rec::a_xz] * xLocal[0] - rc[Rec::c_xzz] + 12 * rc[Rec::c_x] - 12 * rc[Rec::a_z] + rc[Rec::a_xxz]) /
       12;
   interpolatedCurlB[2] = (12 * rc[Rec::b_xyz] * xLocal[1] * xLocal[2] - 12 * rc[Rec::a_xyz] * xLocal[0] * xLocal[2] + 12 * rc[Rec::b_xz] * xLocal[2] - 12 * rc[Rec::a_yz] * xLocal[2] +
                           12 * rc[Rec::b_xyy] * xLocal[1] * xLocal[1] + 24 * rc[Rec::b_xxy] * xLocal[0] * xLocal[1] - 24 * rc[Rec::a_xyy] * xLocal[0] * xLocal[1] + 12 * rc[Rec::b_xy] * xLocal[1] -
                           24 * rc[Rec::a_yy] * xLocal[1] - 12 * rc[Rec::a_xxy] * xLocal[0] * xLocal[0] + 24 * rc[Rec::b_xx] * xLocal[0] - 12 * rc[Rec::a_xy] * xLocal[0] - rc[Rec::b_xyy] +
                           12 * rc[Rec::b_x] - 12 * rc[Rec::a_y] + rc[Rec::a_xxy]) /
                          12;
   ...
   if (lfsc[0] == -1 || lfsc[1] == -1 || lfsc[2] == -1) {
      cerr << "interpolateCurlB: Trying to access nonlocal cell at " << x[0] << ", " << x[1] << ", " << x[2] << ", which would be local coordinate " << lfsc[0] << ", " << lfsc[1] << ", " << lfsc[2]
           << endl;
      return {0, 0, 0};
   }
