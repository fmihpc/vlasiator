...
void calculateDerivatives(
   ...
   if (calculateMoments) {
      ...
      dMoments->at(fsgrids::dmoments::dPedx) = Pe_const * limiter(
   ...
   // pres_e = const * np.power(rho_e, index)
   dMoments->at(fsgrids::dmoments::dPedy) = Pe_const * limiter(
                                                          pow(leftMoments->at(fsgrids::moments::RHOQ) / physicalconstants::CHARGE, Parameters::electronPTindex),
                                                          pow(centMoments->at(fsgrids::moments::RHOQ) / physicalconstants::CHARGE, Parameters::electronPTindex),
                                                          pow(rghtMoments->at(fsgrids::moments::RHOQ) / physicalconstants::CHARGE, Parameters::electronPTindex)
                                                       );
   ...
   if (calculateMoments) {
      // pres_e = const * np.power(rho_e, index)
      dMoments->at(fsgrids::dmoments::dPedz) = Pe_const * limiter(
                                                             pow(leftMoments->at(fsgrids::moments::RHOQ) / physicalconstants::CHARGE, Parameters::electronPTindex),
                                                             pow(centMoments->at(fsgrids::moments::RHOQ) / physicalconstants::CHARGE, Parameters::electronPTindex),
                                                             pow(rghtMoments->at(fsgrids::moments::RHOQ) / physicalconstants::CHARGE, Parameters::electronPTindex)
                                                          );
...
void calculateBVOLDerivatives(
   FsGrid<std::array<Real, fsgrids::volfields::N_VOL>, FS_STENCIL_WIDTH>& volGrid, FsGrid<fsgrids::technical, FS_STENCIL_WIDTH>& technicalGrid, cint i, cint j, cint k, SysBoundary& sysBoundaries
) {
...
void calculateScaledDeltas(SpatialCell* cell, std::vector<SpatialCell*>& neighbors) {
   ...
   std::array<Real, 6> myPressure{
      cell->parameters[CellParams::P_11],
