 ...
 * The three-point limiter functions are minmod, MC, superbee and vanLeer. Their helper functions are the two-point
 * minmod and the sign function.
 */
...
template <typename T> inline T vanLeer(const T& left, const T& cent, const T& right) {
   ...
   /*
   if (rvalue > 1e10) {
      std::cerr << "ERROR vanLeer rvalue too large " << std::max((right-cent)*(cent-left),ZERO) << '\t' << denumerator
   << std::endl; std::cerr << "\t input values are " << left << '\t' << cent << '\t' << right << std::endl; std::cerr <<
   "\t numerator   " << TWO*std::max((right-cent)*(cent-left),ZERO) << std::endl; std::cerr << "\t denumerator " <<
