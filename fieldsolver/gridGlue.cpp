...
/*Compute coupling DCCRG <=> FSGRID
  ...
  onDccrgMapRemoteProcess   maps fsgrid processes (key) => set of dccrg cellIDs owned by current rank that map to  the
  fsgrid cells owned by fsgrid process (val)

  onFsgridMapRemoteProcess  maps dccrg processes  (key) => set of dccrg cellIDs owned by dccrg-process that map to
  current rank fsgrid cells onFsgridMapCells          maps remote dccrg CellIDs to local fsgrid cells
*/
...
void filterMoments(dccrg::Dccrg<SpatialCell, dccrg::Cartesian_Geometry>& mpiGrid,
   ...
   const static Real kernel[5][5][5] = {
       ...
       {{2, 4, 6, 4, 2}, {4, 8, 12, 8, 4}, {6, 12, 18, 12, 6}, {4, 8, 12, 8, 4}, {2, 4, 6, 4, 2}},

       {{3, 6, 9, 6, 3}, {6, 12, 18, 12, 6}, {9, 18, 27, 18, 9}, {6, 12, 18, 12, 6}, {3, 6, 9, 6, 3}},
...
void feedMomentsIntoFsGrid(dccrg::Dccrg<SpatialCell, dccrg::Cartesian_Geometry>& mpiGrid,
   ...
   for (auto const& receives : onFsgridMapRemoteProcess) {
      ...
      MPI_Irecv(receivedData[process].data(),
                count * fsgrids::moments::N_MOMENTS * sizeof(Real),
                MPI_BYTE,
                process,
                1,
                MPI_COMM_WORLD,
...
void getFieldsFromFsGrid(FsGrid<std::array<Real, fsgrids::volfields::N_VOL>, FS_STENCIL_WIDTH>& volumeFieldsGrid,
   ...
   // computeCoupling
   computeCoupling(
       mpiGrid, cells, volumeFieldsGrid, onDccrgMapRemoteProcess, onFsgridMapRemoteProcess, onFsgridMapCells);
      ...
      for (auto const dccrgCell : snd.second) {
         ...
         for (auto const fsgridCell : fsgridCells) {
            ...
            sendBuffer[ii].sums[FieldsToCommunicate::dPERBXVOLdx] +=
                volcell->at(fsgrids::volfields::dPERBXVOLdx) / technicalGrid.DX;
            sendBuffer[ii].sums[FieldsToCommunicate::dPERBXVOLdy] +=
                volcell->at(fsgrids::volfields::dPERBXVOLdy) / technicalGrid.DY;
            sendBuffer[ii].sums[FieldsToCommunicate::dPERBXVOLdz] +=
                volcell->at(fsgrids::volfields::dPERBXVOLdz) / technicalGrid.DZ;
            sendBuffer[ii].sums[FieldsToCommunicate::dPERBYVOLdx] +=
                volcell->at(fsgrids::volfields::dPERBYVOLdx) / technicalGrid.DX;
            sendBuffer[ii].sums[FieldsToCommunicate::dPERBYVOLdy] +=
                volcell->at(fsgrids::volfields::dPERBYVOLdy) / technicalGrid.DY;
            sendBuffer[ii].sums[FieldsToCommunicate::dPERBYVOLdz] +=
                volcell->at(fsgrids::volfields::dPERBYVOLdz) / technicalGrid.DZ;
            sendBuffer[ii].sums[FieldsToCommunicate::dPERBZVOLdx] +=
                volcell->at(fsgrids::volfields::dPERBZVOLdx) / technicalGrid.DX;
            sendBuffer[ii].sums[FieldsToCommunicate::dPERBZVOLdy] +=
                volcell->at(fsgrids::volfields::dPERBZVOLdy) / technicalGrid.DY;
            sendBuffer[ii].sums[FieldsToCommunicate::dPERBZVOLdz] +=
                volcell->at(fsgrids::volfields::dPERBZVOLdz) / technicalGrid.DZ;
   ...
   for (auto const& sends : onFsgridMapRemoteProcess) {
      ...
      MPI_Isend(sendData[remoteRank].data(),
                count * sizeof(Average),
                MPI_BYTE,
                remoteRank,
                1,
