...
// all data
const uint64_t ALL_DATA = CELL_PARAMETERS | CELL_DERIVATIVES | CELL_BVOL_DERIVATIVES | VEL_BLOCK_DATA |
                          CELL_SYSBOUNDARYFLAG | POP_METADATA | RANDOMGEN;

// all data, except the distribution function
const uint64_t ALL_SPATIAL_DATA =
    CELL_PARAMETERS | CELL_DERIVATIVES | CELL_BVOL_DERIVATIVES | CELL_SYSBOUNDARYFLAG | POP_METADATA | RANDOMGEN;
} // namespace Transfer

typedef std::array<unsigned int, 3>
    velocity_cell_indices_t; /**< Defines the indices of a velocity cell in a velocity block.
...
typedef std::array<vmesh::LocalID, 3>
    velocity_block_indices_t; /**< Defines the indices of a velocity block in the velocity grid.
                               * Indices start from 0 and the first value is the index in x direction.
...
struct Population {
   ...
   vmesh::VelocityMesh<vmesh::GlobalID, vmesh::LocalID> vmesh;   /**< Velocity mesh. Contains all velocity blocks that
                                                                  * exist   in this spatial cell. Cells are identified by
                                                                  * their unique   global IDs.*/
...
public:
   ...
   template <int PAD>
   void fetch_data(const vmesh::GlobalID& blockGID, const vmesh::VelocityMesh<vmesh::GlobalID, vmesh::LocalID>& vmesh,
                   const Realf* src, Realf* array);
   template <int PAD>
   void fetch_acc_data(const vmesh::GlobalID& blockGID, const int& dim,
                       vmesh::VelocityMesh<vmesh::GlobalID, vmesh::LocalID>& vmesh, const Realf* src, Realf* array,
   ...
   vmesh::GlobalID get_velocity_block(const uint popID, const velocity_block_indices_t indices,
                                      const uint8_t& refLevel) const;
   vmesh::GlobalID get_velocity_block(const uint popID, const Real* coords, const uint8_t& refLevel = 0) const;
   vmesh::GlobalID get_velocity_block(const uint popID, const Real vx, const Real vy, const Real vz,
                                      const uint8_t& refLevel = 0) const;
   ...
   void get_velocity_block_children_local_ids(const vmesh::GlobalID& blockGID,
                                              std::vector<vmesh::LocalID>& childrenLIDs, const uint popID);
   ...
   unsigned int get_velocity_cell(const uint popID, const vmesh::GlobalID velocity_block, const Real vx, const Real vy,
                                  const Real vz) const;
   Real get_velocity_cell_vx_min(const uint popID, const vmesh::GlobalID velocity_block,
                                 const unsigned int velocity_cell) const;
   Real get_velocity_cell_vx_max(const uint popID, const vmesh::GlobalID velocity_block,
                                 const unsigned int velocity_cell) const;
   Real get_velocity_cell_vy_min(const uint popID, const vmesh::GlobalID velocity_block,
                                 const unsigned int velocity_cell) const;
   Real get_velocity_cell_vy_max(const uint popID, const vmesh::GlobalID velocity_block,
                                 const unsigned int velocity_cell) const;
   Real get_velocity_cell_vz_min(const uint popID, const vmesh::GlobalID velocity_block,
                                 const unsigned int velocity_cell) const;
   Real get_velocity_cell_vz_max(const uint popID, const vmesh::GlobalID velocity_block,
                                 const unsigned int velocity_cell) const;
   ...
   void adjust_velocity_blocks(const std::vector<SpatialCell*>& spatial_neighbors, const uint popID,
   ...
   std::tuple<void*, int, MPI_Datatype> get_mpi_datatype(const CellID cellID, const int sender_rank,
   ...
   // Member variables //
   std::array<Real, bvolderivatives::N_BVOL_DERIVATIVES>
       derivativesBVOL; /**< Derivatives of BVOL needed by the acceleration.
                         * Separate array because it does not need to be communicated.*/
   // Real parameters[CellParams::N_SPATIAL_CELL_PARAMS];                     /**< Bulk variables in this spatial
   // cell.*/
   ...
   // vmesh::LocalID mpi_number_of_blocks;                                    /**< Number of blocks in
   // mpi_velocity_block_list.*/ Realf* neighbor_block_data;                                             /**< Pointers
   // for translation operator. We can point to neighbor
   //                                                                          * cell block data. We do not allocate
   //                                                                          memory for the pointer.*/
   // vmesh::LocalID neighbor_number_of_blocks;
   std::array<Realf*, MAX_NEIGHBORS_PER_DIM>
       neighbor_block_data; /**< Pointers for translation operator. We can point to neighbor
   ...
   std::vector<vmesh::GlobalID> velocity_block_with_content_list; /**< List of existing cells with content, only
                                                                   * up-to-date after call to update_has_content().*/
   vmesh::LocalID velocity_block_with_content_list_size; /**< Size of vector. Needed for MPI communication of size
                                                            before actual list transfer.*/
   std::vector<vmesh::GlobalID>
       velocity_block_with_no_content_list; /**< List of existing cells with no content, only up-to-date after
   ...
   static bool
       mpiTransferAtSysBoundaries; /**< Do we only transfer data at boundaries (true), or in the whole system (false).*/
...
private:
   ...
   void merge_values_recursive(const uint popID, vmesh::GlobalID parentGID, vmesh::GlobalID blockGID, uint8_t refLevel,
                               bool recursive, const Realf* data, std::set<vmesh::GlobalID>& blockRemovalList);
   ...
   // char rngStateBuffer[256];                                                 /**< Random number generator state
   // buffer.*/ random_data rngDataBuffer;                                                /**< Random number generator
   // data buffer.*/

   // Temporary mesh used in acceleration and propagation.
   vmesh::VelocityMesh<vmesh::GlobalID, vmesh::LocalID>
       vmeshTemp; /**< Temporary velocity mesh that is used in Vlasov solver.
...
template <int PAD>
inline void SpatialCell::fetch_acc_data(const vmesh::GlobalID& blockGID, const int& dim,
                                        vmesh::VelocityMesh<vmesh::GlobalID, vmesh::LocalID>& vmesh, const Realf* src,
                                        Realf* array, Real cellSizeFractions[2]) {
      ...
      for (int i_nbr_off = -1; i_nbr_off < 2; i_nbr_off += 2) { // Copy values from x face neighbors:
         ...
         if (nbrIDs.size() > 0) {     // This block has at least one existing neighbor
            if (refLevelDiff == -1) { // Neighbor is one level coarser, interpolate
               if (nbrIDs[0] == invalid_local_id())
                  ptr = null_block_data.data(); // (this check might not be necessary here)
               else
                  ptr = src + nbrIDs[0] * WID3;
               for (uint32_t i = 0; i < PAD; ++i)
                  for (uint32_t k = 0; k < WID; ++k)
               ...
               for (uint32_t i = 0; i < PAD; ++i)
                  for (uint32_t k = 0; k < WID; ++k)
                     for (uint32_t j = 0; j < WID; ++j) {
                        ...
                        int index = (k / 2) * 2 + j / 2;
                        if (nbrIDs[index] == invalid_local_id())
                           ptr = null_block_data.data();
                        else
                           ptr = src + nbrIDs[index] * WID3;
...
template <int PAD>
inline void SpatialCell::fetch_data(const vmesh::GlobalID& blockGID,
                                    const vmesh::VelocityMesh<vmesh::GlobalID, vmesh::LocalID>& vmesh, const Realf* src,
                                    Realf* array) {
            ...
            for (uint32_t i = 0; i < PAD; ++i)
               for (uint32_t k = 0; k < WID; ++k)
                  for (uint32_t j = 0; j < WID; ++j) {
                     ...
                     pos[1] = 2 * (j_block % 2) + j / 2 + 0.5;
                     pos[2] = 2 * (k_block % 2) + k / 2 + 0.5;
                     array[vblock::padIndex<PAD>(i_trgt + i, j + PAD, k + PAD)] =
                         vblock::interp_yz<vblock::interpmethod::NGP>(pos, ptr);
                  }
            ...
            for (uint32_t i = 0; i < PAD; ++i)
               for (uint32_t k = 0; k < WID; ++k)
                  for (uint32_t j = 0; j < WID; ++j) {
                     ...
                     pos[2] = 2 * (k % 2) + 1;
                     array[vblock::padIndex<PAD>(i_trgt + i, j + PAD, k + PAD)] =
                         vblock::interp_yz<vblock::interpmethod::CIC>(pos, ptr);
                  }
         }
      } else { // Neighbor does not exist, return zero values
         for (uint32_t i = 0; i < PAD; ++i)
            ...
            for (uint32_t j = 0; j < PAD; ++j)
               for (uint32_t k = 0; k < WID; ++k)
                  for (uint32_t i = 0; i < WID; ++i) {
                     ...
                     pos[1] = crd + j;
                     pos[2] = 2 * (k_block % 2) + k / 2 + 0.5;
                     array[vblock::padIndex<PAD>(i + PAD, j_trgt + j, k + PAD)] =
                         vblock::interp_xz<vblock::interpmethod::NGP>(pos, ptr);
                  }
         } else if (refLevelDiff == 0) { // nbr at same level, simple data copy
            if (nbrIDs[0] == invalid_local_id())
               ptr = null_block_data.data();
            ...
            for (uint32_t j = 0; j < PAD; ++j)
               for (uint32_t k = 0; k < WID; ++k)
                  for (uint32_t i = 0; i < WID; ++i) {
                     ...
                     pos[2] = 2 * (k % 2) + 1;
                     array[vblock::padIndex<PAD>(i + PAD, j_trgt + j, k + PAD)] =
                         vblock::interp_xz<vblock::interpmethod::CIC>(pos, ptr);
                  }
         }
      } else { // Neighbor does not exist, return zero values
         for (uint32_t j = 0; j < PAD; ++j)
            ...
            for (uint32_t k = 0; k < PAD; ++k)
               for (uint32_t j = 0; j < WID; ++j)
                  for (uint32_t i = 0; i < WID; ++i) {
                     ...
                     pos[1] = 2 * (j_block % 2) + j / 2 + 0.5;
                     pos[2] = crd + k;
                     array[vblock::padIndex<PAD>(i + PAD, j + PAD, k_trgt + k)] =
                         vblock::interp_xy<vblock::interpmethod::NGP>(pos, ptr);
                  }
         } else if (refLevelDiff == 0) { // nbr at same level, simple data copy
            if (nbrIDs[0] == invalid_local_id())
               ptr = null_block_data.data();
            ...
            for (uint32_t k = 0; k < PAD; ++k)
               for (uint32_t j = 0; j < WID; ++j)
                  for (uint32_t i = 0; i < WID; ++i) {
                     ...
                     pos[2] = crd + k;
                     array[vblock::padIndex<PAD>(i + PAD, j + PAD, k_trgt + k)] =
                         vblock::interp_xy<vblock::interpmethod::CIC>(pos, ptr);
                  }
         }
      } else { // Neighbor does not exist, return zero values
         for (uint32_t k = 0; k < PAD; ++k)
...
inline Realf* SpatialCell::get_data(const vmesh::LocalID& blockLID, const uint popID) {
   ...
   if (blockLID >= populations[popID].blockContainer.size()) {
      std::cerr << "ERROR, block LID out of bounds, blockContainer.size() " << populations[popID].blockContainer.size()
                << " in ";
      std::cerr << __FILE__ << ":" << __LINE__ << std::endl;
      exit(1);
   }
...
inline Real* SpatialCell::get_cell_parameters() { return parameters.data(); }

inline const Real* SpatialCell::get_cell_parameters() const { return parameters.data(); }
...
inline int SpatialCell::get_number_of_populations() const { return populations.size(); }

inline Population& SpatialCell::get_population(const uint popID) { return populations[popID]; }

inline const Population& SpatialCell::get_population(const uint popID) const { return populations[popID]; }

inline void SpatialCell::set_population(const Population& pop, cuint popID) { this->populations[popID] = pop; }
...
inline void SpatialCell::get_velocity_block_coordinates(const uint popID, const vmesh::GlobalID& globalID,
                                                        Real* coords) {
 ...
 */
inline vmesh::GlobalID SpatialCell::get_velocity_block(const uint popID, const velocity_block_indices_t indices,
                                                       const uint8_t& refLevel) const {
   return populations[popID].vmesh.getGlobalID(refLevel, indices[0], indices[1], indices[2]);
}

inline vmesh::GlobalID SpatialCell::get_velocity_block(const uint popID, vmesh::GlobalID blockIndices[3],
                                                       const uint8_t& refLevel) const {
 ...
 */
inline vmesh::GlobalID SpatialCell::get_velocity_block(const uint popID, const Real vx, const Real vy, const Real vz,
                                                       const uint8_t& refLevel) const {
   Real coords[3] = {vx, vy, vz};
   return populations[popID].vmesh.getGlobalID(refLevel, coords);
}

inline vmesh::GlobalID SpatialCell::get_velocity_block(const uint popID, const Real* coords,
                                                       const uint8_t& refLevel) const {
   return populations[popID].vmesh.getGlobalID(refLevel, coords);
}

inline vmesh::GlobalID SpatialCell::get_velocity_block_child(const uint popID, const vmesh::GlobalID& blockGID,
                                                             const uint8_t& refLevel, const int& i_cell,
                                                             const int& j_cell, const int& k_cell) {
...
inline void SpatialCell::get_velocity_block_children_local_ids(const vmesh::GlobalID& blockGID,
...
inline vmesh::GlobalID SpatialCell::get_velocity_block_global_id(const vmesh::LocalID& blockLID,
...
inline vmesh::LocalID SpatialCell::get_velocity_block_local_id(const vmesh::GlobalID& blockGID,
                                                               const uint popID) const {
#ifdef DEBUG_SPATIAL_CELL
   if (popID >= populations.size()) {
      std::cerr << "ERROR, popID " << popID << " exceeds populations.size() " << populations.size() << " in ";
...
inline unsigned int SpatialCell::get_velocity_cell(const velocity_cell_indices_t indices) const {
   if (indices[0] >= block_vx_length || indices[1] >= block_vy_length || indices[2] >= block_vz_length) {
 ...
 */
inline unsigned int SpatialCell::get_velocity_cell(const uint popID, const vmesh::GlobalID velocity_block,
                                                   const Real vx, const Real vy, const Real vz) const {
   ...
   if (vx < block_vx_min || vx >= block_vx_max || vy < block_vy_min || vy >= block_vy_max || vz < block_vz_min ||
       vz >= block_vz_max) {
      return error_velocity_cell;
   }
 ...
 */
inline Real SpatialCell::get_velocity_cell_vx_min(const uint popID, const vmesh::GlobalID velocity_block,
                                                  const unsigned int velocity_cell) const {
 ...
 */
inline Real SpatialCell::get_velocity_cell_vx_max(const uint popID, const vmesh::GlobalID velocity_block,
                                                  const unsigned int velocity_cell) const {
   if (velocity_cell == error_velocity_cell) {
      return std::numeric_limits<Real>::quiet_NaN();
   }
...
/*!
 Returns the edge where given velocity cell in the given velocity block starts.
 */
inline Real SpatialCell::get_velocity_cell_vy_min(const uint popID, const vmesh::GlobalID velocity_block,
                                                  const unsigned int velocity_cell) const {
   if (velocity_cell == error_velocity_cell) {
      return std::numeric_limits<Real>::quiet_NaN();
   }
...
/*!
 Returns the edge where given velocity cell in the given velocity block ends.
 */
inline Real SpatialCell::get_velocity_cell_vy_max(const uint popID, const vmesh::GlobalID velocity_block,
                                                  const unsigned int velocity_cell) const {
   if (velocity_cell == error_velocity_cell) {
      return std::numeric_limits<Real>::quiet_NaN();
   }
...
/*!
 Returns the edge where given velocity cell in the given velocity block starts.
 */
inline Real SpatialCell::get_velocity_cell_vz_min(const uint popID, const vmesh::GlobalID velocity_block,
                                                  const unsigned int velocity_cell) const {
   if (velocity_cell == error_velocity_cell) {
      return std::numeric_limits<Real>::quiet_NaN();
   }
...
/*!
 Returns the edge where given velocity cell in the given velocity block ends.
 */
inline Real SpatialCell::get_velocity_cell_vz_max(const uint popID, const vmesh::GlobalID velocity_block,
                                                  const unsigned int velocity_cell) const {
   if (velocity_cell == error_velocity_cell) {
      return std::numeric_limits<Real>::quiet_NaN();
   }
 ...
 */
inline void SpatialCell::set_value(const Real vx, const Real vy, const Real vz, const Realf value, const uint popID) {
#ifdef DEBUG_SPATIAL_CELL
   if (popID >= populations.size()) {
      std::cerr << "ERROR, popID " << popID << " exceeds populations.size() " << populations.size() << " in ";
      std::cerr << __FILE__ << ":" << __LINE__ << std::endl;
...
inline void SpatialCell::increment_value(const Real vx, const Real vy, const Real vz, const Realf value,
   ...
   if (blockLID == invalid_local_id()) {
      std::cerr << __FILE__ << ":" << __LINE__ << " block_ptr == NULL" << std::endl;
      abort();
   }
...
inline bool SpatialCell::add_velocity_block(const vmesh::GlobalID& block, const uint popID) {
   ...
   for (unsigned int i = 0; i < WID * WID * WID; ++i)
      data[i] = 0;
   ...
   // vmesh::VelocityMesh<vmesh::GlobalID,vmesh::LocalID>::getBlockInfo(block,get_block_parameters(
   // blockContainer.push_back() ));
   return success;
}
...
inline bool SpatialCell::add_velocity_block_octant(const vmesh::GlobalID& blockGID, const uint popID) {
   ...
   if (populations[popID].vmesh.getLocalID(blockGID) != invalid)
      return false;
   if (populations[popID].vmesh.count(blockGID) > 0)
      return false;
 ...
 if (vmesh.getLocalID(parentGID) != invalid) {
   ...
   do {
      ...
      for (size_t s = 0; s < siblings.size(); ++s)
         add_velocity_block(siblings[s], popID);
...
#ifdef DEBUG_SPATIAL_CELL
   if (populations[popID].vmesh.size() != populations[popID].blockContainer.size()) {
      std::cerr << "Error, velocity mesh size and block container size do not agree in " << __FILE__ << ' ' << __LINE__
                << std::endl;
      exit(1);
   }
   if (popID >= populations.size()) {
 ...
 */
inline uint64_t SpatialCell::get_mpi_transfer_type(void) { return SpatialCell::mpi_transfer_type; }

/*!
 Set if this cell is transferred/received using MPI in the next communication phase.
 */
inline void SpatialCell::set_mpi_transfer_enabled(bool transferEnabled) { this->mpiTransferEnabled = transferEnabled; }
...
inline vmesh::GlobalID SpatialCell::velocity_block_has_grandparent(const vmesh::GlobalID& blockGID,
                                                                   const uint popID) const {
#ifdef DEBUG_SPATIAL_CELL
   if (popID >= populations.size()) {
      std::cerr << "ERROR, popID " << popID << " exceeds populations.size() " << populations.size() << " in ";
